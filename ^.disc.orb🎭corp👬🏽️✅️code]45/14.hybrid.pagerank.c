...
#define DEFAULT_TOP_N 5 // Default top results
...
int main(int argc, char *argv[]) {
    if (argc < 3 || argc > 4) {
        fprintf(stderr, "Usage: %s \"keyword\" <txt|html> [num_results]\n", argv[0]);
        return 1;
    }
    ...
    int top_n = DEFAULT_TOP_N; // Default to 5 results
    if (argc == 4) {
        top_n = atoi(argv[3]);
        if (top_n < 1 || top_n > MAX_DOMAINS) {
            fprintf(stderr, "Number of results must be between 1 and %d, using default %d\n", MAX_DOMAINS, DEFAULT_TOP_N);
            top_n = DEFAULT_TOP_N;
        }
    }
    ...
    } results[MAX_DOMAINS] = {0};
    int total_results = 0;

    // Add domains (up to MAX_DOMAIN_HITS)
    ...
    if (total_results == 0) {
        FILE *summary_fp = fopen("summary_result.txt", "w");
        if (summary_fp) {
            fprintf(summary_fp, "search summary failed\n");
            fclose(summary_fp);
        } else {
            fprintf(stderr, "Failed to open summary_result.txt: %s\n", strerror(errno));
        }
    ...
    // Display top results
    printf("\nTop %d results with keyword '%s':\n", total_results > top_n ? top_n : total_results, keyword);
    for (int i = 0; i < total_results && i < top_n; i++) {
    ...
    int selected = 0; // Default to first result
    if (top_n == 1) {
        // Automatically select first result
        if (total_results < 1) {
            fprintf(stderr, "No results to select\n");
            goto cleanup;
        }
    } else {
        // Prompt user for selection
        int choice = -1;
        printf("Enter index (1-%d) to save and display, or 0 to exit: ", total_results > top_n ? top_n : total_results);
        if (scanf("%d", &choice) != 1) {
            fprintf(stderr, "Invalid input\n");
            choice = 0;
        }
        while (getchar() != '\n'); // Clear input buffer
        if (choice < 1 || choice > total_results || choice > top_n) {
    ...
    if (size >= 0) {
        ...
        // Write final summary result to summary_result.txt
        FILE *summary_fp = fopen("summary_result.txt", "w");
        if (summary_fp) {
            const char *source_str = results[selected].source == 0 ? "dns" : "page";
            fprintf(summary_fp, "[%d] %s <%s> (%d mentions, Inbound links: %d, Links: ",
                    selected + 1, results[selected].identifier, source_str, results[selected].count, results[selected].inbound);
            if (results[selected].inbound == 0 || !results[selected].inbound_sources) {
                fprintf(summary_fp, "None");
            } else {
                for (int j = 0; j < results[selected].inbound && j < MAX_LINKS; j++) {
                    if (results[selected].inbound_sources[j]) {
                        fprintf(summary_fp, "<link>%s</link>", results[selected].inbound_sources[j]);
                        if (j < results[selected].inbound - 1) fprintf(summary_fp, ", ");
                    }
                }
            }
            fprintf(summary_fp, ")\n");
            if (results[selected].source == 2) {
                char *path = extract_path(results[selected].identifier);
                if (path) {
                    fprintf(summary_fp, "%s\n", results[selected].identifier);
                    free(path);
                }
            }
            if (!save_html && results[selected].context && results[selected].context[0]) {
                char display_context[101];
                strncpy(display_context, results[selected].context, 100);
                display_context[100] = '\0';
                char *newline = strchr(display_context, '\n');
                if (newline) *newline = '\0';
                fprintf(summary_fp, "%s...\n", display_context);
