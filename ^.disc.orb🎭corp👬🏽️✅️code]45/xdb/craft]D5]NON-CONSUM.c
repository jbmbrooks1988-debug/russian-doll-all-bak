...
void check_missing_elements(char chems[][20], int chem_count, char required_elements[][20], int required_count, char* missing_str, size_t max_len) {
    ...
    ptr += snprintf(ptr, remaining, "To craft the simplest element (Pair_up_quarks), you need: ");
...
int find_valid_combination(char chems[][20], int chem_count, int* idx1, int* idx2) {
    for (int i = 0; i < chem_count; i++) {
        for (int j = i + 1; j < chem_count; j++) {
            int elem_idx1 = get_element_index(chems[i]);
            int elem_idx2 = get_element_index(chems[j]);
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strncmp(content, "!craft", 6) == 0) {
                ...
                check_missing_elements(chems, chem_count, required_elements, required_count, missing, sizeof(missing));
                if (chem_count < 2) {
                    ...
                    if (elem_idx1 >= 0 && elem_idx2 >= 0 && result >= 0) {
                        ...
                        read_user_chems(user_id, chems, &chem_count);
                        format_inventory(chems, chem_count, inventory, sizeof(inventory));
                        snprintf(response, sizeof(response), "Success! Crafted: %s. %s", element_names[result], inventory);
                    } else {
                        if (elem_idx1 < 0 || elem_idx2 < 0) {
