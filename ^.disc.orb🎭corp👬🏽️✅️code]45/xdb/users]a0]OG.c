...
void read_user_state(const char* user_id, char* state_content, size_t max_size, int* user_cmd_count) {
    ...
    if (fp) {
        ...
        // Create new state file for new user
        *user_cmd_count = 0;
...
int main() {
    ...
    srand(time(NULL)); // Initialize random seed for point multiplier
    ensure_users_dir();
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strstr(content, "!user")) {
                ...
                read_user_state(user_id, state_content, MAX_STATE_SIZE, &user_cmd_count); // This handles new user state file creation
                int r = rand() % 100;
                int delta;
                if (r < 70) {
                ...
                snprintf(state_content, MAX_STATE_SIZE, "user_command_count=%d\nlast_login=%ld\n", user_cmd_count, time(NULL));
                ...
                FILE* queue_fp = fopen(SEND_QUEUE_FILE, "w");
                if (queue_fp) {
                    fprintf(queue_fp, "%s|User %s received %d points. State: %s\n", channel_id, user_id, delta, state_content);
                    fclose(queue_fp);
                    snprintf(log, sizeof(log), "Wrote to %s: %s|User %s received %d points. State: %s", SEND_QUEUE_FILE, channel_id, user_id, delta, state_content);
                    log_message(log);
                    char cmd[512];
                    snprintf(cmd, sizeof(cmd), "./+x/send.+x");
                    log_message("Executing: ./+x/send.+x");
