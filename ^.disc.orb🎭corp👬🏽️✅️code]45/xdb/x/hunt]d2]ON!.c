...
void update_user_state(const char* user_id, Fighter* user, int next_hunt_time, Fighter allies[], int ally_count) {
    ...
    if (fp) {
        ...
        fclose(fp);
        char log[256];
        snprintf(log, sizeof(log), "Updated state for user %s: level=%d, xp=%d, allies=%d", user_id, user->level, user->xp, ally_count);
        log_message(log);
    } else {
        char err[256];
...
void read_user_state(const char* user_id, Fighter* user, int* next_hunt_time, Fighter allies[], int* ally_count) {
    ...
    *next_hunt_time = 0;
    ...
    if (fp) {
        ...
        fclose(fp);
        if (user->max_hp == 0) user->max_hp = user->level * 10 + 20;
        char log[256];
        snprintf(log, sizeof(log), "Read state for user %s: level=%d, xp=%d", user_id, user->level, user->xp);
        log_message(log);
    } else {
...
void simulate_battle(Fighter* user, Fighter allies[], int ally_count, Fighter enemies[], int enemy_count_in_battle, char* battle_log, int* outcome, int* xp_gained, Fighter* captured, int* captured_flag) {
    int rounds = 10 + (rand() % 91);
    int log_offset = 0;
...
int main() {
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strncmp(content, "!hunt", 5) == 0 && (content[5] == ' ' || content[5] == '\0')) {
                if (strlen(user_id) == 0) {
                    snprintf(log, sizeof(log), "No valid user ID found in content: %s", content);
                    log_message(log);
                    FILE* queue_fp = fopen(SEND_QUEUE_FILE, "w");
                    if (queue_fp) {
                        fprintf(queue_fp, "%s|Error: Invalid user ID format. Please use !hunt @username", channel_id);
                        fflush(queue_fp);
                        fclose(queue_fp);
                ...
                if (next_hunt_time > now) {
                    ...
                    char response[256] = {0};
                    snprintf(response, sizeof(response), "User %s cannot hunt yet. Try again in %d hour(s).", user_id, hours_left);
                    FILE* queue_fp = fopen(SEND_QUEUE_FILE, "w");
                    if (queue_fp) {
                ...
                Fighter captured = {0};
                int captured_flag = 0;
                simulate_battle(&user, allies, ally_count, enemies, enemy_count_in_battle, battle_log, &outcome, &xp_gained, &captured, &captured_flag);
                char response[256] = {0};
                int leveled_up = 0;
                if (outcome == 1) {
                    user.xp += xp_gained;
                    ...
                    snprintf(response, sizeof(response), "Won XP:%d%s", xp_gained, leveled_up ? " LeveledUp" : "");
                    ...
                    snprintf(response, sizeof(response), "Lost");
                    user.hp = 0;
                    for (int i = 0; i < ally_count; i++) {
                        allies[i].hp = 0;
                    }
                    ...
                    snprintf(response, sizeof(response), "Draw");
                    next_hunt_time = 0;
                }
                user.hp = user.max_hp;
                for (int i = 0; i < ally_count; i++) {
