...
pid_t child_pids[12]; // Increased to accommodate hunt.+x
...
int read_keys() {
    ...
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, "#define TOKEN")) {
            sscanf("%s", line, "#define TOKEN \"%[^\"]\"", TOKEN);
...
int main() {
    ...
    if (strstr(buffer, "101 Switching Protocols")) {
        ...
        snprintf(identify, sizeof(identify),
                 ...
                 TOKEN);
        char cmd[512];
        snprintf(cmd, sizeof(cmd), "stdbuf -oL ./+x/send.+x ws \"%s\"", identify);
        printf("Executing: %s\n", cmd);
        log_message(cmd);
        int ret = system(cmd);
        if (ret != 0) {
        ...
        while (1) {
            ...
            if (fgets(cli_input, sizeof(cli_input), stdin)) {
                ...
                if (strlen(cli_input) > 0) {
                    ...
                    snprintf(cmd, sizeof(cmd), "stdbuf -oL ./+x/send.+x");
                    printf("Executing: %s\n", cmd);
                    log_message(cmd);
                    ret = system(cmd);
                    if (ret != 0) {
            ...
            if (heartbeat_interval > 0 && (now - last_heartbeat) >= (heartbeat_interval / 1000)) {
                snprintf(cmd, sizeof(cmd), "stdbuf -oL ./+x/send.+x ws \"{\\\"op\\\":1,\\\"d\\\":%d}\"", sequence ? sequence : -1);
                printf("Executing: %s\n", cmd);
                log_message(cmd);
                ret = system(cmd);
                if (ret != 0) {
