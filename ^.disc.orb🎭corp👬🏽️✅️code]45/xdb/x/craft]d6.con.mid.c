...
void update_user_chems(const char* user_id, const char* chem_name, char chems[][20], int chem_count, int idx1, int idx2) {
    char filepath[256];
    ...
    // Rewrite chems.txt, excluding idx1 and idx2, and adding the new chem
    ...
    if (fp) {
        ...
        fclose(fp);
        char log[256];
        snprintf(log, sizeof(log), "Updated %s for user %s: added %s, removed %s and %s", filepath, user_id, chem_name, chems[idx1], chems[idx2]);
        log_message(log);
    } else {
        char err[256];
...
void read_user_chems(const char* user_id, char chems[][20], int* chem_count) {
    *chem_count = 0;
    ...
    if (fp) {
        ...
        // Initialize with starting elements (Up_quark, Down_quark, Electron)
...
int get_min_elements_needed() {
    return 2; // Crafting always requires at least 2 elements
}

void get_simplest_element(int* simplest_idx, char required_elements[][20], int* required_count) {
    *simplest_idx = -1;
    *required_count = 0;
    for (int i = 0; i < element_count; i++) {
        if (element_combo1[i] >= 0 && element_combo2[i] >= 0) {
            ...
            *required_count = 2;
            break; // First craftable element is the simplest
        }
    }
}
...
void get_next_element(int current_idx, char chems[][20], int chem_count, int* next_idx, char required_elements[][20], int* required_count) {
    ...
    *required_count = 0;
    ...
    // Look for the next craftable element after current_idx
    ...
    // If no craftable element found, fall back to the simplest
    if (*next_idx == -1) {
        get_simplest_element(next_idx, required_elements, required_count);
    }
}
...
void check_missing_elements(char chems[][20], int chem_count, char required_elements[][20], int required_count, char* missing_str, size_t max_len, const char* target_name) {
    ...
    ptr += snprintf(ptr, remaining, "To craft %s%s, you need: ", (strcmp(target_name, "Pair_up_quarks") == 0) ? "the simplest element (" : "the next element (", target_name);
    ...
    for (int i = 0; i < required_count; i++) {
        ...
        if (idx >= 0) {
            ...
            if (needed[i] < 0) needed[i] = 0; // Don't report negative needs
        }
    }
    if (needed[0] == 0 && needed[1] == 0) {
        snprintf(ptr, remaining, "all required elements. Try combining them!");
    } else {
        for (int i = 0; i < required_count && remaining > 0; i++) {
            if (needed[i] > 0) {
                int len = snprintf(ptr, remaining, "%s (%d more)%s", required_elements[i], needed[i], 
                                   (i < required_count - 1 && needed[i + 1] > 0) ? ", " : "");
                ptr += len;
                remaining -= len;
            }
        }
...
int find_valid_combination(char chems[][20], int chem_count, int* idx1, int* idx2) {
    int counts[MAX_ELEMENTS] = {0};
    for (int i = 0; i < chem_count; i++) {
        int idx = get_element_index(chems[i]);
        if (idx >= 0) {
    ...
    // Prioritize higher-index (more advanced) elements
    for (int i = element_count - 1; i >= 0; i--) {
        if (element_combo1[i] >= 0 && element_combo2[i] >= 0) {
            ...
            int count_c2 = counts[c2];
            if (count_c1 > 0 && count_c2 > 0) {
                // Find the first pair of matching elements in inventory
                for (int j = 0; j < chem_count; j++) {
                    if (get_element_index(chems[j]) == c1) {
                        for (int k = j + 1; k < chem_count; k++) {
                            if (get_element_index(chems[k]) == c2) {
                                *idx1 = j;
                                *idx2 = k;
                                return i;
                            }
    ...
    // No valid combination found, select random indices
    *idx1 = rand() % chem_count;
    *idx2 = rand() % chem_count;
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strncmp(content, "!craft", 6) == 0) {
                ...
                get_simplest_element(&simplest_idx, required_elements, &required_count);
                check_missing_elements(chems, chem_count, required_elements, required_count, missing, sizeof(missing), "Pair_up_quarks");
                if (chem_count < 2) {
                    snprintf(response, sizeof(response), "Craft failed: you need at least 2 elements to craft. %s %s", inventory, missing);
                } else {
                    int idx1, idx2;
                    int result = find_valid_combination(chems, chem_count, &idx1, &idx2);
                    int elem_idx1 = get_element_index(chems[idx1]);
                    int elem_idx2 = get_element_index(chems[idx2]);
                    if (elem_idx1 >= 0 && elem_idx2 >= 0 && result >= 0) {
                        update_user_chems(user_id, element_names[result], chems, chem_count, idx1, idx2);
                        // Refresh inventory to include new element
                        read_user_chems(user_id, chems, &chem_count);
                        format_inventory(chems, chem_count, inventory, sizeof(inventory));
                        // Get the next craftable element
                        int next_idx;
                        ...
                        if (elem_idx1 < 0 || elem_idx2 < 0) {
                            snprintf(response, sizeof(response), "Craft failed: one or both elements (%s, %s) are invalid. %s %s", 
                                     chems[idx1], chems[idx2], inventory, missing);
                        } else {
                            snprintf(response, sizeof(response), "Craft failed: %s and %s cannot be combined. %s %s", 
                                     chems[idx1], chems[idx2], inventory, missing);
                        }
                    }
                }
