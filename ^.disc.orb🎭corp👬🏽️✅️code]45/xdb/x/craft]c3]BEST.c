    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strncmp(content, "!craft", 6) == 0) {
                ...
                if (chem_count < 2) {
                    snprintf(response, sizeof(response), "Craft failed: you need at least 2 elements to craft. %s", inventory);
                } else {
                    ...
                    if (elem_idx1 >= 0 && elem_idx2 >= 0) {
                        ...
                        if (result >= 0) {
                            ...
                            snprintf(response, sizeof(response), "Craft failed: %s and %s cannot be combined. %s", 
                                     chems[idx1], chems[idx2], inventory);
                        }
                    } else {
                        snprintf(response, sizeof(response), "Craft failed: one or both elements (%s, %s) are invalid. %s", 
                                 chems[idx1], chems[idx2], inventory);
                    }
                }
