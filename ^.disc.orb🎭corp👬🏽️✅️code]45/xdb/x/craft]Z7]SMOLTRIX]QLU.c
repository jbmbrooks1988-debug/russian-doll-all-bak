...
int combine_elements(int index1, int index2) {
    ...
    if (index1 >= 0 && index2 >= 0 &&
        element_protons[index1] >= 0 && element_protons[index2] >= 0 &&
...
void update_user_chems(const char* user_id, const char* chem_name, int idx1, int idx2, char chems[][20], int chem_count) {
    ...
    if (fp) {
        ...
        if (rename(temp_filepath, filepath) == -1) {
            ...
            format_inventory(new_chems, new_chem_count, inventory, sizeof(inventory));
            snprintf(log, sizeof(log), "Updated %s for user %s: removed idx %d, %d; added %s; %s", 
                     filepath, user_id, idx1, idx2, chem_name, inventory);
            log_message(log);
        }
    } else {
        char err[MAX_LOG];
...
void read_user_chems(const char* user_id, char chems[][20], int* chem_count) {
    *chem_count = 0;
    ...
    if (fp) {
        ...
        if (file_size == 0) {
            char log[MAX_LOG];
            snprintf(log, sizeof(log), "Empty chems file %s for user %s", filepath, user_id);
            log_message(log);
        }
        char line[20];
        ...
        format_inventory(chems, *chem_count, inventory, sizeof(inventory));
        char log[MAX_LOG];
        snprintf(log, sizeof(log), "Read %d chems for user %s from %s: %s", *chem_count, user_id, filepath, inventory);
        log_message(log);
    } else {
...
void get_next_element(int current_idx, char chems[][20], int chem_count, int* next_idx, char required_elements[][20], int* required_count) {
    ...
    *required_count = 0;
    ...
    // Try to find the next element in sequence
    ...
    // If no sequential element found, pick any craftable or next possible
    if (*next_idx == -1) {
        for (int i = 0; i < element_count; i++) {
...
int find_valid_combination(char chems[][20], int chem_count, int target_idx, char required_elements[][20], int* required_count) {
    ...
    *required_count = 0;
    if (target_idx >= 0 && element_combo1[target_idx] >= 0 && element_combo2[target_idx] >= 0) {
        ...
        *required_count = 2;
        ...
        if (can_craft) {
            ...
            log_message(log);
            // Try to craft prerequisites
            int prereq_idx;
            char prereq_elements[2][20] = {0};
            int prereq_count = 0;
            get_prerequisite_elements(target_idx, chems, chem_count, &prereq_idx, prereq_elements, &prereq_count);
            if (prereq_idx >= 0) {
                snprintf(log, sizeof(log), "Crafting prerequisite %s with components %s and %s", 
                         element_names[prereq_idx], prereq_elements[0], prereq_elements[1]);
                log_message(log);
                return prereq_idx;
            }
        }
    ...
    // Fallback to highest craftable element if no target or prerequisites
    int selected_idx = -1;
            ...
            if (token && strcmp(token, "!craft") == 0) {
                ...
                if (result >= 0) {
                    ...
                    if (next_idx >= 0) {
                        ...
                        // Suggest the next element in sequence even if not craftable
