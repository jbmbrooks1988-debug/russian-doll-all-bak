        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (token && strcmp(token, "!craft") == 0) {
                ...
                if (chem_count < get_min_elements_needed()) {
                    ...
                    if (result >= 0) {
                        // Find indices of components to remove
                        int idx1 = -1, idx2 = -1;
                ...
                if (result >= 0) {
                    ...
                    snprintf(response, sizeof(response), "Craft failed: cannot craft %s. %s %s", element_names[target_idx], inventory, missing);
                } else {
                    int next_idx;
                    char next_required[2][20] = {0};
                    int next_required_count = 0;
                    get_next_element(get_highest_element_index(chems, chem_count), chems, chem_count, &next_idx, next_required, &next_required_count);
                    check_missing_elements(chems, chem_count, next_required, next_required_count, missing, sizeof(missing), next_idx >= 0 ? element_names[next_idx] : "Pair_up_quarks");
