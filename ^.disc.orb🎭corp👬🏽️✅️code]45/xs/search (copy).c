...
// Sanitize keyword for file paths
    ...
    while (1) {
        ...
        while (fgets(line, sizeof(line), event_fp)) {
            ...
            if (strncmp(cmd, "!search ", 8) == 0) {
                ...
                if (ret != 0) {
                    snprintf(log, sizeof(log), "13.hybrid.+pagerank.+x failed with return code %d", ret);
                    log_message(log);
                    FILE *queue_fp = fopen(SEND_QUEUE_FILE, "w");
                    if (queue_fp) {
                        fprintf(queue_fp, "%s|Search failed, please try again.\n", channel_id);
                        fclose(queue_fp);
                ...
                log_message("13.hybrid.+pagerank.+x executed successfully");
