...
void process_ollama_chat(const char* message) {
    ...
    char post_data[2048];
    char modified_message[2048];

    // Prepend the rule to the user's message
    const char* rule = "(rule: give a short reply; dont take too long thinking) ";
    snprintf(modified_message, sizeof(modified_message), "%s%s", rule, message);

    // Log the modified message
    snprintf(log, sizeof(log), "Modified message: %.100s...", modified_message);
    log_message(log);
    ...
    // Construct JSON payload with modified message
