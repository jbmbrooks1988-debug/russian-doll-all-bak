...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    if (fd < 0) {
        char msg[MAX_PATH + 50];
        snprintf(msg, sizeof(msg), "Error: Failed to open output file foo_gl.txt");
        log_to_file(msg);
        return 1;
    }
    ...
    */
    ...
    int input_fd = open(commands_path, O_RDONLY);
    if (input_fd < 0) {
        /*
        char msg[MAX_PATH + 50];
                    ...
                    if (sscanf(line, "[%*[^]]] %31[^:]: %127[^\n]", input_type, details) == 2) {
                        if (strcmp(input_type, "Keyboard") == 0) {
                            if (sscanf(details, "key=%c, x=%d, y=%d", &key[0], &pos_x, &pos_y) == 3) {
                                ...
                                switch (key[0]) {
                                    case 'x': z++; break; // x key: decrease x
                                  //  case 'y': y--; break; // y key: decrease y
                                    case 'z': z--; break; // z key: increase z
                                    /*
                                ...
                                if (key[0] == 'x' || key[0] == 'y' || key[0] == 'z' /* || key[0] == 'w' || key[0] == 's' || 
                            ...
                            if (sscanf(details, "key=%31[^,], x=%d, y=%d", key, &pos_x, &pos_y) == 3) {
                                ...
                                if (strcmp(key, "UpArrow") == 0) x++;
                                else if (strcmp(key, "DownArrow") == 0) x--;
                                else if (strcmp(key, "RightArrow") == 0) y--;
                                else if (strcmp(key, "LeftArrow") == 0) y++;
