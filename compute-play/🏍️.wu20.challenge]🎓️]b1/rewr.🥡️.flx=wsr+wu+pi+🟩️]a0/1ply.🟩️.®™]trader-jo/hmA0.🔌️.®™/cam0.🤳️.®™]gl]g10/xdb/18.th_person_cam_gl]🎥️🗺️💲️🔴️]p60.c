...
int player_shape = SHAPE_SPHERE; // Add to global variables
...
// File paths (initialized empty, to be set by locations.txt)
char state_path[MAX_PATH] = "";
char maps_dir[MAX_PATH] = "";
char commands_path[MAX_PATH] = "";
char terminal_input_path[MAX_PATH] = "";
...
void renderEmoji(unsigned int codepoint, float x, float y, float scale) {
    ...
    float normalized_height = pixel_height / window_height;
...
void log_to_file(const char *message) {
    if (log_path[0] == '\0') return; // Skip if log_path not set
...
void log_input_to_file(const char *input_type, const char *details) {
    if (commands_path[0] == '\0') return; // Skip if commands_path not set
...
int is_directory_not_empty(const char *dir_path) {
    if (dir_path[0] == '\0') return 0; // Skip if dir_path not set
...
void read_locations() {
    ...
    if (!fp) {
        log_to_file("Error: Failed to open locations.txt");
        exit(1); // Exit if locations.txt cannot be opened
    }
    ...
    if (state_path[0] == '\0' || maps_dir[0] == '\0' || commands_path[0] == '\0' ||
        terminal_input_path[0] == '\0' || log_path[0] == '\0') {
        log_to_file("Error: Not all required paths (state, maps, commands, terminal_input, log) defined in locations.txt");
        exit(1);
    }

    char msg[MAX_PATH * 8];
    snprintf(msg, sizeof(msg), "Loaded paths from locations.txt: state=%s, maps=%s, commands=%s, terminal_input=%s, log=%s, super_maps=%s, super_states=%s",
...
void read_commands() {
    if (terminal_input_path[0] == '\0') return; // Skip if terminal_input_path not set
    num_command_lines = 0;
...
void draw_terminal() {
    ...
    float scroll_x = term_x + term_width;
    ...
    float char_width = (emoji_scale / window_width) * 0.6f;
...
int check_commands_update() {
    if (terminal_input_path[0] == '\0') return 0; // Skip if terminal_input_path not set
...
void parse_state_file(const char *filename, int is_state_file, int *x, int *y, int *z, int *cam_x, int *cam_y, int *cam_z, int *cursor_pos, char *piece_symbol, int *has_coords) {
    ...
    float player_r_temp = 1.0f, player_g_temp = 0.8f, player_b_temp = 0.8f;
    int player_has_color_temp = 0;
    float player_size_temp = DEFAULT_PIECE_SIZE;
    int player_shape_temp = SHAPE_SPHERE;
        ...
        if (strcmp(key, "color") == 0) {
            ...
            if (sscanf(line, "%*s %15s", shape) == 1) {
                if (strcmp(shape, "cube") == 0) {
                    ...
                    if (is_state_file) {
                        player_shape_temp = SHAPE_CUBE;
                    }
                } else if (strcmp(shape, "sphere") == 0) {
                    current_shape = SHAPE_SPHERE;
                    if (is_state_file) {
                        player_shape_temp = SHAPE_SPHERE;
                    }
                }
            }
            ...
            if (strcmp(key, "x") == 0) {
                ...
                if (sscanf(line, "%*s %d", &value) == 1) {
                    *x = value;
                    if (has_coords) (*has_coords) |= 1;
                }
            } else if (strcmp(key, "y") == 0) {
                int value;
                if (sscanf(line, "%*s %d", &value) == 1) {
                    *y = value;
                    if (has_coords) (*has_coords) |= 2;
                }
            } else if (strcmp(key, "z") == 0) {
                int value;
                if (sscanf(line, "%*s %d", &value) == 1) {
                    *z = value;
                    if (has_coords) (*has_coords) |= 4;
                }
    ...
    if (is_state_file) {
        player_r = player_r_temp;
        player_g = player_g_temp;
        player_b = player_b_temp;
        player_shape = player_shape_temp;
        char debug_msg[256];
        snprintf(debug_msg, sizeof(debug_msg), "Player state: symbol=%s, x=%d, y=%d, z=%d, color=(%.1f, %.1f, %.1f), shape=%s",
                 piece_symbol, *x, *y, *z, player_r, player_g, player_b,
                 player_shape == SHAPE_CUBE ? "cube" : "sphere");
        log_to_file(debug_msg);
    }
...
void read_maps() {
    if (maps_dir[0] == '\0') return; // Skip if maps_dir not set
    free_maps();
...
void read_state() {
    if (state_path[0] == '\0') return; // Skip if state_path not set
...
void draw_mini_map() {
    ...
    float cx = floor((float)player_x);
    float cy = floor((float)player_y);

    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "Mini-map: Raw player pos: (%d, %d, %d), Snapped: (%.0f, %.0f), Rotation: %d", 
             player_x, player_y, player_z, cx, cy, mini_map_rotation);
    log_to_file(log_msg);
    ...
    glRotatef((mini_map_rotation - 1) * 90.0f, 0.0f, 0.0f, 1.0f);
    glTranslatef(-cx, -cy, 0.0f);
    ...
    glColor4f(1.0f, 1.0f, 0.0f, 0.3f);
    glBegin(GL_QUADS);
    glVertex2f(cursor_pos[0] + 0.1, cursor_pos[1] + 0.1);
    glVertex2f(cursor_pos[0] + 0.9, cursor_pos[1] + 0.1);
    glVertex2f(cursor_pos[0] + 0.9, cursor_pos[1] + 0.9);
    glVertex2f(cursor_pos[0] + 0.1, cursor_pos[1] + 0.9);
    glEnd();
    glDisable(GL_BLEND);
...
static const RenderLayer render_order[] = {
    LAYER_WEBCAM,
    LAYER_SCENE,
    LAYER_MINIMAP,
    LAYER_TERMINAL
};
static const int num_layers = sizeof(render_order) / sizeof(render_order[0]);
...
void display() {
    ...
    for (int i = 0; i < num_layers; i++) {
        switch (render_order[i]) {
            ...
            case LAYER_SCENE:
                ...
                glEnable(GL_LIGHTING);
                glEnable(GL_LIGHT0);
                GLfloat light_pos[] = {camera_x, camera_y, camera_z, 1.0};
                ...
                if (player_coords_valid) {
                    glColor3f(player_r, player_g, player_b);
                    if (player_shape == SHAPE_CUBE) {
                        draw_cube(player_x + 0.5f, player_y + 0.5f, player_z + 0.5f, DEFAULT_PIECE_SIZE * 2.0f);
                    } else {
                        draw_sphere(player_x + 0.5f, player_y + 0.5f, player_z + 0.5f, DEFAULT_PIECE_SIZE);
                    }
                }

                draw_cursor(cursor_pos[0], cursor_pos[1], cursor_pos[2]);
                glDisable(GL_LIGHTING);
                break;
    ...
    char coords[64];
    snprintf(coords, sizeof(coords), "x:%d y:%d z:%d", player_x, player_y, player_z);
    draw_text(10, window_height - 20, coords);
...
void keyboard(unsigned char key, int x, int y) {
    ...
    int cursor_move_x = 0, cursor_move_y = 0, cursor_move_z = 0;
    ...
    switch (key) {
        ...
        case 't':
            ...
            snprintf(log_msg, sizeof(log_msg), "Terminal %s", terminal_enabled ? "enabled" : "disabled");
            log_to_file(log_msg);
            break;
        case 'c':
#if ENABLE_AR
            if (!ar_camera_enabled) {
                ar_camera_enabled = init_camera(&ar_camera_fd, ar_camera_buffers, &ar_num_buffers, "/dev/video0", ar_frame_width, ar_frame_height);
                if (ar_camera_enabled) {
                    glGenTextures(1, &ar_texture_id);
                    log_to_file("AR camera initialized");
                }
            } else {
                cleanup_camera(&ar_camera_fd, ar_camera_buffers, ar_num_buffers, &ar_camera_enabled, ar_frame_width, ar_frame_height);
                glDeleteTextures(1, &ar_texture_id);
                log_to_file("AR camera disabled");
            }
#endif
            break;
    }
    ...
    if (move_x != 0 || move_y != 0 || move_z != 0) {
        int new_x = player_x + move_x;
        int new_y = player_y + move_y;
        int new_z = player_z + move_z;
        int can_move = 1;
        for (int i = 0; i < num_maps; i++) {
            int mx = map_coords[i*3 + 0];
            int my = map_coords[i*3 + 1];
            ...
            int width = map_sizes[i*2 + 0];
            int height = map_sizes[i*2 + 1];
            if (new_z == mz && new_x >= mx && new_x < mx + width && new_y >= my && new_y < my + height) {
                if (maps[i][new_y - my][new_x - mx] == '#') {
                    can_move = 0;
                    break;
                }
            }
        ...
        if (can_move) {
            ...
            player_z = new_z;
            player_coords_valid = 1;
            snprintf(log_msg, sizeof(log_msg), "Player moved to x:%d y:%d z:%d", player_x, player_y, player_z);
            log_input_to_file("keyboard", log_msg);
        } else {
            snprintf(log_msg, sizeof(log_msg), "Blocked move to x:%d y:%d z:%d", new_x, new_y, new_z);
            log_input_to_file("keyboard", log_msg);
        }
    }

    if (cursor_move_x != 0 || cursor_move_y != 0 || cursor_move_z != 0) {
        cursor_pos[0] += cursor_move_x;
        cursor_pos[1] += cursor_move_y;
        cursor_pos[2] += cursor_move_z;
        snprintf(log_msg, sizeof(log_msg), "Cursor moved to x:%d y:%d z:%d", cursor_pos[0], cursor_pos[1], cursor_pos[2]);
        log_input_to_file("keyboard", log_msg);
    }
...
void special(int key, int x, int y) {
    ...
    snprintf(log_msg, sizeof(log_msg), "Camera moved to x:%d y:%d z:%d", cam_x, cam_y, cam_z);
    log_input_to_file("special_key", log_msg);
    glutPostRedisplay();
}
...
void mouse(int button, int state, int x, int y) {
    ...
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            dragging = 1;
            snprintf(log_msg, sizeof(log_msg), "Mouse drag started at x:%d y:%d", x, y);
            log_input_to_file("mouse", log_msg);
        } else {
            dragging = 0;
            snprintf(log_msg, sizeof(log_msg), "Mouse drag ended at x:%d y:%d", x, y);
            log_input_to_file("mouse", log_msg);
        }
    } else if (button == 3 && state == GLUT_DOWN && num_command_lines > 10) {
        scroll_offset = (scroll_offset + 1) % (num_command_lines - 9);
        snprintf(log_msg, sizeof(log_msg), "Scroll up, offset:%d", scroll_offset);
        log_input_to_file("mouse", log_msg);
        glutPostRedisplay();
    } else if (button == 4 && state == GLUT_DOWN && num_command_lines > 10) {
        scroll_offset = (scroll_offset - 1 + (num_command_lines - 9)) % (num_command_lines - 9);
        snprintf(log_msg, sizeof(log_msg), "Scroll down, offset:%d", scroll_offset);
        log_input_to_file("mouse", log_msg);
        glutPostRedisplay();
    }
}
...
void motion(int x, int y) {
    if (dragging) {
        char log_msg[256];
        snprintf(log_msg, sizeof(log_msg), "Mouse dragged to x:%d y:%d", x, y);
        log_input_to_file("motion", log_msg);
    }
}
...
void reshape(int w, int h) {
    ...
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "Window reshaped to %dx%d", w, window_height);
    log_to_file(log_msg);
}
...
void init() {
    ...
    glut_initialized = 1;
    log_to_file("Initialization completed");
}
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("18th Person Camera GL");
