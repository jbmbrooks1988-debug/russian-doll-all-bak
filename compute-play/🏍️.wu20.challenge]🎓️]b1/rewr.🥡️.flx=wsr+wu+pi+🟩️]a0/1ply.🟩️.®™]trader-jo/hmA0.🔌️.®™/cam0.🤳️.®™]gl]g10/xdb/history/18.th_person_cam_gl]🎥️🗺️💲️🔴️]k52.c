...
void draw_mini_map() {
    ...
    char log_msg[256];
    snprintf(log_msg, sizeof(log_msg), "Mini-map: Raw player pos: (%d, %d, %d), Snapped: (%.0f, %.0f), Orientation: N=up, E=right, S=down, W=left, Y-flipped", 
             player_x, player_y, player_z, cx, cy);
    log_to_file(log_msg);
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    glOrtho(cy - half_view, cy + half_view, -(cx - half_view), -(cx + half_view), -1, 1);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    ...
    glBegin(GL_QUADS);
    glVertex2f(cy - half_view, cx - half_view);
    glVertex2f(cy + half_view, cx - half_view);
    glVertex2f(cy + half_view, cx + half_view);
    glVertex2f(cy - half_view, cx + half_view);
    glEnd();
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    if (maps[i][y][x] == '#') {
                        ...
                        glBegin(GL_QUADS);
                        glVertex2f(world_y + 0.1, world_x + 0.1);
                        glVertex2f(world_y + 0.9, world_x + 0.1);
                        glVertex2f(world_y + 0.9, world_x + 0.9);
                        glVertex2f(world_y + 0.1, world_x + 0.9);
                        glEnd();
                    } else if (maps[i][y][x] == piece_symbol[0]) {
                        int found = 0;
                        for (int j = 0; j < num_super_state_pieces; j++) {
                            if (super_state_x[j] == mx + x &&
                                ...
                                glBegin(GL_QUADS);
                                glVertex2f(world_y + 0.5f - half_size, world_x + 0.5f - half_size);
                                glVertex2f(world_y + 0.5f + half_size, world_x + 0.5f - half_size);
                                glVertex2f(world_y + 0.5f + half_size, world_x + 0.5f + half_size);
                                glVertex2f(world_y + 0.5f - half_size, world_x + 0.5f + half_size);
                                glEnd();
                        ...
                        if (!found) {
                            glColor3f(1.0f, 0.8f, 0.8f);
                            glBegin(GL_QUADS);
                            glVertex2f(world_y + 0.2, world_x + 0.2);
                            glVertex2f(world_y + 0.8, world_x + 0.2);
                            glVertex2f(world_y + 0.8, world_x + 0.8);
                            glVertex2f(world_y + 0.2, world_x + 0.8);
                            glEnd();
                        }
                    }
                }
    ...
    if (player_coords_valid) {
        ...
        glBegin(GL_QUADS);
        glVertex2f(player_y + 0.2, player_x + 0.2);
        glVertex2f(player_y + 0.8, player_x + 0.2);
        glVertex2f(player_y + 0.8, player_x + 0.8);
        glVertex2f(player_y + 0.2, player_x + 0.8);
        glEnd();
    }
    ...
    glColor4f(1.0f, 1.0f, 0.0f, 0.3f);
    glBegin(GL_QUADS);
    glVertex2f(cursor_pos[1] + 0.1, cursor_pos[0] + 0.1);
    glVertex2f(cursor_pos[1] + 0.9, cursor_pos[0] + 0.1);
    glVertex2f(cursor_pos[1] + 0.9, cursor_pos[0] + 0.9);
    glVertex2f(cursor_pos[1] + 0.1, cursor_pos[0] + 0.9);
    glEnd();
    glDisable(GL_BLEND);

    glColor3f(1.0f, 1.0f, 1.0f);
    glBegin(GL_LINE_LOOP);
    glVertex2f(cy - half_view + 0.05, cx - half_view + 0.05);
    glVertex2f(cy + half_view - 0.05, cx - half_view + 0.05);
    glVertex2f(cy + half_view - 0.05, cx + half_view - 0.05);
    glVertex2f(cy - half_view + 0.05, cx + half_view - 0.05);
    glEnd();
