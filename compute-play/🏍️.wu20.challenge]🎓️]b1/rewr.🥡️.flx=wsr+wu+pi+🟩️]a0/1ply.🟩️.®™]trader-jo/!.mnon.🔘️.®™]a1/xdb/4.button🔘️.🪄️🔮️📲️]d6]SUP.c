...
int main() {
    ...
    while (fgets(line, MAX_LINE, file) && num_threads < MAX_THREADS) {
        ...
        if (strlen(line) > 0 && line[0] != '#') {
            // Check for "&" to determine output redirection
            char* command_part = strtok(line, " \t"); // Get command (first column)
            if (command_part == NULL) continue;

            char* redirect_flag = strtok(NULL, " \t"); // Check for "&" (optional third column)
            const char* redirect;
            if (redirect_flag && strcmp(redirect_flag, "&") == 0) {
                redirect = " > /dev/null 2>&1"; // Suppress output
            } else {
                redirect = " 2>&1 | tee -a gl_cli_out.txt"; // Normal output with tee
            }

            // Separate directory and executable
            char* last_slash = strrchr(command_part, '/');
            char dir_path[MAX_LINE] = ".";
            char* exec_name = command_part;
            ...
            // Create command string with cd, stdbuf (if not suppressed) and redirection
            size_t len = strlen("cd \"\" && ") +
                         (redirect_flag && strcmp(redirect_flag, "&") == 0 ? 0 : strlen("stdbuf -oL ")) +
                         strlen("./\"") + strlen(dir_path) + strlen(exec_name) + strlen(redirect) + 1;
            ...
            if (redirect_flag && strcmp(redirect_flag, "&") == 0) {
                snprintf(commands[num_threads], len, "cd \"%s\" && ./%s%s", dir_path, exec_name, redirect);
            } else {
                snprintf(commands[num_threads], len, "cd \"%s\" && stdbuf -oL ./%s%s", dir_path, exec_name, redirect);
            }
            num_threads++;
        }
    }
