...
// Recursively list directories and files with hierarchical index, optionally summing sizes
long long list_dir(const char* path, int depth, char* parent_index, int show_size, int* index) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (S_ISDIR(st.st_mode) && !S_ISLNK(st.st_mode)) { // Don't follow symlinks
            ...
            total_size += list_dir(full_path, depth + 1, current_index, show_size, index);
        }
        (*index)++;
    }
    closedir(dir);
...
int main(int argc, char* argv[]) {
    ...
    long long total_size = 0;
    if (show_size) {
        // Use metadata size for master directory unless recursive total is needed
        print_to_both(" (%lld bytes)", (long long)st.st_size);
        total_size = st.st_size; // Start with metadata size
    }
    print_to_both("\n");

    // List directory contents and calculate total size if needed
    int index = 1;
    if (show_size) {
        total_size += list_dir(argv[1], 1, "", show_size, &index);
        // Optionally print total recursive size
        print_to_both("Total recursive size: %lld bytes\n", total_size);
    } else {
        list_dir(argv[1], 1, "", show_size, &index);
    }
