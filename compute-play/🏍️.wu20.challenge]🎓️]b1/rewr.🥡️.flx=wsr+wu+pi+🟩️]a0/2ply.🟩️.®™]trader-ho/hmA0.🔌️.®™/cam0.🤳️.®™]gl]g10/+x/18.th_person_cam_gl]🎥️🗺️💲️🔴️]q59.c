...
void read_locations() {
    // First, try to read the location of locations.txt from ../kvp.txt
    FILE *kvp_fp = fopen("../🔐️.txt", "r");
    char locations_file_path[MAX_PATH] = "locations.txt"; // Default fallback path
    
    if (kvp_fp) {
        char line[MAX_LINE];
        while (fgets(line, MAX_LINE, kvp_fp)) {
            line[strcspn(line, "\n")] = '\0';
            // Skip comments
            if (line[0] == '#') continue;
            ...
            // Check for 🤳️ key
            if (strcmp(key, "🤳️") == 0) {
                strncpy(locations_file_path, value, MAX_PATH - 1);
                locations_file_path[MAX_PATH - 1] = '\0';
                break;
            }
        }
        fclose(kvp_fp);
    } else {
        log_to_file("Warning: Failed to open ../🔐️.txt, using default locations.txt path");
    }

    // Now read the actual locations from the determined file
    FILE *fp = fopen(locations_file_path, "r");
    if (!fp) {
        char msg[MAX_PATH + 50];
        snprintf(msg, sizeof(msg), "Warning: Failed to open %s, using default paths", locations_file_path);
        log_to_file(msg);
        return;
    }
    ...
    char msg[MAX_PATH * 8];
    snprintf(msg, sizeof(msg), "Loaded paths from %s: state=%s, maps=%s, commands=%s, terminal_input=%s, log=%s, super_maps=%s, super_states=%s",
             locations_file_path, state_path, maps_dir, commands_path, terminal_input_path, log_path, super_maps_dir, super_states_dir);
    log_to_file(msg);
}
...
void draw_terminal() {
    ...
    // Restore original terminal width and dynamic height for scrollbar compatibility
    float term_width = 0.75f; // Match mouse logic's expected width
    float line_spacing = 0.025f; // Fixed spacing from your change
    ...
    // Select font size based on window size (emoji-compatible: 16, 20, or 24)
    void *font;
    float emoji_scale;
    ...
    float char_width = (emoji_scale / window_width) * 0.6f; // Scale proportionally to font size
...
void parse_state_file(const char *filename, int is_state_file, int *x, int *y, int *z, int *cam_x, int *cam_y, int *cam_z, int *cursor_pos, char *piece_symbol, int *has_coords) {
    ...
    // Temporary storage for player state (for state.txt)
    ...
    // Update player globals if this is state.txt
    if (is_state_file) {
...
void draw_mini_map() {
    int mini_map_rotation = 2; // 1 = 0°, 2 = 90° clockwise, 3 = 180°, 4 = 270° clockwise
    ...
    // Apply rotation around the player's position (cx, cy)
    float angle = (mini_map_rotation - 1) * 90.0f; // 1=0°, 2=90°, 3=180°, 4=270°
    ...
    // Draw background (unrotated, so draw before rotation or reset matrix)
    glPushMatrix();
    glLoadIdentity();
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
// Define rendering layers at the top for user configuration
typedef enum {
    LAYER_WEBCAM,
    LAYER_SCENE,
    LAYER_TERMINAL,
...
// User-configurable rendering order (modify this array to change priorities)
static const RenderLayer render_order[] = {
    LAYER_WEBCAM,   // Webcam first (background)
    LAYER_SCENE,    // Scene (maps, player, super state, cursor)
    LAYER_MINIMAP,  // Minimap
    LAYER_TERMINAL  // Terminal last (foreground)
};
...
void display() {
    ...
    // Process each layer in the specified order
    for (int i = 0; i < num_layers; i++) {
        switch (render_order[i]) {
            ...
            case LAYER_SCENE:
                // Reset depth buffer and set up 3D projection for scene
                ...
                // Draw maps
                for (int j = 0; j < num_maps; j++) {
                    glCallList(map_display_lists[j]);
                }

                // Draw player with correct shape
                if (player_coords_valid) {
                    glColor3f(player_r, player_g, player_b);
                ...
                for (int j = 0; j < num_super_state_pieces; j++) {
                    ...
                    if (super_state_shape[j] == SHAPE_CUBE) {
                        draw_cube(super_state_x[j] + 0.5f, super_state_y[j] + 0.5f, super_state_z[j] + 0.5f, super_state_size[j] * 2.0f);
                    } else {
                        draw_sphere(super_state_x[j] + 0.5f, super_state_y[j] + 0.5f, super_state_z[j] + 0.5f, super_state_size[j]);
                    }
                }
            ...
            case LAYER_TERMINAL:
                if (terminal_enabled) {
                    ...
                    // Draw terminal in 2D orthographic projection
                    glClear(GL_DEPTH_BUFFER_BIT); // Clear depth to ensure terminal is on top
                    glMatrixMode(GL_PROJECTION);
                    glPushMatrix();
                    glLoadIdentity();
                    glOrtho(0, window_width, 0, window_height, -1, 1);
