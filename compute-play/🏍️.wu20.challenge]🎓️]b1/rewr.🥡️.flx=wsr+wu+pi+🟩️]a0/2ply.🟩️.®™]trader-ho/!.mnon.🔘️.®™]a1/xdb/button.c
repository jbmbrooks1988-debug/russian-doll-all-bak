...
int main() {
    ...
    while (fgets(line, MAX_LINE, file) && num_threads < MAX_THREADS) {
        ...
        if (strlen(line) > 0 && line[0] != '#') {
            ...
            char* plus_x_part = strstr(command_part, "/+x/");

            if (plus_x_part != NULL) {
                strncpy(dir_path, command_part, plus_x_part - command_part);
                dir_path[plus_x_part - command_part] = '\0';
                strcpy(exec_path, plus_x_part + 1);
            } else {
                ...
                if (last_slash != NULL) {
                    ...
                    strcpy(exec_path, last_slash + 1);
                } else {
                    strcpy(exec_path, command_part);
                }
            }

            commands[num_threads] = malloc(COMMAND_BUFFER_SIZE);
            ...
            if (redirect_flag && strcmp(redirect_flag, "&") == 0) {
                snprintf(commands[num_threads], COMMAND_BUFFER_SIZE, "cd \"%s\" && ./%s%s", dir_path, exec_path, redirect);
            } else {
                snprintf(commands[num_threads], COMMAND_BUFFER_SIZE, "cd \"%s\" && stdbuf -oL ./%s%s", dir_path, exec_path, redirect);
            }
            num_threads++;
        }
    }
