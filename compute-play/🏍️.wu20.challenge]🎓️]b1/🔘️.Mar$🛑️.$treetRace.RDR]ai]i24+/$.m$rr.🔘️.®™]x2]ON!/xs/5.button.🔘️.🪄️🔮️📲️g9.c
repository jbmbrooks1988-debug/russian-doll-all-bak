...
int main() {
    // Block SIGINT in main thread initially, unblock later for specific handling
    sigset_t set;
    sigemptyset(&set);
    sigaddset(&set, SIGINT);
    pthread_sigmask(SIG_BLOCK, &set, NULL);

    // Set up signal handler for SIGTERM (not SIGINT)
    signal(SIGTERM, signal_handler);

    // Ignore SIGINT in the orchestrator to let children handle it
    ...
    // Unblock SIGINT in main thread to allow it to catch for cleanup
    sigemptyset(&set);
    sigaddset(&set, SIGINT);
