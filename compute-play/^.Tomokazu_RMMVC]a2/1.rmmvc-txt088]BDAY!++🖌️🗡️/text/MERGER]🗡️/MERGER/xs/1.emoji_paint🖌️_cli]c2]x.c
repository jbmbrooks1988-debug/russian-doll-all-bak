...
int selected_emoji = 1;  // 1 = cherry (was 0, but now 0 = blank)
...
void print_ascii_grid() {
    ...
    for (int i = 0; i < NUM_EMOJIS; i++) {
        if (menu_selection == 1 && i == selected_emoji) printf("\033[1;33m[");
        else if (i == selected_emoji) printf("[");
        printf("%d:%s", i + 1, emojis[i]);
        if (menu_selection == 1 && i == selected_emoji) printf("]\033[0m");
        else if (i == selected_emoji) printf("]");
        printf(" ");
    }
    printf("\n2. Color Palette: ");
...
void process_input(int ch) {
    if (ch == '0') {
        ...
        if (menu_selection == 1) { // Emoji Palette
            ...
            } else if (ch == KEY_RIGHT && selected_emoji < NUM_EMOJIS - 1) {
                selected_emoji++;
                set_status_message("Emoji selected");
            } else if (ch == '\r' || ch == '\n') {
                menu_selection = 0;
