...
int palette_mode = 0; // 0 = normal, 1 = emoji palette, 2 = color palette
...
void print_ascii_grid() {
    ...
    printf("%s\n", status_message);
    printf("0.Canvas Editor  1.Emoji Palette  2.Color Palette  3.Layer Toggle\n");
    printf("SPACE: Paint  F: Fill  R: Rect  S: Save  L: Load  T: Tab  Q: Quit\n");
    printf("Arrows: Move Selector (in canvas) or Select Items (in palette)  Enter: Place Tile\n");
}
...
void process_input(int ch) {
    if (ch == ' ') {
        // Reset palette mode when using tools
        palette_mode = 0;
        ...
        // Canvas editor mode (default)
        palette_mode = 0;
        set_status_message("Canvas editor selected - use arrows to move selector");
    } else if (ch == '1') {
        // Emoji palette selection mode
        palette_mode = 1;
        set_status_message("Emoji palette selected - use arrows to select emoji");
    } else if (ch == '2') {
        // Color palette selection mode
        palette_mode = 2;
        set_status_message("Color palette selected - use arrows to select color");
    } else if (ch == 'c' || ch == 'C') {
        ...
        // Reset palette mode when toggling layers
        palette_mode = 0;
    } else if (ch == 's' || ch == 'S') {
        save_canvas();
    } else if (ch == 'l' || ch == 'L') {
        ...
        if (palette_mode == 1) {
            // Emoji palette mode - move to previous emoji
            selected_emoji = (selected_emoji - 1 + NUM_EMOJIS) % NUM_EMOJIS;
            set_status_message("Emoji selected");
        } else if (palette_mode == 2) {
            // Color palette mode - move to previous color
            selected_fg_color = (selected_fg_color - 1 + NUM_COLORS) % NUM_COLORS;
            set_status_message("Color selected");
        } else {
            // Normal mode - move selector up
            if (selector_row > 0) selector_row--;
            set_status_message("Selector moved");
        }
    } else if (ch == KEY_DOWN) { // Down arrow
        if (palette_mode == 1) {
            // Emoji palette mode - move to next emoji
            selected_emoji = (selected_emoji + 1) % NUM_EMOJIS;
            set_status_message("Emoji selected");
        } else if (palette_mode == 2) {
            // Color palette mode - move to next color
            selected_fg_color = (selected_fg_color + 1) % NUM_COLORS;
            set_status_message("Color selected");
        } else {
            // Normal mode - move selector down
        ...
        if (palette_mode == 1) {
            ...
            // Normal mode - move selector left
        ...
        if (palette_mode == 1) {
            ...
            // Normal mode - move selector right
