...
int main() {
    ...
    do {
        ...
        if (choice == 1) { // Edit Map
            ...
            if (choice == item_count + 1) {
                ...
                if (fields == 3) val = 0; // Default value if omitted
    ...
    if (!fp) {
        printf("Error: Could not open %s for writing. Check permissions or path.\n", static_path);
        return 1;
    }
    printf("Saving to %s\n", static_path);
    fprintf(fp, "[Map]\nSize: 5 5\n");
