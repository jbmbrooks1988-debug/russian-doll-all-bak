...
// External variable for module executable from main
extern char* game_executable;
...
void run_module() {
    if (!game_executable) {
        printf("No module executable specified.\n");
        return;
    }
    
    printf("Running module: %s\n", game_executable);

    // 1. Read previous result to get current state, or use default values
    int player_x = 400;  // Default starting x position (center of canvas)
    int player_y = 300;  // Default starting y position (center of canvas)
    int canvas_width = 400;  // Default canvas width
    int canvas_height = 300; // Default canvas height
    char input_command[20] = "NONE";  // Default: no action
    
    FILE* prev_out_fp = fopen("output.csv", "r");
    if (prev_out_fp) {
        if (fscanf(prev_out_fp, "%d,%d", &player_x, &player_y) == 2) {
            // Successfully read previous state, keep default values for others
        }
        fclose(prev_out_fp);
    }

    // 2. Write the current state to input.csv, including canvas dimensions
    FILE* in_fp = fopen("input.csv", "w");
    if (!in_fp) {
        perror("Error creating input.csv");
        return;
    }
    fprintf(in_fp, "%d,%d,%s,%d,%d\n", player_x, player_y, input_command, canvas_width, canvas_height);
    fclose(in_fp);

    // 3. Execute the module
    char command[512];
    snprintf(command, sizeof(command), "%s input.csv output.csv", game_executable);
    ...
    if (fscanf(out_fp, "%d,%d", &new_player_x, &new_player_y) != 2) {
        fprintf(stderr, "Error reading player position from output.csv\n");
        fclose(out_fp);
        return;
    }
    fclose(out_fp);

    printf("Module processed - Player position: (%d, %d)\n", new_player_x, new_player_y);
    
    // Update UI if needed (for now just print to console)
    // In a more complete implementation, this would update canvas state
    glutPostRedisplay(); // Redraw to show updated UI
}

int is_module_active() {
    return game_executable != NULL;
}
...
void special_keys(int key, int x, int y) {
    ...
    // If no textfield is active and a module is specified, handle arrow keys for the module
    if (is_module_active()) {
        char input_command[20] = "NONE";
        switch(key) {
            case GLUT_KEY_UP:
                strcpy(input_command, "UP");
                break;
            case GLUT_KEY_DOWN:
                strcpy(input_command, "DOWN");
                break;
            case GLUT_KEY_LEFT:
                strcpy(input_command, "LEFT");
                break;
            case GLUT_KEY_RIGHT:
                strcpy(input_command, "RIGHT");
                break;
            default:
                return; // Don't process other special keys for module
        }
        
        printf("Processing module input: %s\n", input_command);

        // Read current state to get canvas dimensions and player position
        int player_x = 400;  // Default starting x position
        int player_y = 300;  // Default starting y position
        int canvas_width = 400;  // Default canvas width  
        int canvas_height = 300; // Default canvas height
        ...
        if (prev_out_fp) {
            if (fscanf(prev_out_fp, "%d,%d", &player_x, &player_y) == 2) {
                printf("Previous player position loaded: (%d, %d)\n", player_x, player_y);
            } else {
                printf("Could not read previous player position, using defaults: (%d, %d)\n", player_x, player_y);
            }
            fclose(prev_out_fp);
        } else {
            printf("No previous output.csv found, using defaults: (%d, %d)\n", player_x, player_y);
        }

        // Write to input.csv to send command to module
        FILE* in_fp = fopen("input.csv", "w");
        if (!in_fp) {
            perror("Error creating input.csv for module");
            return;
        }
        fprintf(in_fp, "%d,%d,%s,%d,%d\n", player_x, player_y, input_command, canvas_width, canvas_height);
        fclose(in_fp);

        // Execute the module with the input
        char command[512];
        snprintf(command, sizeof(command), "%s input.csv output.csv", game_executable);
        int result = system(command);
        if (result == -1) {
        ...
        printf("Module processed - Previous position: (%d, %d) -> New position: (%d, %d)\n", 
