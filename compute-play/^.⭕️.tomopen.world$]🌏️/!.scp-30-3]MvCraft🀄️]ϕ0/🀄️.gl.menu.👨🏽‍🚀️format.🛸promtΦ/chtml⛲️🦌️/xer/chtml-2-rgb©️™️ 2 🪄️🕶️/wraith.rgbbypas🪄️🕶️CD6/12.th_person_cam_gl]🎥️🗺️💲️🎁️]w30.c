                    ...
                    for (int j = 0; j < num_super_state_pieces; j++) {
                        if (super_state_x[j] == mx + x &&
                            ...
                            if (super_state_shape[j] == SHAPE_CUBE) {
                                glutSolidCube(super_state_size[j] * 2.0f); // Scale size to match sphere radius
                            } else {
...
void draw_mini_map() {
                    ...
                    if (maps[i][y][x] == '#') {
                        ...
                        for (int j = 0; j < num_super_state_pieces; j++) {
                            if (super_state_x[j] == mx + x &&
                                ...
                                float quad_size = super_state_size[j] * 2.0f; // Scale size for 2D quad
    ...
    for (int i = 0; i < num_super_state_pieces; i++) {
        ...
        float quad_size = super_state_size[i] * 2.0f; // Scale size for 2D quad
        float half_size = quad_size / 2.0f;
        glBegin(GL_QUADS);
        glVertex2f(world_x + 0.5f - half_size, world_y + 0.5f - half_size);
        glVertex2f(world_x + 0.5f + half_size, world_y + 0.5f - half_size);
