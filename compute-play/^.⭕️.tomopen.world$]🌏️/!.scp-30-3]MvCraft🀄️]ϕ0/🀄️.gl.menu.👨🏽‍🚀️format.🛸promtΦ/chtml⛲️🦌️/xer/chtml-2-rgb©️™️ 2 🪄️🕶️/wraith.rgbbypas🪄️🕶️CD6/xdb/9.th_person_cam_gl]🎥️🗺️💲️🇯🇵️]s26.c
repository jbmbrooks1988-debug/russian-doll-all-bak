...
#define MAX_LINE 256
#define TRANSLUCENT_BLOCKS 1 // 1 to enable 30% translucent blocks, 0 to disable
#define ENABLE_AR 1 // 1 to enable AR (USB camera background), 0 to disable
#define CAMERA_WIDTH 640
#define CAMERA_HEIGHT 480
#define INITIAL_MAP_CAPACITY 10 // Initial capacity for dynamic map array
#define MINI_MAP_VIEW_SIZE 10 // Size of the mini-map view (width and height)
#define MAX_MAPS 100 // Maximum number of maps
#define MAX_MAP_SIZE 1000 // Maximum width or height of a map
#define TARGET_FPS 24 // Target frame rate (adjustable: e.g., 24 or 30)
#define MAX_PATH 256 // Maximum length for file paths
#define MAX_SUPER_STATES 100 // Maximum number of super_states files
#define MAX_PIECES_PER_STATE 100 // Maximum pieces per super_states file
...
int cursor_pos[3] = {0, 0, 0}; // Array for cursor position (x, y, z)
char piece_symbol[16] = "P"; // Using 'P' as the piece symbol in the map
int window_width = 800, window_height = 600;
time_t last_map_load_time = 0; // Track last map load time
GLuint *map_display_lists = NULL; // Display lists for map geometry
int glut_initialized = 0; // Flag to track GLUT initialization
...
// Dynamic map storage
char ***maps = NULL; // Array of 2D map layers
int *map_coords = NULL; // Stores x,y,z for each map [map_idx*3 + 0] = x, [map_idx*3 + 1] = y, [map_idx*3 + 2] = z
int *map_sizes = NULL; // Stores width,height for each map [map_idx*2 + 0] = width, [map_idx*2 + 1] = height
int num_maps = 0; // Current number of maps
int map_capacity = 0; // Current capacity of maps array

// Super state storage
struct super_state_piece {
    int x, y, z;
    char symbol[16];
    float r, g, b; // Color components
    int has_color; // Flag to indicate if color is specified
};
struct super_state_piece *super_state_pieces = NULL; // Array of pieces from super_states files
int num_super_state_pieces = 0; // Current number of pieces
int super_state_piece_capacity = 0; // Capacity of pieces array

// AR camera variables (USB camera, /dev/video4)
int ar_camera_fd = -1;
...
int is_directory_not_empty(const char *dir_path) {
    ...
    while ((entry = readdir(dir))) {
        if (entry->d_type == DT_REG) {
            file_count++;
            break; // Found at least one regular file
        }
    }
    closedir(dir);
    return file_count > 0;
...
void read_locations() {
    ...
    fclose(fp);
    char msg[MAX_PATH * 6];
    snprintf(msg, sizeof(msg), "Loaded paths: state=%s, maps=%s, commands=%s, log=%s, super_maps=%s, super_states=%s",
             state_path, maps_dir, commands_path, log_path, super_maps_dir, super_states_dir);
    log_to_file(msg);
}
...
void resize_super_state_pieces(int needed) {
    if (num_super_state_pieces + needed >= super_state_piece_capacity) {
        ...
        struct super_state_piece *new_pieces = realloc(super_state_pieces, new_capacity * sizeof(struct super_state_piece));
        if (!new_pieces) {
            log_to_file("Error: Memory allocation failed in resize_super_state_pieces");
            exit(1);
        }
        super_state_pieces = new_pieces;
...
void read_super_state() {
    ...
    // Free existing super state pieces
    if (super_state_pieces) {
        free(super_state_pieces);
        super_state_pieces = NULL;
    ...
    while ((entry = readdir(dir)) && file_count < MAX_SUPER_STATES) {
        ...
        if (!fp) {
            char msg[MAX_PATH + 50];
            snprintf(msg, sizeof(msg), "Warning: Failed to open super_states file %s", filename);
            log_to_file(msg);
            continue;
        }

        // First pass: collect pieces
        char line[MAX_LINE];
        char current_symbol[16] = "P"; // Default symbol
        int piece_count = 0;
        int temp_piece_count = 0;
        struct super_state_piece temp_pieces[MAX_PIECES_PER_STATE];

        while (fgets(line, MAX_LINE, fp) && temp_piece_count < MAX_PIECES_PER_STATE) {
            line[strcspn(line, "\n")] = '\0';
            char key[MAX_LINE];
            ...
            } else if (strcmp(key, "piece") == 0) {
                int x, y, z;
                if (sscanf(line, "%*s %d %d %d", &x, &y, &z) == 3) {
                    temp_pieces[temp_piece_count].x = x;
                    temp_pieces[temp_piece_count].y = y;
                    temp_pieces[temp_piece_count].z = z;
                    strncpy(temp_pieces[temp_piece_count].symbol, current_symbol, 15);
                    temp_pieces[temp_piece_count].symbol[15] = '\0';
                    temp_pieces[temp_piece_count].has_color = 0; // Default, will update in second pass
                    temp_piece_count++;
                }
            }
        }
        ...
        while (fgets(line, MAX_LINE, fp)) {
            ...
            if (strcmp(key, "symbol") == 0) {
                ...
                if (sscanf(line, "%*s %f %f %f", &r, &g, &b) == 3) {
                    // Apply color to all pieces with matching symbol
                    for (int i = 0; i < temp_piece_count; i++) {
                        if (strcmp(temp_pieces[i].symbol, current_symbol) == 0) {
                            temp_pieces[i].r = r;
                            temp_pieces[i].g = g;
                            temp_pieces[i].b = b;
                            temp_pieces[i].has_color = 1;
                        }
                    }
                }
            }
        ...
        // Store pieces in super_state_pieces
        for (int i = 0; i < temp_piece_count; i++) {
            resize_super_state_pieces(1);
            super_state_pieces[num_super_state_pieces] = temp_pieces[i];
            if (!super_state_pieces[num_super_state_pieces].has_color) {
                // Default color (pink) for pieces without color
                super_state_pieces[num_super_state_pieces].r = 1.0f;
                super_state_pieces[num_super_state_pieces].g = 0.8f;
                super_state_pieces[num_super_state_pieces].b = 0.8f;
            }
        ...
        file_count++;
        char msg[MAX_PATH + 50];
        snprintf(msg, sizeof(msg), "Loaded %d pieces from super_states file %s", piece_count, filename);
        log_to_file(msg);
    }
    regfree(&regex);
    closedir(dir);

    if (file_count >= MAX_SUPER_STATES) {
        log_to_file("Warning: Reached maximum number of super_states files");
    }
    char msg[256];
...
void read_state() {
    ...
    char line[MAX_LINE];
    float piece_r = 1.0f, piece_g = 0.8f, piece_b = 0.8f; // Default color (pink)
    int piece_has_color = 0; // Flag to indicate if color is specified
    while (fgets(line, MAX_LINE, fp)) {
        line[strcspn(line, "\n")] = '\0';
        char key[MAX_LINE];
        if (sscanf(line, "%s", key) != 1) continue;
        if (strcmp(key, "x") == 0) {
            int value;
            if (sscanf(line, "%*s %d", &value) == 1) player_x = value;
        } else if (strcmp(key, "y") == 0) {
            int value;
            if (sscanf(line, "%*s %d", &value) == 1) player_y = value;
        } else if (strcmp(key, "z") == 0) {
            int value;
            if (sscanf(line, "%*s %d", &value) == 1) player_z = value;
            ...
            if (sscanf(line, "%*s %d %d %d", &x, &y, &z) == 3) {
                for (int i = 0; i < num_maps; i++) {
                    ...
                    if (z == mz && x >= mx && x < mx + width && y >= my && y < my + height) {
                        maps[i][y - my][x - mx] = piece_symbol[0];
                    }
                }
                // Add piece to super_state_pieces for rendering with color
                resize_super_state_pieces(1);
                super_state_pieces[num_super_state_pieces].x = x;
                super_state_pieces[num_super_state_pieces].y = y;
                super_state_pieces[num_super_state_pieces].z = z;
                strncpy(super_state_pieces[num_super_state_pieces].symbol, piece_symbol, 15);
                super_state_pieces[num_super_state_pieces].symbol[15] = '\0';
                super_state_pieces[num_super_state_pieces].r = piece_r;
                super_state_pieces[num_super_state_pieces].g = piece_g;
                super_state_pieces[num_super_state_pieces].b = piece_b;
                super_state_pieces[num_super_state_pieces].has_color = piece_has_color;
                num_super_state_pieces++;
            }
        }
    }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (maps[i][y][x] == '#') {
                    ...
                    // Find the piece in super_state_pieces to get its color
                    int found = 0;
                    for (int j = 0; j < num_super_state_pieces; j++) {
                        if (super_state_pieces[j].x == mx + x &&
                            super_state_pieces[j].y == my + y &&
                            super_state_pieces[j].z == mz &&
                            strcmp(super_state_pieces[j].symbol, piece_symbol) == 0) {
                            if (super_state_pieces[j].has_color) {
                                glColor3f(super_state_pieces[j].r, super_state_pieces[j].g, super_state_pieces[j].b);
                            } else {
                                glColor3f(1.0f, 0.8f, 0.8f); // Default pink
                            }
                            found = 1;
                            break;
                        }
                    ...
                    if (!found) {
                        glColor3f(1.0f, 0.8f, 0.8f); // Default pink if not found
                    }
                    glPushMatrix();
                    glTranslatef(mx + x + 0.5f, my + y + 0.5f, mz + 0.5f);
...
void draw_cursor(float x, float y, float z) {
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glColor4f(1.0f, 1.0f, 0.0f, 0.3f); // Yellow with 30% translucency
    glPushMatrix();
...
void draw_mini_map() {
    ...
    // Draw super state pieces in mini-map
    for (int i = 0; i < num_super_state_pieces; i++) {
        float world_x = super_state_pieces[i].x;
        float world_y = super_state_pieces[i].y;
        if (world_x < cx - half_view || world_x >= cx + half_view ||
            world_y < cy - half_view || world_y >= cy + half_view) {
            continue;
        }
        if (super_state_pieces[i].has_color) {
            glColor3f(super_state_pieces[i].r, super_state_pieces[i].g, super_state_pieces[i].b);
        } else {
            glColor3f(0.2f, 0.8f, 0.2f); // Default green
        }
        glBegin(GL_QUADS);
        glVertex2f(world_x + 0.2, world_y + 0.2);
        glVertex2f(world_x + 0.8, world_y + 0.2);
    ...
    // Draw cursor position (snapped to grid)
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glColor4f(1.0f, 1.0f, 0.0f, 0.3f); // Yellow with 30% translucency
    glBegin(GL_QUADS);
...
int check_memory_usage() {
    ...
    fclose(fp);
    unsigned long memory_mb = resident * 4 / 1024; // Convert pages to MB (assuming 4KB pages)
...
void display() {
    ...
    glColor3f(0.0f, 0.0f, 1.0f); // Blue for player
    draw_sphere(player_x + 0.5f, player_y + 0.5f, player_z + 0.5f);

    // Draw super_states pieces
    for (int i = 0; i < num_super_state_pieces; i++) {
        if (super_state_pieces[i].has_color) {
            glColor3f(super_state_pieces[i].r, super_state_pieces[i].g, super_state_pieces[i].b);
        } else {
        ...
        draw_sphere(super_state_pieces[i].x + 0.5f, super_state_pieces[i].y + 0.5f, super_state_pieces[i].z + 0.5f);
    }
...
void keyboard(unsigned char key, int x, int y) {
    ...
    log_input_to_file("Keyboard", details);
}
...
int main(int argc, char *argv[]) {
    ...
    // Explicitly check super_maps/ first
