...
void resize_super_state_pieces(int needed) {
    if (num_super_state_pieces + needed >= super_state_piece_capacity) {
        ...
        if (!new_x || !new_y || !new_z || !new_symbol || !new_r || !new_g || !new_b || !new_has_color) {
...
void display() {
    ...
    for (int i = 0; i < num_super_state_pieces; i++) {
        ...
        draw_sphere(super_state_x[i] + 0.5f, super_state_y[i] + 0.5f, super_state_z[i] + 0.5f);
    }
