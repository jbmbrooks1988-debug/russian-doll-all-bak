...
float cam_x = 0.5f, cam_y = 2.0f, cam_z = 8.5f; // Near player (0.5, 0.5, 6.5)
float cam_yaw = 0.0f, cam_pitch = 0.0f; // Look straight
...
void read_primitive() {
    ...
    float saved_cam_x = cam_x, saved_cam_y = cam_y, saved_cam_z = cam_z;
    float saved_cam_yaw = cam_yaw, saved_cam_pitch = cam_pitch;
    int saved_orbit_mode = orbit_mode;
        ...
        if (strcmp(key, "primitive") == 0) {
            ...
            if (sscanf(line, "primitive %15s %f %f %f %f %f %f %f type=%15s",
                ...
                if (num_primitives < MAX_PRIMITIVES) {
                    ...
                    log_to_file(msg);
                    if (strcmp(temp_type, "cursor") == 0) {
                        snprintf(msg, sizeof(msg), "Cursor moved to (%.1f, %.1f, %.1f)", temp_x, temp_y, temp_z);
                        log_to_file(msg);
                    }
                } else {
    ...
    cam_y = saved_cam_y;
    cam_z = saved_cam_z;
    cam_yaw = saved_cam_yaw;
    cam_pitch = saved_cam_pitch;
    orbit_mode = saved_orbit_mode;
...
void display() {
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(60.0f, (float)WINDOW_WIDTH / WINDOW_HEIGHT, 0.01f, 200.0f); // Wider clipping planes
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // Camera position
    float look_x = 0.0f, look_y = 0.0f, look_z = 0.0f; // Free mode: look at origin
    if (orbit_mode) {
        look_x = player_x;
        look_y = player_y;
        look_z = player_z;
    ...
    // Log camera position
    char msg[256];
    snprintf(msg, sizeof(msg), "Camera: eye=(%.1f, %.1f, %.1f), look=(%.1f, %.1f, %.1f), mode=%s",
             cam_x, cam_y, cam_z, look_x, look_y, look_z, orbit_mode ? "orbit" : "free");
    log_to_file(msg);
