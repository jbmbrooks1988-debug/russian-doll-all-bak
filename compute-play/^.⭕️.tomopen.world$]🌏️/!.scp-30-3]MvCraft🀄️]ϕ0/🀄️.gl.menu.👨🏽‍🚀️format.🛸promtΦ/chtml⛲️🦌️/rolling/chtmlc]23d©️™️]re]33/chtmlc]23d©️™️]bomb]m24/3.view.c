...
void load_directory_contents(UIElement* el) {
    ...
    // Add ".." (parent directory) entry if not in the root or special case
    if (strcmp(el->dir_path, ".") != 0 && strlen(el->dir_path) > 0) {
        strncpy(el->dir_entries[el->dir_entry_count], "../", 255);
    ...
    while ((entry = readdir(dir)) != NULL && el->dir_entry_count < 100) {
        // Skip "." and ".." entries from the listing to avoid duplicates
    ...
    if (is_3d) {
        ...
        for (int i = 0; i < shape_count; i++) {
            ...
            if (strcmp(s->type, "SQUARE") == 0) {
                ...
                glColor4fv(s->color);
                glutSolidCube(s->size / 10.0f); // Scale down the cube size for 3D space
                glPopMatrix();
            }
        }
        ...
        for (int i = 0; i < shape_count; i++) {
            ...
            if (strcmp(s->type, "SQUARE") == 0) {
                glBegin(GL_QUADS);
                glVertex2f(s->x - s->size / 2, s->y - s->size / 2);
                glVertex2f(s->x + s->size / 2, s->y - s->size / 2);
                glVertex2f(s->x + s->size / 2, s->y + s->size / 2);
                glVertex2f(s->x - s->size / 2, s->y + s->size / 2);
                glEnd();
                ...
                for (int j = 0; j <= 20; j++) {
                    ...
                    float dx = s->size / 2 * cos(angle);
                    float dy = s->size / 2 * sin(angle);
