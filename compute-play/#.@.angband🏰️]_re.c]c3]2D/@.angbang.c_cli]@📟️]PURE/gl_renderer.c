...
void write_command_to_file(const char *command_str) {
...
void keyboard(unsigned char key, int x, int y) {
    if (key == 27) { // ESC key
        ...
        write_command_to_file("pickup");
    } else if (key == 'u') {
        if (num_inventory_items > 0) {
            char command_str[256];
            snprintf(command_str, sizeof(command_str), "use %s", inventory[0]);
            write_command_to_file(command_str);
        }
    }
}
...
void special_keyboard(int key, int x, int y) {
    if (key == GLUT_KEY_UP) {
        write_command_to_file("up");
    } else if (key == GLUT_KEY_DOWN) {
        write_command_to_file("down");
    } else if (key == GLUT_KEY_LEFT) {
        write_command_to_file("left");
    } else if (key == GLUT_KEY_RIGHT) {
        write_command_to_file("right");
    }
}

void idle_func() {
    // Continuously load data and redraw
    load_map();
    load_player_pos();
    load_player_stats();
    load_inventory();
