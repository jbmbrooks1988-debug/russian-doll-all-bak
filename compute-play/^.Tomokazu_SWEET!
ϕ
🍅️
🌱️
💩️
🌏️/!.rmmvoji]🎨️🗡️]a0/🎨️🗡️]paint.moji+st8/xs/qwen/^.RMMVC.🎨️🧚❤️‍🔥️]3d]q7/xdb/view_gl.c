...
void render_emoji_3d_quad(unsigned int codepoint, float x, float y, float z, float fg[3], float bg[3]) {
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(0.0, 1.0); glVertex3f(x2, y2, z + 0.01f); // Slightly in front of background
...
void render_sidebar_ui(void) {
    ...
    float sidebar_y_offset = get_file_tab_height() + 50; // Increased offset to avoid overlap
    ...
    render_text(z_level_str, get_sidebar_width() + 10, 30); // Position below status message
...
void display(void) {
    ...
    if (get_view_mode() == 0) { // 2D View
        ...
        // Draw canvas grid
        float grid_color[3] = {0.6f, 0.6f, 0.6f}; // Lighter gray for grid lines
        for (int r = 0; r < get_canvas_rows(); r++) {
            for (int c = 0; c < get_canvas_cols(); c++) {
                ...
                draw_border(x, y, get_tile_size(), get_tile_size(), grid_color); // Draw border instead of solid rect
            }
        }
        ...
        glRotatef(get_camera_pitch(), 1.0f, 0.0f, 0.0f); // Pitch around X-axis
        glRotatef(get_camera_yaw(), 0.0f, 1.0f, 0.0f);   // Yaw around Y-axis

        // Center the canvas in the 3D view
        float center_x = (get_canvas_cols() * get_tile_size()) / 2.0f;
        float center_y = (get_canvas_rows() * get_tile_size()) / 2.0f;
        float center_z = (get_max_layers() * get_tile_size()) / 2.0f;
        ...
        for (int layer = 0; layer < get_max_layers(); layer++) {
            for (int r = 0; r < get_canvas_rows(); r++) {
                for (int c = 0; c < get_canvas_cols(); c++) {
                    ...
                    glPushMatrix();
                    glTranslatef(x + get_tile_size() / 2, y + get_tile_size() / 2, z + get_tile_size() / 2);
                    
                    // Draw cube (front face with border)
                    float cube_color[3] = {0.2f, 0.2f, 0.2f}; // Dark gray for cube
                    float border_color[3] = {0.4f, 0.4f, 0.4f}; // Slightly lighter gray for border
                    draw_rect( -get_tile_size()/2, -get_tile_size()/2, get_tile_size(), get_tile_size(), cube_color); // Front face
                    draw_border( -get_tile_size()/2, -get_tile_size()/2, get_tile_size(), get_tile_size(), border_color); // Border around the face
                    // You would draw all 6 faces of the cube here. For simplicity, I'm just drawing one face.
                    // To properly render emojis on cube faces, you'd need to render a texture on each face.
                    // This is a more complex task involving texture mapping and proper OpenGL setup.
                    // For now, I'll just render the emoji at the center of the cube.
                    ...
                    render_emoji_3d_quad(codepoint, 0, 0, get_tile_size()/2 + 0.01f, fg, bg); // Render emoji on the front face of the cube
    ...
    glLoadIdentity();
    glOrtho(0, get_window_width(), 0, get_window_height(), -1, 1); // Set 2D orthogonal projection for UI

    glMatrixMode(GL_MODELVIEW);
    glPushMatrix(); // Save current modelview matrix
    glLoadIdentity();

    render_sidebar_ui(); // Render the sidebar UI

    glPopMatrix(); // Restore previous modelview matrix
    glMatrixMode(GL_PROJECTION);
    glPopMatrix(); // Restore previous projection matrix
    glMatrixMode(GL_MODELVIEW); // Set back to modelview for subsequent operations
