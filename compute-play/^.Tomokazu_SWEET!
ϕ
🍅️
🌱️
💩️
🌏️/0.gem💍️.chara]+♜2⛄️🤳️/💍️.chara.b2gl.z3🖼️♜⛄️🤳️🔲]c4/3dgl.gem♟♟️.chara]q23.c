...
TextureCache texture_cache[MAX_TEXTURES];
...
typedef struct {
    ...
    int defense;
    int x, y, z;
    char symbol[8]; // Increased for UTF-8 emojis
} Piece;

// Game state
typedef struct {
    char* board[BOARD_SIZE][BOARD_SIZE][BOARD_SIZE]; // piece names or emojis
...
void draw_cube(float x, float y, float z, const char* symbol, const char* piece_name);
void draw_board();
void draw_cursor();
void draw_hud();
void display();
...
// Cleanup FreeType and texture cache
void cleanup() {
    ...
    for (int i = 0; i < texture_cache_count; i++) {
        glDeleteTextures(1, &texture_cache[i].texture);
    }
...
unsigned int get_emoji_codepoint(const char* emoji) {
    if (!emoji || !emoji[0]) {
        log_to_file("Warning: Empty or NULL emoji string, using fallback U+1F300");
        return 0x1F300;
    }
    ...
    if (c < 0x80) {
        ...
        char msg[256];
        snprintf(msg, sizeof(msg), "Warning: Invalid UTF-8 start byte for emoji '%s', using fallback U+1F300", emoji);
        log_to_file(msg);
        return 0x1F300;
    }
    for (int i = 1; i < bytes; i++) {
        if (!emoji[i] || (emoji[i] & 0xC0) != 0x80) {
            char msg[256];
            snprintf(msg, sizeof(msg), "Warning: Invalid UTF-8 continuation byte for emoji '%s', using fallback U+1F300", emoji);
            log_to_file(msg);
            return 0x1F300;
        }
    ...
    if (codepoint < 0x1F300 || codepoint > 0x1FAD6) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Warning: Codepoint U+%04X for emoji '%s' out of emoji range, using fallback U+1F300", codepoint, emoji);
        log_to_file(msg);
        return 0x1F300;
    }
    return codepoint;
...
// Find or create texture for emoji
GLuint get_emoji_texture(unsigned int codepoint) {
    for (int i = 0; i < texture_cache_count; i++) {
        if (texture_cache[i].codepoint == codepoint) {
            return texture_cache[i].texture;
        }
    }

    if (!freetype_initialized) {
        log_to_file("Error: FreeType not initialized, cannot create texture");
        return 0;
    }

    if (FT_Load_Char(face, codepoint, FT_LOAD_RENDER | FT_LOAD_COLOR)) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Error: Could not load glyph for codepoint U+%04X", codepoint);
        log_to_file(msg);
        return 0;
    }
    ...
    if (!slot->bitmap.buffer) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Error: No bitmap for glyph U+%04X", codepoint);
        log_to_file(msg);
        return 0;
    }
    ...
    if (err != GL_NO_ERROR) {
        char msg[256];
        snprintf(msg, sizeof(msg), "OpenGL error during texture creation for U+%04X: %d", codepoint, err);
        log_to_file(msg);
    ...
    if (texture_cache_count < MAX_TEXTURES) {
        texture_cache[texture_cache_count].codepoint = codepoint;
        texture_cache[texture_cache_count].texture = texture;
        texture_cache[texture_cache_count].width = slot->bitmap.width;
        texture_cache[texture_cache_count].height = slot->bitmap.rows;
        texture_cache_count++;
        char msg[256];
        snprintf(msg, sizeof(msg), "Cached texture for codepoint U+%04X (texture ID: %u)", codepoint, texture);
        log_to_file(msg);
    } else {
        log_to_file("Warning: Texture cache full, deleting uncached texture");
        glDeleteTextures(1, &texture);
    }
...
void render_emoji(unsigned int codepoint, float x, float y, float z, float size) {
    GLuint texture = get_emoji_texture(codepoint);
    if (!texture) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Skipping render for codepoint U+%04X: No texture", codepoint);
        log_to_file(msg);
        return;
    }
    ...
    if (err != GL_NO_ERROR) {
        char msg[256];
        snprintf(msg, sizeof(msg), "OpenGL error during emoji render for U+%04X: %d", codepoint, err);
        log_to_file(msg);
    }
}
...
void draw_cube(float x, float y, float z, const char* symbol, const char* piece_name) {
    unsigned int hash = simple_hash(piece_name);
    srand(hash);
    ...
    if (freetype_initialized && symbol && symbol[0]) {
        unsigned int codepoint = get_emoji_codepoint(symbol);
        char msg[256];
        snprintf(msg, sizeof(msg), "Attempting to render emoji for '%s' (U+%04X) at (%.1f, %.1f, %.1f)", symbol, codepoint, x, y, z);
        log_to_file(msg);
        render_emoji(codepoint, x, y, z, base_size);
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg), "Skipping emoji render for piece '%s' at (%.1f, %.1f, %.1f): %s", 
                 piece_name, x, y, z, freetype_initialized ? "invalid symbol" : "FreeType not initialized");
        log_to_file(msg);
    }
}
        ...
        for (int y = 0; y < BOARD_SIZE; y++) {
            for (int x = 0; x < BOARD_SIZE; x++) {
                if (game_state.board[z][y][x]) {
                    ...
                    float pos_z = z + 0.5f;
                    char msg[256];
                    snprintf(msg, sizeof(msg), "Rendering piece '%s' at (%d, %d, %d)", 
                             game_state.board[z][y][x], x, y, z);
                    log_to_file(msg);
                    draw_cube(pos_x, pos_y, pos_z, game_state.board[z][y][x], game_state.board[z][y][x]);
                }
            }
        }
    }
...
int read_first_render_debug() {
    FILE* f = fopen("1rst_render_debug.txt", "r");
    if (!f) {
        log_to_file("Warning: 1rst_render_debug.txt not found, falling back to board_state.txt");
        return 0;
    }

    char line[256];
    int y = 7; // Start at row A (y=7)
    int piece_count = 0;
    log_to_file("Reading 1rst_render_debug.txt for z=0 layer");
    while (fgets(line, sizeof(line), f) && y >= 0) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Processing line: %s", line);
        log_to_file(msg);

        // Skip header, footer, or separator lines
        if (strstr(line, "+-----") || strstr(line, "0      1      2")) {
            continue;
        }

        // Expect lines like "A|  🍵  |  🔝  | ... "
        if (line[0] >= 'A' && line[0] <= 'H') {
            char* ptr = line + 2; // Skip "A|"
            int x = 0;
            while (x < BOARD_SIZE && *ptr) {
                // Skip spaces and |
                while (*ptr == ' ' || *ptr == '|') ptr++;
                if (*ptr == '\0' || *ptr == '\n') break;

                // Extract emoji (UTF-8, up to 7 bytes)
                char emoji[8] = {0};
                int len = 0;
                while (*ptr && *ptr != ' ' && *ptr != '|' && len < 7) {
                    emoji[len++] = *ptr++;
                }
                emoji[len] = '\0';

                if (len > 0 && strcmp(emoji, " ") != 0 && strcmp(emoji, "") != 0) {
                    char piece_name[32];
                    snprintf(piece_name, sizeof(piece_name), "piece_%d_%d_0", x, y);
                    game_state.board[0][y][x] = strdup(emoji);
                    game_state.player_board[0][y][x] = (y >= 4) ? 1 : 2; // y=4-7 for P1, y=0-3 for P2
                    piece_count++;
                    snprintf(msg, sizeof(msg), "Loaded piece '%s' with emoji '%s' at (%d, %d, 0) for player %d", 
                             piece_name, emoji, x, y, game_state.player_board[0][y][x]);
                    log_to_file(msg);
                } else {
                    game_state.board[0][y][x] = NULL;
                    game_state.player_board[0][y][x] = 0;
                }
                x++;
            }
            if (x < BOARD_SIZE) {
                snprintf(msg, sizeof(msg), "Warning: Row %c has only %d entries, padding with none", 'A' + (7 - y), x);
                log_to_file(msg);
                for (; x < BOARD_SIZE; x++) {
                    game_state.board[0][y][x] = NULL;
                    game_state.player_board[0][y][x] = 0;
    ...
    fclose(f);
    char msg[256];
    snprintf(msg, sizeof(msg), "Total pieces loaded from 1rst_render_debug.txt: %d", piece_count);
    log_to_file(msg);
    return 1;
}
...
void read_board_state() {
    if (read_first_render_debug()) {
        // Only populate z=0 from 1rst_render_debug.txt, clear other layers
    ...
    char line[256];
    int z = 0, piece_count = 0, expected_pieces = 0;
    log_to_file("Reading board_state.txt");
    while (fgets(line, sizeof(line), f) && z < BOARD_SIZE) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Processing z-layer %d: %s", z, line);
        log_to_file(msg);
        ...
        while (token && y < BOARD_SIZE) {
            ...
            while (subtoken && x < BOARD_SIZE) {
                ...
                if (strcmp(subtoken, "none") != 0) {
                    char filename[100];
                    snprintf(filename, sizeof(filename), "%s.dir/state.txt", subtoken);
                    FILE* sf = fopen(filename, "r");
                    if (!sf) {
                        snprintf(msg, sizeof(msg), "Warning: Skipping piece '%s' at (%d, %d, %d): No state.txt file", subtoken, x, y, z);
                        log_to_file(msg);
                        x++;
                    ...
                    game_state.player_board[z][y][x] = (z < 4) ? 1 : 2;
                    snprintf(msg, sizeof(msg), "Loaded piece '%s' at (%d, %d, %d) for player %d", 
                             subtoken, x, y, z, game_state.player_board[z][y][x]);
                    log_to_file(msg);
                    piece_count++;
                    
                    Piece piece = {0};
                    if (fscanf(sf, "player:%d\nhp:%d\nattack:%d\ndefense:%d\nx:%d\ny:%d\nz:%d\nsymbol:%7[^\n]\n",
                               &piece.player, &piece.hp, &piece.attack, &piece.defense, 
                               &piece.x, &piece.y, &piece.z, piece.symbol) == 8) {
                        if (piece.symbol[0]) {
                            free(game_state.board[z][y][x]);
                            game_state.board[z][y][x] = strdup(piece.symbol);
                            snprintf(msg, sizeof(msg), "Updated piece '%s' symbol to '%s' at (%d, %d, %d)", 
                                     subtoken, piece.symbol, x, y, z);
                            log_to_file(msg);
                        } else {
                            snprintf(msg, sizeof(msg), "Warning: Empty symbol for piece '%s' at (%d, %d, %d), using piece name", 
                                     subtoken, x, y, z);
                            log_to_file(msg);
                        }
                    } else {
                        snprintf(msg, sizeof(msg), "Warning: Invalid state.txt format for piece '%s' at (%d, %d, %d), using piece name", 
                                 subtoken, x, y, z);
                        log_to_file(msg);
                    }
                    fclose(sf);
            ...
            if (x < BOARD_SIZE) {
                snprintf(msg, sizeof(msg), "Warning: z-layer %d, row %d has only %d entries, padding with none", z, y, x);
                log_to_file(msg);
                for (; x < BOARD_SIZE; x++) {
                    game_state.board[z][y][x] = NULL;
                    game_state.player_board[z][y][x] = 0;
        ...
        if (y < BOARD_SIZE) {
            snprintf(msg, sizeof(msg), "Warning: z-layer %d has only %d rows, padding with empty rows", z, y);
            log_to_file(msg);
            for (; y < BOARD_SIZE; y++) {
                for (int x = 0; x < BOARD_SIZE; x++) {
                    game_state.board[z][y][x] = NULL;
    ...
    if (z < BOARD_SIZE) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Warning: board_state.txt has only %d z-layers, padding with empty layers", z);
        log_to_file(msg);
        for (; z < BOARD_SIZE; z++) {
            for (int y = 0; y < BOARD_SIZE; y++) {
                for (int x = 0; x < BOARD_SIZE; x++) {
    ...
    fclose(f);
    char msg[256];
    snprintf(msg, sizeof(msg), "Total pieces loaded from board_state.txt: %d (expected up to %d)", piece_count, expected_pieces);
    log_to_file(msg);
}
...
int read_inventory(const char* piece_name, Item inventory[]) {
    char piece_path[100];
    snprintf(piece_path, sizeof(piece_path), "%s.dir", piece_name);
    DIR* d = opendir(piece_path);
    if (!d) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Warning: Could not open directory '%s' for inventory", piece_path);
        log_to_file(msg);
        return 0;
    }
    ...
    while ((dir = readdir(d)) != NULL && count < MAX_INVENTORY) {
        if (strstr(dir->d_name, ".dir") != NULL) {
            ...
            if (base_name) {
                char item_path[150];
                snprintf(item_path, sizeof(item_path), "%s.dir/%s.dir/stats.txt", piece_name, base_name);
                FILE* f = fopen(item_path, "r");
                if (f) {
                    char type_str[20];
                    int value;
                    if (fscanf(f, "type:%19s\nvalue:%d\n", type_str, &value) == 2) {
                        ...
                        char msg[256];
                        snprintf(msg, sizeof(msg), "Warning: Invalid stats.txt format in '%s'", item_path);
                        log_to_file(msg);
                    }
                    fclose(f);
                }
