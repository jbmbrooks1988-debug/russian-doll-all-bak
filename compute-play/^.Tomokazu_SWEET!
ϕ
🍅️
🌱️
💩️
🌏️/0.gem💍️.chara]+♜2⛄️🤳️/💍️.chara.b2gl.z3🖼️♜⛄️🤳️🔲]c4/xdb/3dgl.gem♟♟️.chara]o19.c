...
void display() {
    ...
    float cam_z = camera_distance * sin(rad_y);
...
void special(int key, int x, int y) {
    ...
    if (game_state.in_menu) {
        ...
        if (key == GLUT_KEY_LEFT && game_state.cursor_x > 0) {
            ...
            game_state.cursor_y++;
        } else if (key == GLUT_KEY_PAGE_UP && game_state.cursor_z < BOARD_SIZE - 1) {
            game_state.cursor_z++;
        } else if (key == GLUT_KEY_PAGE_DOWN && game_state.cursor_z > 0) {
            game_state.cursor_z--;
        }
    }
    glutPostRedisplay();
