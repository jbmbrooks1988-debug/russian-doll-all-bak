...
int window_height = GRID_ROWS * TILE_SIZE + 100;  // Extra for menu + UI

float emoji_scale;
float font_color[3] = {1.0f, 1.0f, 1.0f};
float background_color[4] = {0.05f, 0.05f, 0.1f, 1.0f};

char status_message[256] = "Welcome! Use arrow keys. F to found city.";
...
// --- World Generation (Fixed: No mountain prison!) ---
void generate_world() {
    srand(time(NULL));
    ...
    // Find a grassland tile with at least 2 non-mountain adjacent tiles
    bool found = false;
    for (int dr = -2; dr <= 2 && !found; dr++) {
        for (int dc = -2; dc <= 2 && !found; dc++) {
            ...
            if (free_neighbors >= 2) {
                ...
                cursor_row = r;
                cursor_col = c;
                set_status_message("Settler landed safely! Ready to explore.");
                found = true;
            }
        }
    }
    ...
    if (!found) {
        ...
        set_status_message("Settler placed (fallback).");
    }
}
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        int row = (int)((tile_area_top - gl_y) / TILE_SIZE);
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            bool show_fog = debug_fog_enabled && !explored[r][c];
            if (show_fog) {
...
int main(int argc, char** argv) {
    ...
    printf(" - Click menu to toggle debug options\n");
