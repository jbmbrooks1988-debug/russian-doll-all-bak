...
// Chinese (Mode 2)
const char* CHINESE_OCEAN = "海";
const char* CHINESE_LAND = "草";
const char* CHINESE_FOREST = "树";
const char* CHINESE_MOUNTAIN = "山";
const char* CHINESE_RIVER = "河";
const char* CHINESE_SETTLER = "人";
const char* CHINESE_CITY = "城";
const char* CHINESE_FOG = "雾";
...
int render_mode = 0; // 0=Emoji, 1=ASCII, 2=Chinese
...
void render_grid() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        ...
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            if (debug_fog_enabled && !explored[r][c]) {
                symbol = (render_mode == 1) ? ASCII_FOG : (render_mode == 2) ? CHINESE_FOG : EMOJI_FOG;
            } else if (debug_terrain_enabled) {
                if (city_map[r][c]) {
                    symbol = (render_mode == 1) ? ASCII_CITY : (render_mode == 2) ? CHINESE_CITY : EMOJI_CITY;
                } else if (has_settler && r == settler_row && c == settler_col) {
                    symbol = (render_mode == 1) ? ASCII_SETTLER : (render_mode == 2) ? CHINESE_SETTLER : EMOJI_SETTLER;
                } else {
                    switch (terrain[r][c]) {
                        case TERRAIN_GRASS:
                            symbol = (render_mode == 1) ? ASCII_LAND : (render_mode == 2) ? CHINESE_LAND : EMOJI_LAND;
                            break;
                        case TERRAIN_FOREST:
                            symbol = (render_mode == 1) ? ASCII_FOREST : (render_mode == 2) ? CHINESE_FOREST : EMOJI_FOREST;
                            break;
                        case TERRAIN_MOUNTAIN:
                            symbol = (render_mode == 1) ? ASCII_MOUNTAIN : (render_mode == 2) ? CHINESE_MOUNTAIN : EMOJI_MOUNTAIN;
                            break;
                        case TERRAIN_RIVER:
                            symbol = (render_mode == 1) ? ASCII_RIVER : (render_mode == 2) ? CHINESE_RIVER : EMOJI_RIVER;
                            break;
                        case TERRAIN_OCEAN:
                            symbol = (render_mode == 1) ? ASCII_OCEAN : (render_mode == 2) ? CHINESE_OCEAN : EMOJI_OCEAN;
                            break;
                    }
                }
            } else {
    ...
    printf("Debug: g (fog), t (terrain), k (cursor), i (input), c (cycle render: Emoji/ASCII/Chinese), q (quit)\n");
    printf("Status: %s\n", status_message);
...
void process_input() {
    ...
    switch (c) {
        ...
        case 'c': case 'C':
            ...
            set_status_message("Rendering: %s", render_mode == 0 ? "Emoji" : (render_mode == 1 ? "ASCII" : "Chinese"));
            break;
        case 'q': case 'Q':
            restore_terminal();
            exit(0);
...
int main() {
    ...
    printf(" - c: Cycle render (Emoji/ASCII/Chinese)\n");
    printf(" - q: Quit\n");
