...
char status_message[256] = "Welcome! Click settler to select.";
...
void generate_world() {
    ...
    // Find safe spawn: grassland with at least 2 non-mountain neighbors
    bool found = false;
    for (int dr = -2; dr <= 2 && !found; dr++) {
        for (int dc = -2; dc <= 2 && !found; dc++) {
            ...
            if (free_neighbors >= 2) {
                ...
                settler_col = c;
                has_settler = true;
                reveal_around(r, c, 3);  // Start with wide vision
                cursor_row = r;
                cursor_col = c;
                set_status_message("Settler landed safely! Click to move.");
                found = true;
            }
        }
    }
...
void move_settler(int row, int col) {
    if (!is_valid_tile(row, col)) {
        set_status_message("Invalid tile (%d,%d)", row, col);
        return;
    }
    ...
    if (terrain[row][col] == TERRAIN_MOUNTAIN) {
        set_status_message("Can't move to mountains!");
        return;
    }
    if (city_map[row][col]) {
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        float grid_y = window_height - y;  // Convert to OpenGL Y
        float tile_area_bottom = 80;
        float tile_area_top = MENU_Y - 80;

        if (grid_y < tile_area_bottom || grid_y > tile_area_top) {
            return;
        }
        ...
        if (!is_valid_tile(row, col)) {
            set_status_message("Out of bounds.");
            return;
        }
        ...
        if (row == settler_row && col == settler_col) {
            set_status_message("👤 Settler selected. Click adjacent tile to move.");
        } else {
            ...
            if (dr + dc == 1) {
                ...
                set_status_message("❌ Not adjacent! Click a tile next to the settler.");
            }
        }
...
int main(int argc, char** argv) {
    ...
    printf("Controls:\n");
    printf(" - Click settler, then adjacent tile to move\n");
    printf(" - F: Found city | Space: End turn\n");
    printf(" - G/T/K: Toggle Fog/Terrain/Cursor\n");
    printf(" - Menu: Click to toggle debug options\n");
