...
const char ascii_symbols[NUM_SYMBOLS] = {'C', 'L', 'O', 'D', 'B'};
const char *chinese_symbols[NUM_SYMBOLS] = {"樱", "柠", "橙", "钻", "铃"};
int display_mode = 0; // 0: Emoji, 1: Letter, 2: Chinese
...
void print_ascii_grid() {
    ...
    const char *mode_str = display_mode == 0 ? "Emoji" : (display_mode == 1 ? "Letter" : "Chinese");
    printf("Slot Machine (%s Mode)\n", mode_str);
    printf("------------\n");
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            if (display_mode == 0) {
                printf("| %s ", symbols[grid[r][c]]);
            } else if (display_mode == 1) {
                printf("| %c ", ascii_symbols[grid[r][c]]);
            } else {
                printf("| %s ", chinese_symbols[grid[r][c]]);
            }
        }
        printf("|\n");
        printf("------------\n");
    }
    printf("Score: %d  Bet: %d\n", score, bet);
    printf("%s\n", status_message);
    printf("Press SPACE to spin, 1 to switch mode, q to quit\n");
}
...
void display() {
    ...
    render_text("SPACE: Spin  1: Switch Mode  Q: Quit", 10, window_height - 60);
...
void keyboard(unsigned char key, int x, int y) {
    if (key == ' ' && !is_spinning) {
        ...
        display_mode = (display_mode + 1) % 3; // Cycle mode
        print_ascii_grid();
        glutPostRedisplay();
    } else if (key == 'q' || key == 'Q') {
