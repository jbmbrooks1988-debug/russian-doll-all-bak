...
int load_clues(const char* filename) {
    ...
    while (fgets(line, sizeof(line), file) && num_clues < MAX_QUESTIONS) {
        ...
        if (idx != 4) {
            fprintf(stderr, "❌ Bad line (field count = %d): '%s'\n", idx, line);
            continue;
        }
        ...
        // ✅ Validation: make sure price isn't zero unless it's supposed to be
        if (c->price == 0 && strcmp(trim(fields[3]), "0") != 0) {
            fprintf(stderr, "⚠️ Price parse failed: '%s' -> %d (line: %s)\n", fields[3], c->price, line);
        }

        printf("✅ Loaded: [%s] Q='$%d' -> '%s'\n", c->category, c->price, c->question);

        num_clues++;
    }

    fclose(file);
    printf("🎉 Total clues loaded: %d\n", num_clues);
    return num_clues > 0;
}
