...
bool debug_input = false; // Debug input logging
bool use_ascii = false; // Toggle ASCII vs. emoji rendering
...
void process_input() {
    char c = getchar();
    if (debug_input) {
        printf("Debug: Input byte=0x%02X\n", (unsigned char)c);
    }

    if (c == '\033') { // Escape sequence
        char seq[3] = {0};
        int bytes_read = 0;
        // Read up to 2 more bytes, with a timeout
        for (int i = 0; i < 2; i++) {
            if (read(STDIN_FILENO, &seq[i], 1) == 1) {
                bytes_read++;
                if (debug_input) {
                    printf("Debug: Seq[%d]=0x%02X\n", i, (unsigned char)seq[i]);
                }
            } else {
                break;
            }
        ...
        if (bytes_read == 2 && seq[0] == '[') {
            if (show_production_menu) {
                if (seq[1] == 'A' && production_choice > 0) { // Up
                    production_choice--;
                } else if (seq[1] == 'B' && production_choice < 2) { // Down
                    production_choice++;
                }
            } else {
                switch (seq[1]) {
                    case 'A': if (cursor_row > 0) cursor_row--; break; // Up
                    case 'B': if (cursor_row < GRID_ROWS - 1) cursor_row++; break; // Down
                    case 'D': if (cursor_col > 0) cursor_col--; break; // Left
                    case 'C': if (cursor_col < GRID_COLS - 1) cursor_col++; break; // Right
                }
            }
        }
        return;
    ...
    if (show_production_menu) {
        ...
        } else if (c == 'w' && production_choice > 0) {
            production_choice--;
        } else if (c == 's' && production_choice < 2) {
            production_choice++;
        }
        return;
    }
