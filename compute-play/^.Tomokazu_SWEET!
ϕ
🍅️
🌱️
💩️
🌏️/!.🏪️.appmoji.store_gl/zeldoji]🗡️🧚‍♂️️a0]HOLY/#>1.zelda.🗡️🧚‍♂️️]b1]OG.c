...
bool special_pressed[100] = {0}; // For arrows: GLUT_KEY_UP etc.
...
void update_hero() {
    float dr = 0, dc = 0;
    if (key_pressed['w'] || special_pressed[GLUT_KEY_UP]) { dr -= SPEED; facing = 1; }
    if (key_pressed['s'] || special_pressed[GLUT_KEY_DOWN]) { dr += SPEED; facing = 0; }
    if (key_pressed['a'] || special_pressed[GLUT_KEY_LEFT]) { dc -= SPEED; facing = 2; }
    if (key_pressed['d'] || special_pressed[GLUT_KEY_RIGHT]) { dc += SPEED; facing = 3; }
