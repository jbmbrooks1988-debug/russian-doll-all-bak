...
char status_message[256] = "Welcome! Explore the world.";
...
float camera_row = 0, camera_col = 0;

// --- Key States ---
bool key_pressed[256] = {0};
bool special_pressed[200] = {0}; // Increased to handle GLUT special keys safely
...
bool is_land(float fr, float fc, int terrain[][MAP_COLS], int rows, int cols);
void reveal_around(float r, float c, int radius, bool explored[][MAP_COLS], int rows, int cols);
...
void enter_cave();
void exit_cave();
int find_enemy_at(float r, float c);
int find_item_at(float r, float c);
void pickup_item(int item_index);
void use_potion();
void slash_attack();
void shoot_arrow();
float dist(float r1, float c1, float r2, float c2);
...
void generate_world(int terrain[][MAP_COLS], int rows, int cols, bool is_cave) {
    ...
    if (!is_cave) {
        ...
        hero_row = center_r;
        hero_col = center_c;
        terrain[(int)hero_row][(int)hero_col] = TERRAIN_GRASS;
        ...
        // Place hero at random open spot
        hero_row = rand() % rows;
        hero_col = rand() % cols;
        while (terrain[(int)hero_row][(int)hero_col] == TERRAIN_WALL) {
            hero_row = rand() % rows;
            hero_col = rand() % cols;
        }
    ...
    for (int i = 0; i < num_enemies; i++) {
        float r = rand() % rows;
        float c = rand() % cols;
        while (!is_land(r, c, terrain, rows, cols) || dist(r, c, hero_row, hero_col) < 5) {
            r = rand() % rows;
            c = rand() % cols;
        }
        enemies[i].row = r;
    ...
    for (int i = 0; i < num_items; i++) {
        float r = rand() % rows;
        float c = rand() % cols;
        while (!is_land(r, c, terrain, rows, cols) || find_enemy_at(r, c) >= 0 || dist(r, c, hero_row, hero_col) < 1) {
            r = rand() % rows;
            c = rand() % cols;
        }
        items[i].row = r;
...
bool is_land(float fr, float fc, int terrain[][MAP_COLS], int rows, int cols) {
    int r = (int)fr, c = (int)fc;
    if (!is_valid_tile(r, c, rows, cols)) return false;
    int t = terrain[r][c];
    return t != TERRAIN_OCEAN && t != TERRAIN_MOUNTAIN && t != TERRAIN_WALL;
}

void reveal_around(float fr, float fc, int radius, bool explored[][MAP_COLS], int rows, int cols) {
    int center_r = (int)fr, center_c = (int)fc;
    for (int dr = -radius; dr <= radius; dr++) {
        for (int dc = -radius; dc <= radius; dc++) {
            int nr = center_r + dr, nc = center_c + dc;
...
int find_enemy_at(float fr, float fc) {
    int r = (int)fr, c = (int)fc;
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].active && (int)enemies[i].row == r && (int)enemies[i].col == c) {
            return i;
        }
    }
    return -1;
...
int find_item_at(float fr, float fc) {
    int r = (int)fr, c = (int)fc;
    for (int i = 0; i < MAX_ITEMS; i++) {
        if (items[i].active && (int)items[i].row == r && (int)items[i].col == c) {
            return i;
        }
    }
    return -1;
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (key == 'p' || key == 'P') use_potion();
    if (key == ' ') slash_attack();
    if (key == 'b' || key == 'B') shoot_arrow();
}
...
void special(int key, int x, int y) {
    if (key < 0 || key >= 200) return;
    special_pressed[key] = true;
    switch (key) {
        case GLUT_KEY_UP: set_status_message("Moving up"); break;
        case GLUT_KEY_DOWN: set_status_message("Moving down"); break;
        case GLUT_KEY_LEFT: set_status_message("Moving left"); break;
        case GLUT_KEY_RIGHT: set_status_message("Moving right"); break;
    }
}
...
void special_up(int key, int x, int y) {
    ...
    set_status_message("Stopped moving");
}
...
void update_hero() {
    float dr = 0, dc = 0;
    if (key_pressed['w'] || special_pressed[GLUT_KEY_UP]) dr -= SPEED;
    if (key_pressed['s'] || special_pressed[GLUT_KEY_DOWN]) dr += SPEED;
    if (key_pressed['a'] || special_pressed[GLUT_KEY_LEFT]) dc -= SPEED;
    if (key_pressed['d'] || special_pressed[GLUT_KEY_RIGHT]) dc += SPEED;

    // Normalize diagonal movement
    float len = sqrt(dr * dr + dc * dc);
    if (len > 0) {
        dr = (dr / len) * SPEED;
        dc = (dc / len) * SPEED;
    }

    // Update facing direction (prioritize last key pressed)
    if (dr > 0) facing = 0; // down
    else if (dr < 0) facing = 1; // up
    else if (dc < 0) facing = 2; // left
    else if (dc > 0) facing = 3; // right

    float nr = hero_row + dr, nc = hero_col + dc;
    ...
    if (is_land(nr, hero_col, terrain, rows, cols)) hero_row = nr;
    if (is_land(hero_row, nc, terrain, rows, cols)) hero_col = nc;

    reveal_around(hero_row, hero_col, 3, explored, rows, cols);

    int tr = (int)hero_row, tc = (int)hero_col;
    ...
    // Update camera
    camera_row = hero_row - VIEW_ROWS / 2.0f;
    camera_col = hero_col - VIEW_COLS / 2.0f;
...
void update_enemies() {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        float d = dist(enemies[i].row, enemies[i].col, hero_row, hero_col);
        if (d < VIEW_RANGE) {
            float dr = hero_row - enemies[i].row;
            float dc = hero_col - enemies[i].col;
            float norm = sqrt(dr*dr + dc*dc);
            if (norm > 0) {
                dr /= norm;
                dc /= norm;
            }
            float nr = enemies[i].row + dr * SPEED * 0.8f;
            float nc = enemies[i].col + dc * SPEED * 0.8f;
            int rows = (state == OVERWORLD) ? MAP_ROWS : CAVE_ROWS;
            int cols = (state == OVERWORLD) ? MAP_COLS : CAVE_COLS;
            int (*terrain)[MAP_COLS] = (state == OVERWORLD) ? terrain_overworld : terrain_cave;
            if (is_land(nr, enemies[i].col, terrain, rows, cols)) enemies[i].row = nr;
            if (is_land(enemies[i].row, nc, terrain, rows, cols)) enemies[i].col = nc;
        }
    }
}
...
void update_projectiles() {
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        ...
        projectiles[i].row += projectiles[i].dr * PROJECTILE_SPEED;
        projectiles[i].col += projectiles[i].dc * PROJECTILE_SPEED;
        int enemy_index = find_enemy_at(projectiles[i].row, projectiles[i].col);
        if (enemy_index >= 0) {
        ...
        int cols = (state == OVERWORLD) ? MAP_COLS : CAVE_COLS;
        int (*terrain)[MAP_COLS] = (state == OVERWORLD) ? terrain_overworld : terrain_cave;
        if (!is_land(projectiles[i].row, projectiles[i].col, terrain, rows, cols)) projectiles[i].active = 0;
    }
}
...
void display() {
    ...
    // === GAME GRID ===
    for (int r = floor(camera_row); r < camera_row + VIEW_ROWS && r < rows; r++) {
        for (int c = floor(camera_col); c < camera_col + VIEW_COLS && c < cols; c++) {
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Zelda-like: Emoji Edition");
