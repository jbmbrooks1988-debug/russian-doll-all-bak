...
int day = 1;
int money = 50;
int age = 0;  // increases every 5 days
int items_food = 3;
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        // Convert y from top-down window coordinates to bottom-up OpenGL
...
void display() {
    ...
    float btn_colors[4][3] = {
        {0.2f, 0.6f, 0.2f},  // Feed
        {0.2f, 0.2f, 0.8f},  // Rest
        {0.8f, 0.4f, 0.0f},  // Train
        {0.6f, 0.2f, 0.6f}   // Clean/Scold
    };
    ...
    if (debug_ui_enabled && show_cursor) {
        float cx = cursor_col * (window_width / GRID_COLS);
        float cy = tile_area_top - (cursor_row + 1) * TILE_SIZE;
        glLineWidth(3.0f);
...
int main(int argc, char** argv) {
    ...
    printf("Controls:\n");
    printf(" - F: Feed | R: Rest | T: Cycle Training Focus\n");
    printf(" - C: Clean | S: Scold | B: Buy Food ($20)\n");
    printf(" - Enter: End Day\n");
    printf(" - Click buttons or use keys!\n");
