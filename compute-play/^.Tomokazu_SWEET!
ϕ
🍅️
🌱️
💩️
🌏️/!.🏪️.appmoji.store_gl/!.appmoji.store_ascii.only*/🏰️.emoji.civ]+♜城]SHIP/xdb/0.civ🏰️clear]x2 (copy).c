...
// --- Emoji Rendering (Fixed State Management) ---
void render_emoji(unsigned int codepoint, float x, float y) {
    ...
    // Save OpenGL state
    GLboolean was_blend_enabled;
    glGetBooleanv(GL_BLEND, &was_blend_enabled);

    GLboolean was_texture_2d_enabled;
    glGetBooleanv(GL_TEXTURE_2D, &was_texture_2d_enabled);
    ...
    // Enable blending and texture
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glEnable(GL_TEXTURE_2D);

    // Use full white to preserve emoji color
    ...
    // Restore state
    if (!was_blend_enabled) glDisable(GL_BLEND);
    if (!was_texture_2d_enabled) glDisable(GL_TEXTURE_2D);
...
void render_text(const char* str, float x, float y) {
    ...
    glColor4fv(current_color); // Restore
}
...
void generate_island_map() {
    ...
    float radius_sq = (GRID_ROWS / 3.0f) * (GRID_COLS / 3.0f) * 2.0f;

    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            float dist = (r - center_r) * (r - center_r) + (c - center_c) * (c - center_c);
            map[r][c] = (dist < radius_sq && rand() % 100 < 80) ? 1 : 0;
        }
    }
...
bool is_land(int r, int c) {
    return is_valid_tile(r, c) && map[r][c] == 1;
}
...
void end_turn() {
    ...
    gold += city_count;
    char msg[128];
    snprintf(msg, sizeof(msg), "Turn %d. Gold: %d. Cities: %d", turn, gold, city_count);
...
void try_move_or_found(int row, int col) {
    if (!is_land(row, col)) {
        set_status_message("Can only act on land!");
        return;
    }
    ...
    if (dr + dc != 1) {
        set_status_message("Move only one tile orthogonally!");
        return;
    }

    if (city_map[row][col]) {
        set_status_message("City already exists there!");
        return;
    }

    // Prompt: Found city?
    printf("Found city at (%d,%d)? (y/n): ");
    // For now: auto-found (simulate player choice)
    bool found = true;
...
// --- Input ---
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && has_settler) {
        ...
        if (settler_row == row && settler_col == col) {
            set_status_message("Selected settler. Click adjacent tile to move/found.");
            return;
        }
...
void display() {
    ...
    float ocean_color[3] = {0.0f, 0.3f, 0.7f};
...
int main(int argc, char** argv) {
    ...
    printf("Controls:\n");
    printf(" - Click adjacent tile to move or found city\n");
