...
#define ENEMY_PROJECTILE_SPEED 0.4f
...
// Camera variables
float x = 0.0f, y = 1.0f, z = 5.0f;  // Lowered player height
float yaw = -90.0f;
float pitch = 0.0f;
...
void draw_hud() {
    ...
    for (int i = 0; i < NUM_ENEMIES; i++) {
        ...
        gluProject(enemies[i].x, enemies[i].y + 1.0f, enemies[i].z,
        ...
        glVertex2f(winX - 20, winY - 5);
        glVertex2f(winX - 20 + 40 * health_ratio, winY - 5);
        glVertex2f(winX - 20 + 40 * health_ratio, winY);
        glVertex2f(winX - 20, winY);
        glEnd();
    }
...
void draw_gun() {
    glColor3f(0.0f, 0.5f, 1.0f);  // Blueish for watergun
    glPushMatrix();
    glTranslatef(0.3f, -0.3f, -0.8f);  // Position relative to camera (right, down, forward)
    glScalef(0.1f, 0.1f, 0.6f);  // Rectangular barrel shape
    glutSolidCube(1.0f);
    glPopMatrix();
}
...
void display() {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        if (projectiles[i].active) {
            ...
            glutSolidSphere(PROJECTILE_RADIUS, 16, 16);  // Smaller projectile
            glPopMatrix();
        }
    }
    ...
    for (int i = 0; i < MAX_ENEMY_PROJECTILES; i++) {
        if (enemy_projectiles[i].active) {
            ...
            glutSolidSphere(PROJECTILE_RADIUS, 16, 16);  // Use same radius
            glPopMatrix();
        }
    }

    // Draw the watergun-like rectangular barrel
    draw_gun();
...
void shoot() {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        if (!projectiles[i].active) {
            // Spawn at muzzle
            projectiles[i].x = x + front_x * spawn_forward + right_x * spawn_right + 0.0f * spawn_down;
            projectiles[i].y = y + front_y * spawn_forward + right_y * spawn_right + 1.0f * spawn_down;
            projectiles[i].z = z + front_z * spawn_forward + right_z * spawn_right + 0.0f * spawn_down;
...
void enemy_shoot(int enemy_idx) {
    ...
    for (int i = 0; i < MAX_ENEMY_PROJECTILES; i++) {
        if (!enemy_projectiles[i].active) {
            enemy_projectiles[i].x = enemies[enemy_idx].x;
            enemy_projectiles[i].y = enemies[enemy_idx].y;
            enemy_projectiles[i].z = enemies[enemy_idx].z;
