...
void draw_hud() {
    ...
    // Enemy health bars (2D overlay above enemies)
    for (int i = 0; i < NUM_ENEMIES; i++) {
        ...
        glBegin(GL_QUADS);
        glColor3f(1.0f - health_ratio, health_ratio, 0.0f); // Red to green
...
void update_enemies() {
    ...
    for (int i = 0; i < NUM_ENEMIES; i++) {
        ...
        if (dist > 1.0f) { // Stop if too close
            dx /= dist;
            dz /= dist;
...
void check_collisions() {
    ...
    for (int i = 0; i < MAX_PROJECTILES; i++) {
        ...
        for (int j = 0; j < NUM_ENEMIES; j++) {
            ...
            float min_distance = 0.1f + enemies[j].radius;
    ...
    for (int i = 0; i < MAX_ENEMY_PROJECTILES; i++) {
        ...
        float distance = sqrt(dx * dx + dy * dy + dz * dz);
        float min_distance = 0.1f + 0.5f; // Player radius ~0.5
