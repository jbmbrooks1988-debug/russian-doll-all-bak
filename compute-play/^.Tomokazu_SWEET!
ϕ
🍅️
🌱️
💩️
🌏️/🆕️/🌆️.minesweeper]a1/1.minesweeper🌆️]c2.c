...
void mouse(int button, int state, int x, int y) {
    ...
    // Ignore clicks in bottom UI panel
    if (y > window_height - 80) return;

    int col = (x - 20) / TILE_SIZE;
    int row = (window_height - y - 80) / TILE_SIZE;
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            float y = window_height - (r + 1) * TILE_SIZE - 80;
