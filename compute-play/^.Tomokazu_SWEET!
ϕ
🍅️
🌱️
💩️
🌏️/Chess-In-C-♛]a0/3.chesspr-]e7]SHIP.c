...
struct termios old_tio; // Global to access in signal handler

void display(int cursor_row, int cursor_col, int selected_piece, int orig_row, int orig_col);
void change(int r1, int c1, int r2, int c2, int piece);
void pawn(int, int);
void rook(int, int);
void horse(int, int);
void camel(int, int);
...
int main()
{
    ...
    while (1) // Run indefinitely until Ctrl+C
    {
        x++;
        system("cls || clear");
    ...
    // Unreachable due to infinite loop, cleanup handled by signal_handler
    return 0;
}
...
void change(int r1, int c1, int r2, int c2, int piece)
{
    board[r1][c1] = ' '; // Clear original position
    board[r2][c2] = piece; // Place piece at destination
}
...
void pawn(int r1, int c1)
{
    ...
    else
    {
        if (r1 + 1 <= 7 && board[r1 + 1][c1] == ' ')
            printf("%d%d, ", r1 + 1, c1);
        if (r1 + 1 <= 7 && c1 + 1 <= 7 && check(r1 + 1, c1 + 1) == 1)
            printf("%d%d*, ", r1 + 1, c1 + 1);
        if (r1 + 1 <= 7 && c1 - 1 >= 0 && check(r1 + 1, c1 - 1) == 1)
            printf("%d%d*, ", r1 + 1, c1 - 1);
    }
}
...
void pawnb(int r1, int c1)
{
    ...
    else
    {
        if (r1 - 1 >= 0 && board[r1 - 1][c1] == ' ')
            printf("%d%d, ", r1 - 1, c1);
        if (r1 - 1 >= 0 && c1 - 1 >= 0 && check2(r1 - 1, c1 - 1) == 1)
            printf("%d%d*, ", r1 - 1, c1 - 1);
        if (r1 - 1 >= 0 && c1 + 1 <= 7 && check2(r1 - 1, c1 + 1) == 1)
            printf("%d%d*, ", r1 - 1, c1 + 1);
    }
}
...
void select_destination(int *row, int *col, int orig_row, int orig_col, int piece, int *cancelled)
{
    ...
    *cancelled = 0;
    ...
    while (1)
    {
        ...
        {
            printf("[Debug: Destination selected at (%d,%d)]\n", r, c);
            *row = r;
            *col = c;
            return;
        }
...
again1:
    ...
    printf("\n[Debug: Entering destination selection]\n");
    ...
    // Relaxed validation for debugging (allow any move)
    printf("Moved ");
    print_unicode(piece);
    printf(" to (%d,%d)\n", r2, c2);
    change(r1, c1, r2, c2, piece);
}
