...
int window_height = GRID_ROWS * TILE_SIZE + 150;
...
void render_string(float x, float y, const char* text, float color[3]) {
    ...
    while (*p) {
        unsigned int codepoint;
        int char_len = decode_utf8((const unsigned char*)p, &codepoint);
        p += char_len;
        if (FT_Load_Char(text_face, codepoint, FT_LOAD_RENDER)) continue;
        FT_GlyphSlot slot = text_face->glyph;
        if (slot->bitmap.width == 0 || slot->bitmap.rows == 0) {
            current_x += (slot->advance.x >> 6);
            continue;
        }
        float pen_x = current_x + slot->bitmap_left;
        float pen_y = y - slot->bitmap_top + 20; // Adjust for baseline
        glRasterPos2f(pen_x, pen_y);
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        ...
        for (int i = 0; i < MAX_ITEMS; i++) {
            if (items[i].active && items[i].row == nr && items[i].col == nc) {
                ...
                if (items[i].type == 0) {
                    ...
                    sprintf(eng, "(Got extra bomb! Max bombs: %d)");
                } else {
                    player_flame_power++;
                    sprintf(chn, "获得火焰升级！火焰范围：%d (Flame - 火焰)", player_flame_power);
                    sprintf(eng, "(Got flame upgrade! Flame range: %d)");
                }
                set_status_message(chn, eng);
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            float x = c * TILE_SIZE, y = window_height - (r + 1) * TILE_SIZE - 50;
            float bg_color[3];
            switch (tiles[r][c]) {
    ...
    // === DYNAMIC OBJECTS ===
    float px = player_col * TILE_SIZE, py = window_height - (player_row + 1) * TILE_SIZE - 50;
    ...
    for (int i = 0; i < MAX_BOMBS; i++) {
        if (bombs[i].active) {
            float bx = bombs[i].col * TILE_SIZE, by = window_height - (bombs[i].row + 1) * TILE_SIZE - 50;
    ...
    for (int i = 0; i < MAX_BOMBS * 5; i++) {
        if (explosions[i].active) {
            float ex = explosions[i].col * TILE_SIZE, ey = window_height - (explosions[i].row + 1) * TILE_SIZE - 50;
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].active) {
            float ex = enemies[i].col * TILE_SIZE, ey = window_height - (enemies[i].row + 1) * TILE_SIZE - 50;
    ...
    for (int i = 0; i < MAX_ITEMS; i++) {
        if (items[i].active) {
            float ix = items[i].col * TILE_SIZE + 8, iy = window_height - (items[i].row + 1) * TILE_SIZE - 42;
            float icol[3];
            if (items[i].type == 0) {
    ...
    draw_rect(0, 0, window_width, 150, panel_color);
    render_string(10, 120, "炸弹人 (Bomberman)", white);
    render_string(10, 90, status_message, white);
    render_string(10, 70, english_status, white);
    ...
    render_string(10, 40, chn_stats, white);
    render_string(10, 20, eng_stats, white);
    glutSwapBuffers();
}
