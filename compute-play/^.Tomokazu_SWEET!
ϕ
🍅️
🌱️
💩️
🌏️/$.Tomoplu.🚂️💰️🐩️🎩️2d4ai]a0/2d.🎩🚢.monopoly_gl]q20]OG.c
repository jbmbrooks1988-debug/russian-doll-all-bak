...
void display() {
    ...
    for (int i = 0; i < game_state.num_players; i++) {
        ...
        float text_width = get_string_width(info, strlen(info));
        // Draw highlight for current player
        if (game_state.current_turn == i) {
            float sel_color[3] = {0.0f, 1.0f, 0.0f};
            // Position rectangle below text to avoid overlap
            draw_rect(panel_x - 5, py - font_size - 2, text_width + 10, font_size + 4, sel_color);
        }
