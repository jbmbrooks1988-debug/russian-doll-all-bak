...
// Space structure
typedef struct {
    char name[10]; // Abbreviated name for ASCII display
...
void init_board(Space* board) {
    ...
    // 4: Income Tax
    strcpy(board[4].name, "Inc Tax");
    board[4].type = TYPE_TAX;
    board[4].price = 200;

    // 5: Reading Railroad
    strcpy(board[5].name, "Read RR");
    ...
    // 12: Electric Company
    strcpy(board[12].name, "Elec Co");
    board[12].type = TYPE_UTILITY;
    board[12].price = 150;
    board[12].mortgage = 75;

    // 13: States Avenue
    strcpy(board[13].name, "Sta Ave");
    ...
    // 20: Free Parking
    strcpy(board[20].name, "Parking");
    board[20].type = TYPE_FREE_PARKING;

    // 21: Kentucky Avenue
    strcpy(board[21].name, "Ken Ave");
    ...
    // 28: Water Works
    strcpy(board[28].name, "Wat Wks");
    board[28].type = TYPE_UTILITY;
    board[28].price = 150;
    board[28].mortgage = 75;

    // 29: Marvin Gardens
    strcpy(board[29].name, "Mar Gdn");
    ...
    // 30: Go to Jail
    strcpy(board[30].name, "To Jail");
    board[30].type = TYPE_GO_TO_JAIL;

    // 31: Pacific Avenue
    strcpy(board[31].name, "Pac Ave");
    ...
    // 38: Luxury Tax
    strcpy(board[38].name, "Lux Tax");
    board[38].type = TYPE_TAX;
    board[38].price = 100;

    // 39: Boardwalk
    strcpy(board[39].name, "Boardwk");
        ...
        for (int line = 0; line < CELL_HEIGHT; line++) {
            ...
            for (int j = 0; j < GRID_SIZE; j++) {
                ...
                if (i == 0 || i == GRID_SIZE - 1 || j == 0 || j == GRID_SIZE - 1) {
                    ...
                    int p = -1;
                    if (i == 0) p = 10 - j;
                    else if (i == GRID_SIZE - 1) p = 30 + j;
                    else if (j == 0) p = 20 - i + 10;
                    else if (j == GRID_SIZE - 1) p = 40 - i;

                    if (p >= 0 && p < NUM_SPACES) {
                        if (line == 0 && game_state.board[p].type == TYPE_PROPERTY) {
                            // Color bar for properties
                            printf("%s%.*s%s", game_state.board[p].color, CELL_WIDTH, "█████", ANSI_RESET);
                        } else if (line == 1) {
                            // Space name
                            printf("%s%*s%s", game_state.board[p].color, CELL_WIDTH, game_state.board[p].name, ANSI_RESET);
                        } else if (line == 2) {
                            // Player tokens and price
                            char tokens[10] = "";
                            int token_count = 0;
                            for (int pl = 0; pl < game_state.num_players; pl++) {
                                if (game_state.players[pl].position == p && token_count < 3) {
                                    strcat(tokens, game_state.players[pl].emoji);
                                    token_count++;
                                }
                            }
                            if (game_state.board[p].type == TYPE_PROPERTY || game_state.board[p].type == TYPE_RAILROAD || game_state.board[p].type == TYPE_UTILITY || game_state.board[p].type == TYPE_TAX) {
                                char price[10];
                                snprintf(price, sizeof(price), "$%d", game_state.board[p].price);
                                printf("%s%*s%s", game_state.board[p].color, CELL_WIDTH, tokens[0] ? tokens : price, ANSI_RESET);
                            } else {
                                printf("%*s", CELL_WIDTH, tokens);
                            }
                        }
                    } else {
                        printf("%*s", CELL_WIDTH, "");
