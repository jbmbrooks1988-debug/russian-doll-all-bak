...
void render_3d() {
    ...
    glLoadIdentity();
    gluLookAt(0, 0, cam_z, 0, 0, 0, 0, 1, 0); // Use cam_z for zoom
            ...
            for (int j = 0; j < global_m->visual_count; j++) {
                if (strcmp(global_m->elements[i].name, global_m->visuals[j].name) == 0 && strcmp(global_m->visuals[j].shape, "point") == 0) {
                    ...
                    if (valid_particles > 0) {
                        ...
                        render_text(win_x, 600 - win_y + i * 15, global_m->elements[i].name); // Offset by i*15 to avoid overlap
                        glMatrixMode(GL_PROJECTION);
                        glPopMatrix();
                        glMatrixMode(GL_MODELVIEW);
                        glPopMatrix();
...
void keyboard_func(unsigned char key, int x, int y) {
    switch (key) {
        case '+':
            cam_z -= 0.5;
            if (cam_z < 2.0) cam_z = 2.0; // Min zoom
            printf("Zoom in: cam_z = %.1f\n", cam_z);
            glutPostRedisplay();
            break;
        case '-':
            cam_z += 0.5;
            if (cam_z > 20.0) cam_z = 20.0; // Max zoom
...
int main(int argc, char* argv[]) {
    ...
    if (m->render_3d) {
        ...
        glutIdleFunc(idle);
        glutKeyboardFunc(keyboard_func); // Register keyboard callback
