...
typedef struct {
    ...
    int sub_stage; // Sub-stage for Stage 5 (-1 for any)
...
typedef struct {
    ...
    char prerequisite[32]; // Tech required to unlock
    int unlocked;
} Tech;

typedef struct {
    char name[32];
    char shape[16]; // sphere, point, or city
...
typedef struct {
    ...
    int universes;      // 🌌 宇宙
    int stage;          // 🕰️ 阶段
    int sub_stage;      // Sub-stage for Stage 5 (0: Stone, 1: Bronze, 2: Iron, 3: Industrial, 4: Information, 5: Space)
...
// Global variables
Multiverse* global_m;
float cam_z = 10.0; // Camera z-position for zoom
...
void industrial_revolution_effect(void* m_ptr) {
    Multiverse* m = (Multiverse*)m_ptr;
    m->technology += 1.0;
    m->matter -= 1e4; // Industrialization consumes resources
...
Event event_list[] = {
    ...
    {"Alien Contact", 0.02, 5, 4, alien_contact_effect}, // Information Age
    {"Industrial Revolution", 0.04, 5, 3, industrial_revolution_effect} // Industrial Age
};
        ...
        case 5: // Humanoid Civilizations
            ...
            switch (sub_stage) {
                ...
                case 2: // Iron Age
                    ...
                    m->technology = 2.0;
                    break;
                case 3: // Industrial Age
                    m->matter = 1e5;
                    m->technology = 5.0;
                    break;
                case 4: // Information Age
                    m->matter = 1e5;
                    m->technology = 10.0;
                    break;
                case 5: // Space Age
...
void update_multiverse(Multiverse* m) {
    ...
    } else if (m->stage == 1) { // Matter Formation
        if (m->matter >= 1e5) {
            m->matter -= 1e5;
            for (int i = 0; i < m->element_count; i++) {
                if (strcmp(m->elements[i].name, "H2") == 0 || strcmp(m->elements[i].name, "O2") == 0) {
    ...
    } else if (m->stage == 2) { // Chemical Evolution
        form_reactions(m);
        Element* water = NULL;
        for (int i = 0; i < m->element_count; i++) {
            if (strcmp(m->elements[i].name, "Water") == 0) water = &m->elements[i];
    ...
    } else if (m->stage == 3) { // Cellular Life
        if (m->cells >= 1e2) {
            m->cells -= 1e2;
            creatures_delta += 10;
    ...
    } else if (m->stage == 4) { // Complex Creatures
        if (m->creatures >= 10) {
            m->creatures -= 10;
            humanoids_delta += 1;
    ...
    } else if (m->stage == 5) { // Humanoid Civilizations
        if (m->humanoids > 0) {
            humanoids_delta += 0.1; // Population growth
            technology_delta += 0.1;
            matter_delta -= 100; // Resource consumption
            if (matter_delta < -m->matter) matter_delta = -m->matter; // Prevent negative matter

            // Sub-stage progression
            if (m->sub_stage == 0 && m->technology >= 1.0) { // Stone to Bronze
                m->sub_stage = 1;
                printf("Advancing to Bronze Age 🗡️: Tools and trade emerge!\n");
            } else if (m->sub_stage == 1 && m->technology >= 2.0) { // Bronze to Iron
                m->sub_stage = 2;
                printf("Advancing to Iron Age ⚔️: Stronger weapons and structures!\n");
            } else if (m->sub_stage == 2 && m->technology >= 5.0) { // Iron to Industrial
                m->sub_stage = 3;
                printf("Advancing to Industrial Age 🏭: Factories and mass production!\n");
            } else if (m->sub_stage == 3 && m->technology >= 10.0) { // Industrial to Information
                m->sub_stage = 4;
                printf("Advancing to Information Age 💾: Computers and networks!\n");
            } else if (m->sub_stage == 4 && m->technology >= 20.0) { // Information to Space
                m->sub_stage = 5;
                printf("Advancing to Space Age 🚀: Preparing for interstellar travel!\n");
            } else if (m->sub_stage == 5 && m->technology >= 30.0) { // Space Age to Stage 6
                m->stage = 6;
                printf("Space travel achieved! 🚀\n");
            }
        }
    } else if (m->stage == 6) { // Space Travel
        if (m->technology >= 1) {
            m->technology -= 1;
            m->universes += 1;
            energy_delta += 1e10;
...
void render_3d() {
    ...
    // Render cities for civilizations (Stage 5)
    if (global_m->stage == 5) {
        for (int i = 0; i < global_m->visual_count; i++) {
            if (strcmp(global_m->visuals[i].name, "City") == 0 && strcmp(global_m->visuals[i].shape, "city") == 0) {
                ...
                glTranslatef(0, 0, 0);
                glColor3f(global_m->visuals[i].r, global_m->visuals[i].g, global_m->visuals[i].b);
                glutSolidCube(global_m->visuals[i].size * global_m->humanoids); // Scale with humanoids
                glPopMatrix();
            }
        }
    }

    // Render elements as particles with labels
    glPointSize(2.0);
    glBegin(GL_POINTS);
            ...
            for (int j = 0; j < global_m->visual_count; j++) {
                if (strcmp(global_m->elements[i].name, global_m->visuals[j].name) == 0 && strcmp(global_m->visuals[j].shape, "point") == 0) {
                    ...
                    if (valid_particles > 0) {
                        ...
                        render_text(win_x, 600 - win_y + i * 15, global_m->elements[i].name);
                        glMatrixMode(GL_PROJECTION);
                        glPopMatrix();
                        glMatrixMode(GL_MODELVIEW);
                        glPopMatrix();
    ...
    sprintf(buffer, "Zoom: %.1f (use +,-)", cam_z);
    render_text(10, 320, buffer);
    if (global_m->stage == 5) {
        switch (global_m->sub_stage) {
            case 0: sprintf(buffer, "Sub-stage: Stone Age 🪨"); break;
            case 1: sprintf(buffer, "Sub-stage: Bronze Age 🗡️"); break;
            case 2: sprintf(buffer, "Sub-stage: Iron Age ⚔️"); break;
            case 3: sprintf(buffer, "Sub-stage: Industrial Age 🏭"); break;
            case 4: sprintf(buffer, "Sub-stage: Information Age 💾"); break;
            case 5: sprintf(buffer, "Sub-stage: Space Age 🚀"); break;
        }
        render_text(10, 300, buffer);
    }
    glMatrixMode(GL_PROJECTION);
    glPopMatrix();
    glMatrixMode(GL_MODELVIEW);
...
void timer_func(int value) {
    update_multiverse(global_m); // Update state every timer tick
    glutPostRedisplay(); // Force redraw
    glutTimerFunc(global_m->debug_mode ? 100 : 1000, timer_func, 0); // 100ms or 1000ms
}

void idle() {
    // Simplified idle to rely on timer_func for updates
