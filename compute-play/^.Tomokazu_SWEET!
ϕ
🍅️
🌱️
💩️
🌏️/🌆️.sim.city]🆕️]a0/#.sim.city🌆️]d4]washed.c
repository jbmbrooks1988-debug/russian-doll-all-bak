...
void render_emoji(unsigned int codepoint, float x, float y) {
    ...
    if (!entry) {
        if (cache_size >= NUM_EMOJIS) return;
...
void place_tile(int row, int col, int type) {
    ...
    propagate_power();  // Update power immediately after placement
    glutPostRedisplay();
}
...
void display() {
    ...
    glDisable(GL_TEXTURE_2D); // Ensure clean state
    glDisable(GL_DEPTH_TEST); // Disable depth test for 2D

    float tile_area_top = window_height - MENU_HEIGHT - 80;

    // Debug: Test emoji rendering
    unsigned int test_code;
    decode_utf8((const unsigned char*)EMOJI_ROAD, &test_code);
    render_emoji(test_code, 50, window_height - 50);
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            // Draw power overlay before emoji if unpowered
