...
// --- FreeType Init (EXACTLY as you had it) ---
void initFreeType() {
...
// --- Emoji Rendering with Caching (ORIGINAL, just fixed state) ---
void render_emoji(unsigned int codepoint, float x, float y) {
    ...
    // ✅ Clean OpenGL state before using textures
    ...
    // ✅ Critical: Cleanup after drawing emoji
    glBindTexture(GL_TEXTURE_2D, 0);
    glDisable(GL_TEXTURE_2D); // <-- This was missing → caused bleed!
}

// --- Text Rendering ---
void render_text(const char* str, float x, float y) {
    // Ensure no texture interference
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        // ✅ CORRECTED Y MAPPING: bottom-up UI offset
        int col = (x - 10) / TILE_SIZE;
        int row = (y - 80) / TILE_SIZE;          // Subtract UI height
...
// --- Drawing ---
void draw_rect(float x, float y, float w, float h, float color[3], float alpha) {
    glDisable(GL_TEXTURE_2D); // Prevent bleed
    glDisable(GL_BLEND);
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            render_emoji(code, cx, cy);  // Now safe + visible
        }
    }
