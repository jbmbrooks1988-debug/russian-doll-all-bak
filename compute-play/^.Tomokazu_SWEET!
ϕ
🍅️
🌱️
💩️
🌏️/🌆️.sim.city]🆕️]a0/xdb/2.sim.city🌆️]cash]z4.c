...
// Emojis (will render as Unicode if supported by font)
...
// --- FreeType Init with Fallback Fonts ---
void initFreeType() {
    ...
    if (!font_loaded) {
        fprintf(stderr, "ERROR: No suitable font found. Install 'fonts-dejavu-core' or similar.\n");
        exit(1);
    }
    ...
    fprintf(stderr, "Font loaded. Size: %d px, Scale: %.2f\n", loaded_size, emoji_scale);
}

// --- Emoji Rendering with Caching (Safe Grayscale) ---
void render_emoji(unsigned int codepoint, float x, float y) {
    ...
    if (!entry) {
        ...
        for (int i = 0; i < width * height; ++i) {
            unsigned char alpha = (slot->bitmap.num_grays > 1) ? src[i] : (src[i] ? 255 : 0);
            rgba_data[i*4+0] = 255;
            rgba_data[i*4+1] = 255;
            rgba_data[i*4+2] = 255;
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        float grid_y = window_height - y - 80;
        int row = (GRID_ROWS * TILE_SIZE - grid_y) / TILE_SIZE;
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            switch (tiles[r][c]) {
                ...
                default: emoji = EMOJI_EMPTY; color[0]=0.3f; color[1]=0.3f; color[2]=0.3f;
            }
    ...
    render_text("Enter: Place | Space: Advance Month | T: Tools | 0-9: Quick Tools", 10, 30);
...
void idle() {
    glutPostRedisplay(); // Optional: keep animating
}
...
int main(int argc, char** argv) {
    ...
    printf(" - 0-9: Select tool (0=bulldoze, 1=res, etc.)\n");
    printf(" - Enter: Place tile or query\n");
    printf(" - Space: Advance month\n");
    printf(" - T: Open tools menu\n");
    printf(" - P: Toggle power overlay\n");
    printf(" - C: Toggle cursor\n");

    atexit(cleanup_cache);  // Safe cleanup when program exits
