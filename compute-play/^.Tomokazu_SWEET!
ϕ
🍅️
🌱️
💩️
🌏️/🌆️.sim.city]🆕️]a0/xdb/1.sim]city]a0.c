...
const char* EMOJI_CURSOR = "🔍";  // Not really used, but for potential
...
void propagate_power() {
    ...
    // Find power plants and propagate (simple flood fill for demo)
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            if (tiles[r][c] == TILE_POWER_PLANT) {
                // Propagate in a radius for simplicity
...
void simulate_growth() {
    // Simple growth: Upgrade low res to high if powered and near roads
    int new_pop = 0;
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            if (tiles[r][c] == TILE_RES_LOW && powered[r][c]) {
                ...
                new_pop += 5;  // Jobs attract pop
            } else if (tiles[r][c] == TILE_INDUSTRIAL && powered[r][c]) {
                new_pop += 10;
            }
        }
    }
    population += new_pop / 10;  // Scale down
}
...
void place_tile(int row, int col, int type) {
    ...
    int cost = 0;
    switch (type) {
        case 0: cost = 10; break;  // Bulldoze cost
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        float grid_y = window_height - y - 80;  // Adjust for UI
...
void select_tool(int choice) {
    if (choice >= 0 && choice <= 9) {
        current_tool = (choice == 0) ? 0 : choice;  // 0 is bulldoze, others match tile types
...
void display() {
    ...
    draw_rect(0, window_height - MENU_HEIGHT, window_width, MENU_HEIGHT, menu_bg);
    ...
    if (show_tools_menu) {
        ...
        draw_rect(mx, my, 300, 200, (float[]){0.1f, 0.1f, 0.3f});
