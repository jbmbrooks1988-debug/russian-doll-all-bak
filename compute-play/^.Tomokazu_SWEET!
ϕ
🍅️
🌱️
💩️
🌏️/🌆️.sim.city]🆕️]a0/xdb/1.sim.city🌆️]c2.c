...
void render_emoji(unsigned int codepoint, float x, float y) {
    ...
    glColor4f(1.0f, 1.0f, 1.0f, 1.0f); // Full opacity for texture
...
void display() {
    ...
    for (int r = 0; r < GRID_ROWS; r++) {
        for (int c = 0; c < GRID_COLS; c++) {
            ...
            draw_rect(x, y, TILE_SIZE, TILE_SIZE, color, 0.8f); // Semi-transparent

            unsigned int code;
            decode_utf8((const unsigned char*)emoji, &code);
            fprintf(stderr, "Rendering emoji %s at (%d,%d) with codepoint %u\n", emoji, r, c, code);
            ...
            if (debug_power_enabled && !powered[r][c] && tiles[r][c] != TILE_EMPTY && tiles[r][c] != TILE_WATER) {
                ...
                // Fallback: Render tile type as text
                // render_text(tool_options[tiles[r][c]], x + 5, y + 15);
            }
        }
    }
