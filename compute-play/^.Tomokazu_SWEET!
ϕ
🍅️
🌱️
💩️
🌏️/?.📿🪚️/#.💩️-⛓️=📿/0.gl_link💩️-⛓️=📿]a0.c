...
int window_width = CANVAS_COLS * TILE_SIZE + SIDEBAR_WIDTH + 20;
int window_height = CANVAS_ROWS * TILE_SIZE + FILE_TAB_HEIGHT + 20;
...
void display() {
    ...
    draw_rect(10, window_height - FILE_TAB_HEIGHT - 10, 60, FILE_TAB_HEIGHT, tab_color);
    draw_rect(80, window_height - FILE_TAB_HEIGHT - 10, 60, FILE_TAB_HEIGHT, tab_color);
    render_text("Save", 20, window_height - FILE_TAB_HEIGHT + 5);
    render_text("Load", 90, window_height - FILE_TAB_HEIGHT + 5);

    // Draw sidebar (left side, below file tab, shifted down by one tile)
    float sidebar_x = 10;
    float sidebar_y_offset = FILE_TAB_HEIGHT + 10 + TILE_SIZE; // Shifted down by TILE_SIZE (32 pixels)
    for (int i = 0; i < NUM_EMOJIS; i++) {
        float x = sidebar_x + 10;
        float y = window_height - sidebar_y_offset - 20 - i * 40;
        unsigned int codepoint;
    ...
    // Draw tools with grey background
    const char *tools[] = {"Paint", "Fill", "Rect"};
    for (int i = 0; i < 3; i++) {
        float x = sidebar_x + 10;
        float y = window_height - sidebar_y_offset - 350 - i * 30;
        // Draw grey background (60x30, like Save/Load)
        draw_rect(x - 5, y - 5, 60, 30, tab_color);
        // Draw text centered in the button
        render_text(tools[i], x, y + 10); // Shift text up to center in 30px height
        if (i == selected_tool) {
            float sel_color[3] = {1.0f, 1.0f, 0.0f};
            glLineWidth(2.0f);
            glColor3fv(sel_color);
            glBegin(GL_LINE_LOOP);
            glVertex2f(x - 5, y - 5);  // Match full 60x30 button size
            glVertex2f(x + 55, y - 5);
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        // File tab click
        else if (x >= 10 && x < 70 && gl_y >= window_height - FILE_TAB_HEIGHT - 10 && gl_y < window_height - 10) {
            save_canvas();
        } else if (x >= 80 && x < 140 && gl_y >= window_height - FILE_TAB_HEIGHT - 10 && gl_y < window_height - 10) {
            load_canvas();
        }
        // Sidebar click
        else if (x >= 20 && x < 60 && gl_y >= window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - NUM_EMOJIS * 40 && gl_y < window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE)) {
            int idx = (window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - gl_y) / 40;
            if (idx >= 0 && idx < NUM_EMOJIS) {
                selected_emoji = idx;
                set_status_message("Emoji selected");
            }
        } else if (x >= 70 && x < 110 && gl_y >= window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - NUM_COLORS * 40 && gl_y < window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE)) {
            int idx = (window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - gl_y) / 40;
            if (idx >= 0 && idx < NUM_COLORS) {
                selected_fg_color = idx;
                set_status_message("Color selected");
            }
        } else if (x >= 120 && x < 160 && gl_y >= window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - tab_count * 40 && gl_y < window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE)) {
            int idx = (window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - gl_y) / 40;
        ...
        } else if (x >= 15 && x < 75 && gl_y >= window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - 355 - 3 * 30 && gl_y < window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - 325) {
            int idx = (window_height - (FILE_TAB_HEIGHT + 10 + TILE_SIZE) - gl_y - 340) / 30;
            if (idx >= 0 && idx < 3) {
                selected_tool = idx;
                start_row = -1;
                start_col = -1;
