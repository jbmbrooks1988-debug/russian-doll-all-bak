...
static void generate_world(bool is_cave) {
    ...
    if (!is_cave) {
        ...
        // Place hero near center to ensure visibility
        center_r = rows / 2;
        center_c = cols / 2;
...
void init(void) {
    glEnable(GL_DEPTH_TEST); // Enabled for potential 3D use, disabled in view_gl.c for 2D
    generate_world(false);
}
...
void set_entity_action(int entity_index, int action_id, float value) {
    ...
    if (action_id == 'w' || action_id == 'W' || action_id == GLUT_KEY_UP + 256) {
        ...
        switch (facing) {
            case 0: ty -= TILE_SIZE; break;
            case 1: ty += TILE_SIZE; break;
            case 2: tx -= TILE_SIZE; break;
            case 3: tx += TILE_SIZE; break;
        }
        ...
        if (num_entities < MAX_ENTITIES) {
            ...
            switch (facing) {
                case 0: entities[num_entities].dr = -1; break;
                case 1: entities[num_entities].dr = 1; break;
                case 2: entities[num_entities].dc = -1; break;
                case 3: entities[num_entities].dc = 1; break;
            }
            num_entities++;
