...
Window x_window;
static int render_mode = 0; // 0: 2D, 1: 3D
...
int get_score(void);
void toggle_render_mode(void);
void view_keyboard(unsigned char key, int x, int y);
...
void render_emoji(unsigned int codepoint, float x, float y, float z) {
    ...
    float x2 = x - w / 2;
    float y2 = y - h / 2;
    printf("Rendering emoji U+%04X at %s (%.1f, %.1f, %.1f), size (%.1f, %.1f)\n", 
           codepoint, render_mode ? "3D" : "2D", x2, y2, z, w, h);
    glPushMatrix();
    glTranslatef(0, 0, z);
    glBegin(GL_QUADS);
    glTexCoord2f(0.0, 1.0); glVertex3f(x2, y2, 0);
    glTexCoord2f(1.0, 1.0); glVertex3f(x2 + w, y2, 0);
    glTexCoord2f(1.0, 0.0); glVertex3f(x2 + w, y2 + h, 0);
    glTexCoord2f(0.0, 0.0); glVertex3f(x2, y2 + h, 0);
    glEnd();
    glPopMatrix();
...
void display(void) {
    ...
    if (render_mode == 0) { // 2D mode
        ...
        glLoadIdentity();
        float center_x = cam_x;
        float center_y = cam_y;
        gluLookAt(center_x, center_y, 100.0, // Camera above grid
                  center_x, center_y, 0.0,   // Look at grid center
                  0.0, 1.0, 0.0);           // Up vector
    }
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // Draw background rectangle (only in 2D mode)
    if (render_mode == 0) {
    ...
    for (int r = 0; r < view_rows; r++) {
        for (int c = 0; c < view_cols; c++) {
            ...
            float x, y;
            if (render_mode == 0) {
                x = 10 + c * tile_size + tile_size / 2;
                y = height - 50 - (r + 1) * tile_size + tile_size / 2;
            } else {
                x = (world_col * tile_size) + tile_size / 2;
                y = (world_row * tile_size) + tile_size / 2;
            }
            ...
            if (type >= 0) {
                ...
                printf("Rendering tile type %d at %s (%.1f, %.1f, 0.0), world (%d, %d)\n", 
                       type, render_mode ? "3D" : "2D", x, y, world_col, world_row);
                render_emoji(codepoint, x, y, 0);
            }
        }
    }
    ...
    for (int i = 0; i < get_num_entities(); i++) {
        ...
        if (z != 0) continue; // Skip 3D entities for now
        ...
        if (abs(entity_row - hero_row) <= view_rows / 2 && abs(entity_col - hero_col) <= view_cols / 2) {
            if (render_mode == 0) {
                ...
                x = (entity_col * tile_size) + tile_size / 2;
                y = (entity_row * tile_size) + tile_size / 2;
            }
            unsigned int codepoint;
            decode_utf8((const unsigned char*)get_entity_emoji(get_entity_type(i)), &codepoint);
            printf("Rendering entity %d (type %d) at %s (%.1f, %.1f, %.1f), world (%d, %d)\n", 
                   i, get_entity_type(i), render_mode ? "3D" : "2D", x, y, z, entity_col, entity_row);
            render_emoji(codepoint, x, y, z);
        }
    }
