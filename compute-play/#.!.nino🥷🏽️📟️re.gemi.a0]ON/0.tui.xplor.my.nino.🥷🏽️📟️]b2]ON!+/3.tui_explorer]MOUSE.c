...
void disableRawMode() {
    write(STDOUT_FILENO, "\x1b[?1000l", 6); // Disable mouse reporting
...
void enableRawMode() {
    ...
    write(STDOUT_FILENO, "\x1b[?1000h", 6); // Enable mouse reporting
}
...
void process_keypress() {
    ...
    if (c == CTRL_KEY('q')) {
        ...
        if (seq[0] == '[') {
            if (seq[1] == 'M') { // Mouse event
                char button, x, y;
                if (read(STDIN_FILENO, &button, 1) != 1) return;
                if (read(STDIN_FILENO, &x, 1) != 1) return;
                if (read(STDIN_FILENO, &y, 1) != 1) return;

                if (x-32 < AppState.explorer_width) {
                    int clicked_item = y-32-1 + AppState.explorer_scroll_offset;
                    if (clicked_item >= 0 && clicked_item < AppState.explorer_entry_count) {
                        if (AppState.explorer_selected_item == clicked_item) {
                             ExplorerEntry *entry = &AppState.explorer_entries[clicked_item];
                            if (!entry->is_dir) {
                                open_file_in_new_tab(entry->name);
                            }
                        } else {
                            AppState.explorer_selected_item = clicked_item;
                        }
                    }
                }
            } else {
