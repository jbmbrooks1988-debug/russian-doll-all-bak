    ...
    for (int i = 0; i < num_instructions; i++) {
        ...
        if (chip_location == 0) { // No logic
            ...
            if (is_blank_a && !is_blank_b && input_b != -1) {
                ...
                output_value = input_a; // Prefer input_a if input_b is blank
            } else if (!is_blank_a && !is_blank_b && input_a != -1) {
                // Both non-blank, prefer input_a and warn about input_b
                output_value = input_a;
                warn_input_b = (raw_input_b == 0);
                printf("Warning: Both inputs non-blank at instruction %d, using input_a (%d). ", i, input_a);
                if (raw_input_b != 2 && raw_input_b != 3) {
                    printf("input_b (%d) should be blank (2 or 3).\n", raw_input_b);
                } else {
                    printf("input_b is blank but both inputs provided.\n");
                }
            } else {
