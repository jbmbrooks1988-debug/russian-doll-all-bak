...
// Load stock data from locations.txt or data/ directory
void load_stock_data() {
    ...
    // If locations.txt is not available or empty, scan data/ directory
    if (!use_locations) {
        ...
        while ((entry = readdir(dir)) != NULL && i < num_stocks) {
            if (strstr(entry->d_name, ".txt")) {
                snprintf(ptr, MAX_PATH, "data/%s ", entry->d_name);
                ptr += strlen(ptr);
                i++;
            }
        }
    ...
    while (token && i < num_stocks) {
        // Extract name from path (strip .txt and directories)
