...
void simulate_trading(int stock_idx, StockState* state) {
    ...
    for (int r = 0; r < NUM_ROUNDS; r++) {
        ...
        for (int week = start_week; week <= end_week && week < data_lengths[stock_idx]; week++) {
            ...
            state->round_percent_gain_loss[r] = (portfolio - round_start_cash[r]) / round_start_cash[r] * 100.0;
        }
    ...
    // Update previous_final_cash for the next stock
    previous_final_cash = state->cash + (state->shares * stocks[stock_idx][data_lengths[stock_idx] - 1]);
}
...
void draw_chart(int stock_idx, int points_to_show, StockState* state) {
    ...
    float overall_percent = (portfolio - state->starting_cash) / state->starting_cash * 100.0;
