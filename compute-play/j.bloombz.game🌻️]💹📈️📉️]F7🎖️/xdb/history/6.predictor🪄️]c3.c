...
typedef struct {
    ...
    float round_start_cash[NUM_ROUNDS];
} StockState;
    ...
    while ((entry = readdir(dir)) != NULL && i < num_stocks) {
        ...
        if (!ticker_file) {
            ...
            for (int j = 0; j < NUM_ROUNDS; j++) {
                ...
                states[i].round_start_cash[j] = INITIAL_CASH;
            }
            i++;
            continue;
        }
...
void simulate_trading(int stock_idx, StockState* state) {
    ...
    for (int r = 0; r < NUM_ROUNDS; r++) {
        state->round_start_cash[r] = state->cash + state->shares * (r > 0 ? stocks[stock_idx][round_starts[r] - 2] : stocks[stock_idx][0]);
        ...
        for (int week = start_week; week <= end_week && week < data_lengths[stock_idx]; week++) {
            ...
            state->round_percent_gain_loss[r] = (portfolio - state->round_start_cash[r]) / state->round_start_cash[r] * 100.0;
        }
...
void draw_chart(int stock_idx, int points_to_show, StockState* state) {
    ...
    // Display starting portfolio for each round
    for (int r = 0; r < NUM_ROUNDS; r++) {
        sprintf(price_text, "Round %d Start: $%.2f", r + 1, state->round_start_cash[r]);
        draw_text(price_text, x_start, 180 - r * 20);
    }
