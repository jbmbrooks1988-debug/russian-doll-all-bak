...
typedef struct {
    ...
    float starting_cash; // Store the starting cash for this stock
} StockState;
...
float previous_final_cash = INITIAL_CASH; // Tracks final portfolio of previous stock
...
void load_stock_data() {
    ...
    while ((entry = readdir(dir)) != NULL && i < num_stocks) {
        ...
        if (!ticker_file) {
            ...
            stocks[i] = NULL;
            seed_data[i] = NULL;
            states[i].cash = previous_final_cash;
            states[i].starting_cash = previous_final_cash;
            states[i].shares = 0.0;
...
float compute_moving_average(int stock_idx, int week, int window) {
    if (week < window || week >= data_lengths[stock_idx]) return stocks[stock_idx][week];
    float sum = 0.0;
...
void simulate_trading(int stock_idx, StockState* state) {
    float round_start_cash[NUM_ROUNDS]; // Track cash at start of each round
    state->cash = state->starting_cash; // Use carried-over cash
    state->shares = 0.0;

    for (int r = 0; r < NUM_ROUNDS; r++) {
        round_start_cash[r] = state->cash; // Record cash at start of round
        ...
        for (int week = start_week; week <= end_week && week < data_lengths[stock_idx]; week++) {
            if (week >= 9) { // Need enough data for 10-week MA
                ...
                if (state->shares == 0 && prev_short_ma <= prev_long_ma && short_ma > long_ma && state->cash > 0) {
            ...
            // Calculate round percent gain/loss
            float portfolio = state->cash + (state->shares * stocks[stock_idx][week]);
            state->round_percent_gain_loss[r] = round_start_cash[r] > 0 ? 
                ((portfolio - round_start_cash[r]) / round_start_cash[r] * 100.0) : 0.0;
        }
...
void write_rounds_file() {
    ...
    fprintf(fp, "ticker,round,week,trend_prediction,round_percent_gain_loss,starting_cash,final_cash\n");
    for (int i = 0; i < num_stocks; i++) {
        float final_portfolio = data_lengths[i] > 0 ? 
            (states[i].cash + (states[i].shares * stocks[i][data_lengths[i] - 1])) : states[i].cash;
        if (data_lengths[i] == 0) {
            fprintf(fp, "%s,0,0,None,0.00,%.2f,%.2f\n", stock_names[i], states[i].starting_cash, final_portfolio);
            continue;
        }
...
void draw_chart(int stock_idx, int points_to_show, StockState* state) {
    ...
    sprintf(price_text, "Starting: $%.2f | Final: $%.2f", state->starting_cash, portfolio);
    ...
    float overall_percent = state->starting_cash > 0 ? 
        ((portfolio - state->starting_cash) / state->starting_cash * 100.0) : 0.0;
...
int main(int argc, char** argv) {
    ...
    previous_final_cash = INITIAL_CASH; // Reset for first stock
    for (int i = 0; i < num_stocks; i++) {
        states[i].starting_cash = previous_final_cash; // Set starting cash before simulation
        states[i].cash = previous_final_cash;
        if (data_lengths[i] > 0) {
            simulate_trading(i, &states[i]);
            // Update previous_final_cash after simulation
            previous_final_cash = states[i].cash + 
                (states[i].shares * stocks[i][data_lengths[i] - 1]);
        } else {
            previous_final_cash = states[i].cash; // No change for invalid stocks
        }
    }
    write_rounds_file();
