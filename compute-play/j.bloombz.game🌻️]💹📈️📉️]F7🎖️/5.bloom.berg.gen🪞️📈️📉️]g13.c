...
char** seed_data; // Dynamic array of seed data
...
float total_cash = 1000.0; // Tracks cumulative portfolio
float initial_total_cash = 1000.0; // Initial portfolio value
...
void load_stock_data() {
    ...
    // Allocate memory for stocks, names, lengths, and seed data
...
void display() {
    ...
    // Always draw stock name above price axis
    char stock_text[50];
    sprintf(stock_text, "Stock: %s", stock_names[current_stock]);
    draw_text(stock_text, window_width * 0.1, window_height * 0.9 + 20);
    ...
    if (!animation_running && !full_revealed) {
        ...
        // Display seed data below final score
        draw_text(seed_data[current_stock], 100, 80);
        // Display percent gain/loss for this round
        float round_percent = (portfolio - (cash + shares * stocks[current_stock][0])) / (cash + shares * stocks[current_stock][0]) * 100.0;
        char round_text[50];
        sprintf(round_text, "Round Gain/Loss: %.2f%%", round_percent);
        draw_text(round_text, 100, 60);
        // Display overall percent gain/loss
        float overall_percent = (total_cash - initial_total_cash) / initial_total_cash * 100.0;
        char overall_text[50];
        sprintf(overall_text, "Overall Gain/Loss: %.2f%%", overall_percent);
        draw_text(overall_text, 100, 40);
        ...
        if (state == GLUT_DOWN) {
            ...
            if (!animation_running && !full_revealed) {
                ...
                if (x >= 500 && x <= 700 && y >= 50 && y <= 100) {
                    // Update cash to reflect final portfolio value before starting new round
                    if (shares > 0) {
                        float current_price = stocks[current_stock][data_lengths[current_stock] - 1];
                        cash = shares * current_price;
                        shares = 0.0;
                    }
                    total_cash = cash; // Update total_cash for overall tracking
                    full_revealed = 0;
                    animation_running = 0;
...
void timer(int value) {
    if (animation_running && current_step < data_lengths[current_stock]) {
        ...
        // Update total_cash for overall gain/loss
        total_cash = cash;
        glutPostRedisplay();
    }
}
...
void draw_chart(int stock_idx, int points_to_show) {
    ...
    for (int i = 0; i < num_trades; i++) {
        ...
        float start_price = trades[i].start_price;
    ...
    // Labels for axes
    char week_text[50];
    sprintf(week_text, "Week #%d", current_step);
