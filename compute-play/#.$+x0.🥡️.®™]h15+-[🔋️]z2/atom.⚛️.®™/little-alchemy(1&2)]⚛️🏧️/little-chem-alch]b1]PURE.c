...
// Maximum limits (expanded for full periodic table + compounds)
...
void load_elements(const char *filename, int cheat_mode) {
    ...
    while (fgets(line, sizeof(line), file) && element_count < MAX_ELEMENTS) {
        ...
        if (sscanf(line, "%19s %d %d %d %d %d", element_names[element_count],
            ...
            // Inventory logic
            if (cheat_mode && inventory_count < MAX_INVENTORY) {
                inventory[inventory_count++] = element_count; // All elements in cheat mode
            } else if (combo1 == -1 && combo2 == -1 && inventory_count < MAX_INVENTORY) {
                inventory[inventory_count++] = element_count; // Starting elements otherwise
            }
            element_count++;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1 && strcmp(argv[1], "--cheat") == 0) {
        cheat_mode = 1;
        printf("Cheat mode enabled: Starting with all elements.\n");
    }

    load_elements("elements.txt", cheat_mode);
    if (element_count == 0) {
