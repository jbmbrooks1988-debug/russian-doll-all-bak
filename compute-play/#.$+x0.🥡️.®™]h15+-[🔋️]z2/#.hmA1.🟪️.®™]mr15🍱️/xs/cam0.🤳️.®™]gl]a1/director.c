...
#define TIMEOUT_MS 100 // Aligned with PURE.c
...
void load_modules() {
    DIR *dir = opendir("./+x");
    if (!dir) {
        debug_log("Failed to open +x directory");
        return;
    }
    module_count = 0;
    struct dirent *entry;
    while ((entry = readdir(dir)) && module_count < MAX_ACTIONS) {
        if (strstr(entry->d_name, ".+x")) {
            snprintf(modules[module_count][0], MAX_LINE, "%d", module_count + 1);
            snprintf(modules[module_count][1], MAX_LINE, "./+x/%s", entry->d_name);
            module_count++;
        }
    }
    closedir(dir);
    debug_log("Loaded +x modules");
}
...
int check_submenu(int pipe_fd, pid_t pid, const char *exec) {
    ...
    if (waitpid(pid, &status, WNOHANG) == pid) {
        snprintf(msg, sizeof(msg), "Child PID %d exited early with status %d", pid, WEXITSTATUS(status));
        debug_log(msg);
        return 0;
    }

    // Use short timeout like PURE.c
    fd_set read_fds;
    FD_ZERO(&read_fds);
    FD_SET(pipe_fd, &read_fds);
    struct timeval timeout = { .tv_sec = 0, .tv_usec = TIMEOUT_MS * 1000 }; // 100ms
    ...
    // Output detected, assume submenu
