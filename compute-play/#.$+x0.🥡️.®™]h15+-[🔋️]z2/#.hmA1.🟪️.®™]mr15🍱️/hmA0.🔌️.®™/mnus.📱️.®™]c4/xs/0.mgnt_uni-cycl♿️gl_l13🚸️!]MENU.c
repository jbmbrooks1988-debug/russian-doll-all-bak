...
int read_command(int in_submenu, int at_main_menu, int prompt_row, char *result_buffer, size_t result_size) {
    ...
    static int enter_pressed = 0; // ADDED: Track if Enter was pressed
            ...
            if (bytes_read > 0) {
                ...
                if (fp) {
                    ...
                    if (key == 10 || key == 13) {
                        write(STDOUT_FILENO, "\n", 1);
                        if (buffer_pos > 0) {
                            enter_pressed = 1; // ADDED: Flag Enter was pressed
    ...
    if (current_line_count > file_processed_lines) {
        ...
        while (fgets(line, MAX_LINE, fp)) {
            ...
            enter_pressed = 0; // ADDED: Reset enter_pressed for each line
            ...
            if (strcmp(line, "q") == 0 || strcmp(line, "Q") == 0) {
                ...
                if (buffer_pos > 0) {
                    ...
                    enter_pressed = 1; // ADDED: Flag Enter was processed
                    buffer_pos = 0;
                    input_buffer[0] = '\0';
                    write(STDOUT_FILENO, "\n", 1);
    ...
    if (in_submenu && enter_pressed) return 1; // ADDED: Return 1 to indicate Enter was pressed in submenu
    enter_pressed = 0; // ADDED: Reset enter_pressed
    return -1;
}
    ...
    while (child_running) {
        ...
        if (select_result > 0) {
            if (FD_ISSET(pipefd_out[0], &read_fds)) {
                ...
                if (bytes_read > 0) {
                    output_pos += (int)bytes_read; // ADDED: Cast to int for safety
        ...
        if (result == -3 || result == -2) { // Handle 'q' and Ctrl+C
            ...
            break;
        } else if (result == 1) { // ADDED: Handle Enter pressed
            write(pipefd_in[1], "\n", 1);
            debug_log("Sent newline to submenu child process");
