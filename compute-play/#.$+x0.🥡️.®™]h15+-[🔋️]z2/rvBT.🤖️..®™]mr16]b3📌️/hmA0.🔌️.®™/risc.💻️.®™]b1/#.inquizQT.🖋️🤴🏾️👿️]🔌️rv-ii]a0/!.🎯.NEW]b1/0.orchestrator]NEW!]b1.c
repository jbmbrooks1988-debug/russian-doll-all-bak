...
int write_inputs(const char* input_file, int state[], char* signal_names[], int state_count, const char* netlist_file, int cycle, int* netlist_idx) {
    FILE* fp = fopen(netlist_file, "r");
    if (!fp) {
        printf("ERROR: Failed to open %s\n", netlist_file);
        return 1;
    ...
    // Count netlist lines to wrap cycles
    int total_lines = 0;
    char line[MAX_LINE];
    while (fgets(line, MAX_LINE, fp)) {
        total_lines++;
...
int update_state(int state[], char* signal_names[], int state_count) {
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        printf("Usage: %s <netlist_file> <num_cycles>\n", argv[0]);
        return 1;
    }
    ...
    for (int cycle = 0; cycle < num_cycles; cycle++) {
        ...
        int netlist_idx;
        if (write_inputs("tmp/input.txt", state, signal_names, state_count, netlist_file, cycle, &netlist_idx)) {
        ...
        snprintf(command, MAX_LINE, "./+x/main.+x %s tmp/output.txt", tmp_netlist);
        ...
        if (update_state(state, signal_names, state_count)) {
