...
int load_input(const char* hash, unsigned char* state, int* state_count, int expected_lines) {
    ...
    int lines = 0;
    *state_count = 0;
    while (fgets(buffer, 10, src) && *state_count < MAX_STATE && lines < expected_lines) {
...
// Create an intermediate file duplicating a single input
int create_duplicate_input_file(const char* file, const char* output, unsigned char* state, int* state_count) {
    FILE* f = fopen(file, "r");
    if (!f) {
        printf("ERROR: Failed to open %s\n", file);
        return 1;
    ...
    if (fgets(buffer, 10, f)) {
        buffer[strcspn(buffer, "\n")] = '\0';
        val = (strcmp(buffer, "0") == 0 || strcmp(buffer, "1") == 0) ? atoi(buffer) : 0;
    }
    fprintf(out, "%d\n%d\n", val, val);
    if (*state_count < MAX_STATE) {
        state[*state_count] = val;
    ...
    fclose(f);
    fclose(out);
    printf("DEBUG: Created %s with %d,%d from %s\n", output, val, val, file);
    return 0;
}
...
void substitute_io_names(const char* line, char* result, const char* hash, char* tmp_files[], int* tmp_count, const char* subhash_a, const char* subhash_b, const char* subhash_2_3, int* tmp2_index, int* tmp3_index) {
    if (*tmp_count >= MAX_TMP_FILES) {
        printf("ERROR: tmp_files array full (%d)\n", *tmp_count);
        result[0] = '\0';
        return;
    }
    const char* pos = line;
    ...
    while (*pos) {
        ...
        } else if (strncmp(pos, "OUTPUT", 6) == 0 && strncmp(pos, "OUTPUT_", 7) != 0) {
...
void sync_state_to_files(const char* hash, char* tmp_files[], int tmp_count, unsigned char* state) {
    for (int i = 0; i < tmp_count && i < MAX_TMP_FILES; i++) {
        if (tmp_files[i]) {
...
int execute_step(const char* line, const char* hash, char* tmp_files[], int* tmp_count, unsigned char* state, int step, FILE* visual_mem_fp, char* last_output_file, int* tmp2_index, int* tmp3_index) {
    if (*tmp_count >= MAX_TMP_FILES) {
        printf("ERROR: tmp_files array full (%d) at step %d\n", *tmp_count, step);
        fprintf(visual_mem_fp, "\nERROR: tmp_files array full at step %d\n", step);
        return 1;
    }
    ...
    if (strstr(ptr, "TMP_A_TMP1") && *tmp_count >= 1) {
        ...
        if (*tmp_count < MAX_TMP_FILES) {
            ...
            printf("ERROR: tmp_files array full after creating %s\n", intermediate_file);
            return 1;
        }
    ...
    } else if (strstr(ptr, "TMP4 TMP4") && *tmp_count >= 1) {
        generate_hash(subhash_a);
        snprintf(intermediate_file, MAX_LINE, "tmp/tmp4_dup.%s.%s.txt", hash, subhash_a);
        input_file1 = tmp_files[*tmp_count - 1];
        if (!input_file1) {
            printf("ERROR: tmp_files[%d] is NULL for TMP4\n", *tmp_count - 1);
            return 1;
        }
        if (create_duplicate_input_file(input_file1, intermediate_file, state, tmp_count)) {
            printf("ERROR: Failed to create %s\n", intermediate_file);
            return 1;
        }
        if (*tmp_count < MAX_TMP_FILES) {
    ...
    } else if (strstr(ptr, "TMP_A_TMP1 TMP_B_TMP1") && *tmp_count >= 1) {
        generate_hash(subhash_a);
        snprintf(intermediate_file, MAX_LINE, "tmp/tmp_ab_tmp1.%s.%s.txt", hash, subhash_a);
        input_file1 = "tmp/input.txt";
        input_file2 = tmp_files[0];
        if (!input_file2) {
            printf("ERROR: tmp_files[0] is NULL for TMP_A_TMP1 TMP_B_TMP1\n");
            return 1;
        }
        if (create_intermediate_file(input_file1, input_file2, intermediate_file, state, tmp_count)) {
            printf("ERROR: Failed to create %s\n", intermediate_file);
    ...
    } else if (strstr(ptr, "TMP3 TMP4 TMP6") && *tmp_count >= 1) {
        generate_hash(subhash_a);
        snprintf(intermediate_file, MAX_LINE, "tmp/tmp3_tmp4.%s.%s.txt", hash, subhash_a);
        input_file1 = tmp_files[*tmp3_index];
        input_file2 = tmp_files[*tmp_count - 1]; // TMP4
        if (!input_file1 || !input_file2) {
            printf("ERROR: tmp_files[tmp3_index=%d]='%s' or tmp_files[%d]='%s' is NULL for TMP3 TMP4\n",
                   *tmp3_index, input_file1 ? input_file1 : "NULL", *tmp_count - 1, input_file2 ? input_file2 : "NULL");
            return 1;
        }
        if (create_intermediate_file(input_file1, input_file2, intermediate_file, state, tmp_count)) {
            printf("ERROR: Failed to create %s\n", intermediate_file);
    ...
    if (command[0] == '\0') {
        printf("ERROR: Failed to substitute IO names at step %d\n", step);
        return 1;
    }
    ...
    } else if (strstr(ptr, "TMP4 TMP4")) {
        snprintf(adjusted_command, MAX_LINE, "./+x/nand.+x %s %s", intermediate_file, tmp_files[*tmp_count - 1]);
        strcpy(command, adjusted_command);
        printf("DEBUG: Adjusted command: %s\n", command);
    } else if (strstr(ptr, "TMP_A_TMP1 TMP_B_TMP1")) {
        snprintf(adjusted_command, MAX_LINE, "./+x/nand.+x %s %s", intermediate_file, tmp_files[*tmp_count - 1]);
        strcpy(command, adjusted_command);
        printf("DEBUG: Adjusted command: %s\n", command);
    } else if (strstr(ptr, "TMP3 TMP4 TMP6")) {
        snprintf(adjusted_command, MAX_LINE, "./+x/nand.+x %s %s", intermediate_file, tmp_files[*tmp_count - 1]);
        strcpy(command, adjusted_command);
        printf("DEBUG: Adjusted command: %s\n", command);
    }
    ...
    if (input_file && output_file && strstr(input_file, ".txt")) {
        ...
        printf("ERROR: Invalid command format at step %d: %s\n", step, command);
        free(tokens);
        return 1;
    }
    free(tokens);
...
int process_netlist(FILE* netlist_fp, const char* hash, char* tmp_files[], int* tmp_count, unsigned char* state, int* state_count, FILE* visual_mem_fp, char* last_output_file, int* tmp2_index, int* tmp3_index, int* step, int expected_lines) {
    char line[MAX_LINE];
    while (fgets(line, MAX_LINE, netlist_fp)) {
        ...
        if (strncmp(line, "INCLUDE ", 8) == 0) {
            ...
            if (process_netlist(include_fp, hash, tmp_files, tmp_count, state, state_count, visual_mem_fp, last_output_file, tmp2_index, tmp3_index, step, expected_lines)) {
                fclose(include_fp);
                return 1;
            }
            fclose(include_fp);
...
int execute_cycle(FILE* netlist_fp, char* last_output_file, long cycle, unsigned char* state, int* state_count, int expected_lines) {
    ...
    if (load_input(hash, state, state_count, expected_lines)) {
    ...
    if (process_netlist(netlist_fp, hash, tmp_files, &tmp_count, state, state_count, visual_mem_fp, last_output_file, &tmp2_index, &tmp3_index, &step, expected_lines)) {
...
int main(int argc, char* argv[]) {
    ...
    if (strstr(netlist_file, "full_adder") || strstr(netlist_file, "include_test")) {
        expected_lines = 3;
    }
    ...
    if (max_cycles > 0 || infinite) {
        ...
        while (running && (infinite || cycles < max_cycles)) {
            printf("Cycle %ld\n", cycles);
            if (execute_cycle(netlist_fp, last_output_file, cycles, state, &state_count, expected_lines)) {
    ...
    if (last_output_file[0]) {
        ...
        // Handle OUTPUT_COUT for Full Adder
        if (strstr(netlist_file, "full_adder")) {
            char cout_file[MAX_LINE];
            snprintf(cout_file, MAX_LINE, "tmp/output_cout.%s.txt", last_output_file + strlen("tmp/output."));
            printf("DEBUG: Copying from %s to test_cout.txt\n", cout_file);
            src = fopen(cout_file, "r");
            dst = fopen("test_cout.txt", "w");
            if (src && dst) {
                char line[10];
                if (fgets(line, 10, src)) {
                    fprintf(dst, "%s", line);
                ...
                printf("DEBUG: Failed to open files for OUTPUT_COUT (src=%p, dst=%p)\n", src, dst);
            }
        }
    }
