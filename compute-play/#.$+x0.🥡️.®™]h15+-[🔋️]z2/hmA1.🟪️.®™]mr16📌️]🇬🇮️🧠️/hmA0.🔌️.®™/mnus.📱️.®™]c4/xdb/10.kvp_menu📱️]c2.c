...
int execute_command(const char *command, int is_submenu) {
    pid_t pid = fork();
    if (pid == -1) {
        perror("Fork failed");
        return 0;
        ...
        // Child process
        if (!is_submenu) {
            // Redirect output to /dev/null for non-submenu commands
        ...
        // Parent process
        if (is_submenu) {
            // Foreground execution for submenu commands
            int status;
            interrupted = 0;
            struct sigaction sa;
            ...
            // Background execution for non-submenu commands
            printf("Running %s in background\n", command);
            return 1; // Don't wait for background process
        }
    }
}
...
int main() {
    ...
    while (1) {
        ...
        if (choice >= 0 && choice < count) {
            printf("Executing: %s\n", values[choice]);
            // Check if command contains "sub_menu" to determine if it's interactive
            int is_submenu = (strstr(values[choice], "sub_menu") != NULL);
            if (!execute_command(values[choice], is_submenu)) {
