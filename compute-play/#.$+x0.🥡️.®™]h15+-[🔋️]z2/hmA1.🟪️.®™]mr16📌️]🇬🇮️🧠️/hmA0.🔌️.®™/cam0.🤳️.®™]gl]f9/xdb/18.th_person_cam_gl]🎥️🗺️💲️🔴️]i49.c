...
char terminal_input_path[MAX_PATH] = "gl_cli_out.txt";
...
void parse_state_file(const char *filename, int is_state_file, int *x, int *y, int *z, int *cam_x, int *cam_y, int *cam_z, int *cursor_pos, char *piece_symbol, int *has_coords) {
    ...
    while (fgets(line, MAX_LINE, fp)) {
        ...
        if (strcmp(key, "color") == 0 && is_state_file) {
            ...
            // Commented out to ignore cam_x, cam_y, cam_z since camera follows player
            /*
...
void display() {
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(45.0, (float)window_width / window_height, 0.1, 100.0);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // First-person POV: Camera at player's position with eye-level offset
    float eye_offset_z = 0.7f; // Eye level above player position
    float look_distance = 1.0f; // Distance to look ahead (along +X axis)
    gluLookAt(
        player_x + 0.5f, player_y + 0.5f, player_z + 0.5f + eye_offset_z, // Camera position
        player_x + 0.5f + look_distance, player_y + 0.5f, player_z + 0.5f + eye_offset_z, // Look-at point
        0, 0, 1 // Up vector
    );
