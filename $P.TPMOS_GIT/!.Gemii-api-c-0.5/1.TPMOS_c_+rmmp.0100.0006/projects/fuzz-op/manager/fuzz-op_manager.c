...
void save_manager_state() {
    ...
    if (f) {
        while (fgets(lines[lc], sizeof(lines[0]), f) && lc < 99) {
            // If active_target_id is 'xlector' and project is 'fuzz-op', change it to 'pet_01'
            if (strncmp(lines[lc], "active_target_id=", 17) == 0) {
                if (strcmp(current_project, "fuzz-op") == 0 && strcmp(active_target_id, "xlector") == 0) {
                    snprintf(lines[lc], sizeof(lines[0]), "active_target_id=%s\n", "pet_01");
                }
            }
            lc++;
        }
                ...
                if (pf) {
                    ...
                    if (fgets(handler, sizeof(handler), pf)) {
                        ...
                        if (strlen(handler) > 0 && strcmp(handler, "NOT_FOUND") != 0) {
                            /* Execute handler - quote everything to handle spaces in project_root */
                            char *quoted_handler = NULL;
                            asprintf(&quoted_handler, "'%s/%s'", project_root, handler);
                            if (quoted_handler) {
                                system(quoted_handler);
                                free(quoted_handler);
                            }
