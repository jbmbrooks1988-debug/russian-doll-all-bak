...
int main() {
    ...
    while (keep_running) {
        ...
        while (keep_running && !action_interrupted) {
            ...
            unlink(tmp_llm);
            char* curl_url = NULL;
            asprintf(&curl_url, "%s/v1beta/models/%s:streamGenerateContent?alt=sse", current_api_url, current_model);

            char* curl_cmd = NULL;
            asprintf(&curl_cmd, "curl -s -N --max-time 120 -H \"Content-Type: application/json\" -H \"%s\" -d @%s \"%s\"", 
                     key_header, tmp_prompt, curl_url);
            ...
            if (action_interrupted) {
                printf("\n\033[91m[Action Cancelled]\033[0m\n");
                if (acc_text) free(acc_text); if (acc_func) free(acc_func);
                break;
            }
