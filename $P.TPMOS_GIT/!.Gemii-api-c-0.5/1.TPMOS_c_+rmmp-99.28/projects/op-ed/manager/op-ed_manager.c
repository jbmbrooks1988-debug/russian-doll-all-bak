...
void read_slot_name() {
    ...
    if (f) {
        char line[MAX_LINE];
        while (fgets(line, sizeof(line), f)) {
            if (line[0] == 's') { /* id="slot_name" -> prefix 's' */
                strncpy(slot_name_buffer, trim_str(line + 1), sizeof(slot_name_buffer) - 1);
            }
        }
        fclose(f);
    }
...
void op_file_save() {
    save_project(); /* Ensure PDL is up to date */
    read_slot_name();
    char *cmd = NULL;
    if (asprintf(&cmd, "'%s/projects/user/ops/+x/file_op.+x' save %s %s > /dev/null 2>&1", 
                 project_root, current_project, slot_name_buffer) != -1) {
        run_command(cmd); free(cmd);
        char msg[MAX_LINE]; snprintf(msg, sizeof(msg), "Saved to %s", slot_name_buffer);
        set_response(msg);
    }
}
...
void op_file_load() {
    read_slot_name();
    char *cmd = NULL;
    if (asprintf(&cmd, "'%s/projects/user/ops/+x/file_op.+x' load %s %s > /dev/null 2>&1", 
                 project_root, current_project, slot_name_buffer) != -1) {
        run_command(cmd); free(cmd);
        load_project_metadata();
        scan_maps();
        set_response("Project Loaded from Slot");
    }
}

void sync_system_project_id() {
    char *path = NULL;
    if (asprintf(&path, "%s/pieces/apps/player_app/manager/state.txt", project_root) != -1) {
        FILE *f = fopen(path, "r");
        char **lines = NULL;
        ...
        if (f) {
            while (fgets(line_buf, sizeof(line_buf), f) && line_count < 99) {
                ...
                if (eq) {
                    *eq = '\0';
                    if (strcmp(trim_str(line_buf), "project_id") == 0) {
                        asprintf(&lines[line_count], "project_id=%s\n", current_project);
                        found = 1;
                    } else {
                        *eq = '=';
                        asprintf(&lines[line_count], "%s\n", line_buf);
...
void handle_command(const char* cmd) {
    if (strncmp(cmd, "SET_PROJECT:", 12) == 0) {
        strncpy(current_project, cmd + 12, sizeof(current_project) - 1);
        load_project_metadata();
        scan_maps();
        ...
        sync_current_map();
        update_current_map_dir();
        sync_system_project_id();
        set_response("Project Loaded into Editor");
    }
    else if (strcmp(cmd, "OP:PLAY_MODE") == 0) {
        /* Launch Player Bootstrap */
        char *pb = NULL;
        if (asprintf(&pb, "'%s/projects/op-ed/player/manager/+x/player_bootstrap.+x' &", project_root) != -1) {
            run_command(pb); free(pb);
        }
        
        /* Transition to game layout */
        transition_to_layout("pieces/apps/playrm/layouts/game.chtpm");
        set_response("Launching Player...");
    }
    else if (strcmp(cmd, "OP:FILE_MENU") == 0) {
        char *path = NULL;
        if (asprintf(&path, "%s/pieces/apps/player_app/manager/state.txt", project_root) != -1) {
            char *sc = NULL;
            if (asprintf(&sc, "'%s/pieces/ops/file-op/+x/sync_op.+x' '%s' 'browser_current_dir' 'projects/%s'", project_root, path, current_project) != -1) {
                run_command(sc); free(sc);
            }
            if (asprintf(&sc, "'%s/pieces/ops/file-op/+x/sync_op.+x' '%s' 'return_layout' 'projects/op-ed/layouts/op-ed.chtpm'", project_root, path) != -1) {
                run_command(sc); free(sc);
            }
            free(path);
        }
...
void build_project_markup(char* out, size_t sz) {
    out[0] = '\0';
    for (int i = 0; i < project_count && strlen(out) < sz - 1000; i++) {
        char btn[512];
        int label_len = strlen(projects[i]);
        int padding = 40 - label_len;
        if (padding < 0) padding = 0;
        snprintf(btn, sizeof(btn), "<text label=\"║  \" /><button label=\"%s\" onClick=\"SET_PROJECT:%s\" />", projects[i], projects[i]);
        strcat(out, btn);
        if (padding > 0) {
            strcat(out, "<text label=\"");
            for (int j = 0; j < padding; j++) strcat(out, " ");
            strcat(out, "\" />");
        }
        strcat(out, "<text label=\" ║\" /><br/>\\n");
    }
}
...
void build_slot_markup(char* out, size_t sz) {
    out[0] = '\0';
    resolve_active_user();
    char saves_dir[MAX_PATH];
    snprintf(saves_dir, sizeof(saves_dir), "%s/projects/user/pieces/profiles/%s/saves/%s", project_root, active_user_name, current_project);
    ...
    if (dir) {
        ...
        while ((entry = readdir(dir)) != NULL && strlen(out) < sz - 1000) {
            ...
            if (stat(entry_path, &st) == 0 && S_ISDIR(st.st_mode)) {
                ...
                int padding = 40 - label_len;
                if (padding < 0) padding = 0;
                snprintf(btn, sizeof(btn), "<text label=\"║  \" /><button label=\"%s\" onClick=\"SET_SLOT:%s\" />", entry->d_name, entry->d_name);
                strcat(out, btn);
                if (padding > 0) {
                    strcat(out, "<text label=\"");
                    for (int j = 0; j < padding; j++) strcat(out, " ");
...
void write_gui_state() {
    ...
    if (asprintf(&path, "%s/projects/op-ed/manager/gui_state.txt", project_root) != -1) {
        ...
        if (f) {
            ...
            fprintf(f, "slot_name_input=%s\n", slot_name_buffer);
            
            char proj_markup[16384] = "";
            build_project_markup(proj_markup, sizeof(proj_markup));
            fprintf(f, "project_selection_markup=%s\n", proj_markup);

            char slot_markup[16384] = "";
            build_slot_markup(slot_markup, sizeof(slot_markup));
            ...
            if (asprintf(&resp_path, "%s/pieces/apps/editor/response.txt", project_root) != -1) {
                ...
                if (rf) {
                    if (fgets(raw_resp, sizeof(raw_resp), rf)) trim_str(raw_resp);
                    fclose(rf);
                }
                free(resp_path);
            }
...
void write_editor_state() {
    ...
    if (asprintf(&path, "%s/pieces/apps/player_app/state.txt", project_root) != -1) {
        ...
        fprintf(f, "project_id=%s\ngui_focus=%d\nactive_gui_index=%d\nlast_key=%s\nselector_pos=(%d,%d,%d)\n", current_project, gui_focus_index, gui_focus_index, last_key_str, cursor_x, cursor_y, cursor_z);
        ...
        /* Removed horizontal projects_list_view - now using vertical markup in gui_state.txt */
...
void process_key(int key) {
    ...
    if (key >= '1' && key <= '9') {
        ...
        else if (key == '7') { op_file_save(); }
        else if (key == '3') { op_file_load(); }
        else if (key == '8') { 
        ...
        if (clf) {
            ...
            if (fgets(line, sizeof(line), clf)) {
                if (strstr(line, "load_menu.chtpm")) {
                    if (key == 10 || key == 13) {
                        /* Project buttons start at interactive index 1 */
                        if (gui_focus_index >= 1 && gui_focus_index <= project_count) {
                            project_idx = gui_focus_index - 1;
