...
bool is_modern_layout(const char* layout) {
    ...
    return (strstr(layout, "playrm") != NULL || strstr(layout, "player") != NULL || 
            ...
            strstr(layout, "mp3") != NULL || strstr(layout, "pieces/ops/") != NULL);
}
...
#else
    current_module_pid = -1;  
    if (project_root_path[0] != '\0') _chdir(project_root_path);
    intptr_t h = _spawnv(_P_NOWAIT, args[0], (const char* const*)args);
    if (h != -1) {
        current_module_pid = (int)GetProcessId((HANDLE)h);
        ...
        if (module_name) {
            char* dot = strchr(module_name, '.'); if (dot) *dot = '\0';
            char *pcmd = NULL;
            /* Use start /B to run proc_manager in background on Windows without a new window */
            asprintf(&pcmd, "start \"\" /B \"%s\" register %s %d", pm, module_name, current_module_pid);
            if (pcmd) { system(pcmd); free(pcmd); }
            free(module_name);
        }
        free(pm);
    }
...
void send_command(const char* cmd) {
    ...
    if (strncmp(cmd, "LOAD_PROJECT:", 13) == 0) {
        ...
        /* DECOUPLED: Delegate resolution to external Op (Dumb Theater Mode) */
        char *op_path = build_path_malloc("pieces/chtpm/ops/+x/resolve_project_op.+x");
        char *sys_cmd = NULL;
#ifndef _WIN32
    asprintf(&sys_cmd, "'%s' '%s' &", op_path, proj_name);
    if (sys_cmd) { system(sys_cmd); free(sys_cmd); }
#else
    /* Windows system() uses cmd.exe. Single quotes around paths are NOT supported. 
       Use double quotes. "start" needs an empty title argument first if the path is quoted. */
    asprintf(&sys_cmd, "start \"\" /B \"%s\" \"%s\"", op_path, proj_name);
    if (sys_cmd) { system(sys_cmd); free(sys_cmd); }
#endif
    free(op_path);
        return;
...
void export_active_index() {
    ...
    else if (focus_index >= 0 && focus_index < element_count) active_gui_idx = elements[focus_index].interactive_idx;
