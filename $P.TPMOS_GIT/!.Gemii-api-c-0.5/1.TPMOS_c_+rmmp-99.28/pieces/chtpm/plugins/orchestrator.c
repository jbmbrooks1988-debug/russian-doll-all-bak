...
#ifdef _WIN32
static int win_kill(int pid, int sig) {
...
#else
    intptr_t h; 
    char ocwd[1024]; _getcwd(ocwd, sizeof(ocwd));
    if (project_root[0] != '\0') _chdir(project_root);
    
    /* Use _P_NOWAIT to get a handle for PID logging, then wait */
    if (args && strlen(args) > 0) h = _spawnl(_P_NOWAIT, abs_path, abs_path, args, NULL);
    else h = _spawnl(_P_NOWAIT, abs_path, abs_path, NULL);
    
    if (h != -1 && h != 0) {
        int pid = (int)GetProcessId((HANDLE)h);
        if (pid > 0) log_pid(pid, name);
        
        /* Synchronous wait mirrors Linux waitpid(pid, &status, 0) */
        _cwait(NULL, h, _WAIT_CHILD);
...
#else
    /* Use launch_and_register to ensure proper tracking and blocking */
    launch_and_register("joystick_input", "pieces/joystick/plugins/+x/joystick_input.+x", ".", false);
#endif
    return NULL;
}
