...
def edit_file(filename, search_block, replace_block):
    try:
        ...
        if search_block.strip() not in content: return "Error: Search block not found."
...
# --- UI UTILS ---
messages = [{'role': 'system', 'content': 'You are a terminal coding agent. Use edit_file for existing files.'}]
...
def main():
    ...
    print("\033[94mGrok-TUI Active. Tab for paths. /yolo, /clear, exit\033[0m")
    ...
    while True:
        try:
            ...
            if user_input.lower() == '/yolo': YOLO_MODE = not YOLO_MODE; continue
