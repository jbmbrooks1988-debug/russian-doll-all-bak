...
static void refresh_ui(int pct, const char* line, const char* suggestions, int selected_idx) {
...
static char* get_line_interactive(int pct) {
    ...
    while (keep_running) {
        ...
        refresh_ui(pct, line, current_suggestions, selected_idx);
    }
    if (current_suggestions) free(current_suggestions);
    disable_raw_mode();
    return line;
...
int main() {
    ...
    while (keep_running) {
        ...
        if (strcmp(line, "/summarize") == 0) {
            ...
            if (m_file) { if (fgets(model, sizeof(model), m_file)) model[strcspn(model, "\r\n")] = 0; fclose(m_file); }
            ...
            char* api_path; asprintf(&api_path, "%s/api/chat", current_api_url);
        ...
        if (strcmp(line, "/api") == 0) {
            ...
            enable_raw_mode();
            while (1) {
                refresh_ui(pct, "Select", names, sel);
