...
```
tpmos-cli/
├── agent.c              # Main loop, tool dispatcher (Native & JSON-in-content)
├── tools/
...
├── state/               # Runtime state (context.json, llm_response.json)
└── config/              # Persona, model selection, context limits, and YOLO flags
```
...
- `config/persona.txt`: Define the system prompt.
...
- [x] State entirely in `state/*.json`
