...
void execute_option(DesktopWindow* win, int index) {
    ...
    if (strcmp(win->type, "project_mirror") == 0) {
        ...
        } else if (strcmp(win->project_id, "piececraft-3d") == 0) {
            if (index == 1) { /* Mining */
                char cmd[MAX_PATH];
                snprintf(cmd, sizeof(cmd), "%s/projects/piececraft-3d/ops/+x/mining.+x xlector", project_root);
                system(cmd);
                add_history(win, "Mining block at Xlector position...");
            } else if (index == 2) { /* Placing */
                char cmd[MAX_PATH];
                snprintf(cmd, sizeof(cmd), "%s/projects/piececraft-3d/ops/+x/placing.+x xlector stone", project_root);
                system(cmd);
                add_history(win, "Placing stone at Xlector position...");
...
void create_mirrored_window(const char* project_id, const char* title) {
    ...
    } else if (strcmp(project_id, "piececraft-3d") == 0) {
        windows[idx].option_count = 5;
        strcpy(windows[idx].menu_options[0], "Control Map");
        strcpy(windows[idx].menu_options[1], "Mining");
        strcpy(windows[idx].menu_options[2], "Placing");
