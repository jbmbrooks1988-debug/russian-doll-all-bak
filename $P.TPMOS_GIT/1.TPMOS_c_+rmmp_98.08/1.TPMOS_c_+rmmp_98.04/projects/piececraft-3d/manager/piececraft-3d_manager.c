...
float cam_rot[3] = {-35.0f, 0.0f, 0.0f};
int camera_mode = 4;
...
void route_input(int key) {
    ...
    // Methods (Keys 2-3)
    if (key == '2' || key == 2002) { // Mining
        char op_path[MAX_PATH];
        snprintf(op_path, sizeof(op_path), "%s/projects/%s/ops/+x/mining.+x", project_root, project_id);
        pid_t pid = fork();
        if (pid == 0) {
            execl(op_path, op_path, active_target_id, NULL);
            exit(1);
        } else waitpid(pid, NULL, 0);
        strcpy(last_response, "Mined block.");
        save_manager_state();
        trigger_render();
    } else if (key == '3' || key == 2003) { // Placing
        char op_path[MAX_PATH];
        snprintf(op_path, sizeof(op_path), "%s/projects/%s/ops/+x/placing.+x", project_root, project_id);
        pid_t pid = fork();
        if (pid == 0) {
            execl(op_path, op_path, active_target_id, "stone", NULL);
            exit(1);
        } else waitpid(pid, NULL, 0);
        strcpy(last_response, "Placed block.");
        save_manager_state();
        trigger_render();
    }
}
