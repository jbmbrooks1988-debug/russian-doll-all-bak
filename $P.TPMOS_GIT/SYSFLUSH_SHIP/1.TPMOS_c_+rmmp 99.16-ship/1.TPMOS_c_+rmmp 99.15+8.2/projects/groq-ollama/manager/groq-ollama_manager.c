...
static char* run_tool(const char* tool_name, char* const args[], bool sandbox) {
    ...
    if (pid == 0) {
        ...
        if (sandbox) {
            ...
            else { if (asprintf(&full_path, "../ops/+x/%s", tool_name) != -1) { execvp(full_path, args); free(full_path); } }
        } else {
            if (strchr(tool_name, '/') || tool_name[0] == '.') { execvp(tool_name, args); }
            else { if (asprintf(&full_path, "projects/groq-ollama/ops/+x/%s", tool_name) != -1) { execvp(full_path, args); free(full_path); } }
        }
        _exit(127);
    }
...
void start_ai_query(const char* input) {
    ...
    if (pf) { fprintf(pf, "{\"model\":\"%s\",\"format\":\"json\",\"stream\":false,\"messages\":%s}\n", g_current_model, context ? context : "[]"); fclose(pf); }
    if (context) free(context);
    char *body_arg = NULL; char *api_path = NULL;
    if (asprintf(&api_path, "%s/api/chat", g_api_url) != -1 && asprintf(&body_arg, "@%s", tmp_prompt) != -1) {
        g_ai_pid = fork();
        if (g_ai_pid == 0) {
            setpgid(0, 0);
            char *curl_args[] = {"curl", "-v", "-s", "--max-time", "300", "-H", "Content-Type: application/json", api_path, "-d", body_arg, "-o", tmp_llm, NULL};
...
int main(int argc, char *argv[]) {
    ...
    while (!g_shutdown) {
                    ...
                    while (fgets(line, sizeof(line), hf)) {
                        ...
                        if (cmd_ptr) {
                            ...
                            if (key == 10 || key == 13) { process_input_trigger(); state_changed = 1; }
                                ...
                                FILE *pf = fopen(tmp_prompt, "w"); if (pf) { fprintf(pf, "{\"model\":\"%s\",\"format\":\"json\",\"stream\":false,\"messages\":[{\"role\":\"system\",\"content\":\"Condense the following conversation into a concise summary of facts, decisions, and current project state. Respond ONLY with a JSON object containing a 'summary' key.\"},{\"role\":\"user\",\"content\":\"%s\"}]}", g_current_model, context); fclose(pf); }
                                free(context); char *body_arg = NULL; char *api_path = NULL;
                                if (asprintf(&body_arg, "@%s", tmp_prompt) != -1 && asprintf(&api_path, "%s/api/chat", g_api_url) != -1) {
        ...
        check_ai_status(); write_gui_state(); if (state_changed) trigger_render();
        usleep(100000); 
    }
    return 0;
}
