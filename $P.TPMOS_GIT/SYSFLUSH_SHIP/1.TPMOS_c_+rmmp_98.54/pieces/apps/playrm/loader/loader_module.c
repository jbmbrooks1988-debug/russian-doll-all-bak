...
void write_gui_state() {
    ...
    /* Capture output of get_projects_op */
    char *op_path = NULL;
    asprintf(&op_path, "%s/pieces/chtpm/ops/+x/get_projects_op.+x", project_root);
    
    FILE *pf = popen(op_path, "r");
    if (pf) {
        char markup[16384];
        size_t n = fread(markup, 1, sizeof(markup)-1, pf);
        markup[n] = '\0';
        pclose(pf);
        ...
        if (gf) {
            ...
            /* Also set piece_methods to empty since we are using project_list now */
