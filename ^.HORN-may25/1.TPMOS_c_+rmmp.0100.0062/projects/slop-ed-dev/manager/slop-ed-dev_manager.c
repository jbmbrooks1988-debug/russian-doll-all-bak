...
static int save_game_to_path(const char *rel_path) {
    ...
    if (strstr(rel_path, "projects/slop-ed-dev/games/") == rel_path) {
        ...
        snprintf(full_dest, sizeof(full_dest), "%s/projects/slop-ed-dev/games/%s", project_root, rel_path);
    }
    ...
    snprintf(cmd, sizeof(cmd), "cp -r '%s/projects/slop-ed-dev/maps' '%s/'", project_root, full_dest);
    run_command(cmd);
    snprintf(cmd, sizeof(cmd), "cp -r '%s/projects/slop-ed-dev/pieces' '%s/'", project_root, full_dest);
    run_command(cmd);
    snprintf(cmd, sizeof(cmd), "cp '%s/projects/slop-ed-dev/project.pdl' '%s/'", project_root, full_dest);
    run_command(cmd);
...
static int load_game_from_path(const char *rel_path) {
    ...
    if (strstr(rel_path, "projects/slop-ed-dev/games/") == rel_path) {
        ...
        snprintf(full_src, sizeof(full_src), "%s/projects/slop-ed-dev/games/%s", project_root, rel_path);
    }

    if (access(full_src, F_OK) != 0) {
        FILE *dbg = fopen("debug.txt", "a");
        if (dbg) {
            fprintf(dbg, "LOADER: Failed to access '%s' (root: %s, rel: %s)\n", full_src, project_root, rel_path);
            fclose(dbg);
        }
    ...
    char cmd[MAX_CMD];
    snprintf(cmd, sizeof(cmd), "rm -rf '%s/projects/slop-ed-dev/maps'/* '%s/projects/slop-ed-dev/pieces'/*", project_root, project_root);
    run_command(cmd);

    snprintf(cmd, sizeof(cmd), "cp -r '%s/maps'/* '%s/projects/slop-ed-dev/maps/'", full_src, project_root);
    run_command(cmd);
    snprintf(cmd, sizeof(cmd), "cp -r '%s/pieces'/* '%s/projects/slop-ed-dev/pieces/'", full_src, project_root);
    run_command(cmd);
    snprintf(cmd, sizeof(cmd), "cp '%s/project.pdl' '%s/projects/slop-ed-dev/'", full_src, project_root);
    run_command(cmd);
...
int process_key(int key) {
    ...
    if (strcmp(layout, "file_menu.chtpm") == 0) {
        ...
        if (key == '9') { // Load
            browser_mode = 0; clear_search_query(); set_file_path_input("projects/slop-ed-dev/games/");
            transition_to_layout("projects/slop-ed-dev/layouts/file_browser.chtpm");
