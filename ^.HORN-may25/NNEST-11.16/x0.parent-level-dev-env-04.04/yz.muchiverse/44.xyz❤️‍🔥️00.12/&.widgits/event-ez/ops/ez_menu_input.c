...
static void house_root_from_pkg_dir(const char *pkg_dir, char *out, size_t out_sz) {
    char probe[PATH_BUF];
    snprintf(probe, sizeof(probe), "%s", pkg_dir);
    /* Old layout: <house>/@.apps/<app>/entities/<name> — cut at /@.apps/. */
    const char *marker = strstr(probe, "/@.apps/");
    if (marker) {
        size_t len = (size_t)(marker - probe);
        if (len >= out_sz) len = out_sz - 1;
        memcpy(out, probe, len);
        out[len] = '\0';
        return;
    }
    /* New layout: house-root / *.monads / *.muchi-pet / entities / name —
     * walk up until we find the dir that holds &.widgits (house root). */
    char wid_check[PATH_BUF];
    while (strchr(probe, '/')) {
        char *slash = strrchr(probe, '/');
        if (!slash) break;
        *slash = '\0';
        snprintf(wid_check, sizeof(wid_check), "%s/&.widgits", probe);
        if (access(wid_check, F_OK) == 0) {
            ...
            if (irf) {
                ...
                while (fgets(line, sizeof(line), irf)) {
                    ...
                    if (strcmp(type_buf, "change_gold") == 0) {
                        ...
                        if (wf) {
                            fprintf(wf, "#!/bin/sh\n");
                            fprintf(wf, "exec '%s/*.monads/*.muchi-pet/ops/+x/mr_change_gold.+x' '%s' '%s'\n",
