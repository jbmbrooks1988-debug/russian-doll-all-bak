...
- **Epoch Management**: A C-based `manager` orchestrates turn rotation, perceptual phases, and training triggers.
...
### B. Compile Binaries
Builds all entity Ops and the central manager.
```bash
./build_ops.sh
gcc -o moke-pet-project-1.0/manager moke-pet-project-1.0/manager.c
```

### C. Run Simulation
Executes one epoch (Turn -> Train -> Increment).
```bash
./moke-pet-project-1.0/manager
```
