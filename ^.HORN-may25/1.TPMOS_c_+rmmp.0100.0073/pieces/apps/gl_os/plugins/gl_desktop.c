...
void execute_option(DesktopWindow* win, int index) {
    ...
    /* Standard Terminal Contexts */
        ...
        if (index == 0) {
            ...
            win->selected_index = 0;
            win->option_count = gl_project_count + 2; /* Removed redundant shortcut */
            for (int i = 0; i < gl_project_count; i++) {
                strcpy(win->menu_options[i], gl_projects[i].id);
            }
            strcpy(win->menu_options[gl_project_count], "3D Cube Demo");
            strcpy(win->menu_options[gl_project_count + 1], "back");
        ...
        if (index < gl_project_count) {
            ...
            if (strcmp(gl_projects[index].id, "piececraft-3d") == 0) {
                create_gltpm_window("piececraft-3d", "projects/piececraft-3d/layouts/main.gltpm", "Piececraft-3D");
            } else if (strcmp(gl_projects[index].id, "fuzz-op-gl") == 0) {
                create_gltpm_window("fuzz-op-gl", "projects/fuzz-op-gl/layouts/main.gltpm", "Fuzz-Op-GL");
            } else if (strcmp(gl_projects[index].id, "op-ed-gl") == 0) {
                create_gltpm_window("op-ed-gl", "projects/op-ed-gl/layouts/main.gltpm", "GL-Op-Ed");
            } else if (strcmp(gl_projects[index].id, "aow-2d") == 0) {
                create_gltpm_window("aow-2d", "projects/aow-2d/layouts/main.gltpm", "AOW-2D");
            } else {
...
void draw_window(DesktopWindow* win) {
    ...
    if (strcmp(win->type, "terminal") == 0) {
        ...
        for (int i = 0; i < win->gltpm_scene.text_count; i++) {
            int line_y = y + h - 62 - (i * get_line_height());
        ...
        for (int i = 0; i < win->gltpm_scene.button_count; i++) {
            ...
            int btn_h = get_button_height();
            int button_y = y + h - 110 - (i * (btn_h + 4));
            ...
            draw_rect(x + 12, button_y, w - 24, btn_h, r, g, b);
            glColor3f(1, 1, 1);
...
void motion(int x, int y) {
    ...
    if (drag_window_id != -1) {
        ...
        if (is_dragging) {
            ...
            if (windows[window_count - 1].y > window_height - get_toolbar_height() - 25) windows[window_count - 1].y = window_height - get_toolbar_height() - 25;
        } else if (is_resizing) {
