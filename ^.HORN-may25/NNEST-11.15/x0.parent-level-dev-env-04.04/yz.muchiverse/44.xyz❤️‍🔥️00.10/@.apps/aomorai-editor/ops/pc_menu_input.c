...
int main(int argc, char **argv) {
    ...
    if (cmd) {
        if (strcmp(cmd, "CONFIRM_START") == 0) {
             ...
             * stealing aomorai-editor's already-open board-viewer
             ...
             * aomorai-editor"), never blindly take the first line - a
            ...
            if (peer_session_root[0] && real_root[0]) {
                ...
                write_kv(peer_state_path, "focused_project_id", "aomorai-editor");
                write_kv(peer_state_path, "focused_project_root", real_root);
                snprintf(message, sizeof(message), "Board widget already open - refocusing on aomorai-editor.");
