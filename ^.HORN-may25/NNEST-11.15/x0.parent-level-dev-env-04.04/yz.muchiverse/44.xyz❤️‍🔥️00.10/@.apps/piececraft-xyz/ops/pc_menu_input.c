 ...
 * decision_mode, no monster_types.txt registry lookup (piececraft has
...
int main(int argc, char **argv) {
    ...
    if (cmd) {
        if (strcmp(cmd, "CONFIRM_START") == 0) {
             ...
             * "clear out or only have session focus on its own widgets
             * ... to prevent these clobbering collisions once and for
             * all" - tactics-txt's own OPEN_BOARD_WIDGET press was
             * stealing piececraft-xyz's already-open board-viewer
             * window and vice versa, since ledger_peers only ever
             * tracked ONE global "board-viewer" identity regardless of
             * which host project it was showing). Real fix, ported to
             * board-viewer's own button.sh too (see that file's own
             * header comment): board-viewer now registers on the ledger
             * as "board-viewer:<host_basename>", not bare "board-
             * viewer" - ledger_peers.+x (unchanged, already returns one
             * line per DISTINCT project_id) now returns one line per
             * currently-live HOST-SCOPED widget, so this loop must scan
             * every line for the ONE matching THIS host ("board-viewer:
             * piececraft-xyz"), never blindly take the first line - a
             * different host's own live session on the ledger is a real
             * possible line here now, not a bug to work around. */
            ...
            if (peer_session_root[0] && real_root[0]) {
                ...
                write_kv(peer_state_path, "focused_project_id", "piececraft-xyz");
                write_kv(peer_state_path, "focused_project_root", real_root);
                snprintf(message, sizeof(message), "Board widget already open - refocusing on piececraft-xyz.");
