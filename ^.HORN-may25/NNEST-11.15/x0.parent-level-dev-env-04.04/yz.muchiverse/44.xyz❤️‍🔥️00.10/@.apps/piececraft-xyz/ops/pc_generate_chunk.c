...
int main(int argc, char **argv) {
    ...
    write_kv_int(world_state_path, "game_time_epoch_sec", 1767225600L);
    write_kv_int(world_state_path, "game_time_epoch_ms", 1767225600000L); /* real ms-precision canonical clock, see pc_clock_daemon.c's own header comment */
    ...
    if (flat_mode) {
        ...
        FILE *pef = fopen(phymoji_entities_path, "w");
        if (pef) {
     ...
     * owner_id), PLUS piececraft-xyz's own pos_z (real Z-axis, this
