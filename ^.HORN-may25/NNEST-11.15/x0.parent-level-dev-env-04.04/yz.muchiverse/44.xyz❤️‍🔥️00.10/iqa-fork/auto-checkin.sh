...
if [ "$BUILDER_PIDS" -eq 0 ]; then
    echo "builder: DEAD"
    if [ "$FIX" -eq 1 ]; then
        cd "/home/no/Desktop/🤖️🪤️🏠️/🥡️🪜️/🪜️-00.00/NNEST_CLEAN_PARENT/NNEST-11.15/x0.parent-level-dev-env-04.04/yz.muchiverse/44.xyz❤️‍🔥️00.10/046.open-gema🤖️+1" && nohup bash builder/run.sh >/tmp/builder.out 2>&1 &
