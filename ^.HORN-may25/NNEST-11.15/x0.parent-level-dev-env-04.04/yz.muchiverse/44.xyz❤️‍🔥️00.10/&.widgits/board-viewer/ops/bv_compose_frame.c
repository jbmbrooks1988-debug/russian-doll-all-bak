...
int main(void) {
    ...
    char focused_project_id[128] = "", focused_project_root[PATH_BUF] = "";
    ...
    if (!bf) {
        ...
        line(rowbuf);
        line("  No board.txt yet (host hasn't confirmed setup / generated a board).");
        blank();
        border();
        fclose(g_view_out);
        ping_chtpm_render_marker(project_root);
    ...
    int default_render_mode = has_z_manifest(focused_project_root, z_base_dummy, sizeof(z_base_dummy), &z_count_dummy) ? 1 : 0;
    int render_mode = read_kv_int(state_path, "render_mode", default_render_mode);
    int camera_mode = read_kv_int(state_path, "camera_mode", 4);
    if (!render_mode) {
        line("  Camera: 2D flat (press 0 for 3D raymarch - not yet rendered)");
    } else {
        int cam_yaw = read_kv_int(state_path, "cam_yaw", 180);
        int cam_pitch = read_kv_int(state_path, "cam_pitch", -90);
