...
#ifdef _WIN32
    /* Prefer "." when CWD has pieces/ — emoji abs paths break MinGW fopen. */
    {
        struct stat st_p;
...
static void write_file_atomic(const char *path, const void *data, size_t len) {
     ...
     * if the dest already exists — rgb_frame.raw then freezes on first
     * write forever (live: GL never updates after boot frame). Match
     * 014.wsr-pal write_file_atomic: remove dest, then rename; fallback
     * to direct write if rename still fails. */
    remove(path);
 ...
 * pure opt-in. */
/* Returns 1 if overlay pixels were blitted, 0 if missing/short/unreadable.
 * Callers MUST NOT skip map text rows when this returns 0 (Win: stale
 * receipt with no raw left map band permanently black). */
...
static void render_once(unsigned char fb[FRAME_H][FRAME_W][4], const char *frame_path) {
    ...
    /* "rb": on Windows, text-mode fopen can mangle multi-byte UTF-8 map
     * emoji lines (live: chrome ASCII lit, map band fully black). Binary
     * read keeps F0 9F … sequences intact for blit_text(). */
    ...
    while (row < MAX_TEXT_ROWS && fgets(line, sizeof(line), f)) {
        ...
        if ((unsigned char)line[0] == 0x01) {
             ...
             * Only skip following map rows if overlay actually blitted.
             * Stale receipt + missing/short raw used to skip 16 map
             * lines and leave a black band (Win 2D emoji map invisible). */
            ...
            /* If overlay failed: do not consume map lines; fall through
             * next fgets rows will be 2D emoji/ASCII map as usual. */
            continue;
        }
