...
#ifdef _WIN32
/* Port of 014.wsr-pal win_spawn (house Win pattern — do not invent a second).
 * Old muta CreateProcessA + bare system/prisc+x failed: no .exe suffix and
 * absolute emoji house paths (debug: launch_module pid=-1). Prefer relative
 * path under CWD, add .exe, CreateProcessW UTF-16. */
...
intptr_t win_spawn(const char* path, char* const args[]) {
    ...
    /* Prefer relative path under project_root (avoids long Unicode abs path) */
    ...
    /* MinGW: system/prisc+x often only exists as prisc+x.exe */
...
void resolve_root() {
    /* Win + emoji house: prefer relative "." when CWD already has pieces/
     * (absolute Unicode getcwd paths break ANSI fopen / weak CreateProcess). */
#ifdef _WIN32
    ...
    }
    {
        ...
        if (env && env[0] && strcmp(env, ".") != 0) {
            /* Only honor env if it has anchors and is not a broken abs emoji path preference */
#ifdef _WIN32
            if (access("pieces", F_OK) == 0 && env[0] != '.' && strchr(env, ':')) {
                /* keep "." already set */
            } else
#endif
            if (root_has_anchors(env)) {
                strncpy(project_root_path, env, sizeof(project_root_path) - 1);
                project_root_path[sizeof(project_root_path) - 1] = '\0';
            }
        }
    }
...
#ifdef _WIN32
                    /* Skip absolute candidates that would reintroduce emoji abs paths */
                    if (!(candidate[0] && candidate[1] == ':') && root_has_anchors(candidate)) {
#else
