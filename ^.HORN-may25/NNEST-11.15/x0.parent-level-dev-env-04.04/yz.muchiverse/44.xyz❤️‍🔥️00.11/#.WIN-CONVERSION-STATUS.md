...
| **KHTPM entity windows** | Working sprites + RMB menus (shared `khtpm_core`) | crypts LAUNCH → `tp_desktop_window.exe` |
| **Livedesk taskbar** | Working (top of screen on Win); 50% alpha layered; shared `khtpm_taskbar_core` | `tp_taskbar.exe` |
...
```
DESIGN (one place)
  khtpm_core.c / khtpm_taskbar_core.c

PLAT SHIMS
  khtpm_plat_win.c / khtpm_plat_x11.c
  khtpm_taskbar_plat_win.c / khtpm_taskbar_plat_x11.c

LEGACY (do not add design)
  tp_desktop_window.c, tp_desktop_window_win.c
  livedesk-taskbar/ops/tp_taskbar.c, tp_taskbar_win.c
```
