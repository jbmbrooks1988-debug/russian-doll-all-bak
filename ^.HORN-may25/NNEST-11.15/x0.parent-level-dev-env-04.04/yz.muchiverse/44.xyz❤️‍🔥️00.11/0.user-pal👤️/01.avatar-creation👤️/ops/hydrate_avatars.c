...
int main(void) {
    ...
    snprintf(inv, sizeof(inv), "%s/%s/home/avatars/inventory.txt", login_root, xyz);
    ...
    while (fgets(id, sizeof(id), f)) {
        ...
        snprintf(remote, sizeof(remote), "%s/%s/home/avatars/%s", login_root, xyz, id);
    ...
    if (d) {
        ...
        while ((ent = readdir(d)) != NULL) {
            ...
            snprintf(remote, sizeof(remote), "%s/%s/home/avatars/%s/state.txt",
                     login_root, xyz, ent->d_name);
            if (stat(remote, &st) == 0) continue; /* already on xyzfs */
            char rdir[PATH_BUF];
            snprintf(rdir, sizeof(rdir), "%s/%s/home/avatars/%s", login_root, xyz, ent->d_name);
            char ldir[PATH_BUF];
