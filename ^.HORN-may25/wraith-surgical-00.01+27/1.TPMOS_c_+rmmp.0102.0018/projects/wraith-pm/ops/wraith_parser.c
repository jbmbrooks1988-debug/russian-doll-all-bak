...
char current_layout[MAX_PATH] = "projects/wraith-pm/layouts/wraith-shell.wrai";
...
bool wraith_shell_owns_top_level_nav(void);
void write_wraith_session_key(const char *value);
...
bool wraith_shell_owns_top_level_nav(void) {
    if (interact_history_path[0] == '\0') return false;
    if (strstr(interact_history_path, "pieces/apps/player_app/history.txt") == NULL) return false;
    if (strstr(current_layout, "projects/wraith-pm/layouts/") == NULL) return false;
    return true;
}
...
void write_wraith_session_key(const char *value) {
    ...
    snprintf(path, sizeof(path), "%s/projects/wraith-pm/session/state.txt", project_root_path);
    snprintf(tmp_path, sizeof(tmp_path), "%s/projects/wraith-pm/session/state.txt.tmp", project_root_path);
    ...
    if (src) {
        while (fgets(line, sizeof(line), src)) {
            ...
            if (eq) {
                *eq = '\0';
                if (strcmp(trim_pmo(line), "current_key") == 0) {
                    fprintf(dst, "current_key=%s\n", value);
                    wrote = 1;
                } else {
                    fputs(original, dst);
                }
    ...
    if (!wrote) fprintf(dst, "current_key=%s\n", value);
    fclose(dst);
...
void load_vars() {
    ...
    if (strlen(proj_id) > 0 && strcmp(proj_id, "template") != 0) {
        ...
        /* WRAITH-SPECIFIC: Load session state and nav_state */
        if (strcmp(proj_id, "wraith-pm") == 0) {
            load_state_file("projects/wraith-pm/session/state.txt", NULL);
            load_state_file("projects/wraith-pm/session/nav_state.txt", NULL);
        }
...
static void reload_live_layout_state(void) {
    int prior_focus = focus_index;
    int prior_active = active_index;
    char prior_active_id[64] = "";

    if (prior_active >= 0 && prior_active < element_count && elements[prior_active].id[0] != '\0') {
        strncpy(prior_active_id, elements[prior_active].id, sizeof(prior_active_id) - 1);
        prior_active_id[sizeof(prior_active_id) - 1] = '\0';
    }

    /* PURGE CACHE FOR READ INTEGRITY */
    var_count = 0;
    ...
    if (prior_active_id[0] != '\0') {
        for (int i = 0; i < element_count; i++) {
            if (strcmp(elements[i].id, prior_active_id) == 0) {
                active_index = i;
                break;
            }
        }
    } else {
        active_index = prior_active;
    }

    /* PRIORITIZE AUTHORITATIVE MANAGER FOCUS */
    const char* m_idx = get_var("active_gui_index");
    if (m_idx && strlen(m_idx) > 0) {
        initialize_focus();
    } else {
        if (prior_focus >= 0 && prior_focus < element_count && is_navigable(prior_focus)) {
            focus_index = prior_focus;
        } else {
            initialize_focus();
        }
    }

    if (active_index >= element_count) active_index = -1;
}
...
void compose_frame() {
    ...
    if (clear_nav_on_next) { nav_buffer[0] = '\0'; clear_nav_on_next = false; }

    /* FINGERPRINT CHECK: Only pulse renderer if frame content actually changed. */
    static unsigned long long last_h1 = 0, last_h2 = 0;
    unsigned long long h1 = 5381, h2 = 0;
    for (size_t i = 0; frame[i]; i++) {
        h1 = ((h1 << 5) + h1) + (unsigned char)frame[i];
        h2 = (unsigned char)frame[i] + (h2 << 6) + (h2 << 16) - h2;
    }
    ...
    if (out_f) {
        ...
        if (h1 != last_h1 || h2 != last_h2) {
            ...
            if (audit) {
                time_t t; struct tm *tm; time(&t); tm = localtime(&t);
                fprintf(audit, "[%02d:%02d:%02d] FRAME PULSE: h1=%llu h2=%llu\n", tm->tm_hour, tm->tm_min, tm->tm_sec, h1, h2);
                fclose(audit);
            }
            last_h1 = h1; last_h2 = h2;
...
void handle_mouse(int btn, int x, int y) {
    /* RE-WRAITH: MOUSE IGNORED FOR NAV TESTING
     * To stop frame spam, we are temporarily disabling all mouse history logging. */
    return;
}
...
void process_key(int key) {
    ...
    /* write_wraith_session_key(key_str); */

    /* Normal nav mode */
    if (active_index == -1) {
        bool wraith_top_level_nav = wraith_shell_owns_top_level_nav();
        /* RE-WRAITH: If no navigable buttons exist, forward arrows to project history */
        bool has_navigable = false;
        for (int i = 0; i < element_count; i++) { if (is_navigable(i)) { has_navigable = true; break; } }

        if (key == ARROW_UP || key == 'w' || key == 'W' || key == JOY_UP) { 
            if (!wraith_top_level_nav && has_navigable) {
...
int main(int argc, char **argv) {
    ...
    if (stat(hist_p, &st) == 0) last_history_position = st.st_size;
    ...
    while (1) {
...
#endif
        /* RE-WRAITH: Input polling removed. Manager now polls global history directly. */
        ...
        if (stat(master_frame_ch, &st) == 0) {
            if (st.st_size < last_master_pulse_size) last_master_pulse_size = 0;
            if (st.st_size != last_master_pulse_size) {
                last_master_pulse_size = st.st_size;
                reload_live_layout_state();
                dirty = 1;
            }
        }
        if (stat(display_frame_ch, &st) == 0) {
            if (st.st_size < last_display_pulse_size) last_display_pulse_size = 0;
            if (st.st_size != last_display_pulse_size) {
                last_display_pulse_size = st.st_size;
                reload_live_layout_state();
                dirty = 1;
            }
        }
        if (stat(state_ch, &st) == 0) {
            if (st.st_size < last_state_file_size) last_state_file_size = 0;
            if (st.st_size != last_state_file_size) {
                ...
                /* RE-WRAITH: Silence state-driven spam. 
                   We only want renders triggered by the explicit display pulse from the Manager. */
                /* dirty = 1; */
            }
        }
        if (stat(layout_ch, &st) == 0) {
            if (st.st_size < last_layout_file_size) last_layout_file_size = 0;
            if (st.st_size != last_layout_file_size) {
