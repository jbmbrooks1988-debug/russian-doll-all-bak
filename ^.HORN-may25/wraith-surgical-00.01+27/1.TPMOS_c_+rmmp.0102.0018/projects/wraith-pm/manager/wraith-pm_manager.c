...
static void format_key_label(int key, char *out, size_t out_sz) {
    if (out_sz == 0) return;
    if (key >= 32 && key <= 126) {
        snprintf(out, out_sz, "%c", key);
    } else if (key == 1000) {
        snprintf(out, out_sz, "LEFT");
    } else if (key == 1001) {
        snprintf(out, out_sz, "RIGHT");
    } else if (key == 1002) {
        snprintf(out, out_sz, "UP");
    } else if (key == 1003) {
        snprintf(out, out_sz, "DOWN");
    } else if (key == 10 || key == 13) {
        snprintf(out, out_sz, "ENTER");
    } else {
        snprintf(out, out_sz, "%d", key);
    }
}
...
static void ensure_default_state(const char *root) {
    ...
    ensure_session_default(root, "projects/wraith-pm/session/nav_state.txt", "active_gui_index", "1");
...
static void normalize_boot_state(const char *root) {
    ...
    update_session_kv(root, "projects/wraith-pm/session/state.txt", "current_key", "None");
    update_session_kv(root, "projects/wraith-pm/session/nav_state.txt", "active_gui_index", "1");
    set_focus_and_scope(root, WINDOW_TERMINAL);
}

static void load_nav_state(const char *root) {
    char selected[32];
    read_session_value(root, "projects/wraith-pm/session/nav_state.txt", "active_gui_index", selected, sizeof(selected));
    if (selected[0]) {
        g_active_gui_index = atoi(selected);
        if (g_active_gui_index < 1) g_active_gui_index = 1;
    }
}

static void clamp_active_gui_index(void) {
    if (g_project_count < 1) {
        g_active_gui_index = 1;
        return;
    }
    if (g_active_gui_index < 1) g_active_gui_index = 1;
    if (g_active_gui_index > g_project_count) g_active_gui_index = g_project_count;
}

static void sync_nav_state(const char *root) {
    char selected[32];
    snprintf(selected, sizeof(selected), "%d", g_active_gui_index);
    update_session_kv(root, "projects/wraith-pm/session/nav_state.txt", "active_gui_index", selected);
}

static void open_selected_project(const char *root) {
    const char *project_id = NULL;
    if (g_active_gui_index < 1 || g_active_gui_index > g_project_count) return;
    project_id = g_projects[g_active_gui_index - 1].id;
...
static void trigger_render(const char *root) {
    ...
    /* STANDARD: Pulse display marker after state change to notify parser. */
...
static void update_terminal_view(const char *root) {
    ...
    if (f) {
        for (int i = 0; i < g_project_count; i++) {
            const char *marker = (g_active_gui_index == (i + 1)) ? "[>] " : "[ ] ";
...
static int route_input(const char *root, const char *line) {
    ...
    if (strstr(line, "COMMAND: OP wraith-pm::focus terminal")) {
        ...
        return 0;
    } else if (isdigit((unsigned char)line[0]) || strstr(line, "KEY_PRESSED:")) {
        const char *p = line;
        if (strstr(line, "KEY_PRESSED:")) {
            p = strstr(line, "KEY_PRESSED:") + 12;
            while (*p && !isdigit((unsigned char)*p)) p++;
        }
        key = atoi(p);
        
        FILE *dbg = fopen("manager.log", "a");
        if (dbg) { fprintf(dbg, "DEBUG: route_input parsed key %d from line: %s", key, line); fclose(dbg); }

        if (key > 0) {
            char label[64];
            format_key_label(key, label, sizeof(label));
            update_session_kv(root, "projects/wraith-pm/session/state.txt", "current_key", label);
        }
        if (key == 1002 || key == 'w') { // UP
            g_active_gui_index--; if (g_active_gui_index < 1) g_active_gui_index = 1;
            changed = 1;
        } else if (key == 1003 || key == 's') { // DOWN
            g_active_gui_index++; if (g_active_gui_index > g_project_count) g_active_gui_index = g_project_count;
            changed = 1;
        } else if (key == 10 || key == 13) {
            ...
            int val = key - '0';
            if (val <= g_project_count) { 
                g_active_gui_index = val; 
            }
            changed = 1;
        }
    }
...
void* history_thread(void* arg) {
    ...
    snprintf(path, sizeof(path), "%s/pieces/keyboard/history.txt", root);
...
int main(void) {
    pthread_t history_tid; setpgid(0, 0); 
    ...
    pthread_create(&history_tid, NULL, history_thread, (void*)g_project_root);
    ...
    while (!g_shutdown) {
        ...
        clock_timer++;
        if (clock_timer >= 30) {
            sync_clock_state(g_project_root);
            trigger_render(g_project_root);
            clock_timer = 0;
        }
    }
    pthread_join(history_tid, NULL);
    return 0;
}
