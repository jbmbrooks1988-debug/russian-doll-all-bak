...
void update_completions(void) {
    ...
    if (!star) { g_completion_mode = false; g_piece_methods[0] = '\0'; return; }
    
    char *prefix = star + 1;
    char *p_args[] = {"projects/gem-dev/ops/+x/complete_path", (strlen(prefix) == 0 ? "." : prefix), NULL};
    char *matches = run_tool(p_args[0], p_args, false);
    
    g_piece_methods[0] = '\0';
    if (matches && strlen(matches) > 0) {
        char cmd[4096];
        snprintf(cmd, sizeof(cmd), "'%s/projects/gem-dev/ops/+x/get_completion_methods_op' '%s'", project_root, matches);
        FILE *pf = popen(cmd, "r");
        if (pf) {
            size_t n = fread(g_piece_methods, 1, sizeof(g_piece_methods) - 1, pf);
            g_piece_methods[n] = '\0';
            pclose(pf);
        }
        free(matches);
    } else {
        snprintf(g_piece_methods, sizeof(g_piece_methods), "(no matches)");
    }
}
...
void handle_choose_path(int index) {
    ...
    if (strlen(last_line) > 0) {
        ...
        if (star) {
            ...
            if (matches && strlen(matches) > 0) {
                ...
                int count = 0;
                int target = index - 2; // KEY:2 is index 0
                while (token) {
                    if (count == target) {
                        choice = strdup(token);
                        break;
                ...
                if (choice) {
                    *star = '\0';
                    char new_input[2048];
                    // Use 'i' prefix explicitly for the cli_io input_text element
    ...
    write_gui_state();
    trigger_render();
    strcpy(last_frame_methods, g_piece_methods);
}
...
void process_input_trigger(void) {
    ...
    if (input && strlen(input) > 0) {
        ...
        if (strcmp(g_ai_state, "PENDING_PERM") == 0) {
            ...
            // Clear internal buffer and completion mode on submission
...
int main(int argc, char *argv[]) {
    ...
    // GLOBAL STARTUP CLEAR: Truncate history and buffer files for a clean session
    FILE *f_clear = fopen(hist_path, "w"); if (f_clear) fclose(f_clear);
    f_clear = fopen(buf_path, "w"); if (f_clear) fclose(f_clear);
    
    // Also clear project-specific gui_state.txt
    f_clear = fopen("projects/gem-dev/manager/gui_state.txt", "w"); if (f_clear) fclose(f_clear);
    ...
    while (!g_shutdown) {
        ...
        if (has_star != g_completion_mode || (has_star && text_changed)) {
            ...
            // Keep internal buffer in sync without triggering a full project render
            // Parser handles live text display locally
            strncpy(g_last_buf_input, last_line, sizeof(g_last_buf_input) - 1);
            if (!has_star && g_completion_mode) {
                g_completion_mode = false;
                g_piece_methods[0] = '\0';
                state_changed = 1;
            }
        ...
        if (state_changed) {
            write_gui_state();
            // Guard against frame spam: only render if menu changed or FSM state changed
            if (strcmp(g_piece_methods, last_frame_methods) != 0 || strncmp(g_fsm_state, "THINKING", 8) == 0) {
                trigger_render();
                strcpy(last_frame_methods, g_piece_methods);
            }
