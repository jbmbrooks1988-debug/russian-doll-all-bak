...
void get_pdl_value(const char* project, const char* section, const char* key, char* out_val, size_t out_sz) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(trimmed, section, strlen(section)) == 0 && (trimmed[strlen(section)] == ' ' || trimmed[strlen(section)] == '\t' || trimmed[strlen(section)] == '\0' || trimmed[strlen(section)] == '|')) {
            in_section = 1;
            /* DO NOT continue; the line might contain Key|Value (Table Style) */
        }
...
void load_gl_projects(void) {
    ...
    /* 1. LOAD FROM KVP (CURATED/PINNED) */
    ...
    /* 2. DYNAMIC SCAN (EVERYTHING ELSE WITH A PDL) */
...
typedef struct {
    ...
    int text_count;
    int node_count;
    int ui_mode; /* 0 = WORLD (Default), 1 = SOVEREIGN (Hide HUD/Xelector) */
...
int g_high_speed_ipc_enabled = 0; /* 1 = Use SHM/Pipes, 0 = File-based (Default) */
...
static void sync_gltpm_menu_from_scene(DesktopWindow* win) {
    ...
    /* Initialize active node on first sync or reload */
    if (!win->gltpm_scene.loaded) {
        win->active_node_index = -1;
    }
    // ... rest of sync logic ...
    win->camera_mode = win->gltpm_scene.camera_mode;

    /* Sync dynamic camera/xelector coordinates */
    for(int i=0; i<3; i++) {
    ...
    /* Populate menu_options only with currently navigable buttons for terminal context parity */
    for (int i = 0; i < win->gltpm_scene.button_count && win->option_count < MAX_TERM_OPTIONS; i++) {
        GLTPMButton *btn = &win->gltpm_scene.buttons[i];
        bool navigable = true;
        if (win->active_node_index != -1) {
            bool is_descendant = false;
            int p = btn->parent;
            while (p != -1) {
                if (p == win->active_node_index) { is_descendant = true; break; }
                p = win->gltpm_scene.nodes[p].parent;
            }
            if (!is_descendant && i != win->active_node_index) navigable = false;
        }

        if (navigable) {
            strncpy(win->menu_options[win->option_count], btn->label, MAX_OPTION_LEN - 1);
            win->menu_options[win->option_count][MAX_OPTION_LEN - 1] = '\0';
            win->option_count++;
        }
    }
...
void dispatch_gltpm_button(DesktopWindow* win, int index) {
    ...
    GLTPMButton *btn = &win->gltpm_scene.buttons[index];

    /* Submenu Navigation: ACTIVATE / BACK */
    if (strcmp(btn->onClick, "ACTIVATE") == 0) {
        win->active_node_index = btn->parent;
        add_history(win, "Container Activated.");
        glutPostRedisplay();
        return;
    }
    
    if (strcmp(btn->onClick, "BACK") == 0) {
        if (win->active_node_index != -1) {
            win->active_node_index = win->gltpm_scene.nodes[win->active_node_index].parent;
        }
        add_history(win, "Navigated Back.");
        glutPostRedisplay();
        return;
    }

    /* Responsive INTERACT handling (Concern #3) */
    if (strcmp(btn->onClick, "INTERACT") == 0) {
    ...
    /* Standard Terminal Contexts */
        ...
        if (index < gl_project_count) {
            ...
            if (entry_layout[0] != '\0') {
                ...
                /* Intelligent Fallback Convention (Alpha-Beta Compliant) */
                char probe[MAX_PATH];
                char abs_p[MAX_PATH];
                
                /* 1. Try index.chtmgl (Modern Standard) */
...
void move_selection(DesktopWindow* win, int direction) {
    ...
    /* RECOVERY: Support legacy/terminal menu selection (OS Desktop) */
    if (strcmp(win->type, "gltpm_app") != 0) {
    ...
    /* GLTPM SCOPED NAVIGATION */
    int prev_idx = win->selected_index;
    int search_idx = win->selected_index;
    int limit = win->gltpm_scene.button_count;

    if (limit <= 0) return;

    while (1) {
        search_idx += direction;
        if (search_idx < 0) search_idx = limit - 1;
        if (search_idx >= limit) search_idx = 0;

        if (search_idx == prev_idx) break; /* Cycled through all, none other found */

        /* Check if search_idx is navigable */
        GLTPMButton *btn = &win->gltpm_scene.buttons[search_idx];
        bool navigable = true;
        if (win->active_node_index != -1) {
            bool is_descendant = false;
            int p = btn->parent;
            ...
            if (!is_descendant && search_idx != win->active_node_index) navigable = false;
        }

        if (navigable) {
            win->selected_index = search_idx;
            break;
        }
    }
}
...
bool is_button_navigable(DesktopWindow *win, int i) {
    if (i < 0 || i >= win->gltpm_scene.button_count) return false;
    GLTPMButton *btn = &win->gltpm_scene.buttons[i];
    
    if (win->active_node_index != -1) {
        /* If a node is active, only its descendants are navigable */
        bool is_descendant = false;
        int p = btn->parent;
        while (p != -1) {
            if (p == win->active_node_index) { is_descendant = true; break; }
        ...
        if (!is_descendant && i != win->active_node_index) return false;
    }
    return true;
}
...
void draw_window(DesktopWindow* win) {
    ...
    if (strcmp(win->type, "terminal") == 0) {
        ...
        if (win->camera_mode == 1) { /* 1st Person - Eye-Level & Forward */
            /* Standard 1st Person: Rotate Pitch then Yaw, then Translate */
            glRotatef(win->cam_rot[0], 1, 0, 0); /* Pitch (Look Up/Down) */
            glRotatef(win->cam_rot[1], 0, 1, 0); /* Yaw (Look Left/Right) */
            /* Offset forward slightly (.3) to push "eyes" to the front of the cube */
            glTranslatef(-target_x, -(target_y + 1.6f), -target_z + 0.3f); 
        } else if (win->camera_mode == 3) { /* 3rd Person - Behind/High & Angled Down */
            glTranslatef(0, -2.5f, -8.0f); /* Camera Offset (Up and Back) */
            glRotatef(25.0f, 1, 0, 0);      /* Look Down at target */
            glRotatef(win->cam_rot[1], 0, 1, 0); /* Orbital Yaw */
        ...
        for (int i = 0; i < win->gltpm_scene.sprite_count; i++) {
            ...
            /* Hide self in 1st person to prevent flickering/clipping */
        ...
        /* Draw Xelector (Artifact-XEL) - Sovereign Position (Conditional) */
        if (win->gltpm_scene.ui_mode == 0) {
            ...
            if (win->camera_mode != 1 && (tv.tv_usec / 200000) % 2 == 0) {
                ...
                /* Inner Legend Block (v1.2 Spec) */
                float legend_faces[6][3] = {
                    {0, 1, 0},      /* Top (Y+): Green */
                    {0.4, 0.2, 0},  /* Bottom (Y-): Brown */
                    {1, 1, 1},      /* Front (Z-): White */
                    {0, 0, 0},      /* Back (Z+): Black */
                    {1, 0, 0},      /* Right (X+): Red */
                    {0, 0, 1}       /* Left (X-): Blue */
        ...
        for (char *p = win->gltpm_scene.title; *p; p++) {
            glutBitmapCharacter(get_font_regular(), *p);
        }
        ...
        for (char *p = key_feedback; *p; p++) {
            glutBitmapCharacter(get_font_small(), *p);
        }

        /* Draw HUD and Minimap (Conditional) */
        if (win->gltpm_scene.ui_mode == 0) {
            ...
            for (char *p = coord_hud; *p; p++) glutBitmapCharacter(get_font_small(), *p);
        ...
        /* Draw CHTMGL Nodes (Panels, Divs) */
        for (int i = 0; i < win->gltpm_scene.node_count; i++) {
            GLTPMNode *node = &win->gltpm_scene.nodes[i];
            int node_x = x + (int)node->x;
            int node_y = y + h - (int)node->y - (int)node->h;
            
            if (node_y < y) continue;
            
            /* Apply color with alpha */
            draw_rect(node_x, node_y, (int)node->w, (int)node->h, node->color[0], node->color[1], node->color[2]);
            
            /* If it's a div, we might want a border or special handling later */
            if (strcmp(node->tag, "div") == 0) {
                /* Draw subtle border */
                glColor3f(node->color[0] + 0.1f, node->color[1] + 0.1f, node->color[2] + 0.1f);
                glBegin(GL_LINE_LOOP);
                glVertex2f(node_x, node_y);
                glVertex2f(node_x + node->w, node_y);
                glVertex2f(node_x + node->w, node_y + node->h);
                glVertex2f(node_x, node_y + node->h);
                glEnd();
            }
        }

        for (int i = 0; i < win->gltpm_scene.text_count; i++) {
            GLTPMText *txt = &win->gltpm_scene.texts[i];
            int text_x = x + (txt->x > 0 ? (int)txt->x : 15);
            int text_y = y + h - (txt->y > 0 ? (int)txt->y : (62 + (i * get_line_height())));

            if (text_y < y + 45) continue;
            glColor3f(0.85f, 0.85f, 0.9f);
            glRasterPos2f(text_x, text_y);
            for (char *p = txt->label; *p; p++) {
                glutBitmapCharacter(get_font_small(), *p);
            }
        }

        int global_counter = 0;
        int scoped_counter = 0;

        for (int i = 0; i < win->gltpm_scene.button_count; i++) {
            GLTPMButton *btn = &win->gltpm_scene.buttons[i];
            bool navigable = is_button_navigable(win, i);

            int display_num = 0;
            if (navigable) {
                if (win->active_node_index != -1) display_num = ++scoped_counter;
                else display_num = ++global_counter;
            }

            int btn_h = (btn->h > 0) ? (int)btn->h : get_button_height();
            int btn_w = (btn->w > 0) ? (int)btn->w : (w - 24);
            int button_x = (btn->x >= 0) ? (x + (int)btn->x) : (x + 12);
            int button_y = (btn->y >= 0) ? (y + h - (int)btn->y - btn_h) : (y + h - 110 - (i * (get_button_height() + 4)));

            if (button_y < y - 50) continue;

            const char *indicator = (i == win->selected_index) ? (win->is_map_control ? "^" : ">") : " ";
            char btn_label[256];
            if (display_num > 0) snprintf(btn_label, sizeof(btn_label), "[%s] %d. [%s]", indicator, display_num, btn->label);
            else snprintf(btn_label, sizeof(btn_label), "[%s] [%s]", indicator, btn->label);

            float r = (i == win->selected_index) ? 0.72f : 0.22f;
            float g = (i == win->selected_index) ? 0.58f : 0.24f;
            float b = (i == win->selected_index) ? 0.18f : 0.36f;
            
            draw_rect(button_x, button_y, btn_w, btn_h, r, g, b);
            glColor3f(1, 1, 1);
            glRasterPos2f(button_x + 6, button_y + (btn_h / 2) - 4);
            for (char *p = btn_label; *p; p++) glutBitmapCharacter(get_font_small(), *p);
        }

        /* Nav Command Prompt (Conditional) */
        if (win->gltpm_scene.ui_mode == 0) {
            char prompt[64];
...
void draw_global_ui(void) {
    ...
    /* Top Bar background (Optional but helps visibility) */
    draw_rect(0, window_height - btn_size - (margin * 2), window_width, btn_size + (margin * 2), 0.05f, 0.05f, 0.1f);
    
    /* Buttons from right to left: X, F, - */
    int x_x = window_width - btn_size - margin;
    int f_x = x_x - btn_size - spacing;
    int m_x = f_x - btn_size - spacing;
    int y = window_height - btn_size - margin;
    
    void* font = get_font_regular();

    /* Exit Button (X) - Red */
    draw_rect(x_x + 2, y - 2, btn_size, btn_size, 0.0f, 0.0f, 0.0f); /* Shadow */
    draw_rect(x_x, y, btn_size, btn_size, 0.8f, 0.2f, 0.2f);
    glColor3f(1.0f, 1.0f, 1.0f);
    glRasterPos2f(x_x + 9, y + 8);
    glutBitmapCharacter(font, 'X');
    
    /* Fullscreen Button (F) - Green */
    draw_rect(f_x + 2, y - 2, btn_size, btn_size, 0.0f, 0.0f, 0.0f); /* Shadow */
    draw_rect(f_x, y, btn_size, btn_size, 0.2f, 0.6f, 0.2f);
    glColor3f(1.0f, 1.0f, 1.0f);
    glRasterPos2f(f_x + 9, y + 8);
    glutBitmapCharacter(font, 'F');
    
    /* Minimize Button (-) - Yellow */
    draw_rect(m_x + 2, y - 2, btn_size, btn_size, 0.0f, 0.0f, 0.0f); /* Shadow */
    draw_rect(m_x, y, btn_size, btn_size, 0.6f, 0.6f, 0.2f);
    glColor3f(1.0f, 1.0f, 1.0f);
    glRasterPos2f(m_x + 11, y + 8);
    glutBitmapCharacter(font, '-');

    /* Global Status Info (Top Left) */
    glColor3f(0.6f, 0.6f, 0.8f);
    glRasterPos2f(margin, y + 8);
    const char* status = "GL-OS MASTER CONSOLE | TPM-COMPLIANT";
    for (const char* p = status; *p; p++) glutBitmapCharacter(get_font_small(), *p);
}

/* Global mouse position tracking helper functions */
void draw_mouse_cursor(int x, int y) {
    /* Draw a tiny white arrow cursor */
    ...
    glVertex2f(x, gl_y);
    glVertex2f(x + 10, gl_y - 10);
    glVertex2f(x + 4, gl_y - 10);
    glEnd();
    
    /* Small black outline for visibility */
    ...
    if (active_idx != -1 && strcmp(windows[active_idx].type, "gltpm_app") == 0) {
        ...
        if (in_map) {
            ...
            if (key >= '0' && key <= '9') {
                int d = key - '0';
                char temp_buf[32];
                snprintf(temp_buf, sizeof(temp_buf), "%s%c", win->nav_buffer, key);
                int target_num = atoi(temp_buf);
                
                int total_navigable = 0;
                for (int i = 0; i < win->gltpm_scene.button_count; i++) {
                    if (is_button_navigable(win, i)) total_navigable++;
                }
                
                if (target_num > 0 && target_num <= total_navigable) {
                    strncpy(win->nav_buffer, temp_buf, sizeof(win->nav_buffer)-1);
                } else {
                    /* Out of bounds: start over with single digit if valid */
                    if (d > 0 && d <= total_navigable) {
                        snprintf(win->nav_buffer, sizeof(win->nav_buffer), "%c", key);
                        target_num = d;
                    } else {
                        win->nav_buffer[0] = '\0';
                        target_num = 0;
            ...
            } else if (key == 27) { /* ESC: Navigate Back */
                if (win->active_node_index != -1) {
                    win->active_node_index = win->gltpm_scene.nodes[win->active_node_index].parent;
                    add_history(win, "Navigated Back (ESC).");
                }
                win->nav_buffer[0] = '\0';
            } else if (key == 13 || key == 10) {
                if (strlen(win->nav_buffer) > 0) {
                    /* Finalize Jump & Activate */
...
void mouse(int button, int state, int x, int y) {
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        /* Global Master Controls (Top Right Corner) */
        int g_btn_size = 30;
        int g_margin = 10;
        int g_spacing = 5;
        
        int x_x_start = window_width - g_btn_size - g_margin;
        int f_x_start = x_x_start - g_btn_size - g_spacing;
        int m_x_start = f_x_start - g_btn_size - g_spacing;
        
        if (y >= g_margin && y <= g_margin + g_btn_size) {
            /* Exit Button (X) */
            if (x >= x_x_start && x <= x_x_start + g_btn_size) {
                printf("[GLOBAL] Exit requested from master console\n");
                handle_sigint(SIGINT);
                return;
            }
            /* Fullscreen Button (F) */
            if (x >= f_x_start && x <= f_x_start + g_btn_size) {
                printf("[GLOBAL] Fullscreen toggle requested from master console\n");
                g_fullscreen_enabled = !g_fullscreen_enabled;
            ...
            /* Minimize Button (-) */
            if (x >= m_x_start && x <= m_x_start + g_btn_size) {
                printf("[GLOBAL] Minimize requested from master console\n");
                glutIconifyWindow();
                return;
            }
        }
        ...
        if (idx != -1) {
            ...
            /* GLTPM APP INTERACTION (Alpha Support) */
            else if (strcmp(windows[idx].type, "gltpm_app") == 0) {
                DesktopWindow *win = &windows[idx];
                int rel_x = x - win->x;
                int rel_y = y - win->y - 25; /* Account for titlebar */

                for (int i = 0; i < win->gltpm_scene.button_count; i++) {
                    GLTPMButton *btn = &win->gltpm_scene.buttons[i];

                    /* Scoping Check: Only clickable if navigable */
                    bool navigable = true;
                    if (win->active_node_index != -1) {
                        bool is_descendant = false;
                        int p = btn->parent;
                    ...
                    int btn_y = (btn->y >= 0) ? (int)btn->y : (110 + (i * (get_button_height() + 4)));

                    if (rel_x >= btn_x && rel_x <= btn_x + btn_w &&
                        rel_y >= btn_y && rel_y <= btn_y + btn_h) {
                        printf("[GLTPM] Clicked button %d: %s\n", i, btn->label);
                        win->selected_index = i;
                        dispatch_gltpm_button(win, i);
                        glutPostRedisplay();
                        break;
                    }
                }
...
void motion(int x, int y) {
    ...
    if (drag_window_id != -1) {
        ...
        if (is_dragging) {
            ...
            /* Enforce Master Window Bounds */
            if (windows[idx].x < 0) windows[idx].x = 0;
            if (windows[idx].x + windows[idx].w > window_width) windows[idx].x = window_width - windows[idx].w;
            
            if (windows[idx].y < 0) windows[idx].y = 0;
            int max_y = window_height - get_toolbar_height() - 25;
            if (windows[idx].y > max_y) windows[idx].y = max_y;
        } else if (is_resizing) {
            windows[idx].w = win_drag_start_w + dx;
            windows[idx].h = win_drag_start_h + dy;
            if (windows[idx].w < 100) windows[idx].w = 100;
            if (windows[idx].h < 50) windows[idx].h = 50;
            
            if (windows[idx].x + windows[idx].w > window_width) windows[idx].w = window_width - windows[idx].x;
            if (windows[idx].y + windows[idx].h > window_height - get_toolbar_height()) 
                windows[idx].h = window_height - get_toolbar_height() - windows[idx].y;
        }
