...
static void gltpm_scene_reset(GLTPMScene *scene) {
    ...
    for (int i = 0; i < MAX_GLTPM_BUTTONS; i++) {
        scene->buttons[i].x = -1.0f;
        scene->buttons[i].y = -1.0f;
    }
}
...
static int gltpm_load_scene(GLTPMScene *scene, const char *project_root,
    ...
    char *p = layout_sub;
    int parent_stack[MAX_GLTPM_NODES];
    int stack_top = -1;
    float offset_x_stack[MAX_GLTPM_NODES];
    float offset_y_stack[MAX_GLTPM_NODES];
    ...
    while (*p) {
        ...
        if (trimmed[0] == '<' && trimmed[1] == '/') {
            if (stack_top >= 0) {
                stack_top--;
                if (stack_top >= 0) {
                    cur_offset_x = offset_x_stack[stack_top];
                    cur_offset_y = offset_y_stack[stack_top];
                } else {
                    cur_offset_x = 0;
                    cur_offset_y = 0;
                }
        ...
        if (strncmp(trimmed, "<scene", 6) == 0) {
            char title[128] = ""; char bg[64] = ""; char camera[32] = ""; char mode[32] = "";
            ...
            if (gltpm_extract_attr(trimmed, "ui_mode", mode, sizeof(mode))) {
                if (strcmp(mode, "SOVEREIGN") == 0) scene->ui_mode = 1;
                else scene->ui_mode = 0;
            }
        }
        ...
        } else if (strncmp(trimmed, "<panel", 6) == 0 || strncmp(trimmed, "<div", 4) == 0 || strncmp(trimmed, "<ul", 3) == 0) {
            if (scene->node_count < MAX_GLTPM_NODES) {
                GLTPMNode *node = &scene->nodes[scene->node_count];
                char tag[32] = "", id[64] = "", x[32] = "", y[32] = "", w[32] = "", h[32] = "", bg[64] = "";
                
                if (trimmed[1] == 'p') strcpy(node->tag, "panel");
                else if (trimmed[1] == 'd') strcpy(node->tag, "div");
                else strcpy(node->tag, "ul");

                if (gltpm_extract_attr(trimmed, "id", id, sizeof(id))) strncpy(node->id, id, 63);
                node->x = (float)atof(gltpm_extract_attr(trimmed, "x", x, sizeof(x)) ? x : "0") + cur_offset_x;
                node->y = (float)atof(gltpm_extract_attr(trimmed, "y", y, sizeof(y)) ? y : "0") + cur_offset_y;
                node->w = (float)atof(gltpm_extract_attr(trimmed, "width", w, sizeof(w)) ? w : "100");
                node->h = (float)atof(gltpm_extract_attr(trimmed, "height", h, sizeof(h)) ? h : "100");
                
                if (gltpm_extract_attr(trimmed, "color", bg, sizeof(bg))) {
                    float rgb[3];
                    gltpm_parse_rgb(bg, rgb, 0.5, 0.5, 0.5);
                ...
                /* Push to stack */
                stack_top++;
                parent_stack[stack_top] = scene->node_count;
                offset_x_stack[stack_top] = node->x;
                offset_y_stack[stack_top] = node->y;
                cur_offset_x = node->x;
                cur_offset_y = node->y;
            ...
            if (gltpm_extract_attr(trimmed, "id", id, sizeof(id))) {
                float sx = (float)atof(gltpm_extract_attr(trimmed, "x", x, sizeof(x)) ? x : "0") + cur_offset_x;
                float sy = (float)atof(gltpm_extract_attr(trimmed, "y", y, sizeof(y)) ? y : "0") + cur_offset_y;
        ...
        } else if (strncmp(trimmed, "<window", 7) == 0) {
            char title[128] = "";
            if (gltpm_extract_attr(trimmed, "title", title, sizeof(title))) strncpy(scene->title, title, MAX_WIN_TITLE - 1);
            ...
            if (scene->button_count < MAX_GLTPM_BUTTONS) {
                ...
                char lbl[128], clk[128], x[32], y[32], w[32], h[32];
                ...
                /* Auto-layout for li/menuitem inside a parent */
                float local_offset_y = 0;
                if (stack_top >= 0) {
                    /* Count how many buttons already have this same parent */
                    int sibling_count = 0;
                    for (int b = 0; b < scene->button_count - 1; b++) {
                        if (scene->buttons[b].parent == parent_stack[stack_top]) sibling_count++;
                    }
                    local_offset_y = sibling_count * 32.0f; /* Compact line height */
                }

                if (gltpm_extract_attr(trimmed, "x", x, sizeof(x))) btn->x = (float)atof(x) + cur_offset_x;
                else btn->x = cur_offset_x + 10.0f;
                
                if (gltpm_extract_attr(trimmed, "y", y, sizeof(y))) btn->y = (float)atof(y) + cur_offset_y;
                else btn->y = cur_offset_y + local_offset_y + 5.0f;
        ...
        } else if (strncmp(trimmed, "<menu", 5) == 0) {
             printf("[PARSER] Found menu\n");
            ...
            if (scene->button_count < MAX_GLTPM_BUTTONS) {
                ...
                btn->w = (float)atof(gltpm_extract_attr(trimmed, "width", w, sizeof(w)) ? w : "80");
                btn->h = (float)atof(gltpm_extract_attr(trimmed, "height", h, sizeof(h)) ? h : "30");
                btn->parent = (stack_top >= 0) ? parent_stack[stack_top] : -1;
            }
            ...
            if (scene->text_count < MAX_GLTPM_TEXTS) {
                ...
                txt->x = (float)atof(gltpm_extract_attr(trimmed, "x", x, sizeof(x)) ? x : "0") + cur_offset_x;
                txt->y = (float)atof(gltpm_extract_attr(trimmed, "y", y, sizeof(y)) ? y : "0") + cur_offset_y;
