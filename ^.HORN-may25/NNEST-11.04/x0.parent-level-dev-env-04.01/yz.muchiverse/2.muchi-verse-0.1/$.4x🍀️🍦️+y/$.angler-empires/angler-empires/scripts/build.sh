#!/bin/sh
# angler-empires R0 build - each TU its own binary (TPMOS / mutaclsym doctrine)
set -e
cd "$(dirname "$0")/.."
...
if $CC $CFLAGS -o system/gl_mirror system/gl_mirror.c -lglut -lGL -lGLU 2>/tmp/angler-empires_gl_build.log; then
    echo "   ok"
else
    echo "   skipped (see /tmp/angler-empires_gl_build.log)"
fi
...
echo "-- ops (R2 ANGLER EMPIRES verbs)"
...
echo "-- ops (R6 angler instances)"
$CC $CFLAGS -o ops/+x/spawn_angler.+x ops/spawn_angler.c

echo "-- ops (R7 job designation)"
$CC $CFLAGS -o ops/+x/designate.+x ops/designate.c
$CC $CFLAGS -o ops/+x/tick_jobs.+x ops/tick_jobs.c
