 ...
 *
 * Commit action (Enter):
 *   - cursor 1 (New Game): reset the live pieces/world_angler_home/ from the
 *     pristine pieces/world_angler_home_template/ (rm -rf + cp -r, matching
 ...
 *     the chosen pieces/saves/save_N/world_angler_home/ instead of the template.
...
static void reset_world(const char *src_world_dir) {
    ...
    snprintf(world_dst, sizeof(world_dst), "%s/pieces/world_angler_home", project_root);
...
int main(int argc, char **argv) {
    ...
    if (key == ARROW_UP) {
        ...
        if (cursor == 1) {
            ...
            snprintf(tpl_dir, sizeof(tpl_dir), "%s/pieces/world_angler_home_template", project_root);
            ...
            snprintf(save_world, sizeof(save_world), "%s/pieces/saves/%s/world_angler_home", project_root, save_names[cursor - 2]);
