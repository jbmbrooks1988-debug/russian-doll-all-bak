...
static int count_in_inventory(const char *item_id) {
    ...
    snprintf(inventory_dir, sizeof(inventory_dir), "%s/pieces/world_angler_home/map_start/hero/inventory", project_root);
...
int main(void) {
    ...
    snprintf(map_dir, sizeof(map_dir), "%s/pieces/world_angler_home/%s", project_root, map_id);
    ...
    /* NPCs (ANGLER EMPIRES cast - Emma Wood, Hae-rin, etc.), drawn on top of
    ...
    /* CORRECTED 2026-07-21 (real, live-caught gap: ops/spawn_angler.c and
     * the whole possession system - !.pal-standards.txt sec. 28/30 -
     * were built and live-verified via direct state.txt inspection, but
     * this compose op NEVER actually read pieces/<world>/<map>/anglers/
     ...
     * ops/spawn_angler.c already writes a glyph= field straight into each
     * instance's own state.txt, no registry re-lookup needed, same as
     * npcs. Drawn on top of monsters/npcs, below xlector. */
    char anglers_dir[PATH_BUF + 32];
    snprintf(anglers_dir, sizeof(anglers_dir), "%s/anglers", map_dir);
    d = opendir(anglers_dir);
    if (d) {
        struct dirent *entry;
        while ((entry = readdir(d)) != NULL) {
            if (entry->d_name[0] == '.') continue;
            char state_path[PATH_BUF + 320];
            snprintf(state_path, sizeof(state_path), "%s/%s/state.txt", anglers_dir, entry->d_name);
    ...
    if (xlector_y >= 0 && xlector_y < rows) { /* CORRECTED 2026-07-21 sec. 32: focus != control-transfer here, xlector always visible */
    ...
    if (strcmp(possessed_id, "none") != 0) {
        ...
        snprintf(poss_path, sizeof(poss_path), "%s/pieces/world_angler_home/map_start/anglers/%s/state.txt", project_root, possessed_id);
    ...
    if (strcmp(active_panel, "craft") == 0) {
        ...
        snprintf(inventory_dir_panel, sizeof(inventory_dir_panel), "%s/pieces/world_angler_home/map_start/hero/inventory", project_root);
        char panel_rows[MAX_RECIPES_DISPLAY][BOX_TEXT_W + 1];
        int panel_row_count = 0;
...
#pragma GCC diagnostic pop
    ...
    if (!out) return 1;
    fprintf(out, "ANGLER EMPIRES   map: %-10s turn: %d%s\n",
    ...
    fprintf(out, "  | Mode: %s | Focused: %s\n", interact_mode ? "ACTIVE[^]" : "NAV[>]", possessed_id);
    /* CORRECTED 2026-07-21 per !.pal-standards.txt sec. 32 (direct user
     * catch: real Dwarf Fortress only ever "focuses a menu" on a
     * selected dwarf, never lets you puppet its movement) - this is the
     * real "focus panel" replacing the old possess-and-move behavior.
     * Shows the focused angler's own info (role/job/hp), reusing fields
     * ops/spawn_angler.c already writes - no new data needed, just the
     * first real consumer of it. Angler movement stays fully autonomous
     * regardless of focus (move_player.c no longer redirects arrows to
     * it at all - see that file's own header comment). */
    if (strcmp(possessed_id, "none") != 0) {
        char focus_path[PATH_BUF + 384];
        snprintf(focus_path, sizeof(focus_path), "%s/pieces/world_angler_home/map_start/anglers/%s/state.txt", project_root, possessed_id);
        char fname[64], frole[32], fjob[32];
        int fhp = read_kv_int(focus_path, "hp", -1);
        read_kv_str(focus_path, "name", fname, sizeof(fname), "?");
        read_kv_str(focus_path, "role", frole, sizeof(frole), "?");
        read_kv_str(focus_path, "job_id", fjob, sizeof(fjob), "none");
        fprintf(out, "  Focus: %s (%s)  HP:%d  Job:%s\n", fname, frole, fhp, fjob);
    }
    fprintf(out, "%s\n", inventory_line);
