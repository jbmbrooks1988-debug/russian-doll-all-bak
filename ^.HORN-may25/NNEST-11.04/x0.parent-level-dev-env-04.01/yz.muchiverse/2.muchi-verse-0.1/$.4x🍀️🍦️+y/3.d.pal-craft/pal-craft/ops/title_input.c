 ...
 *
 * Commit action (Enter):
 *   - cursor 1 (New Game): reset the live pieces/world_pal_craft_home/ from the
 *     pristine pieces/world_pal_craft_home_template/ (rm -rf + cp -r, matching
 ...
 *     the chosen pieces/saves/save_N/world_pal_craft_home/ instead of the template.
...
static void reset_world(const char *src_world_dir) {
    ...
    snprintf(world_dst, sizeof(world_dst), "%s/pieces/world_pal_craft_home", project_root);
    ...
    /* CORRECTED 2026-07-21 (real, live-caught confusion): a fresh world
     * (New Game or Load Game) should start with a clean message log -
     * without this, stale lines from a PREVIOUS play session (e.g. one
     * that never restarted button.sh run, just returned to the title
     * screen and started over) leak into the new game and look exactly
     * like something auto-happened before the player ever pressed a
     * key. button.sh's own `run` action truncates this too (covers the
     * process-start case); this covers New-Game-mid-session too. */
...
int main(int argc, char **argv) {
    ...
    if (key == ARROW_UP) {
        ...
        if (cursor == 1) {
            ...
            snprintf(tpl_dir, sizeof(tpl_dir), "%s/pieces/world_pal_craft_home_template", project_root);
            /* reset_world() already appends the 999 sentinel itself (see
             * its own header comment) - CORRECTED 2026-07-21 (real,
             * live-caught bug): a SECOND, redundant "999\n" append used
             * to live here too, meaning every New Game/Load Game commit
             * queued the sentinel TWICE. The gameplay loop's own
             * `loop:` label (pal/main_loop.pal) has no special case for
             * 999 (only title_loop does) - a stray 999 reaching it fell
             * through move_player/choice like any other unrecognized
             * key, and choice.c's own catch-all silently reset
             * action_cursor to -1 right at the start of every fresh
             * game, before the player ever pressed a real key -
             * visible live as the "[>]" marker vanishing on the very
             * first rendered gameplay frame, looking like a bug in
             * nav cycling when the actual cause was upstream. */
            reset_world(tpl_dir);
        } else if (cursor >= 2 && cursor <= total) {
            char save_world[PATH_BUF];
            snprintf(save_world, sizeof(save_world), "%s/pieces/saves/%s/world_pal_craft_home", project_root, save_names[cursor - 2]);
            reset_world(save_world); /* also appends its own single 999 - see above */
        }
        digit_accum = 0;
    } else {
        digit_accum = 0;
