...
/* Small generic key=value readers, needed by the "attack"/"capture"
 * combat special cases below - this file's own existing helpers all
 * assume a fixed known field set (hero/state.txt's own layout), these
 * two are genuinely generic. Same self-contained-op convention as every
 * other file in this family (own copy, not shared). */
static int read_kv_int_local(const char *path, const char *key, int def) {
    FILE *f = fopen(path, "r");
    if (!f) return def;
    char line[MAX_LINE];
    int val = def;
...
static void examine_at(const char *map_id, int x, int y, char *selected_id_out, size_t selected_id_sz) {
    char items_dir[PATH_BUF + 32], monsters_dir[PATH_BUF + 32], entities_dir[PATH_BUF + 32];
    snprintf(items_dir, sizeof(items_dir), "%s/pieces/world_muchimon_home/%s/items", project_root, map_id);
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_muchimon_home/%s/monsters", project_root, map_id);
...
int main(int argc, char **argv) {
    ...
    if (is_3d_toggle) {
        ...
        if (strcmp(possessed_id, "none") != 0) {
            ...
            snprintf(entity_path, sizeof(entity_path), "%s/pieces/world_muchimon_home/map_start/entities/%s/state.txt", project_root, possessed_id);
            int upx = -1, upy = -1;
        ...
        if (is_digit) {
            ...
            if (action_cursor >= 2 && action_cursor < total) {
                if (strcmp(names[action_cursor], "craft") == 0) {
                ...
                } else if (strcmp(dispatch_id, "hero") != 0 && strcmp(dispatch_id, "xlector") != 0 && strcmp(names[action_cursor], "capture") == 0) {
                    /* MUCHIMON-DESIGN.txt sec. 8 - capture a possessed
                     * muchimon OR trainer: flips its own civ_id to
                     * "player_party" (same read-all/rewrite-one-field
                     * idiom every op in this family already uses). Real
                     * capture should be gated by the capturing item's
                     * own "flexbox" power tier (direct user note: "you
                     * will likely not have a powerful enough flexbox
                     * starting out") - NOT built (muchibox design is
                     * explicitly deferred), this always succeeds once
                     * the HP threshold below is met - real for TESTING
                     * only. CORRECTED 2026-07-21: civ_id check widened
                     * from "== wild" to "!= player_party" (trainers
                     * spawn with civ_id=debug/whatever their own owner
                     * is, never literally "wild" - the real rule is
                     * "not already mine", not "specifically wild"). */
                    char entity_path2[PATH_BUF + 384];
                    snprintf(entity_path2, sizeof(entity_path2), "%s/pieces/world_muchimon_home/map_start/entities/%s/state.txt", project_root, dispatch_id);
                    char civ_id_now[64], ename[64];
                    int chp = read_kv_int_local(entity_path2, "hp", 1);
                    int cmaxhp = read_kv_int_local(entity_path2, "max_hp", chp > 0 ? chp : 1);
                    snprintf(civ_id_now, sizeof(civ_id_now), "wild");
                    snprintf(ename, sizeof(ename), "%s", dispatch_id);
                    FILE *cf2 = fopen(entity_path2, "r");
                    if (cf2) {
                        char line2[MAX_LINE];
                        while (fgets(line2, sizeof(line2), cf2)) {
                            line2[strcspn(line2, "\r\n")] = '\0';
                            if (strncmp(line2, "civ_id=", 7) == 0) snprintf(civ_id_now, sizeof(civ_id_now), "%s", line2 + 7);
                            else if (strncmp(line2, "name=", 5) == 0) snprintf(ename, sizeof(ename), "%s", line2 + 5);
                        }
                        fclose(cf2);
                    }
                    ...
                    if (strcmp(civ_id_now, "player_party") == 0) {
                        if (lf2) fprintf(lf2, "%s is already caught.\n", ename);
                    } else if (chp > cmaxhp / 2) {
                        /* "wear it down" - direct user request: capture
                         * only succeeds once HP is at/below half max,
                         * not on a fresh, undamaged target. */
                        if (lf2) fprintf(lf2, "%s is too strong to catch yet - weaken it first!\n", ename);
                    } else {
                        char clines[24][MAX_LINE];
                        int cn = 0;
                        FILE *rf = fopen(entity_path2, "r");
                        if (rf) { while (cn < 24 && fgets(clines[cn], MAX_LINE, rf)) cn++; fclose(rf); }
                        FILE *wf = fopen(entity_path2, "w");
                        if (wf) {
                            for (int i = 0; i < cn; i++) {
                                if (strncmp(clines[i], "civ_id=", 7) == 0) { fprintf(wf, "civ_id=player_party\n"); continue; }
                                fputs(clines[i], wf);
                            }
                            fclose(wf);
                        }
                        if (lf2) fprintf(lf2, "Captured %s!\n", ename);
                    }
                    if (lf2) fclose(lf2);
                } else if (strcmp(names[action_cursor], "attack") == 0) {
                    /* MUCHIMON-DESIGN.txt sec. 8 - "fight the wild
                     * muchimon to wear it down". Finds an ADJACENT
                     * entity (any of the 4 cardinal directions, any
                     * civ_id different from the attacker's own) and
                     * applies damage = max(1, attacker's own attack -
                     * defender's own defense) - same simple formula
                     * civ-pal's own unit_bank.txt columns were already
                     * shaped for, first real consumer of attack/defense
                     * as combat stats rather than decorative bank
                     * fields. On defeat (hp<=0), the target is removed
                     * outright (fainted, matches real Pokemon's own
                     * "fainted muchimon can't be caught" rule - a
                     * fainted target is simply gone here, not a
                     * separate captureable state, smallest correct
                     * slice). */
                    char attacker_path[PATH_BUF + 384];
                    snprintf(attacker_path, sizeof(attacker_path), "%s/pieces/world_muchimon_home/map_start/entities/%s/state.txt", project_root, dispatch_id);
                    int aatk = read_kv_int_local(attacker_path, "attack", 1);
                    int ax = read_kv_int_local(attacker_path, "pos_x", -1);
                    int ay = read_kv_int_local(attacker_path, "pos_y", -1);
                    char attacker_civ[64];
                    read_kv_str_local(attacker_path, "civ_id", attacker_civ, sizeof(attacker_civ), "");

                    char entities_dir2[PATH_BUF + 32];
                    snprintf(entities_dir2, sizeof(entities_dir2), "%s/pieces/world_muchimon_home/map_start/entities", project_root);
                    DIR *ed = (ax >= 0 && ay >= 0) ? opendir(entities_dir2) : NULL;
                    char target_id[64] = "";
                    if (ed) {
                        struct dirent *ee;
                        while ((ee = readdir(ed)) != NULL) {
                            if (ee->d_name[0] == '.' || strcmp(ee->d_name, dispatch_id) == 0) continue;
                            char tp[PATH_BUF + 384];
                            snprintf(tp, sizeof(tp), "%s/%s/state.txt", entities_dir2, ee->d_name);
                            char tciv[64];
                            read_kv_str_local(tp, "civ_id", tciv, sizeof(tciv), "");
                            if (strcmp(tciv, attacker_civ) == 0) continue; /* never friendly-fire own civ */
                            int tx = read_kv_int_local(tp, "pos_x", -999);
                            int ty = read_kv_int_local(tp, "pos_y", -999);
                            if (abs(tx - ax) + abs(ty - ay) == 1) { snprintf(target_id, sizeof(target_id), "%s", ee->d_name); break; }
                        }
                        closedir(ed);
                    }

                    char log_path3[PATH_BUF];
                    snprintf(log_path3, sizeof(log_path3), "%s/pieces/display/message_log.txt", project_root);
                    FILE *lf3 = fopen(log_path3, "a");
                    if (!target_id[0]) {
                        if (lf3) fprintf(lf3, "Nothing to attack nearby.\n");
                    } else {
                        char target_path[PATH_BUF + 384];
                        snprintf(target_path, sizeof(target_path), "%s/%s/state.txt", entities_dir2, target_id);
                        int tdef = read_kv_int_local(target_path, "defense", 0);
                        int thp = read_kv_int_local(target_path, "hp", 1);
                        char tname[64];
                        read_kv_str_local(target_path, "name", tname, sizeof(tname), target_id);
                        int dmg = aatk - tdef; if (dmg < 1) dmg = 1;
                        thp -= dmg;
                        if (thp <= 0) {
                            remove(target_path);
                            char tdir[PATH_BUF + 384];
                            snprintf(tdir, sizeof(tdir), "%s/%s", entities_dir2, target_id);
                            rmdir(tdir);
                            if (lf3) fprintf(lf3, "%s took %d damage and fainted!\n", tname, dmg);
                        } else {
                            char tlines[24][MAX_LINE];
                            int tn = 0;
                            FILE *trf = fopen(target_path, "r");
                            if (trf) { while (tn < 24 && fgets(tlines[tn], MAX_LINE, trf)) tn++; fclose(trf); }
                            FILE *twf = fopen(target_path, "w");
                            if (twf) {
                                for (int i = 0; i < tn; i++) {
                                    if (strncmp(tlines[i], "hp=", 3) == 0) { fprintf(twf, "hp=%d\n", thp); continue; }
                                    fputs(tlines[i], twf);
                                }
                                fclose(twf);
                            }
                            if (lf3) fprintf(lf3, "%s took %d damage (hp %d).\n", tname, dmg, thp);
                        }
                    }
                    if (lf3) fclose(lf3);
                } else {
