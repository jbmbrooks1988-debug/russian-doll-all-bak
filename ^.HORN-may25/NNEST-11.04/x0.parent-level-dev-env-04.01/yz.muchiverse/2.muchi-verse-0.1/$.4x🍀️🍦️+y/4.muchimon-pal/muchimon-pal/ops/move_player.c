 ...
 * pieces/world_muchimon_home/map_start/hero/ - its own map_id FIELD tracks which
 ...
 * muchi-civ's cursor is transient and doesn't need to persist as its
...
int main(int argc, char **argv) {
    ...
    int key = atoi(argv[1]);
    int dx = 0, dy = 0;
    /* REAL, DIRECT INSTRUCTION: wasd no longer moves the hero/xlector -
     * arrows are the only movement keys now, freeing wasd up entirely
     * for 3D camera pan (see the CAMERA PAN block further down, gated
     * on render_mode==1 && camera_mode==3). Matches mutaclsym's own
     * identical control-scheme change. A real, deliberate behavior
     * change, not an addition - wasd used to also move the hero; it no
    ...
    switch (key) {
        ...
        default: break; /* not an arrow - may still be a camera-pan key,
                          ...
                          * render_mode/camera_mode to know whether
                          * wasd/xz mean anything right now). */
    }

    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/world_muchimon_home/map_start/hero/state.txt", project_root);
    FILE *f = fopen(path, "r");
    if (!f) return 1;
    ...
    char possessed_id[64] = "none"; /* CORRECTED 2026-07-21 (direct user
        walkthrough of real fuzz-op's own live behavior, see ops/choice.c's
        header comment for the full citation): "none" = plain xlector
        cursor movement (free/uncollided, unchanged below). Any other
        value names a units/<possessed_id>/ instance the player currently
        POSSESSES (set by ops/choice.c's Enter-on-a-unit handling) -
        arrows then move THAT UNIT, walkable-checked like any real
        piece's own movement, not the free-roaming cursor. */
    ...
    /* CAMERA PAN: only meaningful once wasd is freed up from movement
     * (see this file's own header comment above) and only while
     * render_mode==1 (3D GL view - a no-op in plain ASCII, matching
     * mutaclsym's own convention that camera commands only matter when
     * the GL view is what's actually being looked at). w/s pan forward/
     * back, a/d pan left/right, x/z pan up/down on the vertical axis.
     * Only meaningful in POV 3 (free camera) - POV 1/2 are fixed
     * presets the 3D renderer itself computes (see
     * ops/compose_rgb_frame.c's own camera_mode switch), so panning is
     * a pure no-op there too, not just in 2D. */
    ...
    /* CORRECTED 2026-07-21 per !.pal-standards.txt sec. 28.4: while
     * interact_mode==0 (NAV), arrow keys are a pure no-op - there is no
     * hero avatar to move in this control model (matches angler-empires'
     * own draft-3 correction), and NAV mode's own digit/Enter dispatch
     * lives entirely in ops/choice.c, a separate op from this one. Only
     * once interact_mode==1 (ACTIVE - entered via choice.c's real
     * "interact" method row, never a bound key) does either of the two
     * blocks below (possession-redirect or free-cursor) apply. */
    ...
    if (interact_mode == 1 && strcmp(possessed_id, "none") != 0 && (dx != 0 || dy != 0)) {
        /* POSSESSION movement (2026-07-21 correction - see this file's
         * own possessed_id declaration comment above and ops/choice.c's
         * header for the full fuzz-op citation): a real, walkable-
         * checked move of the possessed unit itself, NOT the free
         * xlector cursor - matches real fuzz-op's own "xlector moves
         * pet" behavior once a pet is possessed. Deliberately the
         * SMALLEST correct slice: walkable-check only, no monster-
         * attack, no map-transition handling (unlike the hero's own
         * movement below) - a unit walking into another unit/attacking/
         * changing maps is real future work, not designed yet, and
         * silently reusing the hero's full move logic here would imply
         * behavior (e.g. "units can trigger map transitions") nobody
         * has actually decided on. */
        char entities_dir[PATH_BUF + 32];
        snprintf(entities_dir, sizeof(entities_dir), "%s/pieces/world_muchimon_home/%s/entities", project_root, map_id);
        char entity_path[PATH_BUF + 384];
        snprintf(entity_path, sizeof(entity_path), "%s/%s/state.txt", entities_dir, possessed_id);

        int upx = read_int_field(entity_path, "pos_x", -1);
        int upy = read_int_field(entity_path, "pos_y", -1);
        if (upx < 0 || upy < 0) return 0; /* possessed instance vanished/corrupt - no-op, don't crash */

        int unx = upx + dx, uny = upy + dy;
        char terrain_glyph = file_glyph_at(map_path, unx, uny, '#', map_w, map_h);
        char furniture_glyph = file_glyph_at(furniture_path, unx, uny, ' ', map_w, map_h);
        if (glyph_walkable("pieces/registry/terrain/terrain_types.txt", terrain_glyph) &&
            furniture_walkable(furniture_glyph)) {
            FILE *uf = fopen(entity_path, "r");
            char ulines[24][MAX_LINE];
            int unlines = 0;
            if (uf) { while (unlines < 24 && fgets(ulines[unlines], MAX_LINE, uf)) unlines++; fclose(uf); }
            uf = fopen(entity_path, "w");
            if (uf) {
                int ux_found = 0, uy_found = 0;
                for (int i = 0; i < unlines; i++) {
                    if (strncmp(ulines[i], "pos_x=", 6) == 0) { fprintf(uf, "pos_x=%d\n", unx); ux_found = 1; continue; }
                    if (strncmp(ulines[i], "pos_y=", 6) == 0) { fprintf(uf, "pos_y=%d\n", uny); uy_found = 1; continue; }
                    fputs(ulines[i], uf);
                }
                if (!ux_found) fprintf(uf, "pos_x=%d\n", unx);
                if (!uy_found) fprintf(uf, "pos_y=%d\n", uny);
                fclose(uf);
            }

            /* MUCHIMON-DESIGN.txt sec. 8.2 - wild encounter trigger.
             * Reuses the EXISTING t_grass terrain row as "tall grass"
             * (design doc's own t_tall_grass-equivalent) rather than
             * adding a near-duplicate registry row - grass tiles
             * already mean the same thing thematically, no new
             * machinery needed. Only the PLAYER's own possessed
             * category triggers this (a possessed trainer/muchimon
             * wandering around should not re-roll encounters against
             * itself). Deliberately simple: fixed 1-in-5 roll, spawns
             * one of entity_bank.txt's own muchimon-category rows
             * adjacent to the encounter tile via spawn_entity.+x (same
             * op-calls-op pattern this family already uses everywhere -
             * title_input.c's own reset_world(), choice.c's own
             * exec_handler dispatch). civ_id=wild marks it capturable
             * (ops/choice.c's own "capture" special case only acts on
             * civ_id=wild). NOT built: a real per-route encounter TABLE
             * (which species, what odds) - always sparkit for now,
             * proving the trigger mechanism end-to-end first. */
            if (terrain_glyph == '"') {
                char category[32];
                read_str_field(entity_path, "category", category, sizeof(category), "");
                /* Fresh short-lived process every keypress (this
                 * family's own standing convention) - rand() needs a
                 * real per-call seed or every encounter roll in the
                 * same second would be identical. */
                srand((unsigned)time(NULL) ^ (unsigned)getpid());
                if (strcmp(category, "player") == 0 && (rand() % 5) == 0) {
                    char wild_id[64];
                    snprintf(wild_id, sizeof(wild_id), "wild_%d", (int)time(NULL) % 100000);
                    char spawn_cmd[PATH_BUF * 2];
                    int wx = unx, wy = uny - 1;
                    if (wy < 0) wy = uny + 1;
                    snprintf(spawn_cmd, sizeof(spawn_cmd),
                        "'%s/ops/+x/spawn_entity.+x' sparkit '%s' world_muchimon_home wild %d %d >/dev/null 2>&1",
                        project_root, wild_id, wx, wy);
                    ...
                    if (lf) { fprintf(lf, "A wild Sparkit appeared!\n"); fclose(lf); }
                }
            }
        }
        return 0; /* possession movement never touches hero/xlector state */
    }
    ...
    if (interact_mode == 1) {
        ...
        xlector_x = nxl;
        xlector_y = nyl;
        /* CORRECTED 2026-07-21 (direct user request: "the 3d camera
         * needs to move with the xelector" - compose_rgb_frame.c's own
         * render_3d_view() now anchors the camera to xlector_x/
         * xlector_y instead of the hero's static position, but 1st-
         * person view DIRECTION comes from `facing`, which this block
         * used to skip entirely via its own early `return 0` below -
         * the camera would follow position but never rotate to face
         * the way the cursor last moved). Same "only update on an
         * actual arrow press" rule the old hero-movement path already
         * used (see that code's own comment further down this file). */
        facing = key;

        f = fopen(path, "w");
        if (!f) return 1;
        int xx_found = 0, xy_found = 0, facing_found = 0;
        for (int i = 0; i < nlines; i++) {
            char *eq = strchr(lines[i], '=');
            if (eq) {
                *eq = '\0';
     ...
     * 3D renderer's 1st-person view direction (see
     * ops/compose_rgb_frame.c's own camera_mode==1 handling). Only
     * updated on an actual arrow press (dx/dy nonzero) - any other key
     * reaching this point (e.g. a blocked move into a wall, or an
     * attack) leaves the previous facing untouched rather than
     * overwriting it with an irrelevant key value. */
