 ...
 * simpler - flat per-tile top-face color, no raymarch, since muchi-civ
 ...
 * framebuffer - flat top-face color, no raymarch (muchi-civ has no
 ...
 * scope for muchi-civ's HUD text specifically, tracked separately from
 ...
 * in the GL window" - both muchi-civ's dispatch (ops/choice.c) and the
 ...
 * DDA wall renderer, same code, adapted to muchi-civ's own PATH_BUF/
 ...
 *     pieces/registry/emoji_assets/<id>/) - if muchi-civ has no such
 ...
 * sample_voxel_pixel().
 *
 * Frame convention: mutaclsym/muchi-civ have no mouse-orbit - world
 ...
 * dependency on these assets existing in muchi-civ's own registry. */
...
static void render_3d_view(unsigned char *fb, int fb_w, int fb_h,
    ...
    /* Camera presets - values chosen for muchi-civ's own tile scale
...
int main(void) {
    ...
    /* CORRECTED 2026-07-21 - same fix as compose_frame.c's own copy
     * (ASCII and GL/RGB are two separate rendering pipelines,
     * PIECECRAFT-WRAITH-ARCHITECTURE.md sec. 2's own gotcha): units/
     * was never read here either, so spawned units were invisible in
     * the GL mirror too, not just ASCII. */
    ...
    if (d) {
        ...
        while ((entry = readdir(d)) != NULL) {
            ...
            char glyph[4], uname[64];
            read_kv_str(state_path, "glyph", glyph, sizeof(glyph), "?");
            read_kv_str(state_path, "name", uname, sizeof(uname), entry->d_name);
            if (glyph[0]) grid[uy][ux] = glyph[0];
            if (emoji_mode && glyph[0]) snprintf(cell_asset[uy][ux], ASSET_ID_BUF, "%s", uname);
        }
        closedir(d);
    }

    /* Removed 2026-07-21 - same fix as compose_frame.c's own copy (ASCII
     * and GL/RGB are two separate rendering pipelines, PIECECRAFT-
     * WRAITH-ARCHITECTURE.md sec. 2's own documented gotcha): the hero
     * glyph was a dead mansion-sim leftover, drawn at a position that
     * never moves, in a control model with no real hero avatar at all -
     * see compose_frame.c's own header comment for the full citation. */

    /* Xlector cursor - same real xlector active-target pattern as
     * compose_frame.c's own copy (see that file's header comment and
     * !.pal-standards.txt sec. 28 for the full writeup). Drawn last, on
     * top of even the hero.
     *
     * CORRECTED 2026-07-21 - same fix as compose_frame.c's own copy
     * (ASCII and GL/RGB are two separate rendering pipelines per
     * PIECECRAFT-WRAITH-ARCHITECTURE.md sec. 2's own documented
     * gotcha - a fix to one never automatically applies to the other,
     * confirmed real here: this GL-mirror copy still had the OLD
     * interact_mode==1 gate after the ASCII copy was already fixed).
     * Direct user correction: "we dont need to erase xelector position
     * from view, its enough that it stay still" - always drawn now,
     * regardless of interact_mode.
     *
     * FURTHER CORRECTED 2026-07-21 - same possession-hides-xlector fix
     * as compose_frame.c's own copy, see that file's header comment for
     * the full citation. */
     ...
     *
     * CORRECTED 2026-07-21 - same fix as compose_frame.c's own copy
     * (ASCII and GL/RGB are separate pipelines, PIECECRAFT-WRAITH-
     * ARCHITECTURE.md sec. 2's own gotcha): follows xlector, or the
     * POSSESSED piece's own live position while something is possessed
     * (px/py, the dead hero position, never moves - see that file's own
    ...
    if (render_mode == 1) {
         ...
         * needs to move with the xelector") - render_3d_view()'s own
         * px/py params are its CAMERA ANCHOR, not specifically "the
         * hero" (a naming leftover from the mansion-sim era, before
         * this project's own control model became cursor/possession-
         * based per !.pal-standards.txt sec. 28). Pass xlector_x/
         * xlector_y here instead of the hero's own (now permanently
         * static, per sec. 28.4 - no avatar ever moves) px/py, so the
         * 3D view follows whatever the player is actually looking
         * through the cursor at. Following a POSSESSED unit's own
         * position instead, once one exists, is a real follow-up (not
         * done here - this project's compose_rgb_frame.c doesn't read
         * possessed_id at all yet), not silently assumed done. */
        render_3d_view((unsigned char *)framebuf, FRAME_W, FRAME_H, grid, rows, map_w, xlector_x, xlector_y, camera_mode, facing, cam_pan_x, cam_pan_y, cam_pan_z);
    } else {
...
#pragma GCC diagnostic pop
    ...
    if (z != 0) {
        snprintf(header_text, sizeof(header_text), "MUCHIMON  map: %-10s turn: %d  Floor: %d", map_id, turn, z);
    } else {
        snprintf(header_text, sizeof(header_text), "MUCHIMON  map: %-10s turn: %d", map_id, turn);
    }
