#!/bin/sh
# muchimon-pal R0 build - each TU its own binary (TPMOS / mutaclsym doctrine)
set -e
cd "$(dirname "$0")/.."
...
if $CC $CFLAGS -o system/gl_mirror system/gl_mirror.c -lglut -lGL -lGLU 2>/tmp/muchimon-pal_gl_build.log; then
    echo "   ok"
else
    echo "   skipped (see /tmp/muchimon-pal_gl_build.log)"
fi
...
echo "-- dispatch_event (pal-op event editor's own runtime half - see"
echo "   pal-op-handoff.txt sec. 3.1)"
$CC $CFLAGS -o ops/+x/dispatch_event.+x ops/dispatch_event.c

echo "-- ops (R6 entity instances)"
$CC $CFLAGS -o ops/+x/spawn_entity.+x ops/spawn_entity.c
