 ...
 * pieces/world_muchi_civ_home/map_start/hero/ - its own map_id FIELD tracks which
...
int main(int argc, char **argv) {
    ...
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/world_muchi_civ_home/map_start/hero/state.txt", project_root);
    FILE *f = fopen(path, "r");
    if (!f) return 1;
    ...
    if (interact_mode == 1 && strcmp(possessed_id, "none") != 0 && (dx != 0 || dy != 0)) {
        ...
        snprintf(units_dir, sizeof(units_dir), "%s/pieces/world_muchi_civ_home/%s/units", project_root, map_id);
        char unit_path[PATH_BUF + 384];
        snprintf(unit_path, sizeof(unit_path), "%s/%s/state.txt", units_dir, possessed_id);

        int upx = read_int_field(unit_path, "pos_x", -1);
        int upy = read_int_field(unit_path, "pos_y", -1);
