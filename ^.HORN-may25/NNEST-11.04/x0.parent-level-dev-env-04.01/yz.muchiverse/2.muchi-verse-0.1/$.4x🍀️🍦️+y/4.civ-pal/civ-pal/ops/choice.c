...
static void examine_at(const char *map_id, int x, int y, char *selected_id_out, size_t selected_id_sz) {
    char items_dir[PATH_BUF + 32], monsters_dir[PATH_BUF + 32], units_dir[PATH_BUF + 32];
    snprintf(items_dir, sizeof(items_dir), "%s/pieces/world_muchi_civ_home/%s/items", project_root, map_id);
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_muchi_civ_home/%s/monsters", project_root, map_id);
...
int main(int argc, char **argv) {
    ...
    if (is_3d_toggle) {
        ...
        if (strcmp(possessed_id, "none") != 0) {
            ...
            snprintf(unit_path, sizeof(unit_path), "%s/pieces/world_muchi_civ_home/map_start/units/%s/state.txt", project_root, possessed_id);
            int upx = -1, upy = -1;
