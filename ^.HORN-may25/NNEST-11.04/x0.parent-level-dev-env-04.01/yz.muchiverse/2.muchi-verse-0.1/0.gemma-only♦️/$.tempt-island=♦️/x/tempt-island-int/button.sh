#!/bin/bash
# button.sh - launcher for tempt-island, same verb convention as TPMOS's
...
case "$ACTION" in
    ...
    run|r|start)
        ...
        export PRISC_PROJECT_ID="tempt-island"
        ...
        if [ -z "$NO_GL" ] && [ -x "system/gl_mirror" ]; then
            ./system/gl_mirror >/tmp/tempt-island_gl_mirror.log 2>&1 &
            GL_PID=$!
            ...
            echo "/tmp/tempt-island_gl_mirror.log - the terminal session below is" >&2
    ...
    generate|gen)
        ...
        ;;
    kill|k|stop)
        echo "=== Killing tempt-island processes ==="
    ...
    help|h|-h|--help)
        ...
        echo "  kill, k, stop       - Kill any lingering tempt-island processes"
