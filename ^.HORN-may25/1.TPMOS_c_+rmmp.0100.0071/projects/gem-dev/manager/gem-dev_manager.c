...
void check_ai_status(void) {
    ...
    if (tool_name && strlen(tool_name) > 0) {
        ...
        if (result) {
            // Append function call and response to context (Gemini style)
            // Note: json_state append needs to handle functionCall/functionResponse roles
            char *a_args[] = {"projects/gem-dev/ops/+x/json_state", "append", ctx_file, "assistant", function_call, NULL};
        ...
        free(tool_name); free(args_json); free(text_content); free(function_call);
