...
char active_game_name[MAX_LINE] = "template";
            ...
            while (fgets(line, sizeof(line), f)) {
                ...
                if (eq) {
                    *eq = '\0';
                    ...
                    else if (strcmp(k, "active_game") == 0) strncpy(active_game_name, v, sizeof(active_game_name)-1);
                }
            }
            fclose(f);
        }
...
int main() {
    ...
    /* Remove .txt extension first if present */
    char *dot = strstr(base, ".txt");
    if (dot) *dot = '\0';
    ...
    if (asprintf(&map_path, "%s/%s/maps/%s", project_root, active_game_name, map_with_z) != -1) {
        mf = fopen(map_path, "r");
        if (!mf && current_z == 0) {
            /* Fallback to non-z version if z0 not found */
            free(map_path);
            if (asprintf(&map_path, "%s/%s/maps/%s.txt", project_root, active_game_name, base) != -1) {
                mf = fopen(map_path, "r");
            }
        }
    }
    ...
    // Scan directly from the game's pieces directory
    if (asprintf(&pieces_dir, "%s/%s/pieces", project_root, active_game_name) != -1) {
