...
static void set_window_identity(Window *window, int instance_no) {
    ...
    strncpy(window->project_id, "launcher", sizeof(window->project_id) - 1);
...
static void build_launcher_markup(char *out, size_t size, Window *window) {
    ...
    if (strcmp(window->project_id, "launcher") == 0) {
        ...
        if (f) {
            while (fgets(line, sizeof(line), f) && count < 4) {
                if (strncmp(line, "METHOD", 6) == 0) {
                    ...
                    char *cmd_val = trim_ws(val_start + 1);
                    *val_start = '\0';
                    char *name = trim_ws(key_start);

                    char row[512];
                    /* Goal: "| |  [>] 1. [name] ... |" */
                    snprintf(row, sizeof(row), "| |  <button compact=\"true\" label=\"%-12s\" onClick=\"%s\" visibility=\"${desktop_active_window_body_visible}\" />", name, cmd_val);
                    
                    /* Pad to width 95 before | */
                    int current_len = 5 + 4 + 12; 
                    for (int i = current_len; i < 95; i++) strcat(row, " ");
                    strcat(row, "|<br visibility=\"${desktop_active_window_body_visible}\" />");

                    if (strlen(out) + strlen(row) < size - 1) {
                        strcat(out, row);
                        count++;
                    }
                }
    ...
    if (count == 0) {
        char row[512];
        snprintf(row, sizeof(row), "| |  %-85s |<br visibility=\"${desktop_active_window_body_visible}\" />", "[ No Projects Found ]");
        strcat(out, row);
        count++;
    }

    /* Fill remaining lines to maintain fixed window height */
    while (count < 4) {
        strcat(out, "| |                                                                                              |<br visibility=\"${desktop_active_window_body_visible}\" />");
        count++;
    }
}
...
static void write_projection(FILE *f, int last_key) {
    ...
    build_launcher_markup(body_content, sizeof(body_content), window);
    ...
    fprintf(f, "wraith_body_content=%s\n", body_content);
...
static void launch_rgb_pipeline(void) {
    ...
    if (d_pid > 0) log_pid(d_pid, "wraith_rgb_daemon");
#endif
    ...
    if (g_pid > 0) log_pid(g_pid, "wraith_gl");
#endif
}
