...
void write_gui_state() {
    ...
    if (f) {
        ...
        fprintf(f, "<button label=\"[ START ]\" onClick=\"COMMAND: START\" /> ");
        fprintf(f, "<button label=\"[ PAUSE ]\" onClick=\"COMMAND: PAUSE\" /> ");
        fprintf(f, "<button label=\"[ STEP / END TURN ]\" onClick=\"COMMAND: STEP\" />");
        fprintf(f, "\n");
        ...
        int method_count = read_methods_from_pdl(pdl_path, methods, sizeof(methods), 0);
        ...
        if (method_count > 0) {
            ...
            while (cursor && *cursor) {
                ...
                if (next) *next = '\0';
                if (*cursor) {
                    fprintf(f, "<button label=\"%s\" onClick=\"KEY:%d\" /> ", cursor, idx++);
                }
                cursor = next ? next + 1 : NULL;
            }
        }
