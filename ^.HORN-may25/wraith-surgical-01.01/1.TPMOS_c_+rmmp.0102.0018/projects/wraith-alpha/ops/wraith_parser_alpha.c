...
static bool handle_desktop_action_command(const char* cmd) {
    if (strncmp(cmd, "SET_ACTIVE:", 11) != 0) return false;
    int idx = atoi(cmd + 11);
    bool terminal_collapsed = (strcmp(get_var("desktop_window_1_collapsed"), "true") == 0) ||
                              (strcmp(get_var("fold_terminal"), "folded") == 0);
    bool terminal_fullscreen = (strcmp(get_var("desktop_window_1_fullscreen"), "true") == 0) ||
                               (strcmp(get_var("desktop_window_1_mode"), "full") == 0);
    bool terminal_minimized = (strcmp(get_var("desktop_window_1_minimized"), "true") == 0) ||
                              (strcmp(get_var("desktop_window_1_minimized_visible"), "true") == 0);
    int terminal_count = atoi(get_var("desktop_window_count"));
    if (terminal_count < 0) terminal_count = 0;
    FILE *audit = fopen("parser.out", "a");
    if (audit) {
        fprintf(audit, "DESKTOP ACTION: cmd=%s idx=%d collapsed=%s fullscreen=%s open=%s\n",
                cmd,
                idx,
                terminal_collapsed ? "true" : "false",
                terminal_fullscreen ? "true" : "false",
                get_var("desktop_window_1_open"));
        fclose(audit);
    }
    if (idx == 3) {
        apply_desktop_ui_state(true, false, false, terminal_count > 0 ? terminal_count : 1);
    } else if (idx == 2) {
        apply_desktop_ui_state(terminal_collapsed, !terminal_fullscreen, terminal_count > 0, terminal_count);
    } else if (idx == 4) {
        terminal_count = 0;
        apply_desktop_ui_state(false, false, false, terminal_count);
        if (current_module_pid > 0) {
            kill(current_module_pid, SIGTERM);
        ...
        /*
         * Launcher action:
         * open the first terminal instance if none exists yet.
         */
        if (terminal_count == 0) {
            terminal_count = 1;
            apply_desktop_ui_state(false, false, true, terminal_count);
        }
    } else if (idx == 6) {
        /*
         * Restore action:
         * reopen the exact minimized terminal instance from the toolbar.
         */
        if (terminal_count > 0 && terminal_minimized) {
            apply_desktop_ui_state(false, false, true, terminal_count);
        }
    } else {
        return false;
    }
    audit = fopen("parser.out", "a");
    if (audit) {
        fprintf(audit, "DESKTOP ACTION APPLIED: cmd=%s idx=%d -> open=%s collapsed=%s mode=%s\n",
                cmd,
                idx,
                get_var("desktop_window_1_open"),
                get_var("desktop_window_1_collapsed"),
                get_var("desktop_window_1_mode"));
        fclose(audit);
    }
    return true;
}
