...
static int g_desktop_window_minimized = 0;
static int g_desktop_window_open_override = -1;
static int g_desktop_window_minimized_override = -1;
...
static void sync_desktop_window_state(void) {
    char path[MAX_PATH];
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(line, "desktop_window_1_open=", 22) == 0) {
            ...
            g_desktop_window_open = (strcmp(v, "true") == 0) ? 1 : 0;
        }
        if (strncmp(line, "desktop_window_1_minimized=", 27) == 0) {
            char *v = trim_ws(line + 27);
            g_desktop_window_minimized = (strcmp(v, "true") == 0) ? 1 : 0;
        }
    }
    fclose(f);
}
...
void update_state(int last_key) {
    ...
    if (g_desktop_window_count_override >= 0) {
        ...
        if (g_desktop_window_open_override >= 0) g_desktop_window_open = g_desktop_window_open_override;
        if (g_desktop_window_minimized_override >= 0) g_desktop_window_minimized = g_desktop_window_minimized_override;
    } else {
    ...
    int desktop_body_visible = (g_desktop_window_open && !g_desktop_window_minimized) ? 1 : 0;
    ...
    if (f) {
        ...
        if (ui) {
            ...
            fprintf(ui, "desktop_window_1_collapsed=%s\n", g_desktop_window_minimized ? "true" : "false");
            fprintf(ui, "desktop_window_1_minimized=%s\n", g_desktop_window_minimized ? "true" : "false");
            fprintf(ui, "desktop_window_1_minimized_visible=%s\n", g_desktop_window_minimized ? "true" : "false");
            fprintf(ui, "desktop_window_1_restore_title=%s\n", g_desktop_window_minimized ? focused_title : "");
        ...
        g_desktop_window_open_override = -1;
        g_desktop_window_minimized_override = -1;
...
void route_input(int key) {
    ...
    if (key >= '1' && key <= '9') {
        ...
        if (g_active_gui_index == 1 && g_desktop_window_count == 0) {
            ...
            g_desktop_window_minimized = 0;
            g_desktop_window_open_override = 1;
            g_desktop_window_minimized_override = 0;
            log_alpha("Launch requested: opening Terminal #1 from dock");
        } else if (g_active_gui_index == 3 && g_desktop_window_count > 0) {
            /* Minimize/collapse should be handled at the manager layer so the
             * keyboard path does not depend on parser click dispatch. */
            g_desktop_window_count_override = g_desktop_window_count;
            g_desktop_window_open = 0;
            g_desktop_window_minimized = 1;
            g_desktop_window_open_override = 0;
            g_desktop_window_minimized_override = 1;
            log_alpha("Minimize requested: moving Terminal #%d to toolbar", g_desktop_window_count);
        } else if (g_active_gui_index == 4 && g_desktop_window_count > 0) {
            /* Close/quit should clear the desktop instance immediately. */
            g_desktop_window_count = 0;
            ...
            g_desktop_window_open_override = 0;
            g_desktop_window_minimized_override = 0;
            log_alpha("Close requested: quitting Terminal #1");
        }
        changed = 1;
    } else {
        changed = 1; 
