...
void read_super_state() {
    ...
    while ((entry = readdir(dir)) && file_count < MAX_SUPER_STATES) {
        ...
        float current_r = 1.0f, current_g = 0.8f, current_b = 0.8f; // Default color (pink)
        int current_has_color = 0; // Flag to indicate if color is specified
        int piece_count = 0;
        ...
        while (fgets(line, MAX_LINE, fp) && piece_count < MAX_PIECES_PER_STATE) {
            ...
            if (strcmp(key, "symbol") == 0) {
                ...
                if (sscanf(line, "%*s %d %d %d", &x, &y, &z) == 3) {
                    ...
                    super_state_pieces[num_super_state_pieces].r = current_r;
                    super_state_pieces[num_super_state_pieces].g = current_g;
                    super_state_pieces[num_super_state_pieces].b = current_b;
                    super_state_pieces[num_super_state_pieces].has_color = current_has_color;
