...
void read_locations() {
    ...
    fclose(fp);
    char msg[MAX_PATH * 5];
    snprintf(msg, sizeof(msg), "Loaded paths: state=%s, maps=%s, commands=%s, log=%s, super_maps=%s",
             state_path, maps_dir, commands_path, log_path, super_maps_dir);
    log_to_file(msg);
}
...
void mouse(int button, int state, int x, int y) {
    char *button_str;
    switch (button) {
        case GLUT_KEY_LEFT: button_str = "Left"; break;
    case GLUT_MIDDLE_BUTTON: button_str = "Middle"; break;
        case GLUT_KEY_RIGHT: button_str = "Right"; break;
...
int main(int argc, char *argv[]) {
    ...
    // Check if super_maps/ is specified and not empty
    if (super_maps_dir[0] != '\0') {
        if (is_directory_not_empty(super_maps_dir)) {
            strncpy(maps_dir, super_maps_dir, MAX_PATH - 1);
            maps_dir[MAX_PATH - 1] = '\0';
            log_to_file("Using super_maps");
        } else {
            log_to_file("super_maps directory is empty or inaccessible, using default maps path");
        }
    } else {
        log_to_file("super_maps not specified in locations.txt, using default maps path");
    }
