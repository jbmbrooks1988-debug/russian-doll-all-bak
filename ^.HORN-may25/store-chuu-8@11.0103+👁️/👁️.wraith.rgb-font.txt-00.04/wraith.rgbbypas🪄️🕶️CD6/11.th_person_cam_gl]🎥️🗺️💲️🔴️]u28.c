...
// Super state storage (replacing struct with parallel arrays)
...
void resize_super_state_pieces(int needed) {
    if (num_super_state_pieces + needed >= super_state_piece_capacity) {
        ...
        if (!new_x || !new_y || !new_z || !new_symbol || !new_r || !new_g || !new_b || !new_has_color || !new_size) {
...
void parse_state_file(const char *filename, int is_state_file, int *x, int *y, int *z, int *cam_x, int *cam_y, int *cam_z, int *cursor_pos, char *piece_symbol) {
    ...
    // Second pass: apply colors and sizes
    rewind(fp);
    ...
    for (int i = 0; i < temp_piece_count; i++) {
        ...
        char debug_msg[256];
        snprintf(debug_msg, sizeof(debug_msg), "Piece %d: symbol=%s, x=%d, y=%d, z=%d, color=(%.1f, %.1f, %.1f), has_color=%d, size=%.2f",
...
void draw_mini_map() {
                    ...
                    if (maps[i][y][x] == '#') {
                        ...
                        for (int j = 0; j < num_super_state_pieces; j++) {
                            if (super_state_x[j] == mx + x &&
                                ...
                                float quad_size = super_state_size[j] * 2.0f; // Scale sphere size for 2D quad
    ...
    for (int i = 0; i < num_super_state_pieces; i++) {
        ...
        float quad_size = super_state_size[i] * 2.0f; // Scale sphere size for 2D quad
        float half_size = quad_size / 2.0f;
        glBegin(GL_QUADS);
        glVertex2f(world_x + 0.5f - half_size, world_y + 0.5f - half_size);
        glVertex2f(world_x + 0.5f + half_size, world_y + 0.5f - half_size);
