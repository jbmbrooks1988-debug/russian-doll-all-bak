...
static void draw_slider(ObjectRecord *obj, const char *line) {
    ...
    snprintf(label, sizeof(label), "%s %s", obj->label[0] ? obj->label : "slider", bar);
    draw_text(obj->x, obj->y, label, obj->fg, obj->bg, obj->id, "slider");
}
...
static void draw_media(ObjectRecord *obj, const char *kind) {
    ...
    if (obj->src[0]) {
        ...
        draw_text(obj->x + 1, obj->y + obj->h / 2, label, obj->fg, obj->bg, obj->id, "media_label");
    }
}
...
static void parse_layout(const char *layout) {
    ...
    while (line) {
        ...
        if (starts_with(t, "<window") || starts_with(t, "<panel") || starts_with(t, "<canvas") ||
            ...
            if (obj) draw_text(x, y, obj->label[0] ? obj->label : obj->dom_id, fg, bg, obj->id, "text");
