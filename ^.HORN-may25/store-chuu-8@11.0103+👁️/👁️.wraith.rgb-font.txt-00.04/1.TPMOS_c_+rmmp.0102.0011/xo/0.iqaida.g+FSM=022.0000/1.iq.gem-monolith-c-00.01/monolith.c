...
extern int run_list_dir(FILE* out, int argc, char* const argv[]);
extern int run_file_ops(FILE* out, int argc, char* const argv[]);

static char* run_tool(const char* tool, char* const args[]) {
    if (strcmp(tool, "./tools/list_dir") == 0) {
        int argc = 0; while (args[argc]) argc++;
        char* output = malloc(ctx_limit);
        FILE* f = fmemopen(output, ctx_limit, "w");
        if (!f) { free(output); return NULL; }
        run_list_dir(f, argc, args);
        fflush(f); fclose(f);
        return output;
    }
    
    if (strcmp(tool, "./tools/file_ops") == 0) {
        int argc = 0; while (args[argc]) argc++;
        char* output = malloc(ctx_limit);
        FILE* f = fmemopen(output, ctx_limit, "w");
        if (!f) { free(output); return NULL; }
        run_file_ops(f, argc, args);
        fflush(f); fclose(f);
        return output;
    }
...
static void draw_top_bar() {
    ...
    printf("\033[H\033[K\033[90mAPI: %s | Model: %s | Path: %s\033[0m\n\033[K", current_api_url, current_model, cwd);
}
...
static void refresh_ui(int pct, const char* line, const char* suggestions, int selected_idx, const char* sub_suggestions) {
    ...
    // 2. Last Response (fixed at 2nd line)
    if (last_response) {
        printf("\033[2;1H\033[K\033[90mLast Response: %s\033[0m", last_response);
    }
    
    // 3. Prompt (bottom line)
    int rows = get_term_rows();
    printf("\033[%d;1H\033[K", rows); // Move to bottom line, clear it
    printf("[%s%d%%\033[0m] \033[92mAida >> \033[0m%s", 
           pct < 75 ? "\033[92m" : "\033[91m", pct, line);
    
    // 4. Dropdown
    if (suggestions && strlen(suggestions) > 0) {
        printf("\033[%d;1H\033[K\033[90m", rows - 1); // Move to 2nd to bottom line
        ...
        printf("\033[0m"); // Restore styling
        // Move back to prompt line
        printf("\033[%d;%zuH", rows, (size_t)(10 + strlen(line) + 1));
        free(copy);
    }
    fflush(stdout);
}
...
static int check_permission(const char* tool_name, const char* details) {
    ...
    char choice = 'n';
    (void)read(STDIN_FILENO, &choice, 1);
...
int main() {
    ...
    while (keep_running) {
        ...
        if (strcmp(line, "/summarize") == 0) {
            ...
            if (asprintf(&key_header, "x-goog-api-key: %s", api_key) == -1) {}
            free(api_key);
            ...
            unlink(tmp_llm);
            char* curl_url = NULL;
            if (asprintf(&curl_url, "%s/v1beta/models/%s:generateContent", current_api_url, current_model) == -1) {}
            pid_t curl_pid = fork();
            if (curl_pid == 0) {
                char* body_arg = NULL;
        ...
        if (strcmp(line, "/api") == 0) {
            ...
            while (1) {
                ...
                char c; (void)read(STDIN_FILENO, &c, 1);
                if (c == 9) sel = (sel + 1) % ac;
                else if (c == 13 || c == 10) {
        ...
        while (keep_running && !action_interrupted) {
            ...
            unlink(tmp_llm);
            char* curl_url = NULL;
            if (asprintf(&curl_url, "%s/v1beta/models/%s:streamGenerateContent?alt=sse", current_api_url, current_model) == -1) {}

            char* curl_cmd = NULL;
            if (asprintf(&curl_cmd, "curl -s -N --max-time 120 -H \"Content-Type: application/json\" -H \"%s\" -d @%s \"%s\"", 
                     key_header, tmp_prompt, curl_url) == -1) {}
