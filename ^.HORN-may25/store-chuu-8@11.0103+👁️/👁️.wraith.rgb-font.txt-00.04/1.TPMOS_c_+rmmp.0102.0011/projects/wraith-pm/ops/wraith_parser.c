...
static RGB parse_hex_rgb(const char *value, RGB fallback) {
    return parse_rgb(value, fallback);
}
...
static int run_media_decoder(const char *kind, ObjectRecord *obj, const char *out_path) {
    char decoder_path[MAX_PATH_LEN];
    char cols_arg[32];
    char rows_arg[32];
    char object_arg[32];
    char frame_arg[32];
    pid_t pid;
    int status = 0;
    const char *src = obj->src[0] ? obj->src : "";
    int inner_cols = obj->w - 2;
    int inner_rows = obj->h - 2;

    if (inner_cols < 1) inner_cols = obj->w;
    if (inner_rows < 1) inner_rows = obj->h;
    if (inner_cols < 1) inner_cols = 1;
    if (inner_rows < 1) inner_rows = 1;

    snprintf(cols_arg, sizeof(cols_arg), "%d", inner_cols);
    snprintf(rows_arg, sizeof(rows_arg), "%d", inner_rows);
    snprintf(object_arg, sizeof(object_arg), "%d", obj->id);
    snprintf(frame_arg, sizeof(frame_arg), "%d", 0);

    if (strcmp(kind, "IMG") == 0) {
        snprintf(decoder_path, sizeof(decoder_path), "%s/projects/wraith-pm/ops/+x/wraith_image_decode.+x", g_project_root);
    } else {
        snprintf(decoder_path, sizeof(decoder_path), "%s/projects/wraith-pm/ops/+x/wraith_video_decode.+x", g_project_root);
    }

    pid = fork();
    if (pid == 0) {
        if (strcmp(kind, "IMG") == 0) {
            execl(decoder_path, "wraith_image_decode.+x", src, out_path, cols_arg, rows_arg, object_arg, (char *)NULL);
        } else {
            execl(decoder_path, "wraith_video_decode.+x", src, out_path, cols_arg, rows_arg, object_arg, frame_arg, (char *)NULL);
        }
        perror("wraith_parser media decoder execl");
        _exit(127);
    }
    if (pid < 0) return 0;
...
static void merge_media_cells(ObjectRecord *obj, const char *media_pdl_path) {
    FILE *f = fopen(media_pdl_path, "r");
    char line[MAX_LINE];
    int base_x = obj->x + 1;
    int base_y = obj->y + 1;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(line, "MEDIA | warning |", 17) == 0) {
            char warning[512];
            snprintf(warning, sizeof(warning), "Media decode warning for object %d: %s", obj->id, trim_ws(line + 17));
            append_warning(warning);
            continue;
        }

        if (strncmp(line, "CELL |", 6) != 0) continue;
        if (sscanf(line,
                   "CELL | x=%d,y=%d | ch=%x fg=%15s bg=%15s object=%d style=%31s",
                   &lx, &ly, &ch_hex, fg_hex, bg_hex, &object_id, style) < 6) {
            continue;
        }

        fg = parse_hex_rgb(fg_hex, obj->fg);
        bg = parse_hex_rgb(bg_hex, obj->bg);
        set_cell(base_x + lx, base_y + ly, (char)ch_hex, fg, bg, obj->id, style);
    }

    fclose(f);
}

static int decode_media_into_object(ObjectRecord *obj, const char *kind) {
    char cache_dir[MAX_PATH_LEN];
    char out_path[MAX_PATH_LEN];
    int ok = 0;

    snprintf(cache_dir, sizeof(cache_dir), "%s/projects/wraith-pm/session/media_cache", g_project_root);
    ensure_dir(cache_dir);
    snprintf(out_path, sizeof(out_path), "%s/object_%04d.cells.pdl", cache_dir, obj->id);

    ok = run_media_decoder(kind, obj, out_path);
    if (!ok) {
        char warning[512];
        snprintf(warning, sizeof(warning), "Media decoder failed for object %d (%s)", obj->id, obj->src);
        append_warning(warning);
        return 0;
    }

    merge_media_cells(obj, out_path);
    return 1;
}
...
static void parse_layout(const char *layout) {
    ...
    while (line) {
        ...
        if (starts_with(t, "<window") || starts_with(t, "<panel") || starts_with(t, "<canvas") ||
            ...
            if (obj) {
                const char *kind = starts_with(t, "<img") ? "IMG" : "VID";
                draw_media(obj, kind);
                decode_media_into_object(obj, kind);
            }
        } else if (t[0] == '<') {
            char warning[512];
