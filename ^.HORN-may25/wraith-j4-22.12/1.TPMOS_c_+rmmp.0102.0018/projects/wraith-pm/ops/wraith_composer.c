...
static void compute_frame_bounds(void) {
    ...
    taskbar_right = 10 + label_len + 2 + clock_len;
...
static void add_builtin_objects(void) {
    ...
    if (clock_x <= label_x + label_w + 1) clock_visible = 0;
    ...
    if (clock_visible) {
        add_object("taskbar_clock", "text", g_taskbar_clock[0] ? g_taskbar_clock : "--:-- --- -- ----", clock_x, taskbar_y + 1, 18, 1, 8, 0, "", RGB_YELLOW, (RGB){20, 20, 60}, RGB_YELLOW);
    }
}
...
static void draw_builtin_object(const ObjectRecord *obj) {
    ...
    } else if (strcmp(obj->dom_id, "taskbar_start") == 0 || strcmp(obj->dom_id, "taskbar_windows") == 0 || strcmp(obj->dom_id, "taskbar_clock") == 0) {
