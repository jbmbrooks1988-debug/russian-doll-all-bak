...
| `legacy-shared-fix.md` | Separate leg of work from `khtpm-merge-how2.md`: consolidating all 16 legacy-GL projects' `system/`+`ops/` engine binaries (`prisc+x.c`, `chtpm_parser_pal.c`, `keyboard_input.c`, `renderer.c`, `chtpm_rgb_render.c`, `gl_mirror.c`→`x11_mirror.c`, `orchestrator.c`) into ONE shared binary set instead of 16 per-project copies that may have drifted - started 2026-08-17 right after the mutaclysm GL→X11 pilot, currently at SURVEY stage (drift-check in progress, no consolidation done yet) | Every time the drift survey/consolidation plan advances |
...
**Last updated:** 2026-08-16 (Stage 5 of `khtpm-merge-how2.md` complete — all 5 window apps (entity-menu, taskbar-settings, db-hq, events-hq, chat-hai) merged into one binary, `khtpm_entity_menu_render.c`; old standalone sources archived to `_.ARCHIVED-pre-merge-legacy.zip`; 2 real chat-hai bugs fixed post-merge (undecoded escapes, `fgets`-truncated ledger lines); WM-managed-window drag-clamp fix added. Next up: legacy GL migration) by claude
