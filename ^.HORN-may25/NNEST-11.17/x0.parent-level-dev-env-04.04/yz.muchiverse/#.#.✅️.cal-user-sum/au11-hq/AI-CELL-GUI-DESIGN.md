...
**Files:**
- `&.widgits/ai-cell/ops/khtpm_ai_cell_render.c` — the whole window (~570 lines)
- `&.widgits/ai-cell/ops/build_ai_cell.sh` — build script
- `&.widgits/ai-cell/button.sh` — launcher (build-if-missing + setsid nohup, same shape as `open_db_hq.sh`)
- `*.monads/*.livedesk-taskbar/ops/khtpm_taskbar_manager.c` — `livedesk_build_ai_menu()` (new), `ktb_hq_open()`'s `which==14` case (new), `livedesk:open-ai-cell` dispatch case (new)
...
- Real taskbar wiring: cell 14 ("ai") now opens a real 1-row menu ("Open ai-cell"), confirmed launching the window via **direct manager-level dispatch test** (`nav.sh mgrcode 5000`, i.e. `ktb_hq_activate(s, 0)` — the actual code path any real click/digit-select ultimately reaches).
...
- khtpm CSS engine NOT wired in — v1 uses hand-rolled pixel-math layout only (see `build_ai_cell.sh`'s own comment on why).
...
**Real session persistence** — `&.widgits/ai-cell/sessions/<epoch>/
...
Window left running (`ai-cell` process, house-root argv) for direct
...
- **Live**: press `'p'` (only dispatched when the composer isn't armed,
  ...
  keyboard or the same `ai_cell_agent_relay.txt` relay injection.
- **Headless**: `khtpm_ai_cell_render.+x <house_root> --dump-and-exit`
  — renders one frame, dumps, exits immediately, no window interaction
  needed at all.

Writes `/tmp/ai-cell-frame.png` **plus a receipt**,
`/tmp/ai-cell-frame.png.receipt.txt` — `ok=<0|1> w=<px> h=<px>
...
PNG dump + receipt instead. This isn't unique to ai-cell; the same
