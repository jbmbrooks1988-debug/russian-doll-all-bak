...
static void livedesk_kill_stray_entities(const char *house_root) {
    ...
    while ((ent = readdir(pd)) != NULL) {
        ...
        if (strstr(cmdbuf, "tp_desktop_window") && strstr(cmdbuf, house_root)) {
 ...
 * the same catch-all branch below. Cell 14 (date/time) is display-only, not
 * an ACTIVATE cell at all (see khtpm_strip_header.chtpm - it has no onClick),
...
void ktb_hq_open(KtbState *s, int which) {
    ...
    else { ktb_hq_close(s); return; } /* inert cell (6/7/10/11/12/13/14) or unknown - close any open popup, no-op otherwise, matching the legacy exactly */
    if (n <= 0) {
