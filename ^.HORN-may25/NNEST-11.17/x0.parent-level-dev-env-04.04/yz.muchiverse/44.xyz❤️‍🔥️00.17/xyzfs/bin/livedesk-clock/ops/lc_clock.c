...
static void fire_reminder(const char *house, const Reminder *r) {
    ...
    char popup_bin[PBUF];
    snprintf(popup_bin, sizeof(popup_bin), "%s/xyzfs/bin/livedesk-clock/ops/+x/lc_reminder_popup.+x", house);
    char sh[PBUF * 2];
    ...
    if (r->event[0]) {
        ...
        snprintf(evsh, sizeof(evsh),
                 "setsid nohup sh '%s/xyzfs/bin/muchi-pet/ops/play_event.sh' '%s' '%s' >/dev/null 2>&1 &",
                 house, pkg, house);
        int rc2 = system(evsh);
        (void)rc2;
    }
