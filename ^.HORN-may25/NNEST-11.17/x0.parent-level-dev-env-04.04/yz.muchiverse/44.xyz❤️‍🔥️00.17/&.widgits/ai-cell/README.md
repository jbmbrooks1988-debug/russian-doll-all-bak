...
`#.#.✅️.cal-user-sum/au11-hq/AI-CELL-GUI-DESIGN.md` (house root) — read
...
```
cd ops && sh build_ai_cell.sh
sh ../button.sh <house_root>          # launches it detached, logs to pieces/audit/ai-cell.log
```

Also launches from the real taskbar: click header cell "ai" (14th
button) → "Open ai-cell" row.

**`button.sh` is single-instance-safe** (real fix, 2026-08-13): it kills
any already-running `khtpm_ai_cell_render` process (TERM, then escalates
...
pidfile (`pieces/audit/ai-cell.pid`) and handles `SIGTERM`/`SIGINT`
with a clean shutdown (unlinks its own pidfile, exits its event loop
within ~150ms) rather than requiring `kill -9`.

## Files

- `ops/khtpm_ai_cell_render.c` — the whole app, one file (~950 lines).
- `ops/build_ai_cell.sh` — build script. Syncs `stb_image_write.h` from
...
2. **Relay injection** — append bare decimal ASCII codes, one per
   line, to `<house_root>/#.desktop/ai_cell_agent_relay.txt`. Same
...
- **Live**: press `p` (works via real keyboard or relay injection;
  ...
  typing).
- **Headless**: `ops/+x/khtpm_ai_cell_render.+x <house_root>
...
`<house_root>/&.widgits/ai-cell/pieces/audit/`:
- `ai-cell-frame.png` — the actual real frame.
- `ai-cell-frame.png.receipt.txt` — **check THIS, not just
...
output), `ai-cell.log` (runtime log) — every run is auditable by a
human. Emoji voxel CSVs live in
`&.widgits/ai-cell/pieces/registry/emoji_assets/<hex-codepoint>/
