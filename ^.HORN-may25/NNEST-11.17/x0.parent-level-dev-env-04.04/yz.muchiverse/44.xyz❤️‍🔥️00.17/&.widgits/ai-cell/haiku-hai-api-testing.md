...
1. `&.widgits/ai-cell/README.md` — how h-ai works, how to build/run/
   test it (relay injection, PNG-dump+receipt verification — don't use
   `xwd`/screenshots).
2. `&.widgits/ai-cell/ONBOARDING.md` — recent history, what's real.
...
- `g_model_name` in `khtpm_ai_cell_render.c` is hardcoded to
...
- h-ai's own backend code (`send_to_ollama()` in
  `khtpm_ai_cell_render.c`) calls `/api/generate` with a bare
...
4. Record real results (prompt, full response, whether it worked) in
   a new `&.widgits/ai-cell/api-test-results.md` — this is the actual
...
- Relay injection only (`#.desktop/ai_cell_agent_relay.txt`), never a
...
`&.widgits/ai-cell/api-test-results.md` — real prompts, real
...
- Any tool you're about to wire up would let the model execute
  ...
  ai-cell/` without a human in the loop - that's a real safety
