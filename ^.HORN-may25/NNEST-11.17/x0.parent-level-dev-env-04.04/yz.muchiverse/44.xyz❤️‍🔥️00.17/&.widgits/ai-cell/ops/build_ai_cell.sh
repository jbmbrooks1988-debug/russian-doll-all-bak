#!/bin/sh
# build_ai_cell.sh — build the taskbar cell 14 ("ai") window.
set -e
cd "$(dirname "$0")"
mkdir -p +x
...
# AI-CELL-GUI-DESIGN.md §4 - the file still #includes the header for
...
echo "-- ai-cell renderer -> +x/khtpm_ai_cell_render.+x"
$CC $CFLAGS -o +x/khtpm_ai_cell_render.+x \
  khtpm_ai_cell_render.c $LIBS

echo "OK +x/khtpm_ai_cell_render.+x"
