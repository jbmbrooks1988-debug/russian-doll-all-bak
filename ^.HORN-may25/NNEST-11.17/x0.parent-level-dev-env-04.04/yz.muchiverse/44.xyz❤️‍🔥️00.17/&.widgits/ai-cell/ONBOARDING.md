...
names are still `ai-cell`/`khtpm_ai_cell_render.c` — not renamed yet,
...
feature): `button.sh` now kills any existing `khtpm_ai_cell_render`
...
(`pieces/audit/ai-cell.pid`) and handles `SIGTERM`/`SIGINT` with a
...
are a legitimate use case there, unlike ai-cell/db-hq which are
...
- Rename `ai-cell`/`khtpm_ai_cell_render.c` → `hai`/`khtpm_hai_render.c`
  (or similar) for real, everywhere (dir, binary, `#.desktop` relay
  filename, session storage path, build script, `button.sh`,
  `AI-CELL-GUI-DESIGN.md`'s own title) — not done this session, the
...
- Real relay injection (`#.desktop/ai_cell_agent_relay.txt`).
...
Full history/rationale for all of the above: `AI-CELL-GUI-DESIGN.md`
