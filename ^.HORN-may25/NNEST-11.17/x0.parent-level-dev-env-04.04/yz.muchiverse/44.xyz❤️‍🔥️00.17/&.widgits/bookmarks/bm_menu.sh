...
# Mirror:       <pal>/bookmarks/<safe_name> symlinks (one per row), so
#               the folder itself is clickable from any file manager.
...
#   bm_menu.sh sync <pal>                        rebuild symlink mirror
...
safe_name() {  # filesystem-safe mirror link name from display name
    printf '%s' "$1" | tr ' /:' '___' | tr -cd 'A-Za-z0-9._+-'
}

do_sync() {  # do_sync <pal>
    mkdir -p "$1/bookmarks"
    pdl_rows "$1" | while IFS="$TAB" read -r name path; do
        ln -sfn "$path" "$1/bookmarks/$(safe_name "$name")"
    done
}
...
compose_bookmarks() {
    ...
    [ -n "$_house" ] || _house=$(CDPATH= cd -- "$_self_dir/../../.." 2>/dev/null && pwd) || _house=""
    {
        ...
        pdl_rows "$_pal" | while IFS="$TAB" read -r n p; do
            printf '    <item label="%s  -  %s" onClick="open:%s"/>\n' "$n" "$p" "$p"
        done
        # New+ = NATIVE in-window input field (direct instruction "new+
        # should allow input from <cli-io>"; zenity retired). Renderer
        # generic input: mechanism: Enter/digit/click ARMS it, typing
        # edits in place, Enter commits - commit appends the line to
        # .bm_newplus.txt then fires consumed-newplus detached.
        printf '    <item class="bm-btn" label="+ New+ (Enter, type a path, Enter)" onClick="input:%s/.bm_newplus.txt|%s consumed-newplus %s %s"/>\n' \
            "$_pal" "$_self" "$_house" "$_pal"
        printf '    <button class="bm-btn" label="Open Folder" onClick="open:%s/bookmarks"/>\n' "$_pal"
        printf '  </panel>\n'
        printf '</window>\n'
