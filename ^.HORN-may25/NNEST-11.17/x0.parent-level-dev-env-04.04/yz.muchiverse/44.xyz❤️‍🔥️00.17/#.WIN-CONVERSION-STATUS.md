...
**Date:** 2026-08-06 (updated — handoff)  
**Dev model:** Linux primary; Win = compatibility (see `#.WIN-COMPAT-RULE.md`)  
**Related:** `windows-house-guide.md`, `khtpm-win-plan.txt`, `livedesk-win-plan-lvl2.txt`, `KHTPM-ARCH.txt`  
**Handoff:** `yz.muchiverse/#.grok...J28-grok-user-sum/16.A7_GROK_WALKOFF.md` (2026-08-07).
...
| **$.crypts** autostart | Working (relative PDL, CreateProcessW) | `$.crypts/button.ps1` |
| **KHTPM entity windows** | Working sprites + RMB menus (shared `khtpm_core`/`khtpm_plat_win.c` — real Windows implementation, unrelated to the abandoned Linux `khtpm_plat_x11.c` retired 2026-08-11, see `#.WIN-COMPAT-RULE.md`) | crypts LAUNCH → `tp_desktop_window.exe` |
| **Livedesk taskbar** | ⚠️ NEEDS REBUILD 2026-08-11 — legacy `tp_taskbar.c`/`tp_taskbar_win.c` retired (archived, see `*.monads/*.livedesk-taskbar/ops/LEGACY-ARCHIVE-20260811.zip`). Real replacement is `khtpm_strip_parser.c`/`khtpm_taskbar_manager.c`, Linux-only so far — no Windows build exists yet. Do not assume `tp_taskbar.exe` still works; it's the retired binary. | none yet — see `*.monads/*.livedesk-taskbar/README.md` |
...
```
ENTITY — real, current
  DESIGN: tp_desktop_window.c (Linux, permanent — NOT superseded despite
          the original plan below; khtpm_core.c's own X11 half never
          shipped, confirmed dead + archived 2026-08-11)
  WIN:    khtpm_core.c + khtpm_plat_win.c + khtpm_main.c (real, current,
          builds tp_desktop_window.exe)
  WIN LEGACY FALLBACK: tp_desktop_window_win.c (still real, used by
          button.ps1 only if the khtpm_core Win build is missing)

TASKBAR — real, current (2026-08-11)
  DESIGN: *.monads/*.livedesk-taskbar/ops/khtpm_taskbar_manager.c (Linux,
          platform-neutral, no Xlib)
  LINUX:  khtpm_strip_parser.c (real declarative-layout parser + Xlib)
  WIN:    none yet — button.ps1's own taskbar section is stale/broken,
          see that file's own 2026-08-11 comment there
