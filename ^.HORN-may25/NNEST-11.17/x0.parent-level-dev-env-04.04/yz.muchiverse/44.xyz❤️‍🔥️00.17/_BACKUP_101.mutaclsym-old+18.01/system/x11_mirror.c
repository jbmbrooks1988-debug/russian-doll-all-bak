/* x11_mirror - REAL PILOT CONVERSION (2026-08-17, khtpm-merge-how2.md
 * §5c.1) of gl_mirror.c away from GL/GLUT to plain Xlib, per direct
 * instruction: "khtpmos's real X11 approach... now supersedes [GL] as
 * the house's actual preferred display technology." This is the FIRST
 * of 16 real gl_mirror.c copies to be converted - user asked to watch
 * this one directly before the rest get delegated.
 *
...
static void draw_chrome(void) {
    ...
    XFillRectangle(dpy, win, gc, 0, 0, (unsigned)g_window_w, CHROME_H);
    if (font_ui) {
        XftColor title_col = xft_color("#eeeeee");
        const char *title = "mutaclsym RGB mirror (x11)";
    ...
    XFillRectangle(dpy, win, gc, g_window_w - CLOSE_BTN_W, 0, CLOSE_BTN_W, CHROME_H);
    if (font_ui) {
...
static void x11_display(void) {
    if (!ximg) return;
    draw_chrome();
    XPutImage(dpy, win, gc, ximg, 0, 0, 0, CHROME_H, (unsigned)g_frame_w, (unsigned)g_frame_h);
    ...
    if (g_drop_pending && (time(NULL) - g_drop_time < 2)) {
        ...
        XFillRectangle(dpy, win, gc, 0, CHROME_H, (unsigned)g_window_w, (unsigned)(g_window_h - CHROME_H));
...
int main(int argc, char **argv) {
    ...
    XStoreName(dpy, win, "mutaclsym RGB mirror");
