...
char super_state_path[MAX_PATH] = "";
...
void read_locations() {
    ...
    while (fgets(line, MAX_LINE, fp)) {
        ...
        if (strcmp(key, "state") == 0) {
            ...
            strncpy(super_state_path, value, MAX_PATH - 1);
            super_state_path[MAX_PATH - 1] = '\0';
        } else if (strcmp(key, "super_maps") == 0) {
            strncpy(super_maps_dir, value, MAX_PATH - 1);
            super_maps_dir[MAX_PATH - 1] = '\0';
            // Ensure super_maps_dir ends with '/'
    ...
    fclose(fp);
    char msg[MAX_PATH * 6];
    snprintf(msg, sizeof(msg), "Loaded paths: state=%s, maps=%s, commands=%s, log=%s, super_state=%s, super_maps=%s",
             state_path, maps_dir, commands_path, log_path, super_state_path, super_maps_dir);
    log_to_file(msg);
}
...
void draw_mini_map() {
    ...
    // Snap cursor position to grid for mini-map centering
    ...
    // Set mini-map viewport to top-right corner
    ...
    // Center the mini-map on the snapped cursor position
    ...
    // Draw map tiles for maps at the cursor's z-level or closest z-level
        ...
        for (int i = 0; i < num_maps; i++) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    float world_x = mx + x;
                    float world_y = my + y;
                    // Only draw tiles within the mini-map view
...
int main(int argc, char *argv[]) {
    ...
    read_locations(); // Read paths from ../locations.txt

    // Check if super_state.txt exists and super_maps/ is not empty
    int use_super = 0;
    if (super_state_path[0] != '\0' && super_maps_dir[0] != '\0') {
        if (access(super_state_path, F_OK) == 0) { // super_state.txt exists
            if (is_directory_not_empty(super_maps_dir)) {
                strncpy(state_path, super_state_path, MAX_PATH - 1);
                state_path[MAX_PATH - 1] = '\0';
                strncpy(maps_dir, super_maps_dir, MAX_PATH - 1);
                maps_dir[MAX_PATH - 1] = '\0';
                use_super = 1;
                log_to_file("Using super_state and super_maps");
            } else {
                log_to_file("super_maps directory is empty or inaccessible, using default paths");
            }
        } else {
            log_to_file("super_state.txt not found, using default paths");
        }
    } else {
        log_to_file("super_state or super_maps not specified in locations.txt, using default paths");
    }

    read_state();
    glutInit(&argc, argv);
    glut_initialized = 1; // Set flag after glutInit
    ...
    glutMouseFunc(mouse);
    init();
    read_maps(); // Load maps after GLUT initialization
    if (num_maps > 0) {
        init_map_display_lists(); // Initialize display lists after maps are loaded
    }
    glutMainLoop();
