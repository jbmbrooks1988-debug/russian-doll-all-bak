...
void update_completions(void) {
    ...
    if (!star) { g_completion_mode = false; g_completion_area[0] = '\0'; return; }
    
    char *prefix = star + 1;
    char *p_args[] = {"projects/gem-dev/ops/+x/complete_path", prefix, NULL};
    ...
    if (matches && strlen(matches) > 0) {
        ...
        int count = 0;
        while (token && count < 5) { // Limit to 5 suggestions for height
            char btn[1024];
            // Format: ║ [Suggestion]                                         ║
            snprintf(btn, sizeof(btn), "<text label=\"║  \" /><button label=\"%-70.70s\" onClick=\"CHOOSE:%s\" /><text label=\"  ║\" /><br/>", token, token);
        ...
        snprintf(g_completion_area, sizeof(g_completion_area), "<text label=\"║  (no matches)                                                            ║\" /><br/>");
    }
}
...
void write_gui_state(void) {
    char *path = "projects/gem-dev/manager/gui_state.txt"; char *existing_input = NULL; FILE *fr = fopen(path, "r");
    if (fr) {
...
int main(int argc, char *argv[]) {
    ...
    char *buf_path = "pieces/apps/player_app/cli_buffers.txt"; if (stat(buf_path, &st) == 0) g_buf_last_pos = st.st_size;
    while (!g_shutdown) {
        char *ctx_file = "projects/gem-dev/state/context.json";
                ...
                if (strcmp(last_line, g_last_buf_input) != 0) {
                    ...
                    if (star) {
                        ...
                        if (matches && strlen(matches) > 0) {
                            ...
                            char *token = strtok(copy, "  ");
                            int count = 0;
                            while (token && count < 5) { // Limit to 5 suggestions
                                char btn[512];
