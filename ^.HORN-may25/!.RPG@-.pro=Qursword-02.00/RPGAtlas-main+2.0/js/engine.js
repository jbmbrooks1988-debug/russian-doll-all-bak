...
(() => {
  ...
  function skillMpCost(a, skill) {
    return a.skillMpCost(skill);
  }
  function skillPowerRate(a, skill) {
    return a.skillPowerRate(skill);
  }
  function actorIncomingRate(a, element, guarding) {
    return a.incomingRate(element, guarding);
  }
  function canActorEquip(a, kind, itemId) {
    return a.canEquip(kind, itemId);
  }
  function sanitizeEquipment(a) {
    a.sanitizeEquipment();
  }
  function makeActor(actorId) {
    return new Game_Actor(actorId);
  }
  function gainExp(a, amount, log) {
    a.gainExp(amount, log);
  }
  function addInv(kind, id, n) {
    const bag = SESSION.inv[kind];
    bag[id] = clamp((bag[id] || 0) + n, 0, 99);
    if (!bag[id]) delete bag[id];
  }
  function invCount(kind, id) { return SESSION.inv[kind][id] || 0; }
  function dbFor(kind) { return kind === "item" ? DATABASE.items : kind === "weapon" ? DATABASE.weapons : DATABASE.armors; }
  function traitDescription(t) {
    const value = Number(t.value) || 0;
    if (t.type === "param") return String(t.key).toUpperCase() + " " + value + "%";
    if (t.type === "element") {
      const e = RA.typeList(DATABASE, "elements").find((x) => x.key === t.key);
      return (e ? e.name : t.key) + " damage " + value + "%";
    }
    if (t.type === "state") {
      const state = RA.byId(DATABASE.states || [], Number(t.key));
    ...
    if (t.type === "equip") {
      const item = RA.byId(t.key === "armor" ? DATABASE.armors : DATABASE.weapons, value);
  ...
  function pageActive(evId, page) {
    const c = page.cond;
    if (c.switchId && !SESSION.switches[c.switchId]) return false;
    if (c.varId && !((SESSION.vars[c.varId] || 0) >= c.varVal)) return false;
    if (c.selfSw && !SESSION.selfSw[SESSION.mapId + ":" + evId + ":" + c.selfSw]) return false;
    return true;
  }
  ...
  function loadMap(mapId) {
    map = RA.byId(DATABASE.maps, mapId);
  ...
  function canEntityPass(rt, nx, ny) {
    ...
    if (SESSION.player && SESSION.player.x === nx && SESSION.player.y === ny && (!rt.page || rt.page.priority === "same")) return false;
    return true;
  }
  ...
  class Interp {
    ...
    selfKey(key) { return SESSION.mapId + ":" + (this.evRT ? this.evRT.ev.id : 0) + ":" + key; }
    ...
    async exec(c) {
      switch (c.t) {
        ...
        case "switch": SESSION.switches[c.id] = !!c.val; refreshAllPages(); break;
        case "selfsw": SESSION.selfSw[this.selfKey(c.key)] = !!c.val; refreshAllPages(); break;
        case "var": {
          const cur = SESSION.vars[c.id] || 0;
          let v = c.val;
          if (c.op === "rnd") v = c.val + rnd((c.val2 || c.val) - c.val + 1);
          SESSION.vars[c.id] = c.op === "add" ? cur + v : c.op === "sub" ? cur - v : v;
          refreshAllPages();
          break;
        }
        case "if": {
        ...
        case "gold": SESSION.gold = clamp(SESSION.gold + (c.op === "sub" ? -c.val : c.val), 0, 9999999); break;
        case "item": addInv(c.kind || "item", c.id, c.op === "sub" ? -c.val : c.val); break;
        case "party": {
          if (c.op === "add") {
            if (!SESSION.party.find((a) => a.actorId === c.actorId) && SESSION.party.length < 4) {
              const a = makeActor(c.actorId);
              if (a) SESSION.party.push(a);
            }
          } else {
            SESSION.party = SESSION.party.filter((a) => a.actorId !== c.actorId);
            if (!SESSION.party.length) SESSION.party.push(makeActor(DATABASE.system.party[0] || DATABASE.actors[0].id));
          }
          break;
        }
        case "heal": {
          for (const a of SESSION.party) {
            if (c.full) { a.hp = a.mhp; a.mp = a.mmp; a.states = []; }
            else {
              a.hp = clamp(a.hp + (c.hp || 0), 1, a.mhp);
              a.mp = clamp(a.mp + (c.mp || 0), 0, a.mmp);
            }
          }
          break;
        }
        ...
        case "move": {
          const target = c.target === "player" ? SESSION.player : this.evRT;
          if (!target) break;
          if (c.wait) {
        ...
        case "transparency": if (SESSION.player) SESSION.player.transparent = !!c.val; break;
    ...
    testCond(cond) {
      ...
      switch (cond.kind) {
        case "switch": return !!SESSION.switches[cond.id] === (cond.val !== false);
        case "var": return cmp(SESSION.vars[cond.id] || 0, cond.val, cond.cmp || ">=");
        case "selfsw": return !!SESSION.selfSw[this.selfKey(cond.key)];
        case "item": return invCount(cond.itemKind || "item", cond.id) > 0;
        case "gold": return cmp(SESSION.gold, cond.val, cond.cmp || ">=");
        default: return true;
      }
    }
  }
  const scriptApi = {
    setSwitch(id, v) { SESSION.switches[id] = !!v; },
    getSwitch(id) { return !!SESSION.switches[id]; },
    setVar(id, v) { SESSION.vars[id] = v; },
    getVar(id) { return SESSION.vars[id] || 0; },
    addGold(n) { SESSION.gold = clamp(SESSION.gold + n, 0, 9999999); },
    party() { return SESSION.party; },
  ...
  const Plugins = {
    ...
    runAll() {
      ...
      for (const pl of DATABASE.plugins || []) {
        if (!pl.on) continue;
  ...
  async function runEventBlocking(rt) {
    ...
    if (rt.kind === "human" && rt.page.trigger === "action") {
      rt.dir = dirTo(rt.x, rt.y, SESSION.player.x, SESSION.player.y);
    }
    try {
  ...
  function onPlayerStep() {
    ...
    if (enc && enc.rate > 0 && enc.troops.length && !blockingRun) {
      SESSION.encSteps++;
      if (SESSION.encSteps >= enc.rate * (0.7 + Math.random() * 0.6)) {
        SESSION.encSteps = 0;
  ...
  function actorRowHTML(a) {
    ...
    return '<div class="arow"><span class="aface"></span><div class="ainfo">' +
      ...
      "HP " + a.hp + "/" + a.mhp + " " + bar(a.hp, a.mhp, "#58c46a") + "<br>" +
      "MP " + a.mp + "/" + a.mmp + " " + bar(a.mp, a.mmp, "#5a8ad8") +
      "</div></div>";
  }
  ...
  async function pickPartyMember(title) {
    const i = await showList(SESSION.party.map((a) => ({ html: actorRowHTML(a) })), { title, className: "partywin" });
    return i < 0 ? null : SESSION.party[i];
  }
  ...
  async function openMenu() {
    ...
    function refreshPanel() {
      partyBox.innerHTML = SESSION.party.map(actorRowHTML).join("");
      attachFaces(partyBox, SESSION.party);
      goldBox.textContent = SESSION.gold + " " + DATABASE.system.currency;
    }
    try {
      let idx = 0;
      while (true) {
        ...
        const i = await showList(
          [
            ...
            { html: Assets.iconHtml((actorClass(SESSION.party[0]) || {}).icon, "menu-icon") + "Status" },
  ...
  async function menuItems() {
    while (true) {
      const list = DATABASE.items.filter((it) => invCount("item", it.id) > 0);
  ...
  async function menuSkills() {
    ...
    while (true) {
      ...
      const amount = Math.max(1, Math.floor((s.power + a.mat * 1.2) * skillPowerRate(a, s)));
  ...
  async function menuEquip() {
    ...
    if (!a) return;
    while (true) {
      const w = RA.byId(DATABASE.weapons, a.weaponId), ar = RA.byId(DATABASE.armors, a.armorId);
      ...
      ], { title: a.name + " — ATK " + a.atk + " / DEF " + a.def + " / MAT " + a.mat, className: "itemwin" });
      if (slot < 0) return;
      ...
      const opts = candidates.map((e) => ({
        ...
        help: canActorEquip(a, kind, e.id) ? "" : a.actorClass().name + " cannot equip this item.",
      }));
  ...
  async function menuStatus() {
    ...
    await showList([{
      html: Assets.iconHtml(c.icon, "menu-icon") + "<b>" + esc(a.name) + "</b> — " + esc(c.name) + " Lv " + a.level +
        ...
        "Skills: " + (a.learnedSkills().map((s) => esc(s.name)).join(", ") || "none") +
  ...
  async function saveLoadMenu(mode) {
    ...
    if (mode === "save") {
      const payload = {
        ...
        level: SESSION.party[0] ? SESSION.party[0].level : 1,
        data: {
          switches: SESSION.switches, vars: SESSION.vars, selfSw: SESSION.selfSw,
          party: SESSION.party, inv: SESSION.inv, gold: SESSION.gold, steps: SESSION.steps,
          mapId: SESSION.mapId, player: { x: SESSION.player.x, y: SESSION.player.y, dir: SESSION.player.dir, transparent: !!SESSION.player.transparent },
        },
      };
  ...
  function applySave(d) {
    ...
    SESSION.gold = d.gold || 0; SESSION.steps = d.steps || 0;
    initPlayer(d.player.x, d.player.y, d.player.dir);
    SESSION.player.transparent = !!d.player.transparent;
    loadMap(d.mapId);
    scene = "map";
  }
  ...
  const Shop = {
    async run(goods) {
      const goldLine = () => "Gold: " + SESSION.gold + " " + DATABASE.system.currency;
      while (true) {
        ...
        if (i === 0) {
          while (true) {
            ...
            const bi = await showList(entries.map(({ gd, e }) => ({
              html: iconEntryHtml(e) + ' <span class="cnt">' + e.price + " " + DATABASE.system.currency +
                " · own ×" + invCount(gd.kind, gd.id) + "</span>",
              disabled: SESSION.gold < e.price || invCount(gd.kind, gd.id) >= 99,
          ...
          while (true) {
            ...
            for (const kind of ["item", "weapon", "armor"]) {
              for (const idStr of Object.keys(SESSION.inv[kind])) {
            ...
            const si = await showList(owned.map(({ kind, e }) => ({
              html: iconEntryHtml(e) + ' <span class="cnt">×' + invCount(kind, e.id) + " · " +
                Math.floor(e.price / 2) + " " + DATABASE.system.currency + "</span>",
            })), { title: "Sell — " + goldLine(), className: "shopwin" });
            if (si < 0) break;
            const { kind, e } = owned[si];
            addInv(kind, e.id, -1);
            SESSION.gold = clamp(SESSION.gold + Math.floor(e.price / 2), 0, 9999999);
            sysSe("equip");
          }
        }
      }
  ...
  const Battle = {
    async run(troopId, canEscape) {
      const troop = RA.byId(DATABASE.troops, troopId);
      if (!troop) return "win";
      ...
      const enemies = troop.enemies.map((eid, i) => {
        const d = RA.byId(DATABASE.enemies, eid);
        return d ? { d, hp: d.stats.mhp, i, alive: true } : null;
      }).filter(Boolean);

      const sideView = DATABASE.system.battleView === "side";
      ...
      function actorElement(a) {
        const i = SESSION.party.indexOf(a);
      ...
      function refreshParty() {
        partyArea.innerHTML = SESSION.party.map((a) =>
          '<div class="brow' + (a.hp <= 0 ? " dead" : "") + '"><b>' + esc(a.name) + "</b> " +
          "HP " + a.hp + "/" + a.mhp + " " + bar(a.hp, a.mhp, "#58c46a") +
          " MP " + a.mp + "/" + a.mmp + " " + bar(a.mp, a.mmp, "#5a8ad8") +
          stateTagsHtml(a) + "</div>"
        ).join("");
        actorSprs.forEach((w, i) => {
          const a = SESSION.party[i];
      ...
      const livingP = () => SESSION.party.filter((a) => a.hp > 0);
      ...
      async function pickAlly(deadOk) {
        const pool = deadOk ? SESSION.party : livingP();
      ...
      async function actorCommand(a) {
        while (true) {
          const items = [
            ...
            { html: Assets.iconHtml(8, "menu-icon") + "Skills", disabled: !a.learnedSkills().length },
            { html: Assets.iconHtml(24, "menu-icon") + "Items", disabled: !DATABASE.items.some((it) => invCount("item", it.id) > 0) },
      ...
      function enemyAction(en) {
        ...
        const skill = chosen.skillId ? RA.byId(DATABASE.skills, chosen.skillId) : null;
      ...
      function actorDef(a) { return a.def; }

      // ---- states (poison / stun / regen…) ----
      const stateDef = (id) => RA.byId(DATABASE.states || [], id);
      ...
      function refreshStates() {
        ...
        actorSprs.forEach((w, i) => {
          const a = SESSION.party[i], slot = w.querySelector(".battler-states");
      ...
      function actorFlash(a) {
        const w = actorSprs[SESSION.party.indexOf(a)];
        if (!w) return;
      ...
      try {
              ...
              if (c.type === "attack" || (c.type === "skill" && c.skill.scope === "enemy") || (c.type === "skill" && c.skill.scope === "enemies")) {
                ...
                for (const t of targets) {
                  ...
                  const critical = (!skill || skill.type === "phys") &&
                    rnd(100) < RA.traitSum(a.actorClass(), "special", "critChance", 0);
                  if (!skill) {
                    dmg = variance(a.atk * 2 - t.d.stats.def * 1.2);
                    Sfx.play(critical ? "crit" : "hit");
                  } else if (skill.type === "phys") {
                    dmg = variance((skill.power + a.atk * 2 - t.d.stats.def * 1.2) * skillPowerRate(a, skill));
                    Sfx.play("crit");
                  } else {
                    dmg = variance((skill.power + a.mat * 2 - t.d.stats.mdf * 1.5) * skillPowerRate(a, skill));
                    Sfx.play("magic");
                  }
                ...
                for (const t of targets) {
                  const amount = variance((c.skill.power + a.mat * 1.2) * skillPowerRate(a, c.skill));
        ...
        if (result === "win") {
          ...
          sysSe("levelup");
          const lines = [];
          await say("Victory!  +" + exp + " EXP, +" + gold + " " + DATABASE.system.currency, 900);
          SESSION.gold = clamp(SESSION.gold + gold, 0, 9999999);
  ...
  function refreshPlayerCharset() {
    const lead = SESSION.party[0];
    if (lead) SESSION.player.charsetIdx = Math.max(0, Assets.charsetIndex(lead.charset));
  }

  function newGame() {
    DataManager.setupNewGame();
    SESSION = window.SESSION;
    initPlayer(DATABASE.system.startX, DATABASE.system.startY, DATABASE.system.startDir);
    SESSION.player.transparent = !!DATABASE.system.startTransparent;
    loadMap(DATABASE.system.startMapId);
    scene = "map";
  }
  ...
  async function showTitle() {
    ...
    tw.appendChild(el("div", "title-name", esc(DATABASE.system.title || "Untitled")));
  ...
  function fitStage() {
    ...
    const maxScale = (DATABASE && Number(DATABASE.system.screenScale)) || 1.6;
  ...
  window.addEventListener("DOMContentLoaded", async () => {
    ...
    Plugins.runAll();
    document.title = (DATABASE.system.title || "RPGAtlas") + " — RPGAtlas Player";
    scene = "title";
    showTitle();
    loop();
