 ...
 *
 * If launched with a second argument (a session PID, passed by
 * menu_input.c's spawn_egg_window from pieces/system/session.pid), this
 * process checks every poll whether that PID is still alive and exits
 * cleanly if not - so the pet goes away when the terminal session that
 * spawned it does, instead of being left behind as a frozen window
 * nothing can feed/tick/interact with anymore. Standalone runs (no
 * session PID given) skip this check entirely.
 *
 * This process must stay a dumb renderer + input relay per egg-pals.txt
 * - do not put pet decision-making in here, only drawing + relaying
...
static LRESULT CALLBACK wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_KEYDOWN:
        ...
        return 0;
    case WM_RBUTTONUP: {
        /* This window doesn't close itself when the session that opened
         * it ends (see this file's header comment) - a manual way to
         * close one is needed since nothing else will. */
        POINT pt;
        GetCursorPos(&pt);
    ...
    case WM_COMMAND:
        ...
        return 0;
    case WM_TIMER:
        /* The pet goes away when the terminal session that spawned it
         * does - see this file's own header comment for why. No-op if
         * launched standalone (g_session_pid == 0). */
        if (g_session_pid) {
            HANDLE h = OpenProcess(SYNCHRONIZE | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, g_session_pid);
            int alive = h && WaitForSingleObject(h, 0) == WAIT_TIMEOUT;
            if (h) CloseHandle(h);
            if (!alive) { DestroyWindow(hwnd); return 0; }
        }
        if (g_pet_id) {
            PetVitals v;
...
int main(int argc, char **argv) {
    ...
    DWORD session_pid = (argc >= 3) ? (DWORD)strtoul(argv[2], NULL, 10) : 0;
    resolve_root();
    SetUnhandledExceptionFilter(crash_handler);
    g_pet_id = pet_id;
    g_session_pid = session_pid;
...
int main(int argc, char **argv) {
    const char *pet_id = (argc >= 2) ? argv[1] : NULL;
    pid_t session_pid = (argc >= 3) ? (pid_t)atol(argv[2]) : 0;
    resolve_root();
    ...
    while (running) {
        ...
        /* The pet goes away when the terminal session that spawned it
         * does - see this file's own header comment for why. No-op if
         * launched standalone (session_pid == 0). */
        if (session_pid > 0 && kill(session_pid, 0) != 0) break;
