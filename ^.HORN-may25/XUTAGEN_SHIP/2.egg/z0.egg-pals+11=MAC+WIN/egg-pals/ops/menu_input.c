...
static void load_menu_state(char *screen, size_t screen_sz, int *cursor, char *msg, size_t msg_sz,
                             char *selected_pet, size_t selected_pet_sz) {
...
static void save_menu_state(const char *screen, int cursor, const char *msg, const char *selected_pet) {
    char path[PATH_BUF];
    menu_state_path(path, sizeof(path));
    FILE *f = fopen(path, "w");
    if (!f) return;
...
int main(int argc, char **argv) {
    ...
    load_menu_state(screen, sizeof(screen), &cursor, msg, sizeof(msg), selected_pet, sizeof(selected_pet));
    ...
    save_menu_state(screen, cursor, msg, selected_pet);
    return 0;
}
