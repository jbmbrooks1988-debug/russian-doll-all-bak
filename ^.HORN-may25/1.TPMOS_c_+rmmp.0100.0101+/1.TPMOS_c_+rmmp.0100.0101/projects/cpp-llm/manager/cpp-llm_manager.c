...
static void check_yolo_mode(void) {
    g_yolo_mode = (access("config/yolo.flag", F_OK) == 0);
}
...
void execute_tool(const char *tool_name, const char *args_json) {
    char *ctx_file = "projects/cpp-llm/state/context.json";
...
void check_ai_status(void) {
    ...
    if (is_llamacpp) {
        ...
        if (!content_json || strlen(content_json) == 0) {
            if (content_json) free(content_json);
            char *p_flat[] = {"projects/cpp-llm/ops/+x/json_parser", tmp_llm, "content", NULL};
            content_json = run_tool(p_flat[0], p_flat, false);
        }
    } else {
        char *p_cont[] = {"projects/cpp-llm/ops/+x/json_parser", tmp_llm, "message.content", NULL};
        content_json = run_tool(p_cont[0], p_cont, false);
        char *p_calls[] = {"projects/cpp-llm/ops/+x/json_parser", tmp_llm, "message.tool_calls", NULL};
    ...
    if (tool_calls && strlen(tool_calls) > 2) {
        ...
        if (!tool_name || strlen(tool_name) == 0) {
            ...
            char *p_tn2[] = {"projects/cpp-llm/ops/+x/json_parser", "projects/cpp-llm/state/tool_calls.tmp", "[0].name", NULL};
            tool_name = run_tool(p_tn2[0], p_tn2, false);
            char *p_ta2[] = {"projects/cpp-llm/ops/+x/json_parser", "projects/cpp-llm/state/tool_calls.tmp", "[0].args", NULL};
    ...
    if (tool_name && strlen(tool_name) > 0) {
        ...
        // Log the assistant's tool call JSON BEFORE executing
        char *ast_json = NULL;
        asprintf(&ast_json, "{\"tool\":\"%s\",\"args\":%s}", tool_name, args_json ? args_json : "{}");
        char *a_args[] = {"projects/cpp-llm/ops/+x/json_state", "append", ctx_file, "assistant", ast_json, NULL};
    ...
    audit_log(g_pending_input, resp);
    ...
    // Log the FULL assistant JSON back to history
    char *a_args_app[] = {"projects/cpp-llm/ops/+x/json_state", "append", ctx_file, "assistant", content_json ? content_json : "{\"response\":\"...\"}", NULL};
    run_tool(a_args_app[0], a_args_app, false);
                    ...
                    while (fgets(line, sizeof(line), hf)) {
                        ...
                        if (cmd_ptr) {
                            ...
                            if (key == 10 || key == 13) { process_input_trigger(); state_changed = 1; }
                                ...
                                FILE *pf = fopen(tmp_prompt, "w"); 
                                if (pf) { 
                                    // Use a simpler summary prompt that doesn't embed raw JSON array into a string incorrectly
                                    fprintf(pf, "{\"model\":\"%s\",\"stream\":false,\"messages\":%s}\n", g_current_model, context ? context : "[]"); 
                                    // We could append a summarize instruction to the context instead
                                    fclose(pf); 
                                }
                                if (context) free(context); 
                                ...
                                if (api_path) {
                                    ...
                                    if (cpid > 0) log_pid(cpid, "cpp-llm-summarize");
                                    waitpid(cpid, NULL, 0);
                                }
                                free(op_path); free(api_path);

                                char *p_ext[] = {"projects/cpp-llm/ops/+x/json_parser", tmp_llm, "choices[0].message.content", NULL}; char *content_json = run_tool(p_ext[0], p_ext, false);
                                if (content_json) {
