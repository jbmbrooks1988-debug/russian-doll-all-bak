...
async function edit_file(filename, search_block, replace_block) {
  try {
    ...
    if (!content.includes(search_block.trim())) {
      return "Error: Search block not found. Ensure exact match including whitespace.";
    }
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (file_pattern === "*" || entry.name.endsWith(file_pattern.replace("*", ""))) {
          try {
...
async function main() {
  ...
  console.log(chalk.gray('• Type @ and press Tab for path completion'));
  ...
  while (true) {
    ...
    if (input === '/help') {
      console.log(chalk.cyan('\nAvailable commands: /clear, /yolo, /scan, /help, exit'));
      continue;
    }
    if (input === '/scan') {
      try {
      ...
      } catch (e) { console.log(chalk.red("Scan failed")); }
      continue;
    }
        ...
        if (functionCalls.length > 0) {
          ...
          for (const call of functionCalls) {
            ...
            if (tools[toolName]) {
              ...
              toolParts.push({
                functionResponse: { name: toolName, response: { result: resultText } }
              });
            }
          }
