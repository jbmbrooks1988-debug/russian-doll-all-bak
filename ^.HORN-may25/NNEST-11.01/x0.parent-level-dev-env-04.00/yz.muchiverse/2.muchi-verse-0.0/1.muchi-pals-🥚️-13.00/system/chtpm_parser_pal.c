...
/* PAL-NATIVE PATCH (muchi-pals, 2026-07-19, direct user request: "add a
 * tag to dynamic menu that would leave out <br> ... so each entry
 * wasn't its own newline" - for menus with many rows, matching wsr-pal's
 * own "compact menu display (32 options fit on screen)" checklist item,
 * confirmed that item was about wsr-pal's own SEPARATE hand-rendered C
 * compose_frame path, not this real chtpm-native ${piece_methods}
 * mechanism - this function still hardcoded `<br/>` unconditionally,
 * one row per line, with no compact option at all).
 *
 ...
 * targets, same method_idx numbering) but joined with a single space
 * instead of `<br/>` - a layout just writes `${piece_methods_compact}`
 * instead of `${piece_methods}` where it wants multiple entries packed
 * onto shared lines instead of one-per-line. No new tag/attribute
 * parsing needed at all - `${varname}` substitution is already fully
 * generic (works for any var name), so this is genuinely additive: the
 * real `piece_methods` var/behavior is completely unchanged, and every
 * existing layout using it anywhere in this family is unaffected. */
void load_dynamic_methods(const char* active_id) {
    ...
    /* Width-tracked line breaking for the compact variant (added after
     * live-testing a real 32-item menu, wsr-pal): packing every entry
     * onto ONE unbroken line looked fine in a real terminal only
     * because the terminal itself soft-wraps long lines - the GL
     * renderer's own blit_text() does NOT wrap at all (`while (*p &&
     * x_px < FRAME_W)` - confirmed by direct read,
     * system/chtpm_rgb_render.c), so anything past column ~80 was
     * silently clipped, losing most of a long menu entirely in that
     * view. COMPACT_LINE_BUDGET stays comfortably under FRAME_W's own
     * 80-char capacity (640px / 8px GLYPH_W) so a compact menu never
     * overflows either renderer - real terminal width varies, but 80 is
     * the real, hard, shared minimum both this project's GL window and
     * a stock terminal size respect. */
    ...
    while (fgets(line, MAX_LINE, f)) {
        ...
        *val_start = '\0';
        ...
        if (strcmp(cmd_val, "void") == 0 || !is_parser_cmd) {
            ...
            /* Separator baked INSIDE the label attribute, not as a
             * standalone text node between tags - the tokenizer treats
             * whitespace-only text between two tags as insignificant
             * and drops it (confirmed live: a trailing space after the
             * closing /> rendered with zero visible gap to the next
             * button). Two spaces reads clearly as a real gap without
             * wasting much of the box's own 60-char width. */
