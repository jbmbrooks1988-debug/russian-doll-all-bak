...
static void render_frame(void) {
    ...
    if (is_history_on()) {
        ...
        /* Static UI mode - kept as an explicit opt-in (pieces/display/
         * state.txt containing "off") for anyone who genuinely wants an
         * in-place-redrawn screen instead of scrollback. */
...
int main(void) {
    ...
    /* REAL BUG FIX, ported from pal-forum (user-reported "had to press
     * Enter twice" - a screen transition looked blank/stale until an
     * UNRELATED keypress fixed it): chtpm_parser_pal.c's own
     * process_key() grows pieces/display/frame_changed.txt on every
     * KEYPRESS-driven recompose - the ONLY marker this renderer used to
     * watch. But a pure href screen-navigation transition (chtpm's own
     * native handling, never touching ops/muchi_menu_input.c or ops/
     * muchi_compose_frame.c at all) does NOT also grow
     * frame_changed.txt - so the CORRECTED current_frame.txt could sit
     * on disk unseen until an unrelated later keypress happened to grow
     * frame_changed.txt too. chtpm_parser_pal.c's own internal
     * compose_frame() grows pieces/display/renderer_pulse.txt
     * UNCONDITIONALLY every time it runs, regardless of what triggered
     * it - the authoritative "the correct, chrome-composed
     * current_frame.txt is ready" signal chtpm itself already emits.
     * Watch this too, not just frame_changed.txt. */
