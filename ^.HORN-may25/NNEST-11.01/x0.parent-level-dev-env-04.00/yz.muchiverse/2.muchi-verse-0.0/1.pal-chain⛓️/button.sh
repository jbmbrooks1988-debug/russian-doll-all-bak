...
case "$ACTION" in
    ...
    run|r|start)
        ...
        mkdir -p "$SESSION_DIR/pieces/system" "$SESSION_DIR/pieces/display" \
                 ...
                 "$SESSION_DIR/net" "$SESSION_DIR/projects/pal-chain/manager"
...
EOSTATE
        ...
        if [ -x "system/chtpm_rgb_render" ]; then
            ./system/chtpm_rgb_render >/tmp/pal_chain_chtpm_rgb_render.log 2>&1 &
            RGB_PID=$!
        fi

        NET_PID=""
        if [ -z "$NO_NET" ] && [ -x "ops/+x/palnet_peer.+x" ]; then
            ./ops/+x/palnet_peer.+x chain_node pal-chain - net/outbox.txt net/inbox.txt chain_node >/tmp/pal_chain_palnet_peer.log 2>&1 &
            NET_PID=$!
        fi
        ...
        trap 'kill "$RENDERER_PID" "$CHTPM_PID" $RGB_PID $NET_PID 2>/dev/null; kill_own_module; rm -rf "$SESSION_DIR"' EXIT INT TERM

        : > pieces/apps/player_app/history.txt

        ./system/keyboard_input

        kill "$RENDERER_PID" "$CHTPM_PID" $RGB_PID $NET_PID 2>/dev/null
        kill_own_module
        ;;
    kill|k|stop)
    ...
    check|verify)
        for b in system/prisc+x system/keyboard_input system/renderer \
                 system/chtpm_parser_pal ops/+x/chain_create_wallet.+x \
