...
case "$ACTION" in
    ...
    run|r|start)
        ...
        mkdir -p "$SESSION_DIR/pieces/system" "$SESSION_DIR/pieces/display" \
                 ...
                 "$SESSION_DIR/net" "$SESSION_DIR/projects/pal-forum/manager"
...
EOSTATE
        ...
        if [ -x "system/chtpm_rgb_render" ]; then
            ./system/chtpm_rgb_render >/tmp/pal_forum_chtpm_rgb_render.log 2>&1 &
            RGB_PID=$!
        fi

        NET_PID=""
        if [ -z "$NO_NET" ] && [ -x "ops/+x/palnet_peer.+x" ]; then
            ./ops/+x/palnet_peer.+x forum_node pal-forum - net/outbox.txt net/inbox.txt forum_node >/tmp/pal_forum_palnet_peer.log 2>&1 &
            NET_PID=$!
        fi

        WATCHER_PID=""
        if [ -z "$NO_NET" ] && [ -x "ops/+x/forum_inbox_watcher.+x" ]; then
            ./ops/+x/forum_inbox_watcher.+x >/tmp/pal_forum_inbox_watcher.log 2>&1 &
            WATCHER_PID=$!
        fi

        # chtpm_parser_pal has no SIGTERM handler of its own (only
        # SIGINT triggers its own cleanup_module()), so a plain `kill`
        # on $CHTPM_PID would leave its own spawned module
        # (system/prisc+x) orphaned - matches every OTHER project's own
        # button.sh precedent (see pal-chain's own identical comment).
        # MUST be scoped to THIS session's own module only: prisc+x is
        # always launched via a relative `./system/prisc+x` argv (the
        # symlink target is identical across every session), so a
        # plain `pkill -f "system/prisc\+x"` can't distinguish sessions
        # by argv text alone - it would kill every OTHER concurrent
        # session's module too, defeating the entire point of this
        # isolation. Match by each candidate's own real cwd instead
        # (unique per session - the whole reason SESSION_DIR exists).
        kill_own_module() {
            local pid cwd
    ...
    check|verify)
        for b in system/prisc+x system/keyboard_input system/renderer \
                 system/chtpm_parser_pal ops/+x/forum_create_user.+x \
