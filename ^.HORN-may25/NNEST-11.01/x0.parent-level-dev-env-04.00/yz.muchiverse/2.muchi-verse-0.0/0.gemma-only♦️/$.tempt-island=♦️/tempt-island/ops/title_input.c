 ...
 *
 * Commit action (Enter):
 *   - cursor 1 (New Game): reset the live pieces/world_tempt_island/ from the
 *     pristine pieces/world_tempt_island_template/ (rm -rf + cp -r, matching
 ...
 *     the chosen pieces/saves/save_N/world_tempt_island/ instead of the template.
...
static void reset_world(const char *src_world_dir) {
    ...
    snprintf(world_dst, sizeof(world_dst), "%s/pieces/world_tempt_island", project_root);
...
int main(int argc, char **argv) {
    ...
    if (key == ARROW_UP) {
        ...
        if (cursor == 1) {
            ...
            snprintf(tpl_dir, sizeof(tpl_dir), "%s/pieces/world_tempt_island_template", project_root);
            reset_world(tpl_dir);
            /* Signal game start via sentinel keycode 999 to history.txt */
            char history_path[PATH_BUF];
            snprintf(history_path, sizeof(history_path), "%s/pieces/apps/player_app/history.txt", project_root);
            FILE *hf = fopen(history_path, "a");
            if (hf) { fprintf(hf, "999\n"); fclose(hf); }
        } else if (cursor >= 2 && cursor <= total) {
            char save_world[PATH_BUF];
            snprintf(save_world, sizeof(save_world), "%s/pieces/saves/%s/world_tempt_island", project_root, save_names[cursor - 2]);
