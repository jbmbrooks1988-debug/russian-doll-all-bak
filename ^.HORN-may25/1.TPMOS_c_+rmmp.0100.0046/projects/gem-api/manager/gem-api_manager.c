...
void check_ai_status(void) {
    ...
    if (tool_name && strlen(tool_name) > 0) {
        ...
        if (result) {
            char *a_args[] = {"projects/gem-api/ops/+x/json_state", "append", ctx_file, "assistant", function_call, NULL};
    ...
    audit_log(g_pending_input, text_content); format_response(text_content);
