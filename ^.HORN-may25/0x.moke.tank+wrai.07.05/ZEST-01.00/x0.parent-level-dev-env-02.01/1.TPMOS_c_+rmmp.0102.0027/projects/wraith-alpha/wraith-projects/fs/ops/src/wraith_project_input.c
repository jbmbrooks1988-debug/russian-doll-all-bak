...
static void write_scene(const char *root, const FsState *st, FsEntry *entries, int entry_count) {
    ...
    for (row = st->scroll_offset; row < entry_count && row < st->scroll_offset + VISIBLE_ENTRIES; row++) {
        ...
        fprintf(f, "OBJECT tag=control id=fs_row_%02d role=window_toolbar_item x=4 y=%d w=52 h=1 z=30 nav=%d source=semantic:fs_row fg=#E8F1F2 bg=#122333 border=#7EDFF2 action=PROJECT_ACTION:FS_OPEN:%d label=%s src=\n",
            row + 1, scene_y++, nav++, row + 1, label);
    }
    fclose(f);
}
