...
static int append_project_probe_scene_markup(char *out, size_t size, const Window *window, int max_lines) {
    ...
    while (fgets(line, sizeof(line), scene) && count < max_lines) {
        char tag[32] = "";
        char label[256] = "";
        char action[128] = "";
        char value[256];
        ...
        if (line_kvp_value(line, "nav", value, sizeof(value))) nav = atoi(value);
        ...
        if (y != current_y) {
            ...
            strncat(row_buf, " ", sizeof(row_buf) - strlen(row_buf) - 1);
        }

        if ((strcmp(tag, "control") == 0 || strcmp(tag, "text") == 0) && nav > 0 && action[0] && strcmp(action, "-") != 0) {
            char chunk[384];
            snprintf(chunk, sizeof(chunk), "<button compact=\"true\" label=\"%s\" onClick=\"%s\" visibility=\"${desktop_active_window_body_visible}\" />", label, action);
            strncat(row_buf, chunk, sizeof(row_buf) - strlen(row_buf) - 1);
        } else if (strcmp(tag, "text") == 0) {
            strncat(row_buf, label, sizeof(row_buf) - strlen(row_buf) - 1);
        }
    }

    if (row_open && count < max_lines) {
        appendf(out, size, "| |  %s |<br visibility=\"${desktop_active_window_body_visible}\" />", row_buf);
        count++;
    }
...
static void build_desktop_shell_markup(char *out, size_t size, Window *window) {
    ...
    if (window && !active_window_is_terminal(window)) {
        ...
        project_line_count += append_project_probe_scene_markup(out, size, window, 10);
