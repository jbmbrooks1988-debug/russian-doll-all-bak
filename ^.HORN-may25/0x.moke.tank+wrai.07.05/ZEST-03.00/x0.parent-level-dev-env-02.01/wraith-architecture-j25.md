...
For live media surfaces such as webcam and video, the current file-backed redraw seam is acceptable for proof work but will be slower than the eventual target architecture. Writing PNG/video-derived frame artifacts to disk, rereading them, and then reprojection into ASCII/GL is useful because it stays auditable and project-owned. It is not the final performance path.

Future preferred shape:

- keep file-backed receipts/state/markers as the sovereignty and debug seam
...
This should be treated as an optimization pass later, not as a reason to skip the current proof-friendly file contract.
