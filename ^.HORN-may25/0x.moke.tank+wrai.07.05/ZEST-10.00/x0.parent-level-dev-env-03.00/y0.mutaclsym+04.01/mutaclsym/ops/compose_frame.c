...
static void build_choice_footer(char *out, size_t out_sz, int action_cursor) {
    char buf[512];
    ...
    int n = snprintf(buf + len, sizeof(buf) - len, "  [q] Quit");
    if (n > 0 && (size_t)n < sizeof(buf) - len) len += (size_t)n;
...
static void draw_panel_box(char grid[][VIEWPORT_W + 1], int rows, const char *title,
    ...
    for (int r = 0; r < box_h && box_row + r < rows; r++) {
        ...
        while (rowlen < VIEWPORT_W) { grid[gr][rowlen] = ' '; rowlen++; }
        grid[gr][VIEWPORT_W] = '\0';
        int blen = (int)strlen(boxed);
        for (int i = 0; i < blen && box_col + i < VIEWPORT_W; i++) grid[gr][box_col + i] = boxed[i];
    }
}
...
int main(void) {
    ...
    if (py >= 0 && py < rows) {
        ...
        while (rowlen <= px && rowlen < map_w) { grid[py][rowlen] = ' '; rowlen++; grid[py][rowlen] = '\0'; }
        if (px >= 0 && px < map_w) grid[py][px] = '@';
    }
    ...
    if (strcmp(active_panel, "craft") == 0) {
        ...
        draw_panel_box(viewport, viewport_rows, "CRAFT", panel_rows, panel_row_count, cancel_row);
...
#pragma GCC diagnostic pop
        ...
        draw_panel_box(viewport, viewport_rows, "INVENTORY", panel_rows, panel_row_count, cancel_row);
    }
    ...
    if (!out) return 1;
    fprintf(out, "MUTACLSYM   map: %-10s turn: %d\n", map_id, turn);
    for (int r = 0; r < viewport_rows; r++) fprintf(out, "%s\n", viewport[r]);
    ...
    char footer[600];
    build_choice_footer(footer, sizeof(footer), action_cursor);
    fputs(footer, out);
    fclose(out);
    return 0;
}
