...
**Phase 7 — Save/load (not built)**
- Everything so far is always-live state (the current directory tree *is*
  the current game state, per `cdda-tpm-std-fast.txt`'s doctrine) — there
  is no separate "save file" concept yet, and no world/map/piece tree can
  currently be snapshotted, named, and restored independently of just
  being the live state. `dox` (in this doc's §6, "Deferred") already flags
  a related but distinct idea (named save-game *bundles*) for egg-pals;
  the same design question applies here: most likely a save is a
  timestamped/named copy of the relevant `pieces/world_01/` subtree (this
  is naturally easy given everything is already plain files/directories —
  a save is arguably just `cp -r`), with a registry of available saves and
  a `load` op that swaps the live tree for a saved copy. Not designed in
  detail yet; revisit once there's enough live state (post-crafting,
  post-vehicles) to make the save format decisions concrete rather than
  speculative.
...
- **"xelector"** — an entity-selection/targeting mechanism (name references
  ...
  `projects/fuzz-op/pieces/xlector/`; concrete mechanism written up in §3),
  needed later for combat targeting,
