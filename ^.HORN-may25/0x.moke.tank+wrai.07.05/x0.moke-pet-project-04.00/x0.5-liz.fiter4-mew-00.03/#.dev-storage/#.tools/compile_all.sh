...
source_needs_freetype() {
    local src="$1"
    grep -q "ft2build.h" "$src" 2>/dev/null
}

source_needs_gl() {
    local src="$1"
    grep -Eq '#include <(GLUT/glut.h|GL/glut.h|GL/gl.h|OpenGL/gl.h)>|glutInit|glutMainLoop|glBindTexture|glTexImage2D|glClear|glEnable|glBegin|glEnd|glTexCoord2f|glVertex2f|glGenTextures|glTexParameteri' "$src" 2>/dev/null
}
...
compile_source() {
    ...
    if source_needs_gl "$src"; then
        if source_needs_freetype "$src"; then
            compile_gl "$src" "$dst" "$GL_LIBS $FT_CFLAGS $FT_LIBS"
        else
            compile_gl "$src" "$dst" "$GL_LIBS"
        fi
    elif source_needs_freetype "$src"; then
...
for proj_dir in projects/*/; do
    ...
    if [ -f "${proj_dir}manager/${proj_name}_manager.c" ]; then
        if [ "$proj_name" = "user" ]; then
            ...
            compile_source "${proj_dir}manager/${proj_name}_manager.c" "${proj_dir}manager/+x/${proj_name}_manager.+x"
        fi
    fi

    # Special case: pal_editor_module
    if ([ "$proj_name" = "op-ed" ] || [ "$proj_name" = "slop-ed-dev" ]) && [ -f "${proj_dir}manager/pal_editor_module.c" ]; then
        compile_source "${proj_dir}manager/pal_editor_module.c" "${proj_dir}manager/+x/pal_editor_module.+x"
    fi

    # Special case: gl-os uses underscore instead of hyphen
    if [ "$proj_name" = "gl-os" ] && [ -f "${proj_dir}manager/gl_os_manager.c" ]; then
        compile_source "${proj_dir}manager/gl_os_manager.c" "${proj_dir}manager/+x/gl_os_manager.+x"
    fi

    # GL-Op-Ed Manager
    if [ "$proj_name" = "op-ed-gl" ] && [ -f "${proj_dir}manager/op-ed-gl_manager.c" ]; then
        compile_source "${proj_dir}manager/op-ed-gl_manager.c" "${proj_dir}manager/+x/op-ed-gl_manager.+x"
    fi
done
echo ""
...
for proj_dir in projects/*/; do
    ...
    # Check for source-bearing project directories.
