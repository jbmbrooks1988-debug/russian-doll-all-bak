...
static void emit_embedded_line_objects(FILE *objects, int *object_id, const Window *window,
    ...
    while (*p) {
        ...
        if (*p == '<') {
            ...
            if (label[0]) {
                /* Only real buttons (a non-empty action) consume a nav
                   slot -- plain <text> border/padding chunks stay
                   nav=0, visible but not Tab-focusable, matching
                   count_embedded_body_nav_slots()'s job of reserving
                   the right number of slots (that counter only counts
                   labeled chunks in general; giving nav only to
                   actionable ones here means the reserved range is a
                   safe upper bound, never an under-count -- no
                ...
                fprintf(objects, "OBJECT | %04d | tag=text id=%s_body_%02d_%02d role=project_body_embedded x=%d y=%d w=%d h=1 z=14 focused=%s parent_id=%s container_id=%s source_ref=semantic:project_body_embedded ancestor_chain=%s>%s_body_%02d_%02d clip_chain=%s nav=%d nav_selected=%s nav_selector_glyph=%s label_core= fg=#E8F1F2 bg=#162534 border=#E8F1F2 label=%s action=%s src=\n",
                    ...
                    cursor_x, base_y, rendered_len > 0 ? rendered_len : 1,
...
static void build_desktop_shell_markup(char *out, size_t size, Window *window) {
    ...
    appendf(raw, sizeof(raw), "------------------------------------------");
    appendf(raw, sizeof(raw), "<button compact=\"true\" label=\"o\" onClick=\"SET_ACTIVE:2\" />");
    appendf(raw, sizeof(raw), "<button compact=\"true\" label=\"-\" onClick=\"SET_ACTIVE:3\" />");
    appendf(raw, sizeof(raw), "<button compact=\"true\" label=\"x\" onClick=\"SET_ACTIVE:4\" />");
...
static void write_semantic_projection_files(void) {
    ...
    if (window) {
        ...
        fprintf(objects, "OBJECT | %04d | tag=text id=%s_open role=chrome_button x=%d y=%d w=10 h=1 z=21 focused=%s parent_id=%s container_id=%s source_ref=semantic:chrome_open ancestor_chain=%s>%s>chrome_open clip_chain=%s nav=%d nav_selected=%s nav_selector_glyph=%s label_core=o fg=#E8F1F2 bg=#162534 border=#E8F1F2 label=o action=SET_ACTIVE:2 src=\n",
            object_id++, window->id, 63 + win_col_offset, 2 + win_row_offset, g_active_gui_index == 2 ? "true" : "false", window->id, window->id, window_chain, window->id, window_chain, 2, g_active_gui_index == 2 ? "true" : "false", g_active_gui_index == 2 ? ">" : " ");
        fprintf(objects, "OBJECT | %04d | tag=text id=%s_min role=chrome_button x=%d y=%d w=10 h=1 z=21 focused=%s parent_id=%s container_id=%s source_ref=semantic:chrome_min ancestor_chain=%s>%s>chrome_min clip_chain=%s nav=%d nav_selected=%s nav_selector_glyph=%s label_core=- fg=#E8F1F2 bg=#162534 border=#E8F1F2 label=- action=SET_ACTIVE:3 src=\n",
            object_id++, window->id, 74 + win_col_offset, 2 + win_row_offset, g_active_gui_index == 3 ? "true" : "false", window->id, window->id, window_chain, window->id, window_chain, 3, g_active_gui_index == 3 ? "true" : "false", g_active_gui_index == 3 ? ">" : " ");
        fprintf(objects, "OBJECT | %04d | tag=text id=%s_close role=chrome_button x=%d y=%d w=10 h=1 z=21 focused=%s parent_id=%s container_id=%s source_ref=semantic:chrome_close ancestor_chain=%s>%s>chrome_close clip_chain=%s nav=%d nav_selected=%s nav_selector_glyph=%s label_core=x fg=#E8F1F2 bg=#162534 border=#E8F1F2 label=x action=SET_ACTIVE:4 src=\n",
            object_id++, window->id, 85 + win_col_offset, 2 + win_row_offset, g_active_gui_index == 4 ? "true" : "false", window->id, window->id, window_chain, window->id, window_chain, 4, g_active_gui_index == 4 ? "true" : "false", g_active_gui_index == 4 ? ">" : " ");
        if (!active_window_is_terminal(window)) {
            ...
            int pl;
            /* Embedded body-tag objects get real nav= values continuing
               right after whatever this project's OWN scene.objects.pdl
               already declared (author-chosen, contiguous from 5 --
               confirmed against piececraft-wraith's real file, nav=5..22)
               so the two ranges never collide. If this project has no
               scene.objects.pdl at all, scene_nav_count is 0 and body
               objects correctly start at nav=5. (2fix.txt, 2026-07-05) */
            int next_body_nav = 5;
            {
                char nav_project_dir[MAX_PATH];
                if (project_dir_for_window(window, nav_project_dir, sizeof(nav_project_dir))) {
                    next_body_nav = 5 + count_scene_nav_controls(nav_project_dir);
                }
            }
