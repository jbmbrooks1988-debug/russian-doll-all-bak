...
- the current GL cube is a first-pass `zslice_piece` preview, not the final intended projected cube.
- camera/rotation controls are not active yet because no project-owned cube manager/op consumes history and updates state.

Do not treat the current preview as complete. It proves the file bridge, interact bridge, and source-backed RGB seam. It does not yet prove real 3D camera projection.
...
- add a project-owned input op that consumes `session/history.txt` and translates interact-mode keys into `CAMERA_MODE`, `CAMERA_SET`, `CAMERA_MOVE`, and `ROTATE_CUBE`.
