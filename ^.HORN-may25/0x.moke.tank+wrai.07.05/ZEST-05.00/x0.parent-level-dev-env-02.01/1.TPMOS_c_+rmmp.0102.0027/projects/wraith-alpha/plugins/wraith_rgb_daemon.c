...
/*
 * Real camera-relative perspective projection for the 3D grid POC: translate
 * the world point into camera space, rotate by pitch (downward tilt), then
...
/* Scanline fill of a convex quad given 4 projected screen points, in order. */
static void fill_quad_px(unsigned char *buffer, const int px[4], const int py[4],
                          const unsigned char rgb[3]) {
    ...
    for (y = miny; y <= maxy; y++) {
        ...
        if (n >= 2) {
            ...
            fill_rect_px(buffer, (int)xs[0], y, (int)xs[n - 1] + 1, y + 1, rgb);
        }
    }
}
...
typedef struct {
    ...
    double extrude;
    int extrude_up; /* 0 = down from wy=0 (ground/floor default), 1 = up
                        from wy=0 (obstacle standing on the ground).
                        Explicit "extrude_dir=up|down" field in the
                        .tile.txt file — NOT inferred from "walkable" or
                        any other unrelated field. Default (field absent)
                        is "down", matching the ground-plane convention
                        in ARCHITECTURE-RGB-RENDERING.md. */
} TileMeta;
...
static void load_tile_meta(const char *project_dir, const char *registry_rel, char glyph, TileMeta *out) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        } else if (strcmp(line, "extrude_dir") == 0) {
            out->extrude_up = (strcmp(eq + 1, "up") == 0);
        }
    }
    fclose(f);
}

/* Draws a box's top face + camera-facing front face (near edge = smaller
   world Z, since the camera sits behind row 0 - see
   ARCHITECTURE-RGB-RENDERING.md's camera-relative projection notes). Top
   face corners are returned in top_px/top_py for callers that need them
   (selection outline). */
static void draw_box(unsigned char *buffer,
    ...
    project_world_point(wx0, wy_top, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[0], &top_py[0]);
    project_world_point(wx1, wy_top, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[1], &top_py[1]);
    project_world_point(wx1, wy_top, wz1, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[2], &top_py[2]);
    project_world_point(wx0, wy_top, wz1, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[3], &top_py[3]);

    if (wy_top - wy_bottom > 0.02) {
        int bpx[2], bpy[2];
        int fpx[4], fpy[4];
        project_world_point(wx0, wy_bottom, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &bpx[0], &bpy[0]);
        project_world_point(wx1, wy_bottom, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &bpx[1], &bpy[1]);
        fpx[0] = top_px[0]; fpy[0] = top_py[0];
        fpx[1] = top_px[1]; fpy[1] = top_py[1];
        fpx[2] = bpx[1]; fpy[2] = bpy[1];
        fpx[3] = bpx[0]; fpy[3] = bpy[0];
        fill_quad_px(buffer, fpx, fpy, rgb_side);
    }

    fill_quad_px(buffer, top_px, top_py, rgb_top);
}
...
static void draw_tile_zmap_preview_3d(unsigned char *buffer, const FrameObject *obj,
    ...
    /* "camera=" field: mode,pan_x,pan_y,pan_z,_,_,_ (the 3 trailing fields
       are still unused placeholders — pan_y now live, was a dead
       placeholder before Z/X got reassigned to vertical camera pan).
       1/2/3 switch fixed POV presets (WASD/ZX-driven pan stays live
       within whichever preset is active); all key handling lives in
       ops/wraith_project_input.c's apply_key(), this just renders
       whatever state.txt already says. */
    cam_field = strstr(obj->label, "camera=");
    if (cam_field) {
        double d4, d5, d6;
        sscanf(cam_field + 7, "%d,%lf,%lf,%lf,%lf,%lf,%lf", &camera_mode, &pan_x, &pan_y, &pan_z, &d4, &d5, &d6);
    }
    ...
    /* Paint far-to-near (high row index = farthest from camera, painted
       first) so nearer tiles correctly draw over farther ones without
       needing a depth buffer yet — see staged plan in
        ...
        for (col = 0; col < col_count && rows[row][col]; col++) {
            ...
            if (meta->extrude_up) {
                draw_box(buffer, wx0, wx1, wz0, wz1, 0.0, meta->extrude,
                         ...
                         meta->rgb_top, meta->rgb_side, top_px, top_py);
            } else {
                draw_box(buffer, wx0, wx1, wz0, wz1, -meta->extrude, 0.0,
                         cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale,
                         meta->rgb_top, meta->rgb_side, top_px, top_py);
            }
