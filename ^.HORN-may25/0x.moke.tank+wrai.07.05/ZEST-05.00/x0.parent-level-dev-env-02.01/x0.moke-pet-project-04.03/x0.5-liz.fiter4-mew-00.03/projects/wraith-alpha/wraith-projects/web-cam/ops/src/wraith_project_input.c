...
static void write_state(const char *root, const WebcamState *state) {
    ...
    f = fopen(path, "w");
    if (!f) return;
    fprintf(f, "project_id=wraith-alpha/wraith-projects/web-cam\n");
    fprintf(f, "device=%s\n", state->device);
    fprintf(f, "capture_profile=%s\n", state->capture_profile);
    fprintf(f, "state=%s\n", state->state);
    fprintf(f, "detail=%s\n", state->detail);
    fprintf(f, "frame_width=%d\n", state->frame_width);
    fprintf(f, "frame_height=%d\n", state->frame_height);
    fprintf(f, "frame_loaded=%d\n", state->frame_loaded);
...
static void write_scene(const char *root, const WebcamState *state) {
    ...
    fprintf(f, "OBJECT tag=control id=webcam_start role=chtmgl_button x=5 y=12 w=8 h=1 z=30 nav=5 source_ref=%s fg=#E8F1F2 bg=#22384A border=#7EDFF2 action=PROJECT_ACTION:WEBCAM_START label=Start\n", frame_ref);
    fprintf(f, "OBJECT tag=control id=webcam_stop role=chtmgl_button x=14 y=12 w=8 h=1 z=30 nav=6 source_ref=%s fg=#E8F1F2 bg=#22384A border=#7EDFF2 action=PROJECT_ACTION:WEBCAM_STOP label=Stop\n", frame_ref);
    fprintf(f, "OBJECT tag=control id=webcam_fast role=chtmgl_button x=5 y=14 w=8 h=1 z=30 nav=7 source_ref=%s fg=#E8F1F2 bg=#22384A border=#7EDFF2 action=PROJECT_ACTION:WEBCAM_FAST label=Fast\n", frame_ref);
    fprintf(f, "OBJECT tag=control id=webcam_debug role=chtmgl_button x=14 y=14 w=8 h=1 z=30 nav=8 source_ref=%s fg=#E8F1F2 bg=#22384A border=#FFD166 action=PROJECT_ACTION:WEBCAM_DEBUG label=Debug\n", frame_ref);
    fprintf(f, "OBJECT tag=control id=webcam_refresh role=chtmgl_button x=5 y=16 w=17 h=1 z=30 nav=9 source_ref=%s fg=#E8F1F2 bg=#22384A border=#FFD166 action=PROJECT_ACTION:WEBCAM_REFRESH label=Refresh\n", frame_ref);
