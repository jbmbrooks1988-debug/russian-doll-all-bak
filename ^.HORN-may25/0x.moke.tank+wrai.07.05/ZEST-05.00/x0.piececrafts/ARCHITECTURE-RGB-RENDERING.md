...
- Renderer (`draw_tile_zmap_preview_3d()`) now parses `camera=mode,pan_x,_,pan_z,_,_,_`
  from the label instead of using hardcoded `pitch=22/cam_y=7/cam_z=-16`
  constants — pan offsets are added on top of whichever POV preset's base
  position is active.
