...
/* Projects a tile's 4 top corners (constant wy) into screen space. Shared
   by draw_box() (wy = the box's top) and the flat-tile path in
   draw_tile_zmap_preview_3d() (wy = 0, ground level, no box at all —
   see py3d-inspo.md). Corner order: (wx0,wz0), (wx1,wz0), (wx1,wz1),
   (wx0,wz1) — matches draw_box()'s original corner order so gridline
   drawing downstream doesn't need to care which path filled top_px. */
static void project_tile_top(double wx0, double wx1, double wz0, double wz1, double wy,
                              double cam_x, double cam_y, double cam_z,
                              double pitch, double focal,
                              int screen_cx, int screen_cy, double scale,
                              int top_px[4], int top_py[4]) {
    project_world_point(wx0, wy, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[0], &top_py[0]);
    project_world_point(wx1, wy, wz0, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[1], &top_py[1]);
    project_world_point(wx1, wy, wz1, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[2], &top_py[2]);
    project_world_point(wx0, wy, wz1, cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale, &top_px[3], &top_py[3]);
}
...
/*
 * Renders a tile's footprint as an NxN grid of small colored voxels read
 * from a voxels_N.csv (emoji-studio/emoji-studio-wraith convention: a
 * `# resolution=N` header, then N*N `r,g,b,a` rows, row-major).
 *
 * Stands the image up as a thin vertical relief facing the camera —
 * image-x -> world-x (unchanged), image-y -> world-Y (height, row 0 =
 * top of the source picture = top of the standing figure), each visible
 * pixel a shallow slab at a shared depth within the tile's footprint.
 * NOT the emoji-studio POC's orientation: the POC maps image-y -> world-Z
 * and gives every pixel the SAME full height, which reads fine for its
 * own top-down-ish orbiting preview camera, but produces an unrecognizable
 * mosaic-of-pillars when viewed from piececraft-wraith's oblique POV
 * camera (users look at these from the side/front, not straight down) —
 * a standing sign/statue orientation is the one that actually reads as
 * "a tree" / "a dog" from that vantage point. Reuses draw_box() per
 * pixel, so it still gets the same camera-relative projection and
 * (now-general, yaw-aware) multi-face rendering as every other box here.
 * See emoji-entity-feature-report.md for the full pipeline.
 *
 * Alpha<=10 pixels are skipped entirely (no voxel drawn), matching the
 * POC's `p.a > 10` occupancy threshold.
 */
static void draw_voxel_grid_extrusion(unsigned char *buffer, const char *csv_path,
                                       double wx0, double wx1, double wz0, double wz1,
                                       double height,
                                       double cam_x, double cam_y, double cam_z, double pitch, double focal,
                                       int screen_cx, int screen_cy, double scale) {
    ...
    int resolution = 0;
    int i;
    double cell_w, cell_y, thickness, vz0, vz1;
    ...
    cell_w = (wx1 - wx0) / resolution;
    cell_y = height / resolution;
    /* Thin standing slab: a fraction of the cell's depth, centered, so
       this reads as a flat sign/relief rather than a full-depth solid
       block — like a pixel-art cutout standing on the tile. */
    thickness = (wz1 - wz0) * 0.35;
    vz0 = (wz0 + wz1) / 2.0 - thickness / 2.0;
    vz1 = vz0 + thickness;
    ...
    for (i = 0; i < count && i < resolution * resolution; i++) {
        ...
        unsigned char rgb_top[3], rgb_side[3];
        double vx0, vx1, vy0, vy1;
        int top_px[4], top_py[4];

        if (alpha <= 10) continue;

        rgb_top[0] = (unsigned char)values[i][0];
        rgb_top[1] = (unsigned char)values[i][1];
        rgb_top[2] = (unsigned char)values[i][2];
        shade_rgb(rgb_top, rgb_side, 70);

        vx0 = wx0 + px_i * cell_w;
        vx1 = vx0 + cell_w;
        /* image row 0 is the top of the picture -> top of the standing
           figure, so higher rows sit lower (closer to the ground). */
        vy0 = height - (py_i + 1) * cell_y;
        vy1 = vy0 + cell_y;

        draw_box(buffer, vx0, vx1, vz0, vz1, vy0, vy1,
                 cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale,
                 rgb_top, rgb_side, 255, top_px, top_py);
    }
}

/*
 * POC perspective grid renderer. Real camera projection (not a lift/shading
 * trick), reusing project_world_point() per tile corner. Painting order is
 * far-to-near rows (no depth buffer yet — documented simplification in
 * ARCHITECTURE-RGB-RENDERING.md, not an oversight). Tile height comes from
 * per-glyph metadata (load_tile_meta()), defaulting to a full cube.
 * Xelector and pet render as separate small boxes extruding upward
 * (positive wy) from the shared wy=0 ground plane, standing one layer
 * above whatever tile they're on.
 */
static void draw_tile_zmap_preview_3d(unsigned char *buffer, const FrameObject *obj,
    ...
    /* Pass 1: opaque tiles + entities (pet/xelector markers are always
       opaque). Depth-tested + depth-written. */
        ...
        for (col = 0; col < col_count && rows[row][col]; col++) {
            ...
            if (!meta->walkable && meta->alpha >= 255) {
                if (meta->voxel_source[0]) {
                    /* emoji/voxel-sourced tile: stands as a thin vertical
                       relief facing the camera, read from the CSV. See
                       draw_voxel_grid_extrusion() + emoji-entity-feature-
                       report.md. Outline is drawn at ground level (not
                       meta->extrude) since the figure no longer spans the
                       tile's full depth at a shared top height — a
                       footprint outline is what actually corresponds to
                       where it stands. */
                    draw_voxel_grid_extrusion(buffer, meta->voxel_source,
                                               wx0, wx1, wz0, wz1, meta->extrude,
                                               cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale);
                    project_tile_top(wx0, wx1, wz0, wz1, 0.0,
                                      cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale,
                                      top_px, top_py);
                } else {
                    /* solid obstacle (wall/tree/stone/...): always grows up
                       from the shared wy=0 ground datum, no direction field. */
                    draw_box(buffer, wx0, wx1, wz0, wz1, 0.0, meta->extrude,
                             cam_x, cam_y, cam_z, pitch, focal, screen_cx, screen_cy, scale,
                             meta->rgb_top, meta->rgb_side, 255, top_px, top_py);
                }
            ...
            /* walkable tiles (grass/floor): nothing drawn per-tile at
               all — the shared map-wide wireframe grid above already
               covers this cell. See py3d-inspo.md. */

            if (col == pet_x && row == pet_y) {
                int epx[4], epy[4];
                double inset = 0.22;
 ...
 * the same data draw_voxel_grid_extrusion() stands up into a 3D relief.
...
static long count_nonblack(unsigned char *buf) {
    long touched = 0, i;
    for (i = 0; i < (long)WIDTH * HEIGHT; i++) {
        unsigned char *p = buf + i * 4;
        if (p[0] || p[1] || p[2]) touched++;
    }
    return touched;
}

int main(void) {
    unsigned char red[3] = {255,0,0}, red_side[3] = {180,0,0};
    double cam_x=0, cam_y=7, cam_z=-16, pitch=22, focal=1.0;
    int scx=500, scy=300; double scale=210.0;
    int px[4], py[4];

    /* Test A: box fully in front of the camera (baseline, should look
       like a normal box -- same 3-face count as before this change). */
    {
        unsigned char *buf = calloc((size_t)WIDTH*HEIGHT*4, 1);
        draw_box(buf, -1.0, 1.0, 2.0, 4.0, 0.0, 3.0, cam_x, cam_y, cam_z, pitch, focal, scx, scy, scale, red, red_side, 255, px, py);
        printf("Test A (fully in front): pixels drawn = %ld (expect a reasonable nonzero box silhouette)\n", count_nonblack(buf));
        free(buf);
    }

    /* Test B: box fully BEHIND the camera (camera placed way in front of
       it along its own forward axis) -- must draw NOTHING, not distort. */
    {
        unsigned char *buf = calloc((size_t)WIDTH*HEIGHT*4, 1);
        /* camera at cam_z=50, looking toward +z as usual; box at wz 2..4
           is now far BEHIND the camera (which sits at z=50, looking further
           away from the box) */
        draw_box(buf, -1.0, 1.0, 2.0, 4.0, 0.0, 3.0, cam_x, cam_y, 50.0, pitch, focal, scx, scy, scale, red, red_side, 255, px, py);
        printf("Test B (fully behind camera): pixels drawn = %ld (expect 0)\n", count_nonblack(buf));
        free(buf);
    }

    /* Test C: THE key new capability -- a box that straddles the near
       plane (camera placed INSIDE the box's z-range) should now draw a
       real CLIPPED partial shape: nonzero pixels, but nowhere near the
       wild/blown-up pixel count the old clamp-and-distort bug produced,
       and (critically) NOT the old "skip whole box" zero either. */
    {
        unsigned char *buf = calloc((size_t)WIDTH*HEIGHT*4, 1);
        long n;
        /* box spans wz 2..4 in world; camera sits at cam_z=3 (a pitch of
           22 deg and the yaw/pitch transform means "straddles near plane"
           isn't just wz vs cam_z directly, so try a camera very close to
           the box's near face along the actual view axis by using pitch=0
           and placing cam_z just inside the box's z-span). */
        draw_box(buf, -1.0, 1.0, 2.0, 4.0, 0.0, 3.0, 0.0, 1.0, 2.5, 0.0, focal, scx, scy, scale, red, red_side, 255, px, py);
        n = count_nonblack(buf);
        printf("Test C (camera inside box's z-span, straddling near plane): pixels drawn = %ld (expect >0, a real partial/clipped shape, not the whole undistorted box and not 0)\n", n);
        free(buf);
    }
