...
void handle_mouse(int btn, int x, int y) {
    ...
    if (btn == 0 && last_frame_valid) {
        ...
        if (target_idx != -1) {
            ...
            FILE *dbg = fopen("debug.txt", "a");
            if (dbg) { fprintf(dbg, "[PARSER] Mouse Click Hit: Element '%s' (idx %d) Action: %s\n", el->id, target_idx, el->onClick); fclose(dbg); }
            ...
            FILE *dbg = fopen("debug.txt", "a");
            if (dbg) { fprintf(dbg, "[PARSER] Mouse Click Miss: X=%d Y=%d\n", x, y); fclose(dbg); }
        }
    }
