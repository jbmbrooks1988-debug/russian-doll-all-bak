            ...
            if (hit) {
                ...
                if (has_voxel) {
                    ...
                    if (sample_voxel8_pixel(voxel_path, u, v, sampled)) {
                        ...
                        /* Entities/decor (unlike walls) are small emoji
                         * cutouts with real transparent regions around the
                         * silhouette - a solid fallback-color box behind
                         * them just reads as a meaningless colored square.
                         * Leave this pixel untouched (falls through to
                         * whatever Pass 1's floor already drew) instead of
                         * painting a flat color, so the box is only
                         * visible where the emoji itself has opaque
                         * pixels - a real cutout silhouette, still shaded/
                         * extruded wherever it IS opaque. */
                        drew = 0;
                    }
                } else if (hit_kind != 0) {
                    drew = 0; /* no voxel data at all for this asset - see above */
                }
                if (drew) {
