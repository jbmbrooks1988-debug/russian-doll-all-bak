...
static Window xdnd_source = 0; /* window being dragged from */
static int xdnd_have_source = 0;
static Display *g_dpy = NULL;
static Window g_win = 0;

/* Visual feedback for drag-drop */
static int g_drag_over = 0; /* 1 when something is being dragged over us */
...
#ifndef __APPLE__
/* Forward declaration */
static void handle_xdnd_event(XEvent *xev);

/* Idle callback to check for Xdnd events */
static void check_xdnd_events(void) {
    if (!g_dpy || !g_win) return;

    /* Check for pending X11 events */
    while (XPending(g_dpy)) {
        XEvent xev;
        XNextEvent(g_dpy, &xev);
        handle_xdnd_event(&xev);
    }
}
#endif
...
#ifndef __APPLE__
/* Xdnd initialization - set XdndAware property on the GLUT window */
static void init_xdnd(Display *dpy, Window win) {
    g_dpy = dpy;
    g_win = win;
...
/* Handle Xdnd events - called from the GLUT event loop */
static void handle_xdnd_event(XEvent *xev) {
    if (!g_dpy || !g_win) return;

    if (xev->type == SelectionNotify) {
        /* Handle selection data (pet ID from drag source) */
        if (xev->xselection.selection == XA_XdndSelection &&
            ...
            if (XGetWindowProperty(g_dpy, g_win, xev->xselection.property,
                                   0, 1024, False, AnyPropertyType,
                                   &actual_type, &actual_format,
                                   &nitems, &bytes_after,
                                   &data) == Success && data) {
                if (actual_type == XA_text_plain && nitems > 0) {
                    /* Got the pet ID - trigger import */
                    char pet_id[256];
                    snprintf(pet_id, sizeof(pet_id), "%s", (char *)data);
                    pet_id[strcspn(pet_id, "\n")] = '\0';
                    fprintf(stderr, "gl_mirror: got pet_id='%s' via XdndSelection\n", pet_id);
                    trigger_pet_import(pet_id);
                }
                XFree(data);
            }

            /* Delete the property */
            XDeleteProperty(g_dpy, g_win, xev->xselection.property);

            /* Send XdndFinished to source to confirm the drop was processed */
            if (xdnd_source) {
                XEvent finished;
                ...
                finished.xclient.data.l[1] = 1; /* success */
        ...
        else if (xev->xselection.selection == XA_XdndSelection &&
                 xev->xselection.property == None) {
            /* Target rejected - clean up */
            xdnd_source = 0;
            xdnd_have_source = 0;
            g_drag_over = 0;
            glutPostRedisplay();
        ...
        if (xev->xclient.message_type == XA_XdndEnter) {
            /* Drag source entered our window */
            xdnd_source = xev->xclient.data.l[0];
            xdnd_have_source = 1;
            g_drag_over = 1;
            glutPostRedisplay(); /* Update visual feedback */
        }
        else if (xev->xclient.message_type == XA_XdndPosition) {
            /* Drag source moved within our window - send status response */
            if (xdnd_have_source) {
                XEvent status;
                ...
                status.xclient.data.l[1] = 1; /* bit 0 = accepting */
                status.xclient.data.l[2] = 0; /* rectangle (x,y) */
                status.xclient.data.l[3] = 0; /* rectangle (w,h) */
                XSendEvent(g_dpy, xdnd_source, False, NoEventMask, &status);
                XSync(g_dpy, False);
            }
        }
        else if (xev->xclient.message_type == XA_XdndLeave) {
            /* Drag source left our window */
            xdnd_source = 0;
            xdnd_have_source = 0;
            g_drag_over = 0;
            glutPostRedisplay(); /* Update visual feedback */
            ...
            /* Drag source dropped on our window */
            if (xdnd_have_source && xdnd_source) {
                fprintf(stderr, "gl_mirror: XdndDrop received from window %lu\n",
                        (unsigned long)xdnd_source);

                /* Request the selection data (the pet ID) via XConvertSelection.
                 * This triggers a SelectionRequest on the source (egg_window),
                 * which responds with SelectionNotify containing the pet ID. */
                XConvertSelection(g_dpy, XA_XdndSelection, XA_text_plain,
                                  XA_XdndSelection, g_win, CurrentTime);
                XSync(g_dpy, False);
            }
            /* Don't clear xdnd_source yet — wait for SelectionNotify */
        }
    }
}
#endif
...
#ifndef __APPLE__
    /* Visual feedback: highlight border when drag is over */
    if (g_drag_over) {
...
#ifndef __APPLE__
    /* Initialize Xdnd on the GLUT window.
     * CRITICAL: We must use freeglut's OWN display connection (fgDisplay.Display),
     * not XOpenDisplay(NULL). ClientMessage events sent via XSendEvent are
     * delivered to the connection that CREATED the window — that's freeglut's
     * internal connection. A separate connection will never receive them,
     * regardless of XSelectInput. */
    {
        /* freeglut's internal display — first field is Display* */
        extern struct { void *Display; } fgDisplay;
        Display *dpy = (Display *)fgDisplay.Display;
        if (dpy) {
            Window root = RootWindow(dpy, DefaultScreen(dpy));
            Window parent, *children;
            unsigned int nchildren;
            char *window_name = "mutaclsym RGB mirror";
                ...
                for (unsigned int i = 0; i < nchildren; i++) {
                    ...
                    if (XFetchName(dpy, children[i], &name) && name) {
                        if (strcmp(name, window_name) == 0) {
                            init_xdnd(dpy, children[i]);
                            glutIdleFunc(check_xdnd_events);
                            fprintf(stderr, "gl_mirror: Xdnd initialized on window %lu via freeglut display\n",
            ...
            fprintf(stderr, "gl_mirror: WARNING freeglut display is NULL\n");
        }
    }
#endif
