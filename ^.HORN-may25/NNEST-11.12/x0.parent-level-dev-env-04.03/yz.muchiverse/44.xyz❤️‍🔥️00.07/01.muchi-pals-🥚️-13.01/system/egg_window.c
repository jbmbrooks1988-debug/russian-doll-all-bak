...
/* Xdnd atoms for drag-and-drop between windows */
#ifndef _WIN32
...
static int xdnd_version = 5; /* Xdnd protocol version we support */
static Window xdnd_target = 0; /* window we're dragging over */
static int xdnd_have_target = 0;
static const char *g_pet_id_global = NULL; /* pet ID for XdndSelection */
#endif
...
int main(int argc, char **argv) {
    ...
    /* Xdnd initialization - set XdndAware property so other windows
     * know this window supports drag-and-drop. */
#ifndef _WIN32
    XA_XdndAware = XInternAtom(dpy, "XdndAware", False);
    XA_XdndEnter = XInternAtom(dpy, "XdndEnter", False);
    XA_XdndPosition = XInternAtom(dpy, "XdndPosition", False);
    XA_XdndLeave = XInternAtom(dpy, "XdndLeave", False);
    XA_XdndDrop = XInternAtom(dpy, "XdndDrop", False);
    XA_XdndFinished = XInternAtom(dpy, "XdndFinished", False);
    XA_XdndStatus = XInternAtom(dpy, "XdndStatus", False);
    XA_XdndSelection = XInternAtom(dpy, "XdndSelection", False);
    XA_text_plain = XInternAtom(dpy, "text/plain", False);
    XA_XdndActionCopy = XInternAtom(dpy, "XdndActionCopy", False);
    XA_targets = XInternAtom(dpy, "TARGETS", False);

    /* Set XdndAware property with version number */
    long xdnd_aware = xdnd_version;
    XChangeProperty(dpy, win, XA_XdndAware, XA_ATOM, 32,
    ...
    while (running) {
        ...
        while (XPending(dpy)) {
            ...
            if (xev.type == Expose && xev.xany.window == popup_win) {
                ...
                /* Handle XdndSelection request - send pet ID to target */
                XEvent sel;
                memset(&sel, 0, sizeof(sel));
                sel.xselection.type = SelectionNotify;
                sel.xselection.display = xev.xselectionrequest.display;
                sel.xselection.requestor = xev.xselectionrequest.requestor;
                sel.xselection.selection = xev.xselectionrequest.selection;
                sel.xselection.target = xev.xselectionrequest.target;
                sel.xselection.property = xev.xselectionrequest.property;
                sel.xselection.time = xev.xselectionrequest.time;

                if (xev.xselectionrequest.target == XA_targets) {
                    /* Target is asking what types we support */
                    Atom supported[] = { XA_text_plain };
                    XChangeProperty(dpy, xev.xselectionrequest.requestor,
                                    xev.xselectionrequest.property,
                                    XA_ATOM, 32, PropModeReplace,
                                    (unsigned char *)supported, 1);
                } else if (xev.xselectionrequest.target == XA_text_plain &&
                           g_pet_id_global) {
                    /* Send the pet ID */
                    XChangeProperty(dpy, xev.xselectionrequest.requestor,
                                    xev.xselectionrequest.property,
                                    XA_text_plain, 8, PropModeReplace,
                                    (unsigned char *)g_pet_id_global,
                                    strlen(g_pet_id_global));
                } else {
                    /* Unsupported target - set property to None */
                    sel.xselection.property = None;
                }

                XSendEvent(dpy, xev.xselectionrequest.requestor,
                           False, NoEventMask, &sel);
                XSync(dpy, False);
            }
#endif
                ...
                xdnd_have_target = 0;
                xdnd_target = 0;
                g_pet_id_global = pet_id; /* Set pet ID for XdndSelection */
                /* Claim ownership of XdndSelection so the target can
                 * request the pet ID via XConvertSelection. */
                XSetSelectionOwner(dpy, XA_XdndSelection, win, CurrentTime);
#endif
            }
            else if (xev.type == ButtonRelease && xev.xbutton.button == 1) {
#ifndef _WIN32
                /* Send XdndDrop if we have a target */
                ...
                /* DO NOT send XdndLeave after XdndDrop — per the Xdnd spec
                 * the drop ends the drag; the target cleans up via
                 * XdndFinished. Sending Leave here confuses the target. */
                xdnd_have_target = 0;
                xdnd_target = 0;
#endif
                dragging = 0;
...
#ifndef _WIN32
                /* Xdnd: find window under cursor and send XdndPosition */
                if (dragging && pet_id) {
                    /* Get root-window pointer position */
                    Window root_ret, child_ret;
                    int root_x, root_y, win_x, win_y;
                    unsigned int mask_ret;
                    XQueryPointer(dpy, win, &root_ret, &child_ret,
                                  &root_x, &root_y, &win_x, &win_y, &mask_ret);

                    /* XQueryPointer on our own window won't help (we have
                     * no children), and on root it returns OUR window as
                     * subwindow because we're override_redirect and on top.
                     * Instead, enumerate all root children and check which
                     * one the cursor is inside by geometry. */
                    Window target = 0;
                    {
                        Window r, parent, *children;
                        unsigned int nchildren;
                        if (XQueryTree(dpy, RootWindow(dpy, DefaultScreen(dpy)),
                                       &r, &parent, &children, &nchildren)) {
                            for (unsigned int i = 0; i < nchildren; i++) {
                                if (children[i] == win) continue; /* skip self */
                                Window junk;
                                int wx, wy;
                                unsigned int ww, wh, wb, wd;
                                if (XGetGeometry(dpy, children[i], &junk,
                                                 &wx, &wy, &ww, &wh, &wb, &wd)) {
                                    if (root_x >= wx && root_x < wx + (int)ww &&
                                        root_y >= wy && root_y < wy + (int)wh) {
                                        target = children[i];
                                        break;
                                    }
                                }
                    ...
                    /* If we have a new target, send XdndLeave to old, XdndEnter to new */
                    if (target != xdnd_target) {
                        ...
                        if (xdnd_have_target) {
                            ...
                            enter.xclient.data.l[1] = (xdnd_version << 24); /* no special types */
                            enter.xclient.data.l[2] = XA_text_plain;
                            XSendEvent(dpy, xdnd_target, False, NoEventMask, &enter);
                        }
                    }

                    /* Send XdndPosition to current target */
                    if (xdnd_have_target && xdnd_target) {
                        XEvent pos;
                        ...
                        pos.xclient.data.l[2] = (root_x << 16) | (root_y & 0xFFFF); /* x,y packed */
                ...
                /* Handle XdndStatus response from target window */
                if (xev.xclient.message_type == XA_XdndStatus) {
                    /* Target accepted or rejected the drop - we can
                     * update our state if needed. For now, just note
                     * the response. */
                    int accepted = xev.xclient.data.l[1] & 1; /* bit 0 = accepting */
                    if (!accepted) {
                        /* Target rejected - clear target */
                        xdnd_have_target = 0;
                        xdnd_target = 0;
                    }
                }
                /* Handle XdndFinished - target processed the drop */
                else if (xev.xclient.message_type == XA_XdndFinished) {
                    /* Drop was processed - we can clean up */
                    xdnd_have_target = 0;
                    xdnd_target = 0;
                }
            }
