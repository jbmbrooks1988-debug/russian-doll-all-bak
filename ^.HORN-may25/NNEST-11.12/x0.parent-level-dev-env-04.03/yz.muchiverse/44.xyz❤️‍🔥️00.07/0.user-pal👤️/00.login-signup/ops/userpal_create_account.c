/* userpal_create_account - USER-PAL-STANDARD.txt sec. 1. Creates a new
 * user-pal identity: user-pal/users/<user_id>/profile.txt. No password
 * (same v1 shape as forum_create_user.c/chain_create_wallet.c - no app
 * in this family invents an auth layer where none was asked for).
 *
 * Refuses if user_id already exists (same collision rule every other
 * create-account/create-wallet op in this family follows).
 *
