...
/* 2026-07-21: growth-tracked marker, same idiom as every other
 * *_changed.txt file here - an external process (path_nav_manager.c)
 * appends a line the instant its own completion picker appears, asking
 * this parser to force-deactivate whatever cli_io is currently active
 * (active_index = -1) so the very next keypress navigates the picker
 * instead of being typed as literal text into the input field. There was
 * no existing channel for this: active_gui_index.txt (see
 * sync_focus_from_saved_active_index()) only ever moves FOCUS, never
 * touches active_index, and active_index itself has no other external
 * write path anywhere in this file. */
long last_force_deactivate_size = 0;
#ifdef _WIN32
...
/* inject_raw_line - same target-resolution priority chain as
 * inject_raw_key() immediately above (kept in sync deliberately -
 * both write to whatever a project's own <interact src="..."/> or
 * project-history path resolves to), but writes an arbitrary tagged
 * STRING line instead of converting an int to a single key code.
 * Added for NAV_PICK:n (send_command() below, 2026-07-21) - path_nav_
 * manager.c's own file-picker needed a way to select index 10+, and
 * KEY:n's own encoding (single ASCII digit 0-9 only - see this file's
 * own send_command() comment) can't express that: KEY:13 for example
 * would inject raw code 13, colliding with Enter/CR, not "pick #13".
 * A distinctly-tagged line sidesteps the whole single-digit encoding
 * problem instead of trying to decompose/reassemble multi-digit key
 * sequences. */
void inject_raw_line(const char *text) {
    FILE *fp;
...
void send_command(const char* cmd) {
    ...
    /* NAV_PICK:n - path_nav_manager.c's own file-picker selection, ANY
     * n (not just 0-9) - see inject_raw_line()'s own header comment for
     * why this can't reuse KEY:n's single-ASCII-digit encoding. */
    if (strncmp(cmd, "NAV_PICK:", 9) == 0) {
        inject_raw_line(cmd);
        return;
    }
...
int main(int argc, char **argv) {
    ...
    char *force_deactivate_ch = build_path_malloc("pieces/display/force_deactivate.txt");
    ...
    if (stat(force_deactivate_ch, &st) == 0) last_force_deactivate_size = st.st_size;
    ...
    while (1) {
        ...
        /* path_nav_manager.c's own auto-un-activate request (see
         * last_force_deactivate_size's declaration comment): force
         * active_index = -1 the instant the completion picker appears, so
         * the user doesn't have to press ESC by hand before navigating it -
         * the SAME digit/Enter keys that would otherwise get typed
         * literally into the still-active cli_io immediately become
         * navigation instead. */
        if (stat(force_deactivate_ch, &st) == 0 && st.st_size > last_force_deactivate_size) {
            last_force_deactivate_size = st.st_size;
            active_index = -1;
            clear_nav_on_next = true;
            /* Unconditional, NOT gated behind "if (active_index != -1)":
             * this must re-export active_gui_index.txt from focus_index
             * regardless of whether active_index actually needed changing,
             * or a stale export from BEFORE sync_focus_from_saved_active_index()
             * moved focus onto the picker (in the state_ch block just above,
             * same loop iteration) never gets overwritten - live-caught: the
             * gated version left the auto-focus-onto-first-suggestion write
             * (active_gui_index.txt=2) looking like it silently failed,
             * because this skipped re-exporting whenever active_index
             * already happened to be -1 by the time this ran. */
            export_active_index();
            dirty = 1;
        }
