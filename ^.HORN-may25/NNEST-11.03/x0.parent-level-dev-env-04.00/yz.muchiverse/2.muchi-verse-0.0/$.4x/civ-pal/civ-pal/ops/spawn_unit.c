/* spawn_unit - one verb, one binary, no shared headers.
 ...
 *
 * Usage: spawn_unit.+x <bank_id> <instance_id> <world_dir> <civ_id> <spawn_x> <spawn_y>
...
int main(int argc, char **argv) {
    if (argc < 7) {
        fprintf(stderr, "usage: spawn_unit.+x <bank_id> <instance_id> <world_dir> <civ_id> <spawn_x> <spawn_y>\n");
        return 1;
    }
    ...
    snprintf(bank_path, sizeof(bank_path), "%s/pieces/registry/units/unit_bank.txt", project_root);

    BankRow row;
    if (!find_bank_row(bank_path, bank_id, &row)) {
        fprintf(stderr, "spawn_unit: bank_id '%s' not found in %s\n", bank_id, bank_path);
        return 2;
    }

    char instance_dir[PATH_BUF];
    snprintf(instance_dir, sizeof(instance_dir), "%s/pieces/%s/map_start/units/%s",
             project_root, world_dir, instance_id);

    struct stat st;
    if (stat(instance_dir, &st) == 0) {
        fprintf(stderr, "spawn_unit: instance '%s' already exists at %s, refusing to overwrite\n",
                instance_id, instance_dir);
        return 3;
    }
    if (mkdir_p(instance_dir) != 0 && errno != EEXIST) {
        fprintf(stderr, "spawn_unit: could not create %s\n", instance_dir);
        return 3;
    }
    ...
    if (sf) {
        ...
        fprintf(stderr, "spawn_unit: could not write %s\n", state_path);
        return 3;
    }
