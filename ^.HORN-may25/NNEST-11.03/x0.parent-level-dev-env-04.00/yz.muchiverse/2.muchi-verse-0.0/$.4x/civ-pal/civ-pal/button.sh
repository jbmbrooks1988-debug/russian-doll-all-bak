#!/bin/bash
# button.sh - launcher for muchi-civ, same verb convention as TPMOS's
...
case "$ACTION" in
    ...
    run|r|start)
        ...
        # CORRECTED 2026-07-21 (real, live-caught confusion): message_log.txt
        # was never truncated here (only history.txt was) - stale log lines
        # from a PREVIOUS session/testing run (e.g. "Selected Settler."/
        # "Returned to Xlector.") were visible on the very FIRST rendered
        # frame of a brand new game, looking exactly like something had
        # already auto-happened before the player ever pressed a key.
        ...
        export PRISC_PROJECT_ID="muchi-civ"
        ...
        if [ -z "$NO_GL" ] && [ -x "system/gl_mirror" ]; then
            ./system/gl_mirror >/tmp/muchi-civ_gl_mirror.log 2>&1 &
            GL_PID=$!
            ...
            echo "/tmp/muchi-civ_gl_mirror.log - the terminal session below is" >&2
    ...
    generate|gen)
        ...
        ;;
    kill|k|stop)
        echo "=== Killing muchi-civ processes ==="
    ...
    help|h|-h|--help)
        ...
        echo "  kill, k, stop       - Kill any lingering muchi-civ processes"
