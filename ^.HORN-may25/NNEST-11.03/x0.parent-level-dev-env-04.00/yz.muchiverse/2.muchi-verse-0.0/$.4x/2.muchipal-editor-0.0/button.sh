...
EOSTATE
        ...
        # GL/RGB mirror window (added 2026-07-21, same optional-best-
        # effort pattern as the 4 $.4x game projects' own button.sh -
        # see scripts/build.sh's own gl_mirror block; skip with NO_GL=1
        # or if GLUT isn't available on this machine).
        GL_PID=""
        if [ -z "$NO_GL" ] && [ -x "system/gl_mirror" ]; then
            ./system/gl_mirror >/tmp/muchipal-editor_gl_mirror.log 2>&1 &
            GL_PID=$!
            echo "GL/RGB mirror window launching (pid $GL_PID) - mirrors" >&2
            echo "whichever editor screen is active. If no window appears" >&2
            echo "(no DISPLAY/GLUT), see /tmp/muchipal-editor_gl_mirror.log -" >&2
            echo "the terminal session below is unaffected either way." >&2
        fi
        trap 'kill "$RENDERER_PID" "$CHTPM_PID" "$HANDOFF_PID" "$PRISC_PID" $GL_PID 2>/dev/null' EXIT INT TERM
        ...
        kill "$RENDERER_PID" "$CHTPM_PID" "$HANDOFF_PID" "$PRISC_PID" $GL_PID 2>/dev/null
        ;;
    kill|k|stop)
    ...
    check|verify)
        for b in system/prisc+x system/keyboard_input system/renderer \
                 ...
                 ops/+x/project_browser.+x ops/+x/pdl_reader.+x ops/+x/map_edit_input.+x \
                 ops/+x/compose_rgb_frame.+x; do
