...
echo "-- gl_mirror (optional best-effort, matches the 4 $.4x game"
echo "   projects' own build.sh convention - GL/GLUT dev libs may not"
echo "   be present everywhere, so a failure here doesn't fail the build)"
if $CC $CFLAGS -o system/gl_mirror system/gl_mirror.c -lglut -lGL -lGLU 2>/tmp/muchipal-editor_gl_build.log; then
    echo "   ok"
else
    echo "   skipped (see /tmp/muchipal-editor_gl_build.log)"
fi
...
echo "-- compose_rgb_frame (GL mirror's content half, no GL calls itself)"
$CC $CFLAGS -o ops/+x/compose_rgb_frame.+x ops/compose_rgb_frame.c -lm
echo "-- register_project (CLI-only, not wired into the interactive UI"
echo "   yet - see MUCHIPAL-EDITOR-DESIGN.txt sec. 3.1)"
$CC $CFLAGS -o ops/+x/register_project.+x ops/register_project.c
