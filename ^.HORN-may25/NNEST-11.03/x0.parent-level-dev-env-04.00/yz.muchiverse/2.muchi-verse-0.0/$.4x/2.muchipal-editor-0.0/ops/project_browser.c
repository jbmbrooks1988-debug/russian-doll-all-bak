...
typedef struct {
    ...
    /* Added 2026-07-21 for instance placement (sec. 3.2) - carried
     * through from known_projects.txt's own extended schema (see that
     * file's own header) so map_edit_input.c can spawn a real instance
     * without re-reading the registry itself. */
...
static void load_state(EditorState *st) {
    ...
    st->cursor_y = 0;
    st->armed_idx = 0;
    st->bank_rel_path[0] = '\0';
    st->instance_dir[0] = '\0';
    st->spawn_op_rel_path[0] = '\0';
    st->has_civ_id = 0;
    snprintf(st->arm_mode, sizeof(st->arm_mode), "terrain");
 ...
 * bank_rel_path|instance_dir|spawn_op_rel_path|has_civ_id rows from
 * pieces/registry/known_projects.txt - see that file's own header for
 * why every field is explicit rather than guessed from convention.
 * The last 4 fields may be empty/0 (mutaclsym has no *_bank.txt). */
static int load_projects(char names[][64], char paths[][PATH_BUF],
                          char map_rels[][256], char reg_fmts[][16], char reg_rels[][256],
                          char bank_rels[][256], char inst_dirs[][64], char spawn_ops[][128],
                          int has_civ[], int max_rows) {
    ...
    while (n < max_rows && fgets(line, sizeof(line), f)) {
        ...
        char *p = line;
        fields[nf++] = p;
        while (nf < 10 && (p = strchr(p, '|')) != NULL) {
            *p = '\0';
            p++;
            fields[nf++] = p;
        }
        if (nf < 6) continue;
        snprintf(names[n], 64, "%s", fields[1]);
        snprintf(paths[n], PATH_BUF, "%s", fields[2]);
        snprintf(map_rels[n], 256, "%s", fields[3]);
        snprintf(reg_fmts[n], 16, "%s", fields[4]);
        snprintf(reg_rels[n], 256, "%s", fields[5]);
        snprintf(bank_rels[n], 256, "%s", nf > 6 ? fields[6] : "");
        snprintf(inst_dirs[n], 64, "%s", nf > 7 ? fields[7] : "");
        snprintf(spawn_ops[n], 128, "%s", nf > 8 ? fields[8] : "");
        has_civ[n] = nf > 9 ? atoi(fields[9]) : 0;
        n++;
    }
    fclose(f);
    return n;
...
int main(int argc, char **argv) {
    ...
    char row_bank_rels[MAX_ROWS][256];
    char row_inst_dirs[MAX_ROWS][64];
    char row_spawn_ops[MAX_ROWS][128];
    int row_has_civ[MAX_ROWS];
    int count = 0;
    if (strcmp(st.screen, "title") == 0) count = 1;
    else if (strcmp(st.screen, "projects") == 0)
        count = load_projects(row_names, row_paths, row_map_rels, row_reg_fmts, row_reg_rels,
                               row_bank_rels, row_inst_dirs, row_spawn_ops, row_has_civ, MAX_ROWS);
    else if (strcmp(st.screen, "project_menu") == 0) count = 3;
    ...
    if (is_digit) {
        ...
        if (strcmp(st.screen, "title") == 0) {
            ...
            if (st.cursor >= 1 && st.cursor <= count) {
                ...
                snprintf(st.bank_rel_path, sizeof(st.bank_rel_path), "%s", row_bank_rels[i]);
                snprintf(st.instance_dir, sizeof(st.instance_dir), "%s", row_inst_dirs[i]);
                snprintf(st.spawn_op_rel_path, sizeof(st.spawn_op_rel_path), "%s", row_spawn_ops[i]);
                st.has_civ_id = row_has_civ[i];
            ...
            if (st.cursor == 1) {
                ...
                /* Play Project (added 2026-07-21, MUCHIPAL-EDITOR-
                 * DESIGN.txt sec. "playable project") - shells out to
                 * the target project's OWN real button.sh run, exactly
                 * as if launched by hand from its own directory. This
                 * blocks (system() waits for the child), so the editor
                 * genuinely pauses while the game is played and resumes
                 * right back here on exit - a real handoff, not a
                 * simulation of one. Requires that project to have its
                 * own runnable button.sh (all 4 $.4x game projects do). */
                char cmd[PATH_BUF + 64];
                snprintf(cmd, sizeof(cmd), "cd '%s' && ./button.sh run", st.proj_path);
                if (system(cmd) != 0) { /* non-zero exit from the played project is not this editor's error to report */ }
            } else {
                snprintf(st.screen, sizeof(st.screen), "projects");
                st.cursor = 1;
            }
