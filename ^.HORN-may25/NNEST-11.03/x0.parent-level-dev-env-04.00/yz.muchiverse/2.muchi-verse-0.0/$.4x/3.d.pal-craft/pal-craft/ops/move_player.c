 ...
 * pieces/world_pal_craft_home/map_start/hero/ - its own map_id FIELD tracks which
...
int main(int argc, char **argv) {
    ...
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/world_pal_craft_home/map_start/hero/state.txt", project_root);
    FILE *f = fopen(path, "r");
    if (!f) return 1;
    ...
    if (interact_mode == 1 && strcmp(possessed_id, "none") != 0 && (dx != 0 || dy != 0)) {
        ...
        snprintf(player_path, sizeof(player_path), "%s/%s/state.txt", players_dir, possessed_id);

        int upx = read_int_field(player_path, "pos_x", -1);
        int upy = read_int_field(player_path, "pos_y", -1);
