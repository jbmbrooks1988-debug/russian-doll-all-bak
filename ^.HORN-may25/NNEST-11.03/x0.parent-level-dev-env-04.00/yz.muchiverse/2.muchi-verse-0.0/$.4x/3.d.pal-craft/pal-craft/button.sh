#!/bin/bash
# button.sh - launcher for pal-craft, same verb convention as TPMOS's
...
case "$ACTION" in
    ...
    run|r|start)
        ...
        export PRISC_PROJECT_ID="pal-craft"
        ...
        if [ -z "$NO_GL" ] && [ -x "system/gl_mirror" ]; then
            ./system/gl_mirror >/tmp/pal-craft_gl_mirror.log 2>&1 &
            GL_PID=$!
            ...
            echo "/tmp/pal-craft_gl_mirror.log - the terminal session below is" >&2
    ...
    generate|gen)
        ...
        ;;
    kill|k|stop)
        echo "=== Killing pal-craft processes ==="
    ...
    help|h|-h|--help)
        ...
        echo "  kill, k, stop       - Kill any lingering pal-craft processes"
