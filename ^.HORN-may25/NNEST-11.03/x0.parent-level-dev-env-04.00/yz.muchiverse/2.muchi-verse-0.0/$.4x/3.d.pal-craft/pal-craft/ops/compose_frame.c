...
int main(void) {
    ...
    snprintf(map_dir, sizeof(map_dir), "%s/pieces/world_pal_craft_home/%s", project_root, map_id);
    ...
    /* NPCs (PAL CRAFT cast - Emma Wood, Hae-rin, etc.), drawn on top of
    ...
    /* CORRECTED 2026-07-21 (real, live-caught gap: ops/spawn_player.c and
     * the whole possession system - !.pal-standards.txt sec. 28/30 -
     * were built and live-verified via direct state.txt inspection, but
     * this compose op NEVER actually read pieces/<world>/<map>/players/
     ...
     * ops/spawn_player.c already writes a glyph= field straight into each
     * instance's own state.txt, no registry re-lookup needed, same as
     * npcs. Drawn on top of monsters/npcs, below xlector. */
    char players_dir[PATH_BUF + 32];
    snprintf(players_dir, sizeof(players_dir), "%s/players", map_dir);
    d = opendir(players_dir);
    if (d) {
        struct dirent *entry;
        while ((entry = readdir(d)) != NULL) {
            if (entry->d_name[0] == '.') continue;
    ...
    if (strcmp(possessed_id, "none") != 0) {
        ...
        snprintf(poss_path, sizeof(poss_path), "%s/pieces/world_pal_craft_home/map_start/players/%s/state.txt", project_root, possessed_id);
    ...
    if (strcmp(active_panel, "craft") == 0) {
        ...
        snprintf(inventory_dir_panel, sizeof(inventory_dir_panel), "%s/pieces/world_pal_craft_home/map_start/hero/inventory", project_root);
        char panel_rows[MAX_RECIPES_DISPLAY][BOX_TEXT_W + 1];
        int panel_row_count = 0;
...
#pragma GCC diagnostic pop
    ...
    if (!out) return 1;
    fprintf(out, "PAL CRAFT   map: %-10s turn: %d%s\n",
