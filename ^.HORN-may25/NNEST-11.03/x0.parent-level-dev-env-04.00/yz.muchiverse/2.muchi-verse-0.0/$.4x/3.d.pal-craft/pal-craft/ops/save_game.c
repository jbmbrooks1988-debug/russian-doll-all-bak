 ...
 * Outer-bar action: copies the ENTIRE live pieces/world_pal_craft_home/ tree into a
 * freshly auto-numbered pieces/saves/save_N/world_pal_craft_home/ folder via
...
int main(void) {
    ...
    snprintf(world_src, sizeof(world_src), "%s/pieces/world_pal_craft_home", project_root);
    snprintf(world_dst, sizeof(world_dst), "%s/world_pal_craft_home", save_dir);
    ...
    snprintf(turn_path, sizeof(turn_path), "%s/pieces/world_pal_craft_home/map_start/state.txt", project_root);
