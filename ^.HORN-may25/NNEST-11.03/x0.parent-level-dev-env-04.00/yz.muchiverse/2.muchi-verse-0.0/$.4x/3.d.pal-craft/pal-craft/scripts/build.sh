#!/bin/sh
# pal-craft R0 build - each TU its own binary (TPMOS / mutaclsym doctrine)
set -e
cd "$(dirname "$0")/.."
...
if $CC $CFLAGS -o system/gl_mirror system/gl_mirror.c -lglut -lGL -lGLU 2>/tmp/pal-craft_gl_build.log; then
    echo "   ok"
else
    echo "   skipped (see /tmp/pal-craft_gl_build.log)"
fi
...
echo "-- ops (R6 player instances)"
$CC $CFLAGS -o ops/+x/spawn_player.+x ops/spawn_player.c
