...
static void examine_at(const char *map_id, int x, int y, char *selected_id_out, size_t selected_id_sz) {
    char items_dir[PATH_BUF + 32], monsters_dir[PATH_BUF + 32], anglers_dir[PATH_BUF + 32];
    snprintf(items_dir, sizeof(items_dir), "%s/pieces/world_angler_home/%s/items", project_root, map_id);
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_angler_home/%s/monsters", project_root, map_id);
    snprintf(anglers_dir, sizeof(anglers_dir), "%s/pieces/world_angler_home/%s/anglers", project_root, map_id);
...
int main(int argc, char **argv) {
    ...
    if (is_3d_toggle) {
        ...
        if (strcmp(possessed_id, "none") != 0) {
            ...
            char angler_path[PATH_BUF + 384];
            snprintf(angler_path, sizeof(angler_path), "%s/pieces/world_angler_home/map_start/anglers/%s/state.txt", project_root, possessed_id);
            int upx = -1, upy = -1;
            FILE *uf = fopen(angler_path, "r");
            if (uf) {
                char line[MAX_LINE];
        ...
        if (is_enter && strcmp(dispatch_id, "xlector") == 0 && action_cursor < 2) {
            /* Sec. 30.1: examine-and-possess at the cursor's current
             * position (moved by move_player.c, not this file) -
             * UNCONDITIONAL matching route_input()'s own behavior, but
             * ONLY on a BARE Enter (no digit pressed first - action_
             * cursor<2 means nothing is selected). CORRECTED 2026-07-21
             * (real bug, live-caught building the job-designation
             * system, ANGLER-EXPANSES-DESIGN.txt sec. 4): without the
             * action_cursor<2 guard, this branch consumed EVERY Enter
             * unconditionally, including one after the player
             * deliberately digit-selected a DIFFERENT xlector method
             * (e.g. "designate_dig") - that selection could never fire,
             * silently overridden by the possess-check every time. A
             * real digit selection must win over the bare-Enter
             * default. */
