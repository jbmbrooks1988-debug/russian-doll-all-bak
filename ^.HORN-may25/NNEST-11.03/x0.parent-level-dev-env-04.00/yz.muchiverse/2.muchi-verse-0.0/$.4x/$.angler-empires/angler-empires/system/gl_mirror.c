/* gl_mirror - the ONLY file in angler-empires allowed to call GL/GLUT
 ...
 * angler-empires has no window manager and no semantic-object click targets
 ...
 *     frame; angler-empires has no such pulse-file convention yet, so this
...
/* Matches ops/compose_rgb_frame.c's FRAME_W/FRAME_H exactly (40x16
 * viewport tiles at 16px each, plus one 16px text row above and TWO
 * below - HP/stats then the action-bar choice footer, matching
 * compose_frame.c's own two-line HUD footer - for the header/footer nav
 * text that op's font pipeline blits in) - this file doesn't
 * independently pick a texture size, it reads whatever that op
 * documents it writes. HEIGHT bumped 288->304 (one extra GLYPH_H row)
 * when the action-bar row was added there. */
#define WIDTH 640
#define HEIGHT 304
...
static void resolve_root(void) {
    ...
    /* Same pulse marker pal/main_loop.pal's hit_frame opcode grows by
     * "X\n" every single tick (right after compose_frame AND
     * compose_rgb_frame both run - see prisc+x.c's own OP_HIT_FRAME) -
     * and the SAME file system/renderer.c already watches (its own
     * header comment: "pulse marker file's SIZE, never mtime"). This
     * window used to watch rgb_frame.raw's own stat() (mtime+size)
     * instead - a real, now-fixed bug: that file is a FIXED-size RGBA32
     * buffer (WIDTH*HEIGHT*4 bytes, always identical), so its size never
     * changes between frames, and st_mtime is only 1-SECOND resolution
     * (POSIX stat(), not the nanosecond st_mtim) - multiple frames
     * written within the same wall-clock second (normal during held-key
     * movement) were silently missed, which is what made this window
     * visibly lag behind the ASCII renderer. Watching the same
     * monotonically-growing pulse file renderer.c already uses is the
     * correct, already-proven fix - not a new invention. */
    snprintf(frame_pulse, sizeof(frame_pulse), "%s/pieces/display/frame_changed.txt", project_root);
...
static void update_focus_lock(void) {
    ...
    fprintf(f, "project=angler-empires\n");
    fclose(f);
}
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("angler-empires RGB mirror");
