#!/bin/bash
# button.sh - launcher for angler-empires, same verb convention as TPMOS's
...
case "$ACTION" in
    ...
    run|r|start)
        ...
        # !.pal-standards.txt sec. 31.2 - stale message_log.txt lines
        # from a previous session/testing run must not leak into a
        # fresh one (looks exactly like something auto-happened).
        ...
        export PRISC_PROJECT_ID="angler-empires"
        ...
        if [ -z "$NO_GL" ] && [ -x "system/gl_mirror" ]; then
            ./system/gl_mirror >/tmp/angler-empires_gl_mirror.log 2>&1 &
            GL_PID=$!
            ...
            echo "/tmp/angler-empires_gl_mirror.log - the terminal session below is" >&2
    ...
    generate|gen)
        ...
        ;;
    kill|k|stop)
        echo "=== Killing angler-empires processes ==="
    ...
    help|h|-h|--help)
        ...
        echo "  kill, k, stop       - Kill any lingering angler-empires processes"
