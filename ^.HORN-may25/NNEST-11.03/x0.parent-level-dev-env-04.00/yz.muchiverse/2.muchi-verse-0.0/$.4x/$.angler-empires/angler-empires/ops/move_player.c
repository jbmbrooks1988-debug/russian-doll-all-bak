 ...
 * pieces/world_angler_home/map_start/hero/ - its own map_id FIELD tracks which
...
int main(int argc, char **argv) {
    ...
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/world_angler_home/map_start/hero/state.txt", project_root);
    FILE *f = fopen(path, "r");
    if (!f) return 1;
    ...
    /* REMOVED 2026-07-21 per !.pal-standards.txt sec. 32 (direct user
     * catch: "there is no way to actually possess characters in dwarf
     * fortress... just a way to focus menu on them"). This block used
     * to let arrows directly walk a possessed/focused angler around -
     * genre-wrong for a DF-style project (real DF never lets the player
     * puppet a dwarf's movement; the dwarf keeps moving under its own
     * job/AI assignment regardless of what's selected). possessed_id
     * (kept as the field name for minimal diff, functions as a FOCUS
     * target now, not a controllable one) is still set by examine_at()
     * on a units/anglers/ match and still logged/shown (see
     * compose_frame.c's own new focus-info HUD line), but arrows now
     * ALWAYS move only the free xlector cursor below, regardless of
     * possessed_id - selecting an angler never changes what arrows do.
     * Real angler movement should come from a job-designation/idle-
     * labor AI system (ANGLER-EXPANSES-DESIGN.txt sec. 4) - not built
     * yet, this removal doesn't add it, just stops the wrong interim
     * behavior from standing in for it. */
