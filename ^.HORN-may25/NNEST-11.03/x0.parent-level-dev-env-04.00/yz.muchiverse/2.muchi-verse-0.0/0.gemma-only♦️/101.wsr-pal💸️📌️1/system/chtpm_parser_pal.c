...
/* PAL-NATIVE PATCH (ported from z0.muchi-pals-refx-13.00, 2026-07-19,
 * same direct user request applied there first: "add a tag to dynamic
 * menu that would leave out <br> ... so each entry wasn't its own
 * newline" - directly motivated by THIS project's own checklist item,
 * "compact menu display (32 options fit on screen)" / "bracket cursor
 * with multi-option-per-line layout" - checked directly and confirmed
 * those were about wsr-pal's own separate hand-rendered C compose_frame
 * path, not this real chtpm-native ${piece_methods} mechanism, which
 * still hardcoded `<br/>` unconditionally with no compact option here.
 *
 * THE FIX: build a SECOND var, `piece_methods_compact`, in the exact
 * same loop as the real `piece_methods` (same buttons, same onClick
 * targets, same method_idx numbering) but joined with no `<br/>` -
 * separator baked INSIDE each button's own label attribute (two
 * trailing spaces), not as a standalone text node between tags, since
 * the tokenizer drops whitespace-only text between two tags as
 * insignificant (confirmed live: a trailing space after the closing />
 * rendered with zero visible gap to the next button). A layout just
 * writes `${piece_methods_compact}` instead of `${piece_methods}`
 * where it wants multiple entries packed onto shared lines instead of
 * one-per-line. No new tag/attribute parsing needed - `${varname}`
 * substitution is already fully generic - so this is genuinely
 * additive: the real `piece_methods` var/behavior is unchanged, every
 * existing layout using it is unaffected. Live-tested (isolated harness,
 * 4-item piece.pdl): compact renders all 4 on one line with a real
 * visible gap between entries, normal renders one-per-line exactly as
 * before, both correctly digit-jumpable/dispatchable. */
void load_dynamic_methods(const char* active_id) {
    ...
    /* Width-tracked line breaking for the compact variant (live-caught,
     * user-reported: "in the gl window, it doesn't wrap around like in
     * terminal, the extra disappears"). Packing every entry onto ONE
     * unbroken line looked fine in a real terminal only because the
     * terminal itself soft-wraps long lines - the GL renderer's own
     * blit_text() does NOT wrap at all (`while (*p && x_px < FRAME_W)` -
     * confirmed by direct read, system/chtpm_rgb_render.c), so anything
     * past column ~80 was silently clipped, losing most of a 32-item
     * menu entirely in that view. COMPACT_LINE_BUDGET stays comfortably
     * under FRAME_W's own 80-char capacity (640px / 8px GLYPH_W) so a
     * compact menu never overflows either renderer. */
    ...
    while (fgets(line, MAX_LINE, f)) {
        ...
        *val_start = '\0';
        ...
        if (btn_compact) {
            /* Estimated rendered width: label text + a fixed overhead
             * for render_element's own "[>] NN. []" cursor/index/
             * bracket chrome (not generated here - a different
             * function's own job, but it's a small, roughly-constant
             * addition regardless of label content). Start a new line
             * once the running total would exceed budget, rather than
             * letting one line grow unbounded. */
    ...
    if (strcmp(el->type, "br") == 0) strcat(frame, "\n");
        ...
        if (is_active) {
            if (is_password) {
                ...
                masked[mask_len] = '\0';
                if (display_num > 0) (void)asprintf(&line, "%s %s %d. [%s_]                                              %s", BOX_V, pref, display_num, masked, BOX_V);
                else (void)asprintf(&line, "%s %s [%s_]                                                 %s", BOX_V, pref, masked, BOX_V);
            } else {
                if (display_num > 0) (void)asprintf(&line, "%s %s %d. [%s_]                                              %s", BOX_V, pref, display_num, el->input_buffer, BOX_V);
                else (void)asprintf(&line, "%s %s [%s_]                                                 %s", BOX_V, pref, el->input_buffer, BOX_V);
            }
        } else {
            const char* display_val = (strlen(el->input_buffer) > 0) ? el->input_buffer : scratch_substituted;
            if (display_num > 0) (void)asprintf(&line, "%s %s %d. [%s]                                                 %s", BOX_V, pref, display_num, display_val, BOX_V);
            else (void)asprintf(&line, "%s %s [%s]                                                    %s", BOX_V, pref, display_val, BOX_V);
        }
        if (line) { append_colored_text(frame, line, el->fg_expr, el->bg_expr); free(line); }
    }
    else if (interactive) {
