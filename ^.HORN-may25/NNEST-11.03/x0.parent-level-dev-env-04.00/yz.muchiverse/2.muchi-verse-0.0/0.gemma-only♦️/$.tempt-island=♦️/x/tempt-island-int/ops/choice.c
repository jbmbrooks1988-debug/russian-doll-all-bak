 ...
 * research writeup), adapted for tempt-island's own shape (a single game,
...
static void examine_at(const char *map_id, int x, int y) {
    ...
    snprintf(monsters_dir, sizeof(monsters_dir), "%s/pieces/world_tempt_island/%s/monsters", project_root, map_id);
...
int main(int argc, char **argv) {
     ...
     * for the full writeup, not copied verbatim since tempt-island is a
    ...
    if (is_3d_toggle) {
        ...
        if (is_enter) examine_at(map_id, xlector_x, xlector_y);
        digit_accum = 0;
