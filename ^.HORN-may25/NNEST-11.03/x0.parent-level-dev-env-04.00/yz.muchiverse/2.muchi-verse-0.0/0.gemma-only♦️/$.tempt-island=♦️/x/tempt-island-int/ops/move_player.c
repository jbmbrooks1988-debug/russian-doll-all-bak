 ...
 * pieces/world_tempt_island/map_start/hero/ - its own map_id FIELD tracks which
 ...
 * tempt-island's cursor is transient and doesn't need to persist as its
...
int main(int argc, char **argv) {
    ...
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/pieces/world_tempt_island/map_start/hero/state.txt", project_root);
    FILE *f = fopen(path, "r");
    if (!f) return 1;
