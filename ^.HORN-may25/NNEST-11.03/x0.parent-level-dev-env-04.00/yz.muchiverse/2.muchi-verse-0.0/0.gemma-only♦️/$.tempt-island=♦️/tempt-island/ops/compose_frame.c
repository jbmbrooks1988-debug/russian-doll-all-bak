...
static int count_in_inventory(const char *item_id) {
    ...
    snprintf(inventory_dir, sizeof(inventory_dir), "%s/pieces/world_muchimon_home/map_start/hero/inventory", project_root);
 ...
 *
 * CORRECTED 2026-07-21 per !.pal-standards.txt sec. 28 (read from the
 * real shared engine's own source, chtpm_parser.c - see that section
 * for the full citation, not repeated here): interact_mode==1 (ACTIVE)
 * no longer replaces the footer with a fixed, undispatchable hint
 * string - it shows the SAME kind of real numbered method list as NAV
 * mode, just sourced from whichever piece_id is currently active
 * (dispatch_id, computed by the caller with the exact same 3-way rule
 * ops/choice.c's own dispatch uses - "xlector" unpossessed, or the
 * possessed unit's own instance id). This is what makes the "[>]"/
 * digit-dispatch marker actually visible and meaningful in ACTIVE mode,
 * not just a description of what arrows do. '[i] Interact' legend text
 * is REMOVED (a stale leftover from the old bound-key design, sec.
 * 28.3 - "interact" is now an ordinary numbered METHOD row on hero's
 * own list, rendered by the same loop as every other method, no
 * separate legend entry needed). */
static void build_choice_footer(char *out, size_t out_sz, const char *dispatch_id, int action_cursor, int interact_mode, int emoji_mode, int render_mode) {
    char buf[512];

    snprintf(buf, sizeof(buf), interact_mode ? "[wasd/arrows] Move/Look" : "[wasd/arrows] Move");
    size_t len = strlen(buf);

    char cmd[PATH_BUF];
    snprintf(cmd, sizeof(cmd), "'%s/ops/+x/pdl_reader.+x' '%s' list_methods", project_root, dispatch_id);
    FILE *pf = popen(cmd, "r");
    int idx = 0;
    if (pf) {
        char line[MAX_LINE];
    ...
    int n = interact_mode
        ? snprintf(buf + len, sizeof(buf) - len, "  [e] Emoji:%s  [0] 3D:%s  [esc] Back  [q] Quit", emoji_status, render_status)
        : snprintf(buf + len, sizeof(buf) - len, "  [e] Emoji:%s  [0] 3D:%s  [q] Quit", emoji_status, render_status);
    if (n > 0 && (size_t)n < sizeof(buf) - len) len += (size_t)n;
...
int main(void) {
    ...
    snprintf(map_dir, sizeof(map_dir), "%s/pieces/world_muchimon_home/%s", project_root, map_id);
    ...
    /* NPCs (MUCHIMON cast - Emma Wood, Hae-rin, etc.), drawn on top of
    ...
    /* CORRECTED 2026-07-21 (real, live-caught gap: ops/spawn_entity.c and
     * the whole possession system - !.pal-standards.txt sec. 28/30 -
     * were built and live-verified via direct state.txt inspection, but
     * this compose op NEVER actually read pieces/<world>/<map>/entities/
     * at all, so every spawned unit was completely invisible on the
     * real rendered map the whole time - direct user report: "i dont
     * see the settler"). Same shape as the npcs/ loop directly above -
     * ops/spawn_entity.c already writes a glyph= field straight into each
     * instance's own state.txt, no registry re-lookup needed, same as
     * npcs. Drawn on top of monsters/npcs, below xlector. */
    char entities_dir[PATH_BUF + 32];
    snprintf(entities_dir, sizeof(entities_dir), "%s/entities", map_dir);
    d = opendir(entities_dir);
    if (d) {
        struct dirent *entry;
        while ((entry = readdir(d)) != NULL) {
            if (entry->d_name[0] == '.') continue;
            char state_path[PATH_BUF + 320];
            snprintf(state_path, sizeof(state_path), "%s/%s/state.txt", entities_dir, entry->d_name);
            int ux = read_kv_int(state_path, "pos_x", -1);
            int uy = read_kv_int(state_path, "pos_y", -1);
            if (ux < 0 || uy < 0 || ux >= map_w || uy >= rows) continue;
            char glyph[4];
            read_kv_str(state_path, "glyph", glyph, sizeof(glyph), "?");
            if (glyph[0]) grid[uy][ux] = glyph[0];
            if (emoji_mode && glyph[0]) snprintf(cell_emoji[uy][ux], EMOJI_BUF, "%c", glyph[0]);
        }
        closedir(d);
    }

    /* CORRECTED 2026-07-21 (direct user correction, real live confusion:
     * "get rid of @ if it does nothing. wtf?"): this used to draw a
     * hero glyph unconditionally at hero/state.txt's own pos_x/pos_y -
     * a mansion-sim leftover from before this session's control-model
     * correction (!.pal-standards.txt sec. 28/30). The hero never
     * actually moves anymore in this possession-based control model
     * (move_player.c hard no-ops all hero movement), so '@' just sat
     * frozen at spawn forever, looking like a real, controllable player
     * character while doing and meaning nothing - actively misleading,
     * since a player naturally tries to interact with it (real
     * live-caught case: standing xlector next to '@' and pressing Enter
     * expecting to possess IT specifically, when '@' was never a real,
     * scannable piece at all - only real units/ instances are). Removed
     * entirely - the ONLY things ever drawn on the map now are real
     * pieces (units/monsters/items) plus the always-visible xlector
     * cursor (sec. "we dont need to erase xelector position from view"
     * correction, same session). If a real avatar/camera-anchor glyph
     * is ever wanted again, it should represent something ACTUALLY
     * controllable, not this dead leftover. */

    /* Xlector cursor - real 1.TPMOS's active-target pattern, adapted
     * (see ops/choice.c's own header comment and !.pal-standards.txt
     * sec. 28 for the full writeup). Drawn last of all, on top of even
     * the hero, since it starts exactly on the hero's own tile every
     * time interact_mode is entered - it must stay visibly
     * distinguishable. 'X' confirmed unused by every terrain/furniture/
     * item/monster registry in this project.
     *
     * CORRECTED 2026-07-21 (direct user correction: "we dont need to
     * erase xelector position from view, its enough that it stay
     * still"): this used to be gated on interact_mode==1, so leaving
     * ACTIVE mode (ESC, back to NAV) made the cursor glyph vanish from
     * the map entirely - wrong; it should just stay put, visible,
     * wherever it last was. Drawn regardless of interact_mode now.
     *
     * FURTHER CORRECTED 2026-07-21 (direct user clarification, same
     * session: "during possession xelector isnt visible on the map,
     * till relinquishment of possession where it will appear at the
     * new location of the possessed entity like before"): while
     * ACTUALLY POSSESSING something (possessed_id != "none"), xlector
     * itself is NOT drawn at all - the possessed unit's OWN glyph
     * (drawn by the units/ loop above) is the only visible marker,
     * matching real fuzz-op's own [ACTIVE] semantics (control fully
     * transfers, there's no second cursor glyph hovering alongside the
     * possessed piece). ops/choice.c's own ESC/relinquish handler moves
     * xlector_pos_x/y to the possessed unit's own last position before
     * clearing possessed_id, so it reappears exactly where the unit
     * was, not wherever it happened to be left before possessing. */
    if (strcmp(possessed_id, "none") == 0 && xlector_y >= 0 && xlector_y < rows) {
    ...
    /* Camera: keep the viewport centered, clamped so it never scrolls
     * past the map's edges - dox/01-cdda-architecture.md §5a's exact
     * formula. If the map is smaller than the viewport in either
     * dimension, the upper clamp bound would go negative, so floor it
     * at 0 - camera sits at the map's origin and the viewport just
     * shows blank padding past the map's real edge, same as any other
     * out-of-map read already defaults to on this project.
     *
     * CORRECTED 2026-07-21 (real, live-caught gap - direct user
     * question: "are we sure we have a separate camera than view so it
     * will scroll?"): this centered on px/py, the HERO's own position -
     * which never moves in this project's possession-based control
     * model (move_player.c hard no-ops hero movement, sec. 28/32).
     * Camera would have stayed frozen on the dead spawn point forever
     * on any real map bigger than one screen, exactly the "3d camera
     * needs to move with the xelector" bug already found and fixed for
     * the 3D mirror earlier this session - same fix, same reasoning,
     * this is the 2D viewport's own copy of it. Centers on xlector_x/
     * xlector_y - EXCEPT while something is actually possessed, in
     * which case the camera follows THAT piece's own live position
     * instead. This matters specifically for muchimon-pal/civ-pal's own
     * "free, direct control" genre (sec. 32) - move_player.c's own
     * possession-redirect block updates the POSSESSED entity's pos_x/
     * pos_y every step, but never touches xlector_x/xlector_y (xlector
     * just sits wherever it was when Enter was pressed to possess) - a
     * camera that only ever followed xlector would stay frozen at the
     * possession point while the player walked their actual character
     * away from it, defeating scrolling entirely for exactly the genre
     * that needs direct movement most. */
    int cam_anchor_x = xlector_x, cam_anchor_y = xlector_y;
    if (strcmp(possessed_id, "none") != 0) {
        char poss_path[PATH_BUF + 384];
        snprintf(poss_path, sizeof(poss_path), "%s/pieces/world_muchimon_home/map_start/entities/%s/state.txt", project_root, possessed_id);
        int poss_x = read_kv_int(poss_path, "pos_x", -1);
        int poss_y = read_kv_int(poss_path, "pos_y", -1);
        if (poss_x >= 0 && poss_y >= 0) { cam_anchor_x = poss_x; cam_anchor_y = poss_y; }
    }
    int cam_x = cam_anchor_x - VIEWPORT_W / 2;
    int cam_x_max = map_w - VIEWPORT_W;
    if (cam_x_max < 0) cam_x_max = 0;
    if (cam_x < 0) cam_x = 0;
    if (cam_x > cam_x_max) cam_x = cam_x_max;
    int cam_y = cam_anchor_y - VIEWPORT_H / 2;
    ...
    if (strcmp(active_panel, "craft") == 0) {
        ...
        snprintf(inventory_dir_panel, sizeof(inventory_dir_panel), "%s/pieces/world_muchimon_home/map_start/hero/inventory", project_root);
        char panel_rows[MAX_RECIPES_DISPLAY][BOX_TEXT_W + 1];
        int panel_row_count = 0;
...
#pragma GCC diagnostic pop
    ...
    /* CORRECTED 2026-07-21 (direct user request: "remove the mansion
     * text from screen") - room_name/resolve_location_name() looked up
     * mansion-sim room labels (registry/locations/locations.txt:
     * "Outer Gate"/"Mirror Lounge"/etc, matching map.txt's own old
     * baked-in text, also removed this session) that mean nothing in
     * this project - dropped from the header entirely, not just left
     * blank, since there's no real "room" concept in this control
     * model at all. */
    FILE *out = fopen(out_path, "w");
    if (!out) return 1;
    fprintf(out, "MUCHIMON   map: %-10s turn: %d%s\n",
     ...
     * tick regardless of what it meant, PLUS the current mode/
     * possession state alongside it - lets a player (or whoever's
     ...
     *
     * CORRECTED/EXTENDED 2026-07-21 (direct user request, after a real
     * "possession does nothing" report that couldn't be reproduced via
     * synthetic op invocation alone: "pls add a permanent debug
     * variable to layout so each frame will show last button pressed
     * (even esc/enter/arrow, keys etc) no matter interact or not"):
     * this line already existed but only ever showed a raw numeric
     * code for non-printable keys (arrows/Enter/ESC all showed as bare
     * numbers like 1000/13/27, not readable) - decode the same key
     * classes real chtpm_parser.c's own key_str does (UP/DOWN/LEFT/
     * RIGHT/ENTER/ESC), and show interact_mode/possessed_id on the
     * SAME line so a live session's actual mode/possession state is
     * always visible without needing to cat state.txt separately -
     * always rendered, unconditionally, matching "no matter interact
     * or not." */
    const char *last_key_name = NULL;
    if (last_key == 1000) last_key_name = "LEFT";
    else if (last_key == 1001) last_key_name = "RIGHT";
    else if (last_key == 1002) last_key_name = "UP";
    else if (last_key == 1003) last_key_name = "DOWN";
    else if (last_key == 10 || last_key == 13) last_key_name = "ENTER";
    else if (last_key == 27) last_key_name = "ESC";
    if (last_key_name) fprintf(out, "  Last key: %s (%d)", last_key_name, last_key);
    else if (last_key >= 32 && last_key < 127) fprintf(out, "  Last key: %d ('%c')", last_key, (char)last_key);
    else fprintf(out, "  Last key: %d", last_key);
    fprintf(out, "  | Mode: %s | Possessed: %s\n", interact_mode ? "ACTIVE[^]" : "NAV[>]", possessed_id);
    fprintf(out, "%s\n", inventory_line);
    char log_lines[LOG_TAIL][256];
    int log_n = read_message_log_tail(log_lines);
    for (int i = 0; i < log_n; i++) fprintf(out, "%s\n", log_lines[i]);
    char footer[600];
    /* Same 3-way dispatch_id rule as ops/choice.c's own dispatch (sec.
     * 28 of !.pal-standards.txt) - what's shown here must always match
     * what Enter actually commits, or the footer would lie. */
    const char *footer_dispatch_id = (interact_mode == 0) ? "hero"
                                    : (strcmp(possessed_id, "none") != 0) ? possessed_id
                                    : "xlector";
    build_choice_footer(footer, sizeof(footer), footer_dispatch_id, action_cursor, interact_mode, emoji_mode, render_mode);
    fputs(footer, out);
    fclose(out);
    return 0;
}
