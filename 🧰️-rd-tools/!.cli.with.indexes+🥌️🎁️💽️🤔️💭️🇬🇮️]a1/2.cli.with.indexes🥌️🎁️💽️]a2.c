...
void complete_filename(char *input, int *cursor_pos) {
    ...
    // Handle empty input
    if (strlen(prefix) == 0) {
        printf("\nPlease enter a prefix to complete. 😕\n");
        return;
    }
    ...
    // Debugging: Confirm matches found
    printf("\n[DEBUG] Found %d matches for prefix '%s'\n", match_count, prefix);
    ...
    if (match_count == 1) {
        ...
        *cursor_pos = strlen(input);
        ...
        if (common_len > strlen(prefix)) {
            strncpy(input, matches[0], common_len);
            input[common_len] = '\0';
            *cursor_pos = common_len;
            printf("Completed to common prefix: %s\n", input);
        } else {
            input[*cursor_pos] = '\0'; // Ensure input is unchanged if no new completion
        }
    }
