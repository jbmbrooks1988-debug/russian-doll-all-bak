...
void search_directory(const char *path, const char *word) {
    ...
    struct dirent *ent;
    struct stat statbuf;
    char fullpath[4096]; // Increased buffer size for complex paths with emojis
    int found = 0;
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        // If it's a regular file, search it
            ...
            char buffer[4096]; // Increased buffer for potential UTF-8 content
...
int main(int argc, char *argv[]) {
    ...
    // Set locale to support UTF-8 (for emojis)
